...
- Server-side **AI quota** enforcement — `consume_ai_quota` / `refund_ai_quota` RPCs; `analyze-paper` consumes quota before calling Gemini and returns HTTP 402 when exhausted (`20260521020000`). This enforcement is live. Consumption is caller-authenticated. **The refund is server-only in Production** (`SEC-AI-QUOTA-REFUND-AUTHORITY-001`, C47, `20260924193915`, rolled out 2026-09-25): `service_role` may execute `refund_ai_quota`; `authenticated`, `anon` and PUBLIC may not, so no browser can give itself a unit back. Both deployed generation functions refund through a dedicated server-only client ([docs/deployment.md](docs/deployment.md) §6.8).
...
| `SUPABASE_SECRET_KEYS` / `SUPABASE_SERVICE_ROLE_KEY` | `delete-account`; also the two generation functions, each through two separate narrow clients: one content-free telemetry INSERT (live since 2026-09-17) and the server-only AI-quota refund (C47, live since 2026-09-25) | **Auto-injected** by the Supabase Edge runtime — no manual setup. Server-only; see below. |
