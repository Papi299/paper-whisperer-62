...
- Private per-user attachments, with a **recoverable cleanup lifecycle**: Postgres and Storage cannot share a transaction, so deleting an attachment or a paper records the Storage key in `attachment_cleanup_queue` in the *same* transaction that removes the metadata naming it, and uploading finalizes through a single serialized, idempotent RPC so a lost response can never turn a saved attachment into a deleted file (`20260904120000`). `authenticated` holds `SELECT` only on `paper_attachments` and cannot `DELETE` a `papers` row either, while `anon` holds no privilege on either table — metadata is created and destroyed exclusively by those RPCs, through the parent as well as directly — and Storage refuses to delete a binary a live attachment row still names, which together keep an already-loaded pre-migration browser tab from recreating the old destructive orderings. Physical removal is done by the authenticated browser — immediately, and once more at the next sign-in if that failed. There is no scheduled worker, so cleanup is recoverable rather than immediate or guaranteed; account deletion still enumerates Storage itself as the final sweep. **Both migrations are applied to Production** (in the two phases, with a verified drain between them), and the lifecycle passed a bounded Production wet acceptance on 2026-09-10 — see [docs/deployment.md](docs/deployment.md) §6.4.
...
- Server-side **AI quota** enforcement — `consume_ai_quota` / `refund_ai_quota` RPCs; `analyze-paper` consumes quota before calling Gemini and returns HTTP 402 when exhausted (`20260521020000`). This enforcement is live. Consumption is caller-authenticated. **The server-only refund is not live yet.** `SEC-AI-QUOTA-REFUND-AUTHORITY-001` (C47, `20260924193915`) makes `refund_ai_quota` executable by `service_role` alone, so no browser can give itself a unit back. That correction is implemented in the repository and is pending the migration plus the two-function rollout ([docs/deployment.md](docs/deployment.md) §6.8). Until the rollout, Production still grants the refund to `authenticated`.
...
| `SUPABASE_SECRET_KEYS` / `SUPABASE_SERVICE_ROLE_KEY` | `delete-account`; also the two generation functions, for one telemetry INSERT (live since 2026-09-17) and, once C47 is rolled out, the server-only AI-quota refund (in the repository; not yet deployed) | **Auto-injected** by the Supabase Edge runtime — no manual setup. Server-only; see below. |
...
**Caller-authenticated functions.** `fetch-paper-metadata`, `analyze-paper`, `get-gemini-provider-quota`, `search-pubmed` and `suggest-paper-organization` construct their Supabase client with the **caller's** auth header and rely on RLS plus an in-function `auth.getUser()` check for ownership enforcement. Since the 2026-09-17 deploy of `AI-MULTI-PROVIDER-001D`, `analyze-paper` and `suggest-paper-organization` additionally build a server-only client used for exactly one thing: inserting a content-free provider-usage event into `ai_provider_usage_events`, a table no browser role can read or write. In the repository, `SEC-AI-QUOTA-REFUND-AUTHORITY-001` (C47) adds a **second**, separate server-only client for exactly one other thing: calling `refund_ai_quota` to give back a unit an attempt consumed but did not deliver, once the C47 migration grants that RPC to `service_role` only. That client is implemented but **not yet deployed**. The deployed functions still refund on the caller's client until the [docs/deployment.md](docs/deployment.md) §6.8 rollout. Both clients come from the platform-injected key, carry **no** caller Authorization header, and are typed for their one call; the refunded user id is the function's own `auth.getUser()` result. Every product read, model selection and quota **consumption** still goes through the caller's client — see decisions C42 and C47 and [docs/deployment.md](docs/deployment.md) §3.3.
