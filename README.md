...
| `SUPABASE_SECRET_KEYS` / `SUPABASE_SERVICE_ROLE_KEY` | `delete-account`; in repository `main` also the two generation functions, for one telemetry INSERT (not deployed) | **Auto-injected** by the Supabase Edge runtime — no manual setup. Server-only; see below. |
...
**Caller-authenticated functions.** `fetch-paper-metadata`, `analyze-paper`, `get-gemini-provider-quota`, `search-pubmed` and `suggest-paper-organization` construct their Supabase client with the **caller's** auth header and rely on RLS plus an in-function `auth.getUser()` check for ownership enforcement. In repository `main` (`AI-MULTI-PROVIDER-001D`, not yet deployed), `analyze-paper` and `suggest-paper-organization` additionally build a server-only client used for exactly one thing: inserting a content-free provider-usage event into `ai_provider_usage_events`, a table no browser role can read or write. Every product read, model selection and quota call still goes through the caller's client — see decision C42 and [docs/deployment.md](docs/deployment.md) §3.3.
