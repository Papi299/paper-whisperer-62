...
- Server-side **AI quota** enforcement — `consume_ai_quota` / `refund_ai_quota` RPCs; `analyze-paper` consumes quota before calling Gemini and returns HTTP 402 when exhausted (`20260521020000`). Consumption is caller-authenticated; the refund is **server-only** since `SEC-AI-QUOTA-REFUND-AUTHORITY-001` (C47, `20260924193915`) — executable by `service_role` alone, so no browser can give itself a unit back.
...
| `SUPABASE_SECRET_KEYS` / `SUPABASE_SERVICE_ROLE_KEY` | `delete-account`; also the two generation functions, for one telemetry INSERT (live since 2026-09-17) and the server-only AI-quota refund (C47) | **Auto-injected** by the Supabase Edge runtime — no manual setup. Server-only; see below. |
