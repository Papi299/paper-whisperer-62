...
5. **Check the provider adapter** — the runtime registry must hold a reviewed adapter for the row's provider. The registry in `main` holds `google`, `anthropic` and `openai` (§4.9c), and every catalog row today is Google; Production's pre-001A runtime still makes this check as a Google-only provider comparison (§4.7).
...
- **Repository-only, not deployed.** 001A leaves every provider request, user-visible response and quota/refund outcome unchanged — its one deliberate difference is a log-only privacy hardening (C39) — and has not been deployed, nor have 001B or 001C: Production continues to run the pre-001A Edge artifacts until an authorized deploy of **both** generation functions (see [deployment.md](deployment.md) §7c for the bundle closure, which now includes the three new `_shared` modules).
...
- **Current state, after 001C (§4.9c is authoritative).** `google`, `anthropic` and `openai` are all registered in `main`, and the Settings provider-family filter (§4.10) includes all three. PaperLume's own per-operation reasoning policy now exists (C41). Anthropic and OpenAI are still **not live**, and registration is no longer the reason: no Anthropic/OpenAI catalog row exists, neither credential is installed, and Production generation Edge still runs the pre-001A runtime.
