...
- **Current state, after 001C (§4.9c is authoritative).** `google`, `anthropic` and `openai` are all registered in `main`, and the Settings provider-family filter (§4.10) includes all three. PaperLume's own per-operation reasoning policy now exists (C41). Anthropic and OpenAI are still **not live**, and registration is no longer the reason: no Anthropic/OpenAI catalog row exists and neither credential is installed, although the deployed runtime has contained both registered adapters since the 2026-09-17 Phase 6 deploy.
