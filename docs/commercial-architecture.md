...
|---|---|---|
| **AI quota** | `analyze-paper` calls the `consume_ai_quota` SECURITY DEFINER RPC **before** invoking the routed AI provider; returns the structured **HTTP 402** without calling the provider when quota is unavailable; calls `refund_ai_quota` best-effort if the provider call or parsing fails after a successful consume. The path is provider-neutral — the consume, the dispatch through the provider registry and the refund are identical whichever of the three registered families (§4.9a) the request is routed to. Consumption is caller-authenticated: `consume_ai_quota` is executable by `authenticated` only and runs on the caller's client. The refund is server-only (C47): `refund_ai_quota` is executable by `service_role` alone, called through a dedicated server client with the user id the function's own `auth.getUser()` returned. Before C47 it was executable by `authenticated` with only an `auth.uid()` check, so a signed-in browser could give itself units back. | ✅ **Live in Production** (consume before the provider call, HTTP 402, best-effort refund). ✅ **C47 server-only refund: live in Production since 2026-09-25** — migration `20260924193915` applied and both generation functions redeployed ([deployment.md](deployment.md) §6.8). |
