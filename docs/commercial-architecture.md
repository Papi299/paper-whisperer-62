...
**This table is the runtime allowlist.** `_shared/aiModelSelection.ts` deliberately hard-codes **no** list of model strings — a TypeScript copy would be a second authorization surface that could disagree with this one. A saved preference is honoured only when its catalog row exists, its `id` matches the id that was requested, `enabled` is `true`, `provider_model` is a non-empty trimmed string, and the row's `provider` has a registered runtime adapter (§4.9a). Today the only registered provider is `google`.
