...
5. **Check the provider adapter** — the runtime registry must hold a reviewed adapter for the row's provider. The registry holds `google`, `anthropic` and `openai` (§4.9c) — in `main` and, since the 2026-09-17 Phase 6 deploy, in Production — and every catalog row today is Google.
