...
Consumption is likewise unchanged, and is now harder to get wrong. `finalize_attachment_upload` performs an ordinary INSERT on `paper_attachments`, so the same BEFORE INSERT check-and-consume trigger runs with the same ownership guard reading `auth.uid()`. Its one addition is that the INSERT sits in a subtransaction: a refusal — a quota refusal included — rolls back both the row and the bytes the trigger had just consumed for it, while the transaction continues to commit the cleanup intent for the uploaded object. So a rejected upload charges nothing, and a successful one charges exactly once no matter how many times an ambiguous response makes the client repeat the call — including a repeat that arrives long after the failed upload's object was removed, because the durable `attachment_cleanup_tombstone` makes that replay resolve to cleanup rather than to a second metadata row.
