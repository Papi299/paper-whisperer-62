...
|---|---|---|
| **AI quota** | `analyze-paper` calls the `consume_ai_quota` SECURITY DEFINER RPC **before** invoking the routed AI provider; returns the structured **HTTP 402** without calling the provider when quota is unavailable; calls `refund_ai_quota` best-effort if the provider call or parsing fails after a successful consume. The path is provider-neutral — the consume, the dispatch through the provider registry and the refund are identical whichever of the three registered families (§4.9a) the request is routed to. Consumption is caller-authenticated. **Who may refund differs between Production and the repository:** in Production `refund_ai_quota` is still executable by `authenticated` and checks only `auth.uid()`, so a signed-in browser can give itself units back. C47 (`20260924193915` plus both generation functions) makes it executable by `service_role` alone, called through a dedicated server client with the user id the function's own `auth.getUser()` returned. | ✅ **Existing AI quota enforcement: live in Production** (consume before the provider call, HTTP 402, best-effort refund). ⚠️ **C47 server-only refund: implemented in the repository, NOT yet in Production.** The Production refund is still browser-callable until the migration and both functions are rolled out ([deployment.md](deployment.md) §6.8). |
...
`sort_order` is sparse on purpose, and 001D **appended** 30 and 40 rather than renumbering — a preference someone already saved keeps its position. Nothing beyond the six rows above is in the catalog: Gemini Pro models, preview models and the floating `gemini-flash-latest` alias remain absent by decision, and a floating alias is excluded on its own terms since it is not a stable thing for a user to have chosen. *(Until `AI-MULTI-PROVIDER-001E` this sentence also excluded Anthropic/Claude and OpenAI/GPT; `20260917201856` seeded one model from each, so that part is historical.)* **Being in this table makes a model selectable, never default** — Paperlume's system default is `gemini-3.5-flash`, resolved server-side from `GEMINI_MODEL` (C34), and no migration changes it.
...
`provider` is deliberately **not** CHECK-constrained to a closed list so an Anthropic or OpenAI model is a seed row plus a runtime adapter rather than a constraint migration. **That affordance has since been used:** the adapters were implemented (001B), registered (001C), and `AI-MULTI-PROVIDER-001E` seeded one row per paid provider, so the catalog now names all three registered families. *(Historical — at 001C, before 001E: no non-Google catalog row existed and neither paid credential was installed, so the affordance was unused.)* The gate on a **fourth** provider is unchanged: a seed row for it still needs a reviewed adapter, that provider's own server-side credential, and explicit provider, privacy and cost acceptance (previous paragraph). Since AI-MODEL-SELECTION-001B the runtime enforces that boundary rather than trusting the seed: a catalog row naming a provider PaperLume has no adapter for is **refused and falls back to the system default**, and no external URL is constructed for it. Since AI-MULTI-PROVIDER-001A (C39) the authority for that check is the runtime **provider-adapter registry** (§4.9a) rather than a string comparison — so this table authorizes **models**, and the registry authorizes **provider protocols**. A row alone can never make PaperLume call a provider.
...
### 4.9 Runtime routing — LIVE across all three registered provider families (AI-MODEL-SELECTION-001B; widened by AI-MULTI-PROVIDER-001C/001E)
...
5. **Check the provider adapter** — the runtime registry must hold a reviewed adapter for the row's provider. The registry holds `google`, `anthropic` and `openai` (§4.9c) — in `main` and, since the 2026-09-17 Phase 6 deploy, in Production — and the catalog today holds six rows across all three families: four Google, one Anthropic and one OpenAI. Authorization still comes from `ai_model_catalog` alone; the registry only decides which provider protocols a row may name, never which models exist.
...
**Only the model changes — as written for 001B, when every routable model was Gemini.** At that phase the model component of the Gemini URL was the entire provider delta. Prompts, request bodies, `responseMimeType`, parsing, the suggestion contract, quota consumption/refund, provider-error classification and the 90-second / zero-retry Gemini transport (permanent since 2026-09-19, C46) are all untouched. There is **no per-model API key** — every Gemini model is served by the same existing server-side `GEMINI_API_KEY`, sent the same way (`x-goog-api-key`), and provider credentials never reach the browser. *(Written for 001B. Two things have since changed and are current: since the 2026-09-17 Phase 6 deploy, Gemini request bodies also carry PaperLume's reasoning level, which a routed model resolves from its own catalog row (§4.9c); and since 001C there is a per-**provider** credential name — `GEMINI_API_KEY`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY` — so routing to Anthropic or OpenAI changes the endpoint, the request envelope and the credential read, not just a URL segment. There is still no per-**model** key, and no generic shared key. Provider credentials remain server-side on every path.)*
...
**Current AI provider architecture — read this before the chronology below.**
...
> - **Six enabled and selectable models** in the catalog established by the migration chain: four Gemini (3.5/3.6/3.7/3.8 Flash), `anthropic/claude-sonnet-5`, and `openai/gpt-5.6-terra`.
...
> - **Data flow** — which provider receives research content, and what it receives — is [privacy-data-flow-audit.md](privacy-data-flow-audit.md) §8 and is not restated here.
>
> **Production versus repository.** The bullets above are repository evidence. The dates and Production steps recorded in §§4.9a–4.9c are the **deployment record** ([deployment.md](deployment.md)), not a live check performed while writing this section.

### 4.9a Provider-adapter seam — deployed 2026-09-17 (AI-MULTI-PROVIDER-001A, C39)
...
- **At 001A completion, `google` was the only registered provider.** [`_shared/aiProviderRegistry.ts`](../supabase/functions/_shared/aiProviderRegistry.ts) held one entry. There was no Anthropic or OpenAI entry, no placeholder that throws "not implemented", no endpoint and no credential accessor for either — an unimplemented provider is **absent**, because a stub is something a later edit can finish by accident. **Since superseded:** 001B added the Anthropic and OpenAI adapters without registering them (§4.9b), and 001C registered `google`, `anthropic` and `openai` in `main` (§4.9c); the absent-not-stubbed rule still holds for any provider without a reviewed adapter. The registry is typed as a map from each provider id to the adapter *for that provider*, and an adapter's `generate` accepts only its own provider's models, so pairing the Google adapter with a model resolved for another provider is a compile error rather than a request.
...
- **The transport policy did not move and was not generalised.** The adapter calls `_shared/geminiTransport.ts`, whose 90-second / zero-retry policy is untouched and is explicitly *not* asserted to be correct for any future provider. That policy is Gemini-specific and **permanent** as of 2026-09-19 (C46 in [decisions-and-triggers.md](decisions-and-triggers.md)); Anthropic and OpenAI keep their own independent 60-second constants.
- **All four selectable models remained Gemini at 001A**, served by the one existing `GEMINI_API_KEY` (the two paid models arrived with 001E — §4.9c). No new secret, no migration, no catalog row and no Settings change was part of this work, and a future catalog row stays unusable until a real adapter is registered **and** given its own credential name.
- **Deployed with Phase 6 (2026-09-17).** 001A left every provider request, user-visible response and quota/refund outcome unchanged — its one deliberate difference is a log-only privacy hardening (C39). It stayed repository-only through 001B–001D and went live when **both** generation functions were deployed together (see [deployment.md](deployment.md) §7c for the bundle closure, which includes the three `_shared` modules it added).

### 4.9b Anthropic and OpenAI adapters — 001B checkpoint: implemented, deliberately unregistered (AI-MULTI-PROVIDER-001B, C40)

`AI-MULTI-PROVIDER-001B` added two complete provider protocols to the repository and made neither reachable. This section records that checkpoint; 001C has since registered both, and §4.9c is the current state.
...
- **At 001B completion, registered and routeable: `google` only.** The registry described in §4.9a was unchanged: it held one entry and imported neither new module. A catalog row naming `anthropic` or `openai` fell back with `unsupported_provider`, and the Settings provider filter (§4.10) offered Google rows only. Both providers were implemented, so they were unregistered rather than absent.
- **Why 001B left them unregistered.** Both intended models reason by default: Claude Sonnet 5 runs adaptive thinking at effort `high`, and GPT-5.6 Terra reasons at effort `medium`. PaperLume intended its own per-operation policy, which did not exist yet, so registration waited for `AI-MULTI-PROVIDER-001C`.
- **Catalog at 001B completion:** Gemini rows only; no Claude Sonnet 5 or GPT-5.6 Terra row existed yet. Both were added later by `AI-MULTI-PROVIDER-001E` (`20260917201856`) and opened to users at Phase 8 (`20260918210017`, 2026-09-19) — see §4.9c.
...
- **User-visible behaviour:** unchanged. Production then still ran the pre-001A Google-only artifacts.
- **State immediately after 001C (historical checkpoint; superseded by §4.9c, which is authoritative for current state).** `google`, `anthropic` and `openai` were all registered in `main`, and the Settings provider-family filter (§4.10) included all three. PaperLume's own per-operation reasoning policy existed (C41). Anthropic and OpenAI were still **not live at that point**, and registration was no longer the reason: no Anthropic/OpenAI catalog row existed and neither credential was installed, although the deployed runtime had contained both registered adapters since the 2026-09-17 Phase 6 deploy. Both became live later, through `AI-MULTI-PROVIDER-001E` — see §4.9c.

### 4.9c Model-aware reasoning policy and provider activation — schema, frontend and generation Edge LIVE; paid providers LIVE since 2026-09-19; manual reasoning LIVE since 2026-09-19 (AI-MULTI-PROVIDER-001C, C41; AI-MANUAL-REASONING-001, C45)

`AI-MULTI-PROVIDER-001C` gives PaperLume its own reasoning policy, registers the two adapters 001B held back, and staged every user-facing part of it **off**; each of those locks has since been released by its own separately authorized step, the last of them on 2026-09-19.
...
- **LIVE in Production since 2026-09-19.** 001C shipped both manual-reasoning locks on — `reasoning_selectable = false` on every row and the setter RPC granted to **no role** — and `AI-MANUAL-REASONING-001`'s `20260919075655` (C45) released them together. Today all **six** current models expose their catalog-defined manual levels, and `set_current_user_ai_reasoning` is executable by `authenticated` only. Entitlement is unchanged and remains `can_select_ai_model`: the same capability as model selection, with no second flag. A manual level still requires a **pinned model** — PaperLume's own default permits no manual level, because the default can move server-side — and **Automatic remains the first and recommended option**, unchanged per model and per operation, applying to both operations when a manual level is chosen instead. Activation backfilled nobody, changed no Automatic level, added no second entitlement, and left the column DEFAULT `false`, so a **future** model starts `reasoning_selectable = false` until its own reviewed migration opens it. The bounded Production acceptance on 2026-09-19 verified the whole path — preference → resolver → reasoning policy → adapter → provider → telemetry — across **all three provider families** ([deployment.md](deployment.md) §15.2).
- **Providers registered: `google`, `anthropic`, `openai`.** The Settings provider filter moved with the runtime registry. At 001C this added **no model option** — no Claude Sonnet 5 or GPT-5.6 Terra catalog row existed and neither provider's secret was installed. **Both have since arrived** (`AI-MULTI-PROVIDER-001E`): the rows were staged on 2026-09-18, both credentials were installed, the Phase-7 canaries passed, and Phase 8 made the rows `selectable` on 2026-09-19 — so the filter now really does offer six models. Nothing in this file or in the Settings code changed to make that happen, which is the property §4.9a describes. Each provider reads only its own credential, and there is no generic key.
...
- **Schema, frontend and generation Edge live.** Migration `20260912120000` was applied to Production on 2026-09-12, **before** the 001C pull request merged, as the rollout order requires ([deployment.md](deployment.md) §6.6). PR #280 then merged on 2026-09-13, and the automatic Vercel deployment put the 001C frontend — the Reasoning level control and account export v3 — live on `app.paperlume.app`. Neither step activated anything: at that point every staging lock above was still in place, so the control showed Automatic and offered no manual choice. The generation Edge Functions followed on **2026-09-17** (Phase 6: `analyze-paper` v27, `suggest-paper-organization` v11), so Gemini requests now carry the Automatic level. Anthropic and OpenAI were registered in the deployed runtime but were not yet live providers at that point. `AI-MULTI-PROVIDER-001E` (C43) then completed them in three authorized steps: migration `20260917201856` was applied on **2026-09-18**, adding `anthropic/claude-sonnet-5` and `openai/gpt-5.6-terra` as `enabled` but **not `selectable`**, and both credentials were installed the same day; the Phase-7 Production canaries passed on both providers (Analyze and Suggest each, one attempt); and Phase 8 (`20260918210017`) was applied on **2026-09-19**, setting `selectable = true` on exactly those two rows. **Both paid providers are now live and choosable by entitled users** — selection stays gated on `can_select_ai_model`, manual reasoning was opened catalog-wide the same day by `AI-MANUAL-REASONING-001` (C45, `20260919075655`), and the system default remains Google (C34).

### 4.9d Provider usage and cost-estimate telemetry — schema, writer and collection LIVE since 2026-09-17 (AI-MULTI-PROVIDER-001D, C42)
...
- **`ai_provider_usage_events`** — one server-written row per AI operation that reached a provider: operation, provider, public model, reasoning decision, bounded outcome, real provider attempt count, provider-reported tokens (input with disjoint cached and cache-write subsets, output with a reasoning subset, and the provider's own total) and a cost estimate. NULL means **not known**, never zero.
- **The money column is a list-price estimate** (`list_price_estimate_usd`, exact `numeric`) at the provider's published standard paid-tier rate. It is not an invoice or actual spend, and it can be non-zero for a request that cost nothing on a provider free tier. `cost_status` says whether it is complete, a lower bound (retries, unmodeled usage), or absent because usage or pricing is missing.
- **Prices are versioned and snapshotted.** An append-only, effective-dated price book in code supplies rates; every estimated row stores the record id **and** the rates it used, so a later price change never re-prices history. At 001D launch only the four routeable Gemini models were priced, and Anthropic and OpenAI were deliberately left unpriced until their staging phase. `AI-MULTI-PROVIDER-001E` then added reviewed records for `anthropic/claude-sonnet-5` and `openai/gpt-5.6-terra` (C44), and the successful Phase-7 paid-provider telemetry was estimated against exactly those records — so all six catalog models are priced today.
...
- **Schema live since 2026-09-13; collection live since 2026-09-17.** Migration `20260913120000` was applied to Production on 2026-09-13, and the Privacy Policy disclosure is published (effective September 17, 2026). The table held no rows until the separately authorized Phase 6 deploy of both generation functions on 2026-09-17 put the provider-neutral runtime and its writer live ([deployment.md](deployment.md) §6.6a, §6.7). The bounded acceptance that followed recorded three events: two Gemini successes with provider-reported usage and list-price estimates, and one Google HTTP 503 with unknown usage and no estimate. Telemetry remains accounting only — it holds no entitlement, quota or pricing authority, and no dashboard reads it. Google was the only reachable provider when collection first went live. Claude Sonnet 5 and GPT-5.6 Terra later passed their Phase-7 canaries (2026-09-18) and were opened to users by Phase 8 (2026-09-19), so **all three provider families are live today**. Choosing a paid model remains gated on `can_select_ai_model`, and manual reasoning went live on 2026-09-19 when its activation migration was applied (C45), so telemetry now records a manual `reasoning_source` when a user has chosen a level. Reasoning level changes **no** quota or billing behaviour either way: one successful AI operation is one PaperLume quota unit at every level, and telemetry stays accounting only.
...
- **The catalog supplies the options.** `ai_model_catalog` is read for `id, provider, display_name, enabled, selectable, sort_order`, ordered `sort_order` then `id`, and filtered to `enabled AND selectable` rows from provider families the shipped UI can route to. That provider boundary — `google`, `anthropic` and `openai` since 001C moved it with the runtime registry (§4.9c), `google` alone before that — names *providers*, never models, so it is not a duplicate model allowlist; it mirrors the runtime's own `unsupported_provider` refusal. The catalog held only Google rows until 001E, so until then the control offered only Gemini models; since Phase 8 (2026-09-19) it holds six `enabled AND selectable` rows and the control offers all of them — four Gemini plus Claude Sonnet 5 and GPT-5.6 Terra — to entitled users, with no frontend change required.
...
2. **Server-side AI quota enforcement inside `analyze-paper`.** ✅ **Implemented, applied, and deployed.** Migration `20260521020000_add_ai_quota_rpcs.sql` is applied to Production and the `analyze-paper` Edge Function is deployed. `consume_ai_quota` is called before the routed provider; on `allowed=false` the function returns HTTP 402 `Payment Required` with a structured body and invokes no provider at all. `refund_ai_quota` is called best-effort if the provider call or response parsing fails after a successful consume. See [migration-history.md](migration-history.md) under "Commercial foundation — AI quota enforcement".
