...
9. **Privacy + AI disclosure + support links.** Two of the three halves are done. **Account deletion** shipped in PFA-C04 (the `delete-account` Edge Function plus the Settings → Danger zone surface). The **Privacy Policy** shipped in PAPERLUME-PRIVACY-001B — app-served at `/privacy` and linked from the sign-in page, so it needs no marketing site (§11). What remains is the AI-disclosure and support URL wiring, which still depends on the marketing site.
...
- **In-app surface.** *Implemented today:* one **Privacy Policy** link in the legal footer of the sign-in page (`/auth`), pointing at the in-app `/privacy` route. There is **no Settings → Privacy link**, and none is required — the policy is public, so it does not need an authenticated surface to be reachable. *Still unimplemented:* "Terms" and "Support" links out to the external URLs, and the "AI disclosure" line at the Analyze action.
