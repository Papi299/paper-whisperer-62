...
### 4.9c Model-aware reasoning policy and provider activation — schema, frontend and generation Edge LIVE; paid providers LIVE since 2026-09-19; manual reasoning still staged off in Production, activation PREPARED (AI-MULTI-PROVIDER-001C, C41; AI-MANUAL-REASONING-001, C45)
...
- **Staged, not live in Production.** `reasoning_selectable = false` on every row, and the manual-reasoning setter RPC is granted to **no role**. No user can create a manual level yet. The migration that flips both together is `AI-MANUAL-REASONING-001`'s `20260919075655` (C45) — **prepared in the repository and not applied**, so this bullet still describes Production exactly. It opens all six rows at once and grants the setter to `authenticated` only; it backfills nobody, changes no Automatic level, adds no second entitlement, and leaves the column DEFAULT `false` so a future model starts closed.
