...
**Verification status.** Repository implementation and deterministic local verification only. **Neither 3.7 nor 3.8 has been provider-verified**, in Production or anywhere else — `AI-MODEL-SELECTION-001D` made **no** Gemini request of any kind, and the migration is **not applied to Production**. Production provider-path verification for both models remains **pending** until after the catalog migration is rolled out and the owner intentionally selects each. That pending state belongs to 3.7 and 3.8 alone: C34's own explicit **3.6** preference path was separately Production-verified on 2026-09-03, for both Suggest and Analyze, once the 001C Settings control went live — see C34's Verification status. Nothing about that verification says anything about 3.7 or 3.8, neither of which has been called.

**Re-evaluation trigger:** a Google deprecation, shutdown date or pricing change affecting either model; a documented request-contract change that Paperlume's `generateContent` call would no longer satisfy; sustained provider-error evidence for either model once they are live; the first successful owner-driven provider-path verification of 3.7 or 3.8 (which closes the pending item above); or an owner decision to move the system default onto one of them, which would be a C34 change rather than a catalog change.
