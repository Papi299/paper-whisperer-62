...
**Account export.** `ai_model_catalog` is **permanently** out of the account export — it is global product metadata, not account data. `user_ai_preferences` is **deferred, not disclaimed**: a saved preference is genuinely the user's own setting and belongs in a portability archive, but while the preference is unreachable (no control, no shipped caller, table empty for every account) including it would add an always-`null` file to every archive. `AI-MODEL-SELECTION-001C` must promote it to an export category when it makes the preference settable. Both classifications are recorded in `ACCOUNT_EXPORT_EXCLUDED_TABLES` and pinned by test, so the promotion is a deliberate edit rather than a silent one.

**Consequence:** `AI-MODEL-SELECTION-001A` — one additive migration (`20260902120000_add_ai_model_selection_foundation.sql`), regenerated Supabase types, the additive `canSelectAiModel` on `useCurrentUserAccess`, the account-export classification above, database and unit tests, and documentation. **Runtime behavior is unchanged:** `analyze-paper` and `suggest-paper-organization` still resolve the model solely through the global `GEMINI_MODEL` configuration and `supabase/functions/_shared/geminiModel.ts`, neither reads the new tables, and the current 90-second / zero-retry Gemini transport (PR #265) and Production `gemini-3.6-flash` selection are untouched. **No Settings control exists yet.** No Production migration, Edge deploy, secret change or provider request is part of this work. Runtime routing is `AI-MODEL-SELECTION-001B`; the Settings control is `AI-MODEL-SELECTION-001C`.
