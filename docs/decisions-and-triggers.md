...
- **`paper_tags` / `paper_projects` keep `SELECT, INSERT, DELETE`.** Today's browser only reads them and every mutation goes through a SECURITY DEFINER RPC, so the grants are arguably surplus — but narrowing them is a change to the repository's established contract, not convergence toward it, and it is out of scope here.
...
- **Provider-specific transports need not share a retry policy.** The Google adapter calls [`_shared/geminiTransport.ts`](../supabase/functions/_shared/geminiTransport.ts) and 001A neither changed its constants nor generalised them: the temporary `AI-PROVIDER-90S-PROD-DIAGNOSTIC-001A` policy (90 s per attempt, zero retries) is untouched, and it is **not** asserted to be right for any future provider. Status semantics, `Retry-After` handling, idempotency and streaming differ between providers, so a shared *adapter contract* is claimed here and a shared *transport policy* is deliberately not.
...
- **No Production data flow exists to either provider.** No shipping Edge Function imports either adapter module, so neither is in any deployment closure. Production still runs the pre-001A Google-only artifacts, and no request has been sent to Anthropic or OpenAI.
...
**Staging — nothing is activated by this decision.** Five locks apply, and each alone keeps manual reasoning and non-Google providers unreachable:
...
5. No Edge Function is deployed. Production still runs the pre-001A `analyze-paper` v26 and `suggest-paper-organization` v10.
...
- **One event per provider-call sequence, not per HTTP request.** An event exists exactly when `generateWithRegisteredAiProvider` was called. `provider_attempts` (1–10) counts the real requests inside it, so the schema stays correct whether a transport retries or not; the current Gemini 90 s / zero-retry diagnostic is neither assumed nor changed. A request refused before the provider — missing auth, a malformed body, a title-only or foreign paper, a PaperLume `402`, a missing provider credential — records nothing: those are not provider usage.
...
- **Provider mappings** (first-party documentation, read 2026-09-13):
  ...
  | Google | `totalTokenCount` | provider total | the REST reference says prompt + thoughts + candidates and the proto comment says prompt + candidates, so either sum (plus tool-use prompt tokens) is accepted and any other value rejects the report |
...
- **Absence has a protocol meaning only on Google.** Every `UsageMetadata` count is a proto3 `int32` with implicit presence, and ProtoJSON omits a default value, so inside a present `usageMetadata` a missing count is the wire spelling of 0. Two guards make that honest: the prompt and total must be positive, and the total must equal its parts, which is what stops a renamed or missing output count from reading as a false zero. On Anthropic and OpenAI (JSON APIs) absence is **unreported**, never zero. A missing usage block is no report on every provider, and a network error, timeout, HTTP error or unreadable body always carries **no** usage.
...
**Staging.** 001D is repository-only. Migration `20260913120000` is **not applied** to Production, no Edge Function is deployed (Production still runs the pre-001A generation runtime, which writes no telemetry), and no telemetry UI exists. Paid providers and manual reasoning remain staged off exactly as C41 left them. Activation needs the migration (a separately authorized step, [deployment.md](deployment.md) §6.7) before the generation Edge deploy, and an owner/legal review of the public Privacy Policy before telemetry is live.
...
**Scope, explicitly.** No Production migration, Edge deployment, secret, live provider call, paid-provider catalog row, reasoning activation, quota or billing change, telemetry UI or external analytics service. The Gemini transport diagnostic, prompts, parsers, suggestion caps and output ceilings are untouched; the Google request bytes are unchanged (golden SHA-256 `e26b9bca…`).
...
- Prompt caching or tools are introduced: the unmodeled-usage flags start firing and the cache-write classes need real rates.
