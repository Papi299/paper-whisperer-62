...
**No Settings control exists yet**, so with no UI an ordinary user does not newly encounter this capability; only an already-existing server-side preference now affects AI execution. No Production migration, Production database write, Edge deploy, secret change, `GEMINI_MODEL` change or provider request is part of 001B. The Settings control is `AI-MODEL-SELECTION-001C`.
