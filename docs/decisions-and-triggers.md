...
**Consequence:** No Supabase plan, billing, or Auth setting changes are authorized during development by this decision. PFA-C08's database hardening remains a separate track: migration `20260810152125_harden_remaining_function_search_paths` must still be merged, deployed to Production under separate authorization, and verified. Once that database remediation is deployed and verified, PFA-C08 may close for the current development scope; leaked-password protection becomes an explicit **commercialization prerequisite**, not an unresolved development blocker.
