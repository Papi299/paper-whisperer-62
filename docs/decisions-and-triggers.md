...
- Migration `20260913120000` has been **live in Production since 2026-09-13**, and a read-only check on 2026-09-17 found the table present and empty.
- The owner-approved **Privacy Policy disclosure is published**, effective September 17, 2026.
- **Runtime telemetry is not live.** No generation Edge Function is deployed: Production still runs the pre-001A `analyze-paper` v26 and `suggest-paper-organization` v10, which write no telemetry.
- No telemetry UI exists.
- Paid providers and manual reasoning remain staged off exactly as C41 left them.

The next step is the separately authorized deploy of both generation functions, followed by a bounded Production telemetry canary ([deployment.md](deployment.md) §6.6a, §6.7).
