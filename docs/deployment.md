...
What to expect today: a `provider_status=` warning is **terminal** and will be followed by a refund and a neutral provider-unavailable failure, never by `outcome=ok` from the same sequence. That is the policy working as designed, not a regression — the user pays nothing, because the unit is refunded. Re-run the check; a second transient `503` on a retry is the provider being unavailable, not this deployment failing. A real failure of *this* deployment is a non-200 whose logs show something other than a provider class — a contract, auth, quota or parsing error.
