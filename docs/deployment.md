...
Rotation takes effect on the next function invocation **because the in-memory OAuth token cache is keyed by a non-sensitive credential identity** — the service-account email, the project id, and a SHA-256 fingerprint of the private key. When any of those changes, the identity changes, the cached token is not reused, and a fresh token is minted from the new credential on the next call. (The raw private key is never used or stored as a cache key.) The provider-response cache is likewise keyed by project + configured model, so a project/model change is never served stale. No code redeploy is needed. **Delete the old key in Google Cloud** after confirming the panel still returns data. Never place Monitoring credentials in any `VITE_`-prefixed variable, the client bundle, a PR description, or a commit.
...
- **Daily usage** sums DELTA points from the current **Pacific-day** boundary (DST-safe; resolved via the wall-time→UTC fixpoint, not by subtracting wall-clock elapsed time).
- **Minute usage** is the value of the **newest complete 60-second bucket** (requested with 60s DELTA alignment over a short lookback) — never a multi-minute sum. When no complete bucket exists, usage is null (never fabricated as zero), and `remaining` stays null unless both usage and limit are known for a reliable window.
- **Fail-soft:** missing credentials, a disabled API, an HTTP failure, or a timeout yield a bounded `status: "unavailable"` result (HTTP 200) with no raw Google body — the panel is observational only and never blocks user analysis. Values are approximate and may lag; do not present them as real-time or guaranteed.
