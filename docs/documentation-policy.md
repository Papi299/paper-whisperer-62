...
| [chrome-web-store-readiness.md](chrome-web-store-readiness.md) | Chrome extension distribution: Store policy audit, privacy/data-flow record, packaging contract, real-browser verification and its limits, the mandatory manual release gate, and the outstanding listing gaps |
| [chrome-web-store-listing.md](chrome-web-store-listing.md) | The Chrome Web Store **submission package**: listing copy, single-purpose statement, permission justifications, drafted privacy-practices answers, the listing images and their provenance, and the distribution paths. What a human would type into the Developer Dashboard — the readiness document above is the audit behind it |
| [privacy-data-flow-audit.md](privacy-data-flow-audit.md) | Product-wide privacy and data-flow inventory: what is collected, where it is stored and processed, which third parties receive it, retention and deletion evidence, and the classified list of owner/legal inputs a Privacy Policy still needs. **Factual authority only** — it does not own the published policy wording |
| [`src/pages/Privacy.tsx`](../src/pages/Privacy.tsx) (the public `/privacy` page) | The **owner-approved published Privacy Policy copy**. Its wording is legal text, not product copy: it is changed only on owner approval, never edited to match an implementation. If source and policy disagree, raise the discrepancy — do not reword the page |
