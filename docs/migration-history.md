...
- **One migration:** `20260904120000_add_recoverable_attachment_cleanup_queue.sql`. It creates two tables (`public.attachment_cleanup_queue`, `public.attachment_cleanup_tombstone`), two indexes, two RLS policies, four `SECURITY DEFINER` functions executable by `authenticated` (`delete_attachment_with_cleanup(uuid)`, `delete_papers_with_attachment_cleanup(uuid[])`, `finalize_attachment_upload(uuid, text, text, text, integer)` and the Storage-policy predicate `attachment_object_has_live_metadata(text)`), one `SECURITY DEFINER` trigger function plus its BEFORE INSERT trigger on `paper_attachments` (`reject_attachment_over_cleanup_intent()` / `trg_paper_attachments_block_cleanup_intent`) and one `SECURITY INVOKER` internal helper (`attachment_cleanup_path_is_safe(uuid, text, uuid)`). It replaces exactly one existing object — the `attachments_owner_delete` Storage policy, which keeps its name and its owner-prefix condition and gains one more (see below) — and narrows exactly two existing grants: `INSERT`, `UPDATE`, `DELETE` and `TRUNCATE` on `public.paper_attachments`, and `DELETE` and `TRUNCATE` on `public.papers`, are revoked from `authenticated`, `anon` and `PUBLIC` — leaving `SELECT` on the former and `SELECT`/`INSERT`/`UPDATE` on the latter (see below). It re-declares one existing function, `merge_exact_duplicates`, for a single added `LOCK TABLE` statement and no behaviour change. Otherwise it alters no existing table, column, trigger or policy, backfills and repairs no row, and touches no Storage *object*. The whole file runs inside an explicit `BEGIN … COMMIT` and opens with `LOCK TABLE auth.users IN SHARE ROW EXCLUSIVE MODE`, then `LOCK TABLE public.papers IN SHARE MODE`, then `LOCK TABLE public.paper_attachments IN ACCESS EXCLUSIVE MODE`. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C37**.
