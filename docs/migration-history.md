...
**Final review gaps closed (RECON-METADATA-PARITY-001B, same migration file, still local-only/unapplied):** three residual review items were resolved without changing any canonical decision. (1) The `projects.updated_at` **whole-row routine-consumer scan** was completed: beyond a qualified `projects.updated_at` reference or `updated_at` used against the projects table, it now rejects any routine that binds a **complete projects row** to an alias (`FROM public.projects p`), a record/`%ROWTYPE`/`SELECT * INTO` variable, a `FOR … IN SELECT * FROM public.projects` loop record, or the bare table name, and then projects, serializes or aggregates that whole row — `to_jsonb`/`to_json`/`row_to_json`/`json_agg`/`jsonb_agg`/`array_agg`, a bare `SELECT <row>` (incl. `RETURN QUERY SELECT <row>`), or `RETURN <row>` — even when `updated_at` is never named. Alias correlation keeps it precise: a routine that merely mentions projects, uses `paper_projects`, or serializes an unrelated scalar (e.g. `merge_exact_duplicates`, which uses `paper_projects` and `array_agg` over scalar subqueries) is **not** matched. (2) The full owner/ACL routine fingerprint is now enforced **state-specifically** — S1 must equal `1f658861…` and S2 must equal `c0b444c8…` (the alternate environment's image is rejected), superseding the earlier "matches one of the two baselines" acceptance; behavioral `c0c14ef5…` remains common to both states and Phase C re-proves the state-specific full fingerprint. (3) The PR description was consolidated into a single authoritative report (removing the superseded 16-case suite and synthetic-S2-fixture claims). Verified: clean replay applies all 65 migrations as an S1 no-op → canonical; S2 re-validated read-only against the live linked project (state S2, routine count 23, behavioral `c0c14ef5…`, full equals **only** `c0b444c8…`, seven bare indexes absent, no duplicate triggers, zero-NULL `study_type_pool.created_at`, all 10 manifest predicates pass, no mutation); the existing 31-case adversarial matrix still all-correct plus **14 new end-to-end cases** (8 whole-row consumers failing in Phase A before any DDL, 4 whole-row negative controls the detector does not flag, 2 cross-state fingerprint rejections) and two direct rollback-safe harnesses (a whole-row detector isolation proof — 9 positive idioms matched, 5 negative controls incl. `merge_exact_duplicates` not matched — and a cross-state predicate proof rejecting the alternate-environment fingerprint in both directions), with the database returning to canonical S1 (`CANONICAL-LOCAL-STATE-RESTORED`). Application validation re-run in full: lint 0 errors, Vitest 300 passed, node typecheck passes, application typecheck unchanged at the pre-C25 baseline (48 diagnostics, 0 new), build passes, Playwright 9/9, zero residual E2E artifacts.

## 2026-07-20 — RECON-METADATA-PARITY-001C: final reconciliation migration deployed; reconciliation complete

**Deployment of `20260719162013_reconcile_metadata_parity.sql`** (PR #156, decision C24/C26). PR #156 was merged into `main` with a regular merge commit **`4f26c85d6efb69f7a9163585e256c8a70bca50be`** (parents `6dcbd2c…` old main, `ba09509…` approved head), which auto-deployed to Vercel production (`app.paperlume.app`). The migration was then applied to the linked project exactly once via `supabase db push` — classified **S2** (production), so Phase B converged only the **eight `created_at` defaults** from `timezone('utc', now())` to `now()` (all other items already canonical in production) and Phase C confirmed the canonical end state. The aligned migration ledger is now **65** rows, all Local = Remote, last `20260719162013`. Pre/post read-only fingerprinting proved **zero object diffs and zero data diffs** across all eight touched tables (row counts unchanged: papers 730, projects 46, tags 107, keyword_pool 623, synonym_pool 65, keyword_exclusion_pool 105, study_type_pool 23, study_type_exclusion_pool 13). The post-deploy `supabase db diff --linked` residual is entirely the two approved exclusions — the benign `papers.search_vector` textual-expression difference and the SEC-4 default-grant shadow-database artifact — with zero unexplained statements. Production-backed Playwright 9/9 passed with clean cleanup and zero residual `_e2e_` artifacts. **Schema reconciliation is complete** (all five RECON tasks merged, applied remotely, and verified).

## 2026-07-21 — TYPESCRIPT-BASELINE-001: authoritative Supabase types committed; TypeScript baseline restored (completed end-to-end, PR #157 merged)

**Completed end-to-end and merged.** PR #157 was merged into `main` as a **regular merge commit `2161ba6e831bccbd0c37050f39ceb931cd1400a2`** (2026-07-21T06:27:06Z; parents `4f26c85d…` old main and approved feature head `9abd989e4d71c38e8d75ca7b67de8ab2f7553861`) — no squash or rebase, all **five** feature commits preserved and individually addressable (`4064eb2`, `f58c224`, `347b323`, `7454bde`, `9abd989`). Cumulative scope 30 files, +1608/−230; the authoritative generated-types blob on merged `main` is `97534296d6f32bc6df45abdeba76bd76afd97b5b`. No migration, no remote database mutation, no CI workflow, and no branch-protection change. With reconciliation complete, the committed `src/integrations/supabase/types.ts` was regenerated from the reconciled schema. Freshly generated **local** and **linked** `public`-schema types are semantically identical (only the linked-only `__InternalSupabase` client-version header differs), and the committed file now equals the linked output — replacing a stale partial snapshot that had omitted seven real tables (`paper_attachments`, `subscriptions`, `subscription_events`, `usage_counters`, `usage_credits`, `user_entitlements`, `user_storage_usage`), several RPC functions, and papers columns, and had modelled the jsonb array columns as `string[]`. The regeneration confirms the reconciliation results in the types: `projects` has no `updated_at`; `study_type_pool.created_at` is non-nullable; junction tables use the composite-key model; `papers.statistical_methods` is `Json`. The former application TypeScript baseline (~48 diagnostics under `tsc -p tsconfig.app.json`) was then eliminated to **0** without weakening type safety — 11 diagnostics resolved directly by the authoritative types (previously-missing RPC/table shapes) and the remainder by root-cause source fixes (typed PostgREST query-builder boundaries, a runtime-validating `parseDuplicateGroups` RPC parser, threading filter/sort params into the project/tag mutation hooks to fix optimistic-cache targeting, correcting the `PoolsContext` async signatures and the `DuplicateGroup.match_type` domain union, and stale test fixtures). Added the truthful `typecheck` / `typecheck:app` / `typecheck:node` npm scripts; `npm run typecheck` passes with 0 diagnostics for both projects (Node diagnostics were and remain 0).

Two reviewed **remediation commits within the same PR #157** (not separate PRs) then hardened the runtime type boundaries: **TYPESCRIPT-BASELINE-001A** (`7454bde`) made the duplicate-scan RPC JSON runtime-validated, enforced an **at-least-two `DuplicatePaperSet` type invariant** on duplicate groups (raw `"both"` rejected, malformed/under-two groups discarded), added **mandatory Playwright project/tag persistence assertions** (replacing an informational check that could never fail), and added focused tests proving the **project/tag active-cache-key correction** (the mutation hooks now derive the papers-list cache key from the supplied filter/sort params); **TYPESCRIPT-BASELINE-001B** (`9abd989`) replaced the first-match merge with a **connected-component-safe (union-find) duplicate consolidation** so transitive bridges collapse into one group with every paper ID globally unique, plus a fail-closed uniqueness guard at the merge mutation boundary. Final validation on the approved head: **lint 0 errors**; `npm run typecheck` **0** (application + Node); **Vitest 339 passed across 25 files**; **build success**; **Playwright mutations 9/9** against production. The exact merge commit `2161ba6e` **auto-deployed to Vercel Production** (`app.paperlume.app`) as an automatic Git deployment and reached READY; a production frontend smoke returned HTTP 200 with the app shell, and the post-deployment mutation suite again passed 9/9 with clean `_e2e_` cleanup. The `baseline/typescript` feature branch was deleted locally and remotely after the deployment reached READY. At that point `CI-BASELINE-001` was the next task and had not started; no CI workflow or branch protection existed yet — see the following `CI-BASELINE-001` entry.

## 2026-07-21 — CI-BASELINE-001: required GitHub Actions Validate workflow and main branch protection

The repository's first deterministic CI baseline. A single **`Validate`** workflow (`.github/workflows/validate.yml`; workflow name `Validate`, job and check name `validate`) runs on pull requests to `main`, pushes to `main`, and manual dispatch. On `ubuntu-24.04` with Node 22 (resolved `v22.23.1`) via `actions/setup-node` — npm download cache keyed on `package-lock.json` — it runs, as separate non-suppressed steps: `npm ci`, a tool-version report, `npm run lint`, `npm run typecheck` (application + Node projects), `npm test` (Vitest) and `npm run build`. The production build receives inert public placeholders for the two required `VITE_` variables (`https://example.supabase.co` / `ci-placeholder-publishable-key`); the client env validator only needs non-empty build-time values, so no real project is contacted. Security posture: read-only workflow token (`permissions: contents: read`), no repository secret referenced, no `pull_request_target`, no shell interpolation of untrusted PR text, official actions only pinned to full commit SHAs (`actions/checkout` v7.0.1 `3d3c42e5aac5ba805825da76410c181273ba90b1`, `actions/setup-node` v7.0.0 `820762786026740c76f36085b0efc47a31fe5020`); the workflow cannot deploy or mutate GitHub or Supabase state. **Playwright is deliberately excluded** — the E2E suite is production-backed and no isolated staging project exists, so it must not run in CI and is not a required check. The initial run surfaced one genuine CI-portability issue — `npx tsc --version` before `npm ci` fetched an unrelated package on a clean runner — fixed by reporting tool versions after install so `npx tsc` resolves the project's TypeScript.

`main` is protected (classic branch protection) to require the exact **`validate`** check, bound to the GitHub Actions app, with strict/up-to-date checking. Administrators are included; pull requests are required with **zero** human approvals (no code-owner review, no last-push approval, no signed-commit or linear-history requirement); conversation resolution is required; force pushes and branch deletion are disabled. **Regular merge commits remain allowed** and **Vercel is not a required check**. Because the check is required under strict mode, a PR is not mergeable while its `validate` check is pending or failing on the current head and becomes eligible only after that exact check succeeds on the latest pushed commit — with no human approval or administrator bypass involved.

The first real workflow run (event `pull_request`) passed every step: `npm ci` from the lockfile (508 packages; npm cache populated then saved), lint 0 errors (pre-existing warnings only), `npm run typecheck` 0 diagnostics (both projects), **Vitest 339 passed across 25 files**, and a successful production build. No migration, no database mutation, no Supabase command, no Edge Function deployment, and no manual Vercel deployment occurred; the authoritative generated-types blob is unchanged (`97534296d6f32bc6df45abdeba76bd76afd97b5b`). This fulfils decision C25's CI → branch-protection tail.

## 2026-07-24 — PRODUCT-DIRECTION-RESET-001: launch work paused; feature development becomes the active priority (documentation only)

A documentation-only product-direction change recording owner decision **C27**: public-launch and commercial-launch **implementation** are paused and are no longer on the active critical path, and the active engineering priority returns to product feature and workflow development (new features, completing incomplete user workflows, improving existing functionality). Owner-side Paddle Sandbox setup is no longer described as the immediate next task in any authoritative current-state document. Commercialization is **paused, not cancelled**: the already-implemented entitlement, quota, subscription, usage and storage infrastructure and its security controls are **preserved**, and C17/C18 (Paddle as the future Merchant-of-Record provider) remain the approved future billing direction. The updated Markdown documents are `README.md`, `docs/start-here.md`, `docs/owner-decisions.md` (new C27 row + paused §2/§3 framing), `docs/decisions-and-triggers.md` (new C27), `docs/commercial-architecture.md`, `docs/quotas-and-pricing.md`, `docs/store-launch-checklist.md`, and this entry. No application code, test, package, TypeScript configuration, database, migration, CI configuration, branch-protection, or deployment configuration changed. No manual Vercel action (deploy, promotion, rollback, or redeploy) and no Production mutation occurred; the PR's automatic Vercel Preview was the normal repository integration behavior and represents no product or Production change. The generated-types blob is unchanged (`97534296d6f32bc6df45abdeba76bd76afd97b5b`). The next task is a focused product-feature and incomplete-workflow audit (planning only; no feature selected or implemented in this change).

## 2026-07-24 — PROJECT-TAG-SELECTOR-UX-001: dashboard Project/Tag filters become searchable multi-select; long-name overflow fixed (application only)

The Papers dashboard **Project** and **Tag** filters were converted from single-value Radix `Select` controls into **searchable multi-select** controls (new reusable `SearchableEntityMultiFilter`, built on the existing Popover + cmdk `Command` primitives). Each category now accepts zero, one or many selections with an in-dropdown case-insensitive substring search, per-item toggle, visible selected state, clear-all, keyboard accessibility, and a bounded trigger that shows `All Projects`/`All Tags` (zero), the single name truncated with a full-name `title`/`aria-label` (one), or a compact `N Projects`/`N Tags` count (many) — long names no longer wrap or overflow the fixed-width trigger. **Filtering semantics:** selections **OR within** each category (a paper matches if it belongs to at least one selected Project / has at least one selected Tag), resolved by a single bounded `.in(...)` junction query per category with deduplicated paper IDs; categories continue to **AND across** each other and with keyword/search/year filters, preserving the existing four-state (`null`/`undefined`/`[]`/resolved) intersection contract. Dashboard filter state changed from `selectedProjectId`/`selectedTagId` scalars to `selectedProjectIds`/`selectedTagIds` arrays; React Query junction keys are canonicalized (deduped + sorted) so selection order never spawns redundant cache entries.

Saved filter-presets were upgraded from payload **version 1** (scalar `selectedProjectId`/`selectedTagId`) to **version 2** (`selectedProjectIds`/`selectedTagIds` arrays). Existing version-1 presets still load: `parsePresetPayload` reads them via a retained v1 schema and normalizes them to v2 **in memory** (`null` → `[]`, `"id"` → `["id"]`); a legacy preset is persisted as v2 only when the user explicitly re-saves or updates it. Stale references are dropped **partially** (each missing Project/Tag ID is removed individually; valid siblings are kept) with an accurately-pluralized skip toast, and preset dirty-state comparison treats the Project/Tag arrays as order-insensitive sets. **No database migration and no remote Supabase mutation** were required — `filter_presets.payload` is JSONB, so the payload-version change is schema-transparent; no automatic background DB rewrite occurs. The generated-types blob is unchanged (`97534296d6f32bc6df45abdeba76bd76afd97b5b`). The separate Import-continuation bug (Import Results Summary hiding the assignment controls) is **intentionally out of scope** and remains the next remediation task, `IMPORT-CONTINUATION-WORKFLOW-001`.

## 2026-07-24 — PROJECT-TAG-MATCH-MODE-001: independent Any/All match modes for the dashboard Project and Tag filters (application only)

Each of the dashboard **Project** and **Tag** searchable multi-select filters (from PROJECT-TAG-SELECTOR-UX-001) gained an **independent Any/All match mode**, surfaced as an accessible in-dropdown segmented control (single-mode Radix `ToggleGroup`, `role="radio"` per option, category-specific `aria-label` descriptions) that appears only once two or more items are selected. The closed trigger shows the active mode exactly once for multi-selections (`2 Projects · Any`, `3 Tags · All`); zero- and one-selection triggers are unchanged (Any/All is a logical no-op below two items). The two modes are fully independent — changing one never affects the other — and there is **no** global cross-category OR: Projects and Tags remain ANDed with each other and with keyword/search/year/study-type/notes filters.

**Filtering semantics.** State gained `projectMatchMode`/`tagMatchMode` (`"any" | "all"`, default `"any"`), with guarded setters, in `useFilterState`. `"any"` is the OR-union shipped previously; `"all"` requires membership in **every** selected member. Each category runs exactly **one deterministic paginated junction-query pipeline per active category, never one query per selected Project or Tag** — selecting the entity ID column too (`paper_id, project_id` / `paper_id, tag_id`). The pipeline pages through PostgREST via `src/lib/fetchAllPages.ts` under a **stable composite ordering** (`.order("paper_id").order("project_id")` / `.order("tag_id")`, both ascending) so offset/range pages are contractually stable — rows cannot shift, duplicate, or drop across page boundaries; no arbitrary total-row ceiling is imposed. All raw pages are **concatenated before** the pure helper `resolveJunctionPaperIds(rows, selectedIds, mode)` in `src/lib/filterSets.ts` runs **once** on the complete set: Any unions matching papers (deduped); All groups rows by paper and includes a paper only when its count of **unique** matched selected entities equals the number of unique selected IDs, so duplicate junction rows cannot inflate the count, unselected rows are ignored defensively, and a cross-page membership is resolved correctly. React Query keys include the mode (`junctionQueryKey`), so Any and All never share a cache entry while order-insensitive selections still reuse one; the four-state (`null`/`undefined`/`[]`/resolved) contract is preserved and a page-fetch failure surfaces as the query's error state rather than a partial success. Emptying a category (clear or deselecting the last member, or a global Clear) resets that category's mode to `"any"`, upholding the "no selection ⇒ Any" invariant. **No RPC was added, and no migration, schema change, or remote Supabase action occurred.**

Saved filter-presets were upgraded from payload **version 2** to **version 3**, adding `projectMatchMode`/`tagMatchMode` (parsed with a strict enum — corrupted mode values fail the schema and the row is dropped). Backward compatibility is read-only: a retained **version-2** schema and the existing **version-1** schema both normalize forward to v3 **in memory** with both modes defaulting to `"any"` (the exact semantics those presets had when saved); a legacy preset is persisted as v3 only when the user explicitly re-saves or updates it. On load, each category's saved mode is restored when at least one valid selected ID remains after the stale-reference guard, and normalized to `"any"` when every selected ID in that category was stale (independently per category). Preset dirty-state compares the two modes exactly (scalars), so an Any↔All change reads as dirty while ID reordering does not. **No database migration and no remote Supabase mutation** were required — `filter_presets.payload` is JSONB, so the payload-version change is schema-transparent; no automatic background DB rewrite occurs. The generated-types blob is unchanged (`97534296d6f32bc6df45abdeba76bd76afd97b5b`). The separate Import-continuation bug remains the next remediation task, `IMPORT-CONTINUATION-WORKFLOW-001`.

## 2026-07-24 — IMPORT-CONTINUATION-WORKFLOW-001: Add Papers no longer dead-ends after an import; continuous multi-batch importing (application only)

The Add Papers dialog (`src/components/papers/AddPaperDialog.tsx`) previously hid the Project/Tag **Assign on Import** controls once an import completed and left the completed identifier input and its Import action in place, so the only way to start another batch was to close and reopen the dialog. Both the **identifier-import** and **file-import** flows now transition into an explicit continuation state instead of dead-ending.

- **Identifier-import result summary no longer dead-ends the workflow.** In the completed state the `Import Results Summary` is shown together with the Project/Tag assignment controls; the source textarea and its `Import N Papers` action are hidden, and a primary **Import More** action replaces them.
- **File-import result summary no longer dead-ends the workflow.** In the completed state `File Import Results` is shown together with the assignment controls and a primary **Import Another File** action; the previously parsed file's Import action is not rendered.
- **Project/Tag assignment controls remain available for the next import.** The shared selector is rendered in both states by one in-component `renderAssignSection(context)` (no duplicated Project/Tag selector implementation); after completion it uses the heading **"Assignments for next import"** with the helper text "These selections apply to the next batch, not the completed import."
- **Completed-run and next-run semantics are explicitly distinguished.** Assignments captured when an import starts apply to that run (unchanged `useBulkMutations` behavior); post-completion selections configure only the next run. Changing selections while a summary is shown invokes no import or database call, and there is no "apply to imported papers" action — inserted paper IDs are never returned to the dialog.
- **`Import More` resets identifier-run state only** (`bulkInput`, `bulkRunning`, `bulkProgress`, `bulkResults`) and returns focus to the textarea; **`Import Another File` resets file-run state only** (`parsedFile`, `fileName`, `fileImportRunning`, `fileImportComplete`, `fileImportProgress`, file drag state) and returns focus to the dropzone. Neither closes the dialog, invokes an import callback, or issues a database request.
- **Assignment selections persist across continuation and tab changes**; only a full dialog **close still resets everything** (input, results, parsed file, manual form, Project/Tag selections, active tab back to Import IDs).
- **Accidental re-import of the completed identifier batch is prevented**: the Import action is rendered only in the ready/runnable state, and `handleBulkImport` has a defensive guard that no-ops in the completed state until `Import More` returns to the ready state. A file import that throws no longer marks a partial run complete (completion is set only after the callback resolves), so the parsed file and selections survive a failed run.
- Bounded accessibility corrections on the touched controls: the icon-only badge remove buttons gained category-specific labels (`Remove project <name>` / `Remove tag <name>`), and the continuation actions carry meaningful labels.

A new focused Vitest suite (`src/components/papers/__tests__/AddPaperDialog.test.tsx`, 16 cases) covers both continuation workflows, preserved selections, second-run options, the full close reset, running-state safety, and that post-completion selection changes invoke no callback. **No callback-contract change** (the public `onBulkImport`/`onFileImport`/`onSubmitManual` signatures are unchanged and still never return inserted IDs), **no `useBulkMutations` or RPC semantic change, no schema migration, no remote Supabase action, no generated-type change** (blob remains `97534296d6f32bc6df45abdeba76bd76afd97b5b`), and **no CI or branch-protection change**. `Dashboard.tsx` and `useBulkMutations.ts` were not modified. The product-feature audit, `PRODUCT-FEATURE-AUDIT-001`, remains the next task.

## 2026-07-24 — PFA-C01-AI-QUOTA-UX-001: read-only AI quota status RPC (owner-selected feature)

**Migration:** `supabase/migrations/20260724120000_add_ai_quota_status_rpc.sql` — additive, one new function, no table/policy/quota changes.

- **Function:** `public.get_ai_quota_status(p_user_id uuid)` — `RETURNS TABLE(allowed, reason, plan, plan_status, period_type, used, quota, remaining, reset_at)`.
- **Read-only:** `LANGUAGE plpgsql STABLE SECURITY DEFINER`. Performs **no** INSERT/UPDATE/DELETE/lock/consume/refund and **does not create a missing counter row** (a missing counter is treated as `used = 0`, `remaining` clamped to ≥ 0). It is the read-side companion to the write-side `consume_ai_quota`/`refund_ai_quota` RPCs (`20260521020000`) and mirrors their bucket selection (monthly when `ai_monthly_quota > 0`, else lifetime, else unavailable).
- **S1 ownership guard:** raises `Unauthorized: user mismatch` unless `p_user_id` is non-null, `auth.uid()` is non-null, and `p_user_id = auth.uid()`.
- **Grants:** `EXECUTE` revoked from `PUBLIC` and `anon`; granted only to `authenticated`.
- **No client SELECT on `usage_counters`:** this migration does **not** add any policy to `usage_counters` and does not weaken its `FORCE ROW LEVEL SECURITY`. Quota usage is exposed **only** through this SECURITY DEFINER RPC — honoring the decision recorded in `20260521010000_add_entitlement_usage_schema.sql`. The client never reads `usage_counters` directly.
- **Enforcement boundary unchanged:** the server (`analyze-paper` + `consume_ai_quota`) remains authoritative; this function is advisory display only and can be momentarily stale.

**Local verification (no remote DB):** the migration was replayed into a **local** Supabase stack (`supabase db reset`, aligned ledger of 66 migrations) and validated with a bounded local SQL procedure (13 assertions): own-user lifetime/monthly reads and UTC reset boundary; cross-user rejection (S1); `anon` execute denied; null-`auth.uid()` rejected; read-only + idempotent across repeated calls; missing-counter → `used 0` with no row created; exhausted → `remaining` clamped to 0; inactive and missing entitlements return explicit non-success; and a positive check that a direct client `SELECT` on `usage_counters` is denied. No permanent DB-test framework was added.

**Frontend (companion, same task):** `src/hooks/useAiQuota.ts` (read-only status hook), `src/lib/analyzeError.ts` (pure structured-`402` parser + non-commercial message formatter), `src/components/papers/AiQuotaIndicator.tsx` (header indicator; fails soft), `src/hooks/usePaperAnalysisActions.ts` (single-paper specific quota message + no update on `402`; bulk stops after the first authoritative `402` and reports unattempted papers), `src/lib/queryKeys.ts` (quota query key), and `src/pages/Dashboard.tsx` (wiring). Quota-query synchronization: the single-paper path invalidates only when the Edge Function was actually invoked (a pre-invoke failure such as an abstract-fetch rejection does not); the bulk path invalidates once after the whole run completes. **Quota transparency only — no checkout/paywall/upgrade path; C27 unchanged.**

**Generated types:** `src/integrations/supabase/types.ts` regenerated locally (`supabase gen types typescript --local --schema public`) to add `get_ai_quota_status`; blob `97534296d6f32bc6df45abdeba76bd76afd97b5b` → `43eb1a939070b288d547bdb58e1e393c768814f3`. Not hand-edited (only a trailing blank line normalized to satisfy `git diff --check`).

**Deployment status:** the migration is **committed but NOT applied remotely** while the PR is under independent review. Because the frontend depends on the new RPC, the deploy sequence must **apply this additive migration before** the frontend merge auto-deploys (`supabase db push` per `deployment.md`). No remote Supabase action, no Edge Function change/deploy, and no Gemini/Production access occurred in this task.

## 2026-07-25 — PFA-C01-AI-QUOTA-UX-001A: correct AI quota workflow edge cases (application + docs only; same PR #166)

Frontend-only correction commit on `feat/pfa-c01-ai-quota-ux`. The approved read-only RPC migration `20260724120000` is **unchanged** (signature, S1 guard, `SECURITY DEFINER`, fixed `search_path`, read-only behavior, grants/revokes, bucket selection, and absence of client `usage_counters` access all preserved). No remote DB action, no Edge Function change/deploy, no Gemini/Production access.

- **Single-analysis failure boundary restored.** `handleAnalyzePaper` now wraps the whole operation (`fetchAbstract` → `invoke` → merge → `updatePaper`) in one `catch`. A `fetchAbstract` rejection no longer leaks a rejected promise: it shows one generic destructive `AI Analysis failed` toast (original message preserved), never invokes `analyze-paper`, never updates, and always clears `analyzingPaperId`. Quota is invalidated **only** when the Edge Function was actually invoked.
- **Exactly one bulk quota notification.** The prior double-toast (one inside the loop + one summary) is replaced by a single post-run notification combining the authoritative parsed-`402` allowance/reset detail with the run accounting. No billing/upgrade language.
- **Complete bulk accounting.** The quota-denied paper is counted as **failed**; papers after it are **not attempted**. Invariant `successCount + failCount + unattempted === total` holds (e.g. 1 analyzed / 1 failed / 1 not attempted for a 3-paper run whose 2nd paper hits the wall).
- **Strict `402` validation.** `parseAnalyzeError` returns `quota_exceeded` only when `details` is an object with finite non-negative `used`/`quota`/`remaining`, `period_type ∈ {monthly, lifetime, null}`, and string-or-null `reset_at`/`plan`. Malformed or incomplete payloads fall back to the generic message — a missing/malformed `details` is **never** treated as an authoritative zero-quota response.
- **UTC reset dates.** New shared `formatResetDate` renders `reset_at` as its UTC calendar date (so `2026-08-01T00:00:00Z` is Aug 1, not a timezone-shifted Jul 31) and fails soft on invalid input; reused by `formatQuotaExceededMessage` and `AiQuotaIndicator`.
- **Neutral unavailable wording.** When `period_type` is null (no active AI bucket), the quota message says "AI analysis is currently unavailable." instead of describing a nonexistent "lifetime" allowance.
...
## 2026-07-25 — OWNER-MANAGER-ACCESS-AND-GEMINI-QUOTA-001: internal owner/manager access + AI quota exemption (LOCAL-ONLY — under review)

**Migration:** `supabase/migrations/20260725090000_add_internal_access_and_ai_quota_exemption.sql` — one additive migration (two sections). Timestamp strictly later than the aligned baseline `20260724120000`.

**Status: LOCAL-ONLY — NOT applied remotely.** Created and verified on a clean local replay (`supabase db reset --local` → 67 migrations) as part of the implementation PR `feat: add owner access and Gemini quota monitoring`. **No `supabase db push`, no MCP apply, no remote Supabase mutation, no migration-history change on the linked project.** Remote application is a later, separately-authorized deployment step (see deployment ordering below and `deployment.md`).

- **Part A — internal access:**
  - **`public.internal_user_access`** — server-only role model: `user_id` PK → `auth.users`, `role text` CHECK (`owner`|`manager`), `ai_quota_exempt boolean NOT NULL DEFAULT false`, `created_by`, `created_at`/`updated_at` (canonical `update_updated_at_column` trigger). **ENABLE + FORCE RLS with NO client policy** (server-only, same posture as `usage_counters`); no email / credential / secret / billing column.
  - **`public.get_current_user_access()`** — `STABLE SECURITY DEFINER`, fixed `search_path = public`, **no parameter** (derives the caller from `auth.uid()`; rejects null auth; cannot inspect another user). Returns `role`/`is_internal`/`can_view_provider_quota`/`ai_quota_exempt`/`plan`/`plan_status`/`premium_taxonomy_enabled`/`labs_team_enabled`. Ordinary users (no row) default to role `user`. `EXECUTE` revoked from `PUBLIC`/`anon`, granted to `authenticated`.
- **Part B — AI quota exemption:**
  - **`consume_ai_quota`** (signature unchanged) — an `ai_quota_exempt` user is always allowed (`reason = 'quota_exempt'`), never blocked at the cap, still atomically increments the correct bucket (race-safe; S1 guard unchanged).
  - **`refund_ai_quota`** (signature unchanged) — mirrors the exempt bucket selection (incl. lifetime fallback) so exempt usage is reversed from the same bucket; still floored at 0.
  - **`get_ai_quota_status`** — **additive `is_exempt boolean`** column (DROP+CREATE for the return-type change); exempt users get `allowed=true`/`reason='quota_exempt'` with usage still visible. Non-exempt output is bit-compatible except for the additive column. Grants re-affirmed.
  - Internal role is separate from the commercial plan; runtime authorization is UUID/role-based, never email-based (decision **C28**).
- **Owner bootstrap row is NOT in this migration.** The Production owner grant (`internal_user_access` row + Pro-baseline entitlement for the resolved UUID of the owner account) is a separate, bounded, explicitly-authorized deployment-time transaction — see `deployment.md`. This keeps an environment-specific email out of a migration that runs in every environment.

**Edge Functions (repo-only; NOT deployed):** new `get-gemini-provider-quota` (owner/manager-gated via `get_current_user_access`; server-side Google Cloud Monitoring `monitoring.read`; fails soft; never leaks credentials/tokens/raw Google bodies) with a `supabase/config.toml` entry (`verify_jwt = false`); `analyze-paper` centralizes the model string (Part D) and adds structured provider-error classification with neutral user wording (Part F) — no prompt/schema/retry/timeout/generation change, best-effort refund preserved, provider limits never converted to a Paperlume 402. Pure normalization/classification/model helpers live in `supabase/functions/_shared/` and are Vitest-covered without a Deno runtime.

**Local DB verification:** `supabase/tests/owner_access_and_quota_verification.sql` — one bounded, self-rolling-back transaction asserting all 17 required role + quota cases (role resolution, no direct table read/insert/update by clients, anon/null-auth rejection, Free 15-lifetime + Pro monthly caps unchanged, exempt allowed-beyond-cap + increment + same-bucket refund, missing/inactive safety, `is_exempt` reported, no negative usage, no email-based role check). Passes on a clean local replay. No pgTAP framework added.

**Generated types:** `src/integrations/supabase/types.ts` regenerated from the **local** DB (`supabase gen types typescript --local --schema public`); adds `internal_user_access` + `get_current_user_access` and the additive `is_exempt` on `get_ai_quota_status`. Blob `43eb1a939070b288d547bdb58e1e393c768814f3` → `30f0137ab42c7af8fa97d6c26cff0de71610c07a`. Not hand-edited (only the generator's trailing blank line normalized for `git diff --check`); the linked/Production schema was not used for generation.

**Deployment ordering (later, each Production mutation separately authorized):** approve exact PR head → owner configures/enables Google Monitoring + narrowly-privileged service account (`roles/monitoring.viewer`) + credential → `supabase db push` (this migration) → bounded owner UUID/role/entitlement bootstrap → set Google Edge secrets → deploy `get-gemini-provider-quota` (and `analyze-paper` since it changed) → verify role security + provider data → merge exact head → verify merged-main CI + Vercel Production → owner-account smoke test.

### 2026-07-25 — OWNER-MANAGER-ACCESS-AND-GEMINI-QUOTA-001A: provider-monitoring + runtime-UX corrections (same PR #168)

A single correction commit `fix: correct Gemini quota monitoring edge cases` on the same branch. **The migration `20260725090000` is UNCHANGED** (no DB defect surfaced) — approved internal-role architecture, RLS posture, quota-exemption RPCs, `is_exempt`, C28, and the bootstrap design are all preserved. Still local-only; no remote migration, no owner grant, no secret, no Edge deploy, no Google/Gemini call. Generated types unchanged (blob `30f0137ab42c7af8fa97d6c26cff0de71610c07a`). Corrections (all in Edge helpers / Edge functions / client, with expanded Node-Vitest coverage):

- **One metric type per Monitoring request** — the request plan now issues a single-metric `timeSeries.list` per type (Monitoring rejects OR-ed types), page-followed, results combined after parsing. Extracted into a pure/DI request-plan module.
- **Correct minute aggregation** — minute usage is the newest **complete 60-second bucket** (60s DELTA alignment over a short lookback), never a multi-minute sum; null when no complete bucket; daily still sums over the Pacific day.
- **DST-safe Pacific boundary** — a pure helper resolves midnight America/Los_Angeles via a wall-time→UTC fixpoint (correct through spring-forward/fall-back), not by subtracting wall-clock elapsed time.
- **Structured provider-500 client parsing** — `parseAnalyzeError` recognizes the `analysis_unavailable` body (`provider_rate_limit`/`provider_unavailable`/`malformed_response`/`unknown`); single analysis shows the neutral message, bulk counts it as a non-terminal failure; a provider 429/403/5xx is never a Paperlume 402.
- **Authoritative exempt indicator** — "Unlimited" only when `isExempt && allowed && reason === "quota_exempt"`; role-neutral wording; inactive/missing entitlement renders unavailable even if `isExempt` is set. `useCurrentUserAccess` also clamps `aiQuotaExempt` behind `isInternal`.
- **No fabricated blocked attempts** — the card renders `—` for a null `exceededAttempts` (never `0`).
- **Credential-cache consistency** — the OAuth token cache is keyed by a non-sensitive credential identity (email + project + private-key SHA-256 fingerprint); the response cache by project + model. A rotated credential is used on the next invocation (no stale reuse); the raw key is never a cache key. `deployment.md §13.6/§13.7` updated to match.

### 2026-07-25 — OWNER-MANAGER-ACCESS-AND-GEMINI-QUOTA-001B: Monitoring alignment + cache-identity corrections (same PR #168)

A single correction commit `fix: correct Gemini Monitoring alignment` on the same branch (sixth commit). **The migration `20260725090000` is UNCHANGED** (no DB defect surfaced) — the internal-role architecture, RLS posture, quota-exemption RPCs, `is_exempt`, C28, and the bootstrap design are all preserved. Still local-only; no remote migration, no owner grant, no secret, no Edge deploy, no Google/Gemini call. Generated types unchanged (blob `30f0137ab42c7af8fa97d6c26cff0de71610c07a`). Corrections (all in Edge helpers / Edge function / tests, with expanded Node-Vitest coverage):

- **Correct Monitoring aligner** — minute-window DELTA usage/exceeded now use `perSeriesAligner = ALIGN_SUM` (`alignmentPeriod = 60s`) to total a count within a bucket; `ALIGN_DELTA` is never requested for any metric; GAUGE `/limit` metrics are fetched unaligned and their newest point is selected. The chosen aligner is explicit in the pure request contract (`TimeSeriesRequest.perSeriesAligner`), serialized verbatim by a pure query-parameter builder that Vitest asserts.
- **Synchronized minute buckets** — usage/exceeded values combine only across the newest `interval.endTime` bucket that **every** contributing series shares; values from different minutes are never cross-summed and an absent/forming series is never treated as zero. When no common complete bucket exists, the value is null; a single-series dimension reduces to its own newest complete bucket.
- **Credential-aware response cache** — the response-cache identity now includes the full credential (project + configured model + service-account email + private-key SHA-256 fingerprint); the fingerprint is computed before the cache lookup, so a rotated key / changed service account misses the cache and is exercised on the next invocation. The raw key is never an identity or cache key.
...
### 2026-07-26 — Current Production state + OWNER-MANAGER-ACCESS-AND-GEMINI-QUOTA-001Y provider-quota deferral (C29, repository-only)

Chronological update superseding the "Not deployed / awaiting approval" notes in the two entries immediately above. As of 2026-07-26:

- **Both additive migrations are applied in Production** (`lioxtgiputfniqbktcsz`): `20260725090000` and the grant-hardening `20260726120000`. The ledger is aligned through `20260726120000` — **68 rows**, Local = Remote. No schema change was made by 001Y.
- **Edge Functions deployed:** `fetch-paper-metadata` **v10**, `analyze-paper` **v15**, `get-gemini-provider-quota` **v3**. The owner bootstrap is complete and verified.
- **001Y is a repository-only change (decision C29):** the manager-facing Gemini provider-quota **dashboard is deferred** during the Gemini Free Tier development phase (Google Cloud billing intentionally disabled). The frontend provider-quota card, fetch hook, client library, their frontend-only tests, and the orphaned `geminiProviderQuota` query key were removed; the deployed v3 function and shared backend monitoring modules are **retained as deferred infrastructure** (unchanged blobs). **No migration was added, changed, or applied by 001Y; no remote database, secret, Edge deploy/invocation, or Google/billing/IAM mutation occurred.** Generated types unchanged. See decision **C29** in [decisions-and-triggers.md](decisions-and-triggers.md) and the current-state callout in [deployment.md §13](deployment.md).

### 2026-07-28 — OWNER-MANAGER-ACCESS-AND-GEMINI-QUOTA-001ZZ: PR #168 merged (regular exact-head merge commit)

PR #168 was merged into `main` via a **regular merge commit** (task 001ZZ; the merge was the only mutation, performed at the independently approved head `e1b6ea3b32258e8a3b93710a40d7e509ea2378bd`).

- **Merge commit:** `a1fc2cea53e33c8b34c557c7087236a939bb783c` — a true two-parent merge (first parent `9d4e791f59e4e05903e3355fd0f32b9532fcb5eb`, second parent the approved head `e1b6ea3b…`). Merged 2026-07-28T11:59:48Z. All 11 branch commits and 36 changed files are preserved; the three review threads are resolved.
- **Merged-main CI:** `Validate` run **30357049945** (branch `main`, event `push`, head `a1fc2cea…`) completed **success**.
- **Vercel Production:** deployment **`dpl_Bx1GyYog6KCDUjHtcHTFVWqySwoV`** (target `production`, state **READY**) built from `a1fc2cea…` on `main`; the alias **`app.paperlume.app`** points to that exact deployment.
- **Supabase postflight — unchanged:** 68 aligned migrations through `20260726120000` (Local = Remote); functions `fetch-paper-metadata` v10, `analyze-paper` v15, `get-gemini-provider-quota` v3 (IDs/hashes/ACTIVE/`verify_jwt=false` unchanged); required secret names present, `GEMINI_MODEL` absent.
- **The GitHub merge applied no migration and deployed no Edge Function** — Supabase migrations and Edge Functions are operated separately and did not change during the merge.
- **Provider-dashboard state:** deferred under **C29**; `get-gemini-provider-quota` v3 remains deployed but intentionally unused. **Google Cloud billing remains disabled.** C27/C28/C29 unchanged.

### 2026-07-31 — PFA-C03A1-L-GRANT-PARITY-001: reconcile Data API object grants (`20260731162729`)

Adds **one** additive migration, `20260731162729_reconcile_data_api_grants.sql`, that makes the Data API's table/sequence GRANTs **reproducible on a fresh replay**. **Merged to `main` and applied to Production on 2026-07-31** — see the *Merge and Production deployment* sub-events at the end of this entry. (Originally developed and opened as a PR for independent review; the development-time findings below are preserved as history.)

- **Discovered cause.** The tracked migration chain enables/forces RLS and defines per-user policies but never granted the underlying table privileges to the API roles. Production works because it was provisioned under Supabase's *old* auto-grant default (raw ACL `arwdDxtm` for `anon`/`authenticated`/`service_role` on every `public` table). Supabase is moving to **opt-in Data API exposure**, so a fresh local `supabase db reset` grants the API roles **no** DML — `authenticated` hits `42501 permission denied`, the dashboard cannot read `papers`/`profiles`/etc., and the app is non-functional on a clean replay. Reproduced independently before the fix (all API roles: 0 table DML grants).
- **Least-privilege grant scope (explicit; no `PUBLIC`, no blanket client `GRANT ALL`, no `ALTER DEFAULT PRIVILEGES`).** `authenticated` receives, per table, exactly the operations that table's RLS policy already exposes: full SELECT/INSERT/UPDATE/DELETE on `papers`, `projects`, `tags`, `filter_presets`, `study_type_pool`, `synonym_pool`; SELECT/INSERT/UPDATE on `profiles`; SELECT/INSERT/DELETE on `paper_tags`, `paper_projects`, `paper_attachments`, `keyword_pool`, `keyword_exclusion_pool`, `study_type_exclusion_pool`; SELECT-only on `user_entitlements`, `usage_credits`, `user_storage_usage`; plus `USAGE` on `papers_insert_order_seq` (direct manual-add insert). `anon` gets **no** table DML (every policy predicate is `auth.uid() = user_id`). The server-only tables `usage_counters`, `subscriptions`, `subscription_events`, and `internal_user_access` receive **no** client grant. `service_role` (server-only, RLS-bypassing, never client-exposed) receives an explicit SELECT/INSERT/UPDATE/DELETE object grant on the current application tables (all except `internal_user_access`, already granted by `20260726120000`) plus sequence `USAGE`, matching Production's established server boundary.
- **Local replay + isolation results.** Applies cleanly on a fresh reset and **reproducibly** on a second reset (16 authenticated client tables, 0 server-only leakage to clients, 0 `anon` DML, sequence USAGE present). Functional proof via authenticated clients: own-user reads/writes and `safe_bulk_insert_papers`/`get_ai_quota_status` RPCs succeed; **cross-user SELECT/UPDATE/DELETE are RLS-blocked**; server-only tables return `42501` to clients; `service_role` server paths succeed; signup-trigger profile/entitlement/counter rows exist.
- **Production not modified at development time.** During development, read-only ledger comparison (remote `supabase_migrations.schema_migrations` = 68 rows, latest `20260726120000`) vs. local showed exactly **one** pending migration (`20260731162729`) and no remote-only migration; the CLI `--linked` dry-run was then environment-blocked at the link step, so equivalence was established from the read-only ledger and no Production DDL/DML was executed. Grants are a strict **subset** of Production's existing ACLs, so applying the migration would not broaden client access. *(The linked dry-run and the application were completed later — see the Merge and Production deployment sub-events below.)*
- **No other change.** No table/column/type/policy/trigger/function/owner/data change; RLS unchanged and authoritative; generated types byte-identical; no package/lockfile/workflow change. Standard gates on the PR head: lint 0 errors, typecheck 0, Vitest 39 files/662 tests, build OK.
- **Merge and Production deployment (2026-07-31).**
  - **Merged:** PR #171 → `main` via a regular exact-head two-parent merge commit `1f00800e5c2f1e1c2d405aa0061ecc38c8738004` (first parent `2be1acef7bd6aaae0f648228b856633431a6323a`, second parent the approved head `49ac5b3ea21d09a2d65ddc8daf6df7be24ff162d`), merged 2026-07-31T21:12:52Z. Merged-main `Validate` run **30665760591** (branch `main`, event `push`, head `1f00800e…`) completed **success**; Vercel **Production** deployment **`dpl_3DgKjyw5z94K3W2kMEbLG7ZjQVjh`** (target `production`, state **READY**, source commit `1f00800e…`) is live on **`app.paperlume.app`**.
  - **Mandatory linked dry-run — obtained:** `supabase db push --dry-run --linked` succeeded and proposed **exactly** `20260731162729_reconcile_data_api_grants.sql` (resolving the earlier environment-blocked link step).
  - **Applied:** `supabase db push --linked --yes` applied **only** that migration to Production (`lioxtgiputfniqbktcsz`), 2026-07-31T21:45:47Z–21:45:53Z. Ledger advanced **68 → 69** (latest `20260731162729`); **Local = Remote (69/69, 0 local-only, 0 remote-only)**.
  - **Effective ACL no-op:** the pre/post effective grant fingerprint was **identical** (Production already held the broader legacy ACLs); **no privilege was revoked and no `anon`/`PUBLIC` grant was added**. No table/column/type/constraint/index/RLS/policy/function/trigger/owner/data change; Edge Functions unchanged (`fetch-paper-metadata` v10, `analyze-paper` v15, `get-gemini-provider-quota` v3); no new Vercel deployment from the database-only apply; no Auth/Storage/Google/Gemini/Resend/billing action.
- **Unblocks but does not complete `PFA-C03A1-L`.** With the grant-parity prerequisite now merged, applied, and verified, **C03A1-L is unblocked and is the next implementation task to resume**; its local-stack foundation work stays uncommitted and preserved in its own workspace and **remains incomplete**. **C03A2-L not begun; PFA-C03 not complete; C27–C29 unchanged.**

## 2026-08-09 — PUBMED-API-PUBLICATION-TYPE-STRUCTURE-001: structured publication-type provenance (`20260809051802`) + `fetch-paper-metadata` v12

**PR #194**, merged into `main` via a regular exact-head two-parent merge commit `7c4f3686a0d298dd940c941b584fb08a46a520ec` (second parent, the approved head, `ee0c4827f5194b2db5966739012960af8145e093`). Migration and Edge deployment were separately authorized steps taken after the merge.

- **Why.** PubMed states publication types as discrete `<PublicationType>` elements, and an official type may itself contain a comma (`Clinical Trial, Phase II`, `Research Support, N.I.H., Extramural`). The Edge function flattened them into one comma-joined `study_type` and later code recovered individual values by splitting that string, so a single official type re-read as two false, less specific ones and matched the wrong entry in the user's study-type pool. Once joined, the boundaries are unrecoverable — splitting is ambiguous, not merely lossy.
- **Repository implementation (PR #194).** Structured values flow through `fetch-paper-metadata` (an additive `publication_types: string[]` alongside the unchanged joined `study_type`, on every PubMed path and transferred as a pair when a Crossref record is enriched from PubMed), the client `PaperMetadata` transport, `RawPaperData` → `normalizePaperData()` → `evaluateStudyType()` (the route native NBIB already used since PR #192), persistence, and `reevaluateStudyTypes()`. `evaluateStudyType()`'s ranking is unchanged; `raw_study_type` is preserved as the legacy representation.
- **Migration `20260809051802_add_structured_publication_type_provenance`.** Adds `papers.raw_publication_types jsonb` (**nullable, no default**) plus a validated CHECK admitting only `NULL` or a non-empty JSON array of strings; updates `safe_bulk_insert_papers(uuid, jsonb)` to canonicalize a missing key / JSON `null` / empty array to `NULL` and return a per-row `error` for any other malformed shape; and updates `merge_exact_duplicates(uuid, uuid[])` to take `raw_study_type` and `raw_publication_types` as a coherent pair from one source row rather than unioning arrays across duplicates. Both RPCs keep their signature, `SECURITY DEFINER`, `search_path=public`, owner, and least-privilege EXECUTE ACLs. **No historical backfill** — a stored joined string is genuinely ambiguous, so reconstructing it would fabricate provenance; every pre-existing row remains `NULL` and behaves exactly as before.
- **Production rollout (2026-08-09).** Ledger advanced **71 → 72** (latest `20260809051802`, Local = Remote) via `supabase db push --linked`; `fetch-paper-metadata` was then deployed from the merge commit and is **v12 ACTIVE**, `ezbr_sha256 a7f05833a66bfacf8ec2dfe71ad4d9af2d093c7e558f329b05109704d33d404d`, with **`verify_jwt = false` intentionally preserved** (the function authenticates in-body via `auth.getUser()`). The live bundle's four files were read back and verified byte-identical to the merged source. `analyze-paper` v15 and `get-gemini-provider-quota` v3 unchanged; no data, Auth, Storage, secret, or Vercel mutation.
- **Outcome.** The runtime rollout is complete: PubMed-derived publication-type boundaries now survive the API → import → persistence → re-evaluation path instead of being inferred later from an ambiguous joined string.

## 2026-08-09 — IMPORT-DOI-URL-DETECTION-001: DOI resolver URL detection deployed to metadata Edge path (`fetch-paper-metadata` v13)

**PR #196**, merged into `main` via a regular exact-head two-parent merge commit `b9d65337d8393de1956ebd145521adbeaa25a690` (first parent `d48ee1fbc2f9d66890cd226e0a2688e28b09b627`, second parent the approved head `08312bb65301c59854c86ee7904214cda855713b`). **Edge Function source, test, and import-dialog copy only — no migration.**

- **Why.** The identifier-import path accepted DOIs written directly (`10.1000/example`, `doi:10.1000/example`) but classified `https://doi.org/10.1000/example` — the resolver form the DOI Foundation and Crossref both document as canonical — as a *title*, so the most reliable identifier a reader can copy out of a reference list went to free-text search instead of DOI lookup.
- **Repository implementation (PR #196).** Recognition is **structural**, on the boundary already used for PubMed record URLs: the WHATWG `URL` parser decides from `protocol` + `hostname` + `pathname` only, restricted to http(s), with the hostname compared **exactly** against `doi.org` and the backward-compatible `dx.doi.org`. The DOI is read from the **path** — whole, suffix slashes preserved, percent-decoded exactly once so the provider layer's own encoding cannot double-escape it — so a query string or fragment can never supply it, and near-hosts, suffix-hosts, the `…@evil.example` user-info form, foreign URLs carrying a DOI URL, and scheme-less values all **fail closed** to the existing title path with no scheme guessed. `detectIdentifier()` now returns `{ type: "doi"; doi }` and the provider dispatch consumes `detected.doi`, so the raw resolver URL no longer reaches a provider that expects a DOI *name*; the former local `cleanDoi()` was removed so DOI normalization has a single authority. Bare PMID, PubMed record URLs, direct DOI forms, title fallback, provider precedence, structured publication types, and bulk ordering/accounting are unchanged.
- **Merged-main CI.** `Validate / validate`, `DB Tests / db-tests`, and `E2E (local) / e2e-local` all completed **success** on the merge commit, and the automatic **Vercel Production** deployment built from `b9d65337…` completed **success**.
- **Production rollout (2026-08-09).** `fetch-paper-metadata` was deployed from the merged source: **v12 → v13**, status **ACTIVE**, `verify_jwt = false` **preserved**, bundle `caaee84b2488c31c0fc780538d2d0988903aba2340d5b4a625574511a25cd02a`. The v13 bundle contains exactly `fetch-paper-metadata/index.ts` plus `_shared/env.ts`, `_shared/identifierDetection.ts`, and `_shared/publicationTypes.ts`. `analyze-paper` **v15** and `get-gemini-provider-quota` **v3** are unchanged.
- **Scope.** **No migration; no schema/RPC/RLS/table/policy/trigger change; no dependency or lockfile change; no Auth, Storage, or secrets mutation; no workflow change.** The ledger remains **72 rows** (latest `20260809051802`, Local = Remote). The function's in-body `supabase.auth.getUser()` remains the authoritative check.

## 2026-08-09 — DOI-RESOLVER-LINK-ENCODING-001: canonical DOI resolver-link encoding (`fetch-paper-metadata` v14)

**PR #198**, merged into `main` via a regular exact-head two-parent merge commit `ca2f0542c7fcd8c7ec43f8425ac2f188a15c2d56` (first parent `bcfb539b43bcf720ebacf663a56ce0a680d40e79`, second parent the approved head `f9555a347cd8c63ec8f39369389b3dd9fa6168d7`). **Application source, Edge Function source, and tests only — no migration.**

- **Why.** Resolver links were built by raw interpolation (`` `https://doi.org/${doi}` ``), so URL-significant characters that are legal DOI-name data became URL *syntax* rather than data. `10.1000/456#789` produced a link a browser truncates at the `#`, and the paper never resolves; it has to be `https://doi.org/10.1000/456%23789`. The defect is a conflated representation: a DOI **name** and its resolver **URL** are not the same string.
- **Repository implementation (PR #198).** A shared canonical builder converts a DOI name into its resolver URL: prefix and suffix are percent-encoded **separately** and rejoined with the single structural `/`, so a suffix-internal `/` becomes `%2F` while the prefix/suffix separator stays literal; a literal `%` becomes `%25` and an apparent `%xx` is never assumed to be already encoded; comma, `#`, `?`, spaces, quotes and Unicode are encoded per **UTF-8 byte**. The builder deliberately performs **no trimming or normalization** of DOI-name code points — whitespace is legal DOI data, so `10.1000/example ` yields `…/example%20` (this whitespace-preservation correction came out of independent review and is the behaviour that shipped). The representation boundary is explicit in both directions: **identifier** fields keep raw DOI names (CSV `DOI`, RIS `DO`, BibTeX `doi`) and only **resolver-URL** fields are encoded — CSV URL / RIS `UR` / BibTeX `url` on the export side, and the PubMed metadata, Crossref metadata, and `fetchByDoi` fallback paths in `fetch-paper-metadata`. **Provider-API encoding is untouched and stays separate**: `api.crossref.org/works/<encodeURIComponent(doi)>` and the PubMed `<encodeURIComponent(doi)>[doi]` search term. Merged delta: 9 files, +1107 / -7.
- **Merged-main CI.** `Validate / validate`, `DB Tests / db-tests`, and `E2E (local) / e2e-local` all completed **success** on the merge commit, and the automatic **Vercel Production** deployment built from `ca2f0542…` is **READY** on `app.paperlume.app`.
- **Production rollout (2026-08-09).** `fetch-paper-metadata` was deployed from the merged source: **v13 → v14**, status **ACTIVE**, `verify_jwt = false` **preserved** (the function authenticates in-body via `auth.getUser()`), bundle `ezbr_sha256 6fed85c0b888ee22263a6c344dcb7a4860e808cc979615f25b3fac24819c1744`. Deployment provenance was pinned by deploying from an isolated worktree proven byte-identical to the merge commit, and the deployed bundle was read back and compared against the pre-deployment v13 read-back: `_shared/env.ts` and `_shared/publicationTypes.ts` unchanged, `index.ts` changed only at the import and the three resolver-link sites, `identifierDetection.ts` purely additive. `analyze-paper` **v15** and `get-gemini-provider-quota` **v3** are unchanged.
- **Scope.** **No migration; no schema/RPC/RLS/table/policy/trigger change; no dependency or lockfile change; no Auth, Storage, secrets, Google Cloud, or workflow change.** The ledger remains **72 rows** (latest `20260809051802`, Local = Remote).
- **No historical backfill.** Stored `journal_url` values were **not** rewritten or reinterpreted. `journal_url` is a generic external-link field that may legitimately hold a publisher URL rather than a DOI resolver link, so a global rewrite would corrupt legitimate data; existing rows keep exactly their previous values and change only when normal application activity re-fetches metadata.
...
- **Migration `20260810152125_harden_remaining_function_search_paths`.** Four `ALTER FUNCTION … SET search_path = pg_catalog` statements and nothing else. `ALTER FUNCTION … SET` writes only `pg_proc.proconfig`, so body, language, volatility, parallel-safety, `SECURITY INVOKER` status, owner, and EXECUTE ACL are preserved by construction rather than by re-assertion — which is why this is an `ALTER` and not a `CREATE OR REPLACE`.
- **Why `pg_catalog` and not `public`.** None of the four references an application relation or an unqualified non-built-in function; their entire external surface is `now()`, `to_tsvector()`, `COALESCE`, the `::text` casts, and the `'english'::regconfig` lookup, all of which resolve in `pg_catalog`. The `search_path=public` convention used by the SECURITY DEFINER RPCs is correct for RPCs that genuinely read public tables, and would be wider than these four need.
- **Why `papers.search_vector` is unaffected.** PostgreSQL inlines simple SQL functions when it stores a generated-column expression. Read-only inspection of the linked project confirms Production's `search_vector` holds the **inlined** `to_tsvector('english'::regconfig, COALESCE(…, ''))` form and all three wrappers report **zero** dependent catalog objects. A non-NULL `proconfig` makes a SQL function ineligible for inlining, but only for expressions parsed *after* the change — an expression already stored is not re-derived. So no generated column is recomputed, no GIN index is rebuilt, and no stored `tsvector` value changes. On a clean local replay the same column stores the **wrapper calls** instead (the long-standing `papers.search_vector` benign/artifact difference recorded in [schema-reconciliation.md](schema-reconciliation.md)); there the wrappers are simply called rather than inlined, which is semantically identical.
- **Behavior-preservation evidence.** Two clean local replays — one with the migration held out, one with it applied — produced a 72-line catalog/behavior snapshot whose **only** difference was `proconfig=(null)` → `proconfig=search_path=pg_catalog` on the four functions. Function bodies (md5), language, volatility, parallel-safety, `prosecdef`, owner, ACL, both `papers` generated-column expressions, all `papers` index definitions, the trigger definition, 29 wrapper outputs over a NULL/empty/prose/punctuation/unicode/`text[]`/`jsonb` corpus, and the end-to-end generated `search_vector`, its A/B/C/D weighting and `set_updated_at` firing were byte-identical.
- **Regression coverage.** New pgTAP suite `supabase/tests/database/007_function_search_path_hardening.test.sql` (**55 assertions**) pins the pinned path, pins that the hardening stayed execution-environment-only (still `SECURITY INVOKER`, same volatility/parallel-safety/language/return type, no second GUC in `proconfig`), proves `pg_catalog` is *sufficient* by re-checking each wrapper against the raw built-in form it wraps, and proves the generated `search_vector` and the `set_updated_at` trigger still behave. `npm run test:db:local` runs **8 pgTAP files / 595 assertions** with a clean 73-migration replay, negative control, sensitivity probe, concurrency probe, and residue check.
- **Not in scope.** The 17 `authenticated_security_definer_function_executable` warnings are the intended, already-hardened application RPC surface and were **not** changed; the four `rls_enabled_no_policy` INFO entries are the intentional server-only deny-all tables and were **not** given policies. **Auth leaked-password protection was not enabled** — it requires the Supabase **Pro Plan** and the organization is on **Free**; the owner has since resolved this as **C30** (stay on Free during development; defer the control to commercialization), see [owner-decisions.md](owner-decisions.md) §1.5.

### Production rollout (2026-08-10, `PFA-C08-SECURITY-HARDENING-001P`)

Deployed from a worktree at the PR #200 merge commit `7c61ba397e775fa580479139baef48a2d3330a8e`, whose merged-main `Validate`, `DB Tests`, and `E2E (local)` checks were all green before any Production mutation.

- **Dry run first.** `supabase db push --dry-run` (CLI 2.111.0, linked project `lioxtgiputfniqbktcsz`) proposed **exactly one** migration — `20260810152125_harden_remaining_function_search_paths.sql` — with no seeds and no roles. The pre-deploy ledger was the expected **72 aligned rows plus this one local-only row**, with zero misaligned and zero remote-only rows.
- **Applied with the plain command.** `supabase db push`, exit 0, applying only `20260810152125`. **No `--include-all`, no `migration repair`, no ad-hoc DDL, and no other migration.** Ledger **72 → 73**, all 73 rows Local = Remote, latest `20260810152125`.
- **Catalog verification.** All four functions now report exactly `proconfig = search_path=pg_catalog` with `cardinality(proconfig) = 1` (no second GUC). Against the pre-deploy snapshot, `proconfig` is the **only** field that changed: body md5 and length, language, `prosecdef` (all still `SECURITY INVOKER`), volatility, parallel-safety, leakproof, strictness, return type, owner, and the EXECUTE ACL are byte-identical for all four.
- **Nothing else moved.** `papers.search_vector` still holds the same **inlined** `setweight(to_tsvector('english'::regconfig, COALESCE(…)), …)` expression with unchanged A/B/C/D weighting; the `has_abstract` generated column, all six `papers` indexes (including the `idx_papers_search_vector` GIN index), and `trg_papers_updated_at` are unchanged; wrapper dependent-object counts stayed 0/0/0 and `set_updated_at`'s stayed 1. No table rewrite, no index rebuild, no recomputed `tsvector`.
- **Literal-only semantic smoke test.** Eleven synthetic values (NULL, empty, English prose, punctuation, unicode, `text[]`, `jsonb`) run through the three wrappers all matched `to_tsvector('english'::regconfig, COALESCE(…))` exactly, confirming `pg_catalog` resolves the built-ins; weighting and `ts_rank` still separate `A` (0.607927) from `D` (0.0607927). **No paper or user rows were read, and no write-based smoke test was performed** — the pre-merge pgTAP suite already covers behavior.
- **Advisor after.** The four `function_search_path_mutable` warnings are **gone** (26 → 22 findings). What remains is unchanged and intentional: **17** `authenticated_security_definer_function_executable` WARNs (same functions as before), **4** `rls_enabled_no_policy` INFOs on the server-only deny-all tables, and **1** `auth_leaked_password_protection` WARN deferred under C30. **No new finding appeared.**
- **No unrelated Production mutation.** Exactly one ledger row was added. Auth configuration, the organization plan (still **Free**), billing, users, user data, Storage, Edge Functions, secrets, Google Cloud, Vercel configuration, and branch protection were all untouched. PR #200 required no Edge Function deploy and no manual Vercel action.

## 2026-08-17 — AUTHOR-IDENTITY-PROVENANCE-001B: structured authorship provenance (`20260817120000`) — **implemented; Production migration and Edge deploy NOT yet authorized**

> **Status superseded.** This record is preserved exactly as written on 2026-08-17, when the migration and Edge deploy were genuinely unauthorized. The rollout completed on 2026-08-18 — see the record at the end of this file.

Branch `feat/author-identity-provenance-001b`, cut from `main` at `39f9c61e9a8ff8f361770d8b9953150f3f4c9d61`. **Status: reviewed-ready implementation only.** No Production migration, no Edge Function deployment, and no merge have been performed under this task.

- **Why.** `papers.authors` stores one author *mention* as a source wrote it. What the source actually stated about that mention — personal vs collective authorship, the given/family split, initials, suffix, affiliations, author-associated identifiers, an ORCID — is destroyed the moment it is flattened to a display string, and no later parse can recover it. Splitting `Ricardo Soto-Rifo` into components would fabricate provenance, not restore it. PubMed, Crossref and native NBIB all state that structure discretely and it was being discarded at the door.
- **What this is not.** It is **not** an identity model. Provenance records what a source stated; it does not assert that two mentions are the same researcher, and a matching ORCID across two rows does not assert it either. There is deliberately no author/person/alias/link table and no global uniqueness on any identifier — pgTAP asserts the *absence* of both, so the identity design stays open for a later task. Analytics grouping is untouched: it still uses the AUTHOR-NAME-CANONICALIZATION-001A textual mention key, so `S M Phillips` and `Stuart M Phillips` remain separate mentions even when both rows carry the same ORCID.
- **Repository implementation.** One authoritative TypeScript contract (`src/lib/authorProvenance.ts`) plus a deterministic ORCID normalizer (`src/lib/orcid.ts`, ISO/IEC 7064 MOD 11-2, offline, no registry lookup). Structured extraction is added to the PubMed and Crossref Edge paths (`_shared/pubmedAuthors.ts`, `_shared/crossrefAuthors.ts`) and to the NBIB file parser; BibTeX, RIS, EndNote, CSV and manual entry emit honest `kind: "unknown"` entries with nothing guessed. Every `authors` projection is byte-for-byte unchanged. Two DTD/format facts drove the extraction and are worth keeping: PubMed's `Identifier` element occurs **both** under `Author` and nested inside `AffiliationInfo`, so affiliation blocks are stripped before author identifiers are read (an institution's GRID/ISNI must never become author identity evidence); and MEDLINE's `AUID`/`AD` carry no author index, so the NBIB parser walks the record in original field order rather than zipping `allFieldValues()` arrays, which would shift every later ORCID onto the wrong person.
- **Migration `20260817120000_add_structured_author_provenance`.** Adds `papers.author_provenance jsonb` (**nullable, no default**) plus a validated CHECK; updates `safe_bulk_insert_papers(uuid, jsonb)` to canonicalize a missing key / JSON `null` / empty array to `NULL` and fail the row for anything else; updates `merge_exact_duplicates(uuid, uuid[])` so `authors` and `author_provenance` are always taken from **one** source row under the pre-existing authors rule — never unioned, and a winning row with `NULL` provenance never borrows a discard's. Both RPCs keep their signature, `SECURITY DEFINER`, `search_path=public`, owner, and least-privilege EXECUTE ACLs; the effective post-replay definitions were diffed against the `20260809051802` baseline and the only substantive changes are the provenance ones. **No historical backfill** — every pre-existing row stays `NULL`, which is the truthful state, and every read path already treats `authors` as sufficient.
- **Review remediation 1 of 2 — `kind` is required, not merely constrained (migration repaired in place).** Independent review flagged the CHECK's `kind` predicate as the one required field written as a bare value comparison instead of the `exists() + type()` guard `source` and `source_name` use. It was a real hole, though not the reported one: a **missing** `kind` key was already rejected, because against an absent key the comparison yields false rather than unknown and the negation catches it. What actually leaked was every shape where the comparison *cannot* return false — a number, boolean or object compares **unknown** against `"personal"` and a SQL/JSON filter selects an item only when its predicate is TRUE, so the violation probe found nothing; and lax mode auto-unwraps arrays before comparing, so `{"kind": ["personal"]}` and even `{"kind": ["personal","editor"]}` compared TRUE and stored as though they were the scalar. All five shapes persisted an entry whose `kind` could not be read as one of the three known values. Repaired to `exists(@.kind) && @.kind.type() == "string" && (one of the three)`; `.type()` applies to the item itself and does not unwrap, so it rejects the array outright. Proved table-level against the real constraint text read back from `pg_constraint` with only this clause reverted. Because the migration had **never been applied to Production**, it was repaired in place rather than followed by a corrective migration, and no older migration was touched. Suite `009` plan **75 → 88**; seven of the added assertions fail against the reviewed constraint and pass against the repaired one. Nothing outside the CHECK changed.
- **Review remediation 2 of 2 — optional scalar members hardened against the same lax-mode coercion.** The first remediation fixed `kind` but left the rest of the CHECK in the form that caused it, so a second review correctly found the defect was not confined to one field. Any probe written as `$[*].field ? (@.type() != …)` selects the member and *then* filters it, and lax mode unwraps an array operand before the filter evaluates — so the array was tested as its own element and admitted. A focused re-audit of every predicate in the constraint found **eleven** affected: the six optional strings (`source_field`, `given_name`, `family_name`, `initials`, `suffix`, `collective_name`), `orcid` (where `["0000-0002-1825-0097"]` passed as canonical), `orcid_authenticated` (where `[true]` passed as `true`), and — beyond the reported scope — a nested-array **affiliation**, a nested-array **identifier**, and a whole array-wrapped provenance **entry**, which slipped past the rule that types entries as objects. All eleven now evaluate type from a parent that is never unwrapped: members through `@.field.type()` filtered at the provenance object, and array elements through `.type()` called *before* the filter (`$[*].affiliations[*].type() ? (@ != "string")`), so the filter operand is a type string with nothing left to unwrap. The required-field predicates, `scheme`/`value`, and the `affiliations`/`identifiers` container-type rules were probed and confirmed already sound, and were left unchanged. Suite `009` plan **88 → 111**; sixteen of the added assertions fail against the previous constraint and pass against the repaired one. Again no TypeScript, parser, RPC-signature or frontend change was required.
- **The alignment invariant.** When non-null, the array is the same length as `authors` and element `i` describes `authors[i]`. The CHECK enforces the length equality, so a partial array can never be stored: once the indexes stop lining up every entry describes the wrong mention, and an ORCID on the wrong name is a false claim about a person. Writers degrade to `NULL` rather than persist a misaligned array.
- **Manual-edit safety.** Editing a paper's authors atomically replaces its provenance with `manual`/`unknown` entries in the same `papers` UPDATE, so a previous source's ORCID can never remain attached to text the user rewrote. Provenance is preserved when the authors array is unchanged — including the Edit dialog's normal case of resubmitting the same array while editing an unrelated field.
- **Account export.** `ACCOUNT_EXPORT_VERSION` **1 → 2**: `data/papers.json` gains a persisted field, which reshapes an existing archive file rather than adding a new one, and a v1 reader would silently drop it.
- **Required Production rollout order (hard dependency, not a preference).** The migration **must** be applied before the frontend ships. Three frontend paths select or write the new column directly — the full account export's explicit column list, `addPaperManually`'s direct insert, and `updatePaper` when the authors array changed — and PostgREST resolves a select/insert list before permissions, so against a pre-migration schema each fails with `42703` rather than degrading. (`safe_bulk_insert_papers` is unaffected: the pre-migration function body ignores an unrecognized payload key, exactly as it did for `raw_publication_types`.) The authorized order is therefore: **1.** apply the migration, **2.** verify schema/RPC state, **3.** deploy the reviewed `fetch-paper-metadata`, **4.** verify, **5.** merge the PR, **6.** allow the automatic Vercel Production deploy, **7.** post-deploy verification. Merging before step 1 would break paper creation and account export in Production. Standing authorization to merge ready PRs does not authorize steps 1–3.

## 2026-08-18 — AUTHOR-IDENTITY-PROVENANCE-001B: Production rollout and PR #229 merge — complete

Closes the chronology opened by the 2026-08-17 record above: that entry described a reviewed-ready implementation, this one records it migrated, deployed, merged and validated in Production. Implementation detail is not repeated here — only the durable operational facts.

- **Migration applied.** `20260817120000_add_structured_author_provenance.sql` was applied to Production project `lioxtgiputfniqbktcsz` via `supabase db push --linked`. A dry run immediately beforehand proved the pending set was **exactly** that one migration (no seeds, no roles, no unrelated migration); the ledger advanced **73 → 74** and `20260817120000` is now the latest applied version.
- **Column and CHECK.** `papers.author_provenance` exists as **nullable `jsonb` with no default**, and `papers_author_provenance_shape_check` exists and is **validated**. The Production constraint text was read back and carries the hardened predicates from both review remediations — required `kind`, and the optional scalars, ORCID, authenticated flag, affiliation elements, identifier elements and provenance entries all typed from a parent that lax mode cannot unwrap. No pre-remediation path/filter form survives.
- **No historical backfill.** Immediately after the migration Production held **783 papers with 0 non-null `author_provenance`**, and no `UPDATE` was run. Rows that predate the rollout keep `NULL`, which is the truthful state: the semantics were never persisted for them, and deriving provenance from an author string would invent it.
- **RPC verification.** `safe_bulk_insert_papers(uuid, jsonb)` and `merge_exact_duplicates(uuid, uuid[])` kept their signature, `SECURITY DEFINER`, owner `postgres`, `search_path=public`, single overload, and unwidened EXECUTE ACL (`postgres=X/postgres | authenticated=X/postgres`). Verified by byte-level parity rather than inspection: the Production `pg_get_functiondef` and `pg_get_constraintdef` md5s matched a from-scratch local replay of the same migration on the same engine (PG 17.6) exactly — constraint `7d6ecbcf…` (3138 B), `safe_bulk_insert_papers` `dffa5295…` (5742 B), `merge_exact_duplicates` `418ad2bb…` (12200 B). The reviewed pgTAP suite therefore describes Production's actual behaviour.
- **Security invariants unchanged.** `papers` RLS remained enabled **and forced** with the same four own-rows policies, no `anon`/`PUBLIC` grant was added, and the public schema gained no table, no identity/person/alias/link table and no ORCID uniqueness rule.
- **Edge deployment.** `fetch-paper-metadata` deployed from the exact reviewed commit `9a5f775eeea86fef0cb6a562b2c69f421c15d9ff` against a clean tree: version **15 → 16**, `ACTIVE`, bundle `ezbr_sha256 155ba78bc31317f4d77b90557112c52d0717430079857b421fcaecb72189fe1d`. **`verify_jwt: false` was preserved** (the established posture for this function) — confirmed both by configuration and by an unauthenticated request returning the function's own `Missing Authorization header` rather than a gateway rejection.
- **Deployed-source reconciliation.** Not inferred from a version bump: the deployed bundle's source was retrieved and all **9 files compared byte-for-byte against the reviewed commit — 9 identical, 0 differing**, covering `index.ts` and the `_shared` graph (`authorProvenance.ts`, `orcid.ts`, `pubmedAuthors.ts`, `crossrefAuthors.ts`, `htmlEntities.ts`, `env.ts`, `identifierDetection.ts`, `publicationTypes.ts`).
- **No sibling Edge deployment.** `analyze-paper` (v16), `get-gemini-provider-quota` (v4) and `delete-account` (v1) were byte-identical in version, hash and `updated_at` before and after.
- **Merge.** PR #229 merged **2026-08-18T14:45:45Z** as a **regular two-parent GitHub merge commit**, pinned to the approved head with `--match-head-commit`. Merge SHA **`c8196357a1b8cb652a614a2868770fbbf8ec59ed`** (GitHub-verified); parent 1 `39f9c61e9a8ff8f361770d8b9953150f3f4c9d61` (base), parent 2 `9a5f775eeea86fef0cb6a562b2c69f421c15d9ff` (reviewed head). No squash, rebase, force-push or admin bypass; `main` advanced exactly to the merge SHA.
- **Merged scope.** **34 files, +5632/−79**, and the merge commit's tree is identical to the reviewed head's tree, so no intervening commit contributed content. Exactly one migration; zero changes to workflows, dependency manifests, Vercel configuration or `supabase/config.toml`; the only Edge function touched is `fetch-paper-metadata` and the `_shared` modules it imports.
- **Post-merge CI on the merge SHA.** Validate `32150324743`, DB Tests `32150324800`, E2E (local) `32150324732` — all **completed / success** on `main`.
- **Vercel Production.** The merge triggered the normal automatic deployment `dpl_8twQQ3RTZ2mqtVMCGVBXUj8Y738r`, `target: production`, `READY`, `meta.githubCommitSha = c8196357a1b8cb652a614a2868770fbbf8ec59ed`, no alias error. `app.paperlume.app` is bound to it and serves the application (HTTP 200). No manual Production deployment and no Vercel configuration change.
- **Post-deploy schema compatibility.** The rollout dependency that made ordering load-bearing is resolved, verified in both directions with controls. Read path: the account export's explicit column list including `author_provenance` returns **200**, while an unknown column still returns **`42703`**. Write path: a PATCH payload carrying `author_provenance` resolves (**204**, matching no row), while an unknown column returns **`PGRST204` — not found in the schema cache**. The former `42703` failure mode cannot occur. No write test was performed against real rows and no Production RPC was invoked as a smoke test.
- **Branch preserved.** `feat/author-identity-provenance-001b` remains at `9a5f775eeea86fef0cb6a562b2c69f421c15d9ff` and was not deleted.
- **No unrelated Production mutation.** Only the authorized migration, the authorized `fetch-paper-metadata` deployment, and the automatic Vercel Production deployment that followed the merge. No backfill, no test-paper creation, no other DML, no other Edge deploy, no secret, Google Cloud, Vercel-config, dependency, workflow or branch-protection change.
- **001B was complete; 001C was not, at this date.** As of this rollout `AUTHOR-IDENTITY-RESOLUTION-001C` was unimplemented: no person, author, alias or identity-link table, no auto-linking, no author-merge UI. It has since been implemented — see the 2026-08-21 entry below. What did **not** change is the boundary: a matching ORCID across two rows is still **provenance a source stated, not a claim that the mentions are the same person**, and only an explicit user action resolves them.

---

## 2026-08-21 — AUTHOR-IDENTITY-RESOLUTION-001C: user-scoped author identity (`20260818120000`) — **implemented; Production migration NOT yet authorized**

> **Status superseded.** This record is preserved exactly as written earlier on 2026-08-21, when the Production migration was genuinely unauthorized. The owner authorized it later the same day and the rollout completed — see the record at the end of this file.

Completes the three-layer author model. 001A compares author *strings*, 001B records what a *source* stated, and 001C records what *this user decided* — the first layer permitted to create a person, and only from an explicit action.

- **Migration `20260818120000_add_user_scoped_author_identity_resolution.sql`.** Additive: four tables (`author_identities`, `author_identity_aliases`, `author_identity_links`, `author_identity_merges`), **nine** functions (six client RPCs, two internal helpers, one trigger function — the count the migration's own verify block enumerates), one trigger on `papers`, and one new UNIQUE constraint `papers (user_id, id)`. Nothing existing is altered or dropped, and no historical migration is touched. **Not applied to Production** — it requires explicit owner authorization. The one lock worth planning around is `ALTER TABLE public.papers ADD CONSTRAINT papers_user_id_id_key UNIQUE (user_id, id)`, which takes ACCESS EXCLUSIVE on `papers` and builds an index under it; `CREATE UNIQUE INDEX CONCURRENTLY` cannot run inside a migration transaction, so apply it off peak and expect the pause to scale with the row count.
- **No backfill, by construction.** The migration creates no identity row, and its fail-closed verify block asserts a total row count of zero across all four tables. Two historical papers carrying the same valid ORCID stay unresolved: identity records begin only from user action after rollout.
- **User-scoped, enforced structurally — on both endpoints of every edge.** All five relationships are **composite on `(user_id, …)`**: alias → identity, link → identity, link → **paper**, and both merge endpoints. The paper endpoint is why the migration adds `papers (user_id, id)`: with `paper_id` referencing `papers (id)` alone, a privileged or direct write could store `user_id = A`, `identity_id = A's identity`, `paper_id = B's paper` — rejected by every RPC and hidden by RLS, but *storable*, which is weaker than the claim this section makes. Pointing the edge at `papers (user_id, id)` makes it a foreign-key violation before any policy is consulted. `papers.id` remains the primary key; the new constraint forbids nothing that was previously legal. Suite 010 proves it with a **direct privileged INSERT** as `postgres` (BYPASSRLS, no RPC) expecting SQLSTATE `23503`, alongside a positive control on the caller's own paper, and a generic assertion that every foreign key leaving the four tables carries `user_id`. No global uniqueness on a preferred name or an ORCID — two users may reach different conclusions about the same person, and two people may share a name.
- **Merges are reversible edges, not reassignments.** `source_identity_id → target_identity_id` means "resolve the source through the target"; nothing is copied, moved or deleted, so undo is one row removed rather than a state reconstruction, and unmerging A from `A → B → C` leaves `B → C` standing. `source_identity_id` is the **primary key**, which enforces at most one outgoing edge per identity with no trigger and no race. `merge_author_identities` resolves the target's root, rejects cycles, and serializes per account with a transaction-scoped advisory lock.
- **Stale-link invalidation.** A link is bound to `paper_id + author_index + author text`. `papers_clear_author_identity_links_on_authors_change` — `AFTER UPDATE OF authors … WHEN (NEW.authors IS DISTINCT FROM OLD.authors)` — deletes that paper's links in the same transaction as the change. An insertion or reorder would otherwise silently re-point an identity at a different human. Editing notes, year, title or `author_provenance` alone changes nothing, and `merge_exact_duplicates` keeps a winning paper's links because it re-assigns the same value.
- **Grants are REVOKE-then-GRANT, and that is load-bearing.** The Supabase database template grants ALL on new tables to `anon`, `authenticated` and `service_role` by default; a first attempt using additive grants alone failed the migration's own verify block with *"anon holds a privilege on public.author_identities"*. The final surface: `authenticated` gets SELECT on all four, INSERT/DELETE on aliases, and UPDATE on **`preferred_name` alone**; `anon`, `service_role` and PUBLIC get nothing. No INSERT or DELETE on `author_identities`, so `delete_empty_author_identity`'s emptiness rule cannot be bypassed by a cascade.
- **Account deletion needs no Edge change.** `delete-account` removes Storage objects and the Auth user and enumerates no tables, so identity data disappears through the `auth.users` FK cascade alone — asserted directly in suite 010. **No Edge Function was modified or deployed for 001C.**
- **Suggestions never assert.** Candidates come only from an exact valid ORCID or an exact 001A mention key against a cluster's preferred name, manual aliases and already-linked spellings. A mention whose ORCID contradicts a cluster's ORCID evidence gets no name candidate, and the same suppression applies to duplicate review. No initials expansion, edit distance, phonetics, transliteration, accent folding, affiliation, coauthor, topic or AI matching, and no registry lookup — 001C makes no network call.
- **…and suggestions are advice, not authorization.** Every unresolved mention can be linked to any owned person through a searchable chooser (`resolution_basis = manual`), and any two owned people can be merged from the People tab — neither path depends on the algorithm having proposed it. This matters most exactly where the automatic path is silent: contradicting ORCIDs suppress the suggestion, and the user may still know that one of those iDs is wrong. Contradictions are surfaced before a two-step confirmation, both values are stated, **neither is called wrong**, and no provenance is altered. Automatic suppression is unchanged; cancelling writes nothing.
- **Choices between people are told apart by evidence, never by a UUID.** Preferred names are deliberately not unique, so **every** surface where a user chooses or acts on a person annotates it with their own data — ORCIDs, aliases, linked spellings, linked-mention counts, a recognisable paper title — in the accessible name as well as on screen: the manual link chooser, the manual merge chooser, the Duplicates tab's two directional buttons (which now name **both** ends, since direction is the whole decision and DOM order conveys nothing to a screen reader), the undo controls, and each People card's own name/alias/merge/delete controls. `A → B → C` renders "Undo merge of A into B" and "Undo merge of B into C" rather than two identical controls.
  Where two people are genuinely indistinguishable by their own evidence — same name, no ORCIDs, no aliases, nothing linked, which is exactly what creating two people and resolving neither produces — the last resort is **creation order** ("created 1st", "created 2nd"), read from the order the data layer already fetches identities in. It is true, it is the user's own data, and a person can reason about it; the identity's UUID is unique but answers the question with a different question.
- **Stale links are reported, not obeyed — and can be repaired.** A link is used only when its paper is present, its index still exists, and the author text is byte-for-byte its `author_name_snapshot` — the read-side twin of the invalidation trigger. A row failing that check contributes no grouping and no ORCID/name evidence, and is listed with the name it was saved under, its paper and the person it points at.
  Detection is read-only; nothing is cleaned up automatically, because a background effect deleting rows the frontend disliked would be Paperlume editing identity history on a hunch. Repair is always an explicit click, and there are two of them. Where the author position still exists the mention is re-offered as unresolved and **both** Link and Create can take it — `create_author_identity_from_mention` gained `p_replace_stale_existing`, which displaces only a row the SERVER re-proves stale from data it reads inside the locked transaction; a link whose snapshot still matches the current text is refused however the caller asks, so this can never overwrite a real decision. Where the position is gone, nothing can render a mention for it, so an explicit **Remove stale saved link** uses the existing `unlink_author_mention_identity` (which needs no author text and is scoped to the caller). Neither path touches `papers.authors`, `author_provenance`, aliases or merge edges.
- **Analytics.** Counts one entity per resolved identity root and one per unresolved 001A mention key; a paper is credited once per entity.
- **Identity evidence is user-wide, never the current Analytics filter.** What a person *is* — their linked spellings, the ORCIDs their linked papers state, whether they hold anything at all — is durable account state, so it is built from a separate minimal read of the papers the identity graph actually links to (`id, title, authors, author_provenance, year, journal`; no abstract, chunked and paginated, skipped entirely when nothing is linked). Deriving it from the filtered collection let a dropdown redefine the graph: narrowing the view removed the paper carrying a person's ORCID, so the exact-ORCID candidate that person exists to produce was never offered, their linked spellings stopped being searchable, duplicate pairs dissolved, and an identity with links elsewhere rendered as empty — offering a Delete the database then correctly refused. The current filter still decides which mentions are *examined*; it no longer decides what exists.
- **Selections reconcile on read, and are never written back.** `lib/authorSelection` separates the stored key, the effective entity key and the label. A selected identity follows a merge to its root; a selected mention follows a link to the person it became; a spelling the user resolved two different ways expands to **every** descendant rather than this layer picking one, because picking would be an assertion the feature refuses to make. Undo needs no bookkeeping precisely because nothing is rewritten — the same stored key simply resolves elsewhere once the edge is gone. Internal keys (`identity:<uuid>`, `mention:<001A key>`) are how a selection is addressed and never how it is described: a selection with no readable label is dropped rather than rendered.
- **A failed identity read fails closed, and never impersonates the compatibility case.** `useAuthorIdentities` now reports one of five states — `loading`, `unavailable` (the 001C schema is genuinely absent), `ready`, `failed` (a real read error with no usable dataset: permission, RLS, network, malformed query, or the linked-paper evidence read), and `stale` (a failed refetch over a dataset that had loaded). The distinction that matters is `unavailable` versus `failed`: both leave a consumer without identity data, and collapsing them would silently regroup a user's resolved people back into separate 001A mentions and tell them nothing — a screen that looks correct and is wrong. A real error is classified **before** the schema-absence sentinel, because TanStack keeps the last successful `data` beside a failed refetch. `failed` shows "Author identities could not be loaded" with a retry and no editable controls; `stale` keeps the known-good graph on screen, labels it last-known, and disables every write. Mutations are gated in the hook itself, not only in the UI, so a stale render cannot get a decision through against a graph nobody could read. Analytics keeps its keyword, year and study-type charts either way and warns above the author selector rather than quietly reverting to name grouping. No raw Postgres/PostgREST text reaches the interface.
- **Compatibility with a database that predates the migration.** Reads distinguish "not installed here" (`null`) from "installed, nothing decided" (empty). The detector requires **both** a missing-object code (`42P01`/`42883`/`42704`/`PGRST205`/`PGRST202`) **and** a 001C object name, so a permission failure or an unrelated missing table still surfaces as the real error. A Preview built against pre-migration Production falls back to 001A grouping and reports the manager unavailable.
- **Account export.** Four additive category files (`data/author_identities.json`, `_aliases`, `_links`, `_merges`), every column exported, with compile-time exhaustiveness guards. **`ACCOUNT_EXPORT_VERSION` stays 2**: the contract bumps only when an existing file changes shape, and 001C reshapes none — the 1 → 2 rationale states the same boundary explicitly.
- **Export survives the rollout window, all-or-none.** Unconditional identity reads would fail a full export on any environment that predates the migration — including this branch's own Preview. `author_identities` is the probe: if *it* reports a missing object, all four categories export as present-but-empty; if it answers, the other three must succeed on their own terms, so a **partial** installation fails closed rather than silently dropping decisions the user made. Permission denials, RLS refusals, network failures, malformed queries, missing *unrelated* tables and ownership-integrity violations all still fail the export — the classifier is the same narrow one the UI uses, demanding both a missing-object code and a 001C object name.
- **Test-suite ownership updated, not weakened.** Suite 009's blanket "no identity table exists" assertion stopped being a statement about 001B the day 001C shipped, so it was narrowed to the designs 001C did **not** choose (a flat `authors`, a global `people`/`person`, a user-independent `author_aliases`) and paired with a stronger permanent invariant asserted against the ORCID-bearing fixtures: **nothing 001B stores ever creates an identity row**. Suite 003's SECURITY DEFINER inventory went 20 → 29 with all nine new functions classified.
- **Two real defects found by the E2E flows, both fixed.** `ANALYTICS_SELECT` omitted `author_provenance`, so ORCID candidates and collective detection silently could not work; and no paper mutation invalidated the identity query, so after an authors edit the client kept showing links the database had already deleted. Paper update, paper delete and duplicate merge now all invalidate the identity dataset.
- **The one invariant a constraint cannot enforce, tested under real concurrency.** A link race is decided by the unique `(paper_id, author_index)`, and a second outgoing merge edge is unstorable because `source_identity_id` is the primary key — but the merge **cycle** rule depends on root resolution reading rows a concurrent transaction has not committed. Without the per-user `pg_advisory_xact_lock`, two tabs merging A into B and B into A would each pass their own cycle check and both commit, leaving a cluster in which no identity has a root. A second true-concurrency probe in the db-tests lifecycle releases both merges from a shared barrier and requires exactly one commit and one refusal **whose message is the cycle rejection** — a weaker assertion would pass even if the two calls never actually raced.
- **Local verification.** Full migration replay from empty; 959 pgTAP assertions across 11 suites (134 in suite 010) plus the framework-free verification, both concurrency probes and the catalog-fingerprint residue checks; 2676 unit tests across 102 files; 162 local E2E tests including eleven identity acceptance flows and a seed-time cross-user isolation probe over the real Data API; lint at its 0-error baseline; typecheck and build clean.

---

## 2026-08-21 — AUTHOR-IDENTITY-RESOLUTION-001C: Production rollout and PR #231 merge — complete

Closes the chronology opened by the record above: that entry described a reviewed-ready implementation whose migration was deliberately unapplied, this one records it migrated, merged and deployed in Production. Implementation detail is not repeated here — only the durable operational facts.

- **Owner authorization.** The owner explicitly authorized applying migration `20260818120000` to Production and merging PR #231. The authorization covered **the migration and the merge only** — not Production DML, not unrelated DDL, not an Edge Function deployment, not a secrets or configuration change.
- **Migration applied.** `20260818120000_add_user_scoped_author_identity_resolution.sql` was applied to Production project `lioxtgiputfniqbktcsz` through the normal linked Supabase CLI path (`supabase db push --linked`), **2026-08-21T12:14:57Z → 12:15:19Z**, so the ledger records the exact reviewed version rather than a regenerated one. A dry run immediately beforehand proved the pending set was **exactly** that one migration (no seeds, no roles, no unrelated migration); the ledger advanced **74 → 75** and `20260818120000` is now the latest applied version. No other migration was applied.
- **Four identity tables installed, and empty.** `author_identities`, `author_identity_aliases`, `author_identity_links` and `author_identity_merges` exist and held **0 rows** immediately after the migration and again after the frontend deployment. **No historical identity backfill occurred** — and structurally could not have: the migration creates no identity row and its own verify block asserts the zero total. Against **783** existing papers that is the visible form of the product rule — two papers sharing a valid ORCID remain two unresolved mentions until a user resolves them.
- **Existing paper data unchanged.** A content fingerprint over `papers (id, authors, author_provenance, updated_at)` was captured **before** the migration and re-checked after it and again after deployment: `7231abc3d90e9df439ea4e15105c111e` at every probe. `papers.id` remains the primary key.
- **User scoping is structural, on both endpoints of every edge.** All five composite `(user_id, …)` foreign keys are live — alias → identity, link → identity, link → **paper**, and both merge endpoints — which is why the migration adds **`papers_user_id_id_key UNIQUE (user_id, id)`**, verified present alongside the unchanged `papers_pkey`. A cross-user edge cannot be stored, not merely cannot be reached.
- **FORCE RLS and least privilege, verified against the catalog rather than `information_schema`.** RLS is **enabled and forced** on all four tables under the seven reviewed policies. `information_schema.role_table_grants` silently filters by the querying role and returns nothing useful here, so the grants were read from `aclexplode()` over `pg_class.relacl` and `pg_attribute.attacl`: **zero** privileges are held by `anon`, `service_role` or `PUBLIC`, and `authenticated` holds only `SELECT` plus column-level `UPDATE (preferred_name)` on identities, `SELECT, INSERT, DELETE` on aliases, and `SELECT` on links and merges. The REVOKE-then-GRANT block therefore held against Supabase's default grants in Production, as it did locally. Confirmed live at the API layer too: an unauthenticated Data API read of `author_identities` returns **`42501` permission denied** — not `PGRST205` — which simultaneously proves PostgREST reloaded its schema cache.
- **Nine functions, hardened, carrying the final reviewed signature.** All nine 001C functions are installed, each `SECURITY DEFINER`, owner `postgres`, `search_path=public`; the Remediation 002 signature is the live one, `create_author_identity_from_mention(uuid, integer, text, text, boolean)`. The trigger `papers_clear_author_identity_links_on_authors_change` is installed.
- **Production equals a clean replay of the reviewed migration.** Not inferred from a successful push: a from-scratch local replay on the same engine and Production were fingerprinted over the same **144 catalog items** — columns, constraints, indexes, policies, RLS flags, table grants, column grants, function definitions (`md5(pg_get_functiondef())`), EXECUTE grants and trigger definitions — producing the identical digest `ca066c8a174a6614780b9820eccdfdbf` on both sides. No temporary Production object was created to obtain it. Treat this as rollout verification evidence, not an API contract: recompute it, never cite it as a fixed value.
- **No Edge Function deployment.** 001C required none, and none occurred. `fetch-paper-metadata` (v16), `analyze-paper` (v16), `get-gemini-provider-quota` (v4) and `delete-account` (v1) were byte-identical in version, bundle hash and `updated_at` before and after. Identity data still disappears on account deletion through the `auth.users` FK cascade alone, with no function change.
- **Merge.** PR #231 merged **2026-08-21T14:12:33Z** as a **regular two-parent GitHub merge commit**, pinned to the approved head with `--match-head-commit`. Merge SHA **`bab7e9d17fec10ca117dc22d89069d7dbce9ac18`** (GitHub-verified); parent 1 `14c59a88d52fe0c51f918484b758eb62fc429533` (base), parent 2 `6e29922eb111a2fab32be095ddde2d41d847e273` (reviewed head). No squash, rebase, force-push or admin bypass; `main` advanced exactly to the merge SHA.
- **Merged scope.** **41 files, +12564/−160**, and the merge commit's tree is **identical to the reviewed head's tree**, so no intervening commit contributed content. Exactly one migration; zero changes to `supabase/functions/`, workflows, dependency manifests, Vercel configuration or `supabase/config.toml`.
- **Post-merge CI on the merge SHA.** Validate `32490896499`, DB Tests `32490896147`, E2E (local) `32490896139` — all **completed / success** on `main`.
- **Vercel Production.** The merge triggered the normal automatic deployment `dpl_3FjbUfYrkPccaajUWzTijgoLRbUZ` — Git-triggered, `target: production`, `READY`, `meta.githubCommitSha = bab7e9d17fec10ca117dc22d89069d7dbce9ac18`, no alias error. `app.paperlume.app` is bound to it and serves the application (HTTP 200). No manual Production deployment or promotion, and no Vercel configuration change.
- **No Production test data, and no unrelated Production mutation.** Only the authorized migration, the regular merge, and the automatic Vercel Production deployment that followed it. **No identity, alias, link or merge row was created to demonstrate behaviour** — the zero-row result above is the real state, not a cleaned-up one. No other DML, no Edge deploy, no secret, Google Cloud, Vercel-config, dependency, workflow or branch-protection change.
- **Branch preserved.** `feat/author-identity-resolution-001c` remains at `6e29922eb111a2fab32be095ddde2d41d847e273` and was not deleted.
- **001C is complete and live — and the boundary it inherited is unchanged.** The three layers stay distinct: 001A compares author *strings*, 001B records what a *source* stated, 001C records what *this user decided*. A matching ORCID across two rows is still **provenance a source stated, not a claim that the mentions are the same person**; person-level identity arises only from an explicit user action, and no automatic, fuzzy, affiliation, coauthor or AI matching was introduced by this rollout. The schema being live does not mean any historical author was resolved.

---

## 2026-08-23 — PUBMED-IN-APP-SEARCH-001: in-app PubMed discovery + `search-pubmed` Edge rollout — live in Production

- **No database change.** This rollout applied **no migration** and made no schema, RPC, RLS, grant or generated-type change. It is recorded here because it added and deployed a **new Edge Function**, which is an operational change to the Production runtime. The Local = Remote migration ledger is unaffected.
- **Edge deployment came before the frontend merge.** Under separate owner authorization, `search-pubmed` was deployed from the independently approved exact head `324f6ae70887bdb6b0e424501574acf1c1d9683f` with `supabase functions deploy search-pubmed --project-ref lioxtgiputfniqbktcsz`, completing **2026-08-23T08:14:59Z**. Production runtime: slug `search-pubmed`, id `583d859f-b4d8-409d-9818-2ef2be98c51b`, initial version **1**, `ACTIVE`, bundle `2d8dacaca2e280c139e9b4915278c03435025fff0aeb67ce7bd4220560dd3af4`. Only that function was added — `fetch-paper-metadata`, `analyze-paper`, `get-gemini-provider-quota` and `delete-account` were **not** redeployed and kept their versions and bundle hashes. **No new secret**, and no Production DML.
- **Non-destructive verification.** Against the Production endpoint: `OPTIONS` → **200** with the committed CORS headers; `GET` → **405 `method_not_allowed`**; unauthenticated `POST` → **401 `unauthenticated`**; and, from an already-existing signed-in Production session, `{"query":""}` → **400 `{"error":"invalid_request","message":"query is required."}`**. That last case is the informative one: it proves the worker boots, builds the caller-scoped client and validates the JWT through `auth.getUser()`, then stops at request validation **before** the `profiles.pubmed_api_key` lookup and before ESearch/ESummary. **No upstream PubMed request was made by these checks**, and no user data was touched.
- **Merge.** PR **#237** merged **2026-08-23T09:02:42Z** as a regular two-parent merge commit **`63b7d6a9ba775f8befa62f6128f34aaeb0468a9f`** — parent 1 the prior `main` `923934f09a69c506bbb2b192aaded85726a036ba`, parent 2 the approved head `324f6ae70887bdb6b0e424501574acf1c1d9683f`. The merge commit's tree is identical to the reviewed head's tree, so no intervening commit contributed content.
- **Post-merge CI on the merge SHA.** Validate `32629906561`, DB Tests `32629906550`, E2E (local) `32629906554` — all **completed / success** on `main`.
- **Vercel Production.** The merge triggered the normal automatic deployment `dpl_7a7Zxy2Trebs72WJbNPXYEsLtFWT` — Git-triggered, `target: production`, `READY`, `meta.githubCommitSha = 63b7d6a9ba775f8befa62f6128f34aaeb0468a9f` on `main`, no alias error, `app.paperlume.app` bound and serving (HTTP 200). No manual Production deployment or promotion, and no Vercel configuration change.
- **Owner acceptance.** The owner performed signed-in Production acceptance: Add Papers → PubMed Search returned real results against the live endpoint for the query `resistance training hypertrophy` (PubMed reported **2,509** matches — an observed 2026-08-23 acceptance-time count, not a stable invariant), and a real-iPhone pass reported the experience as correct.
- **Architecture boundary (C31).** PubMed discovery returns **transient search summaries**; the selected **PMIDs alone** enter the pre-existing canonical importer (`bulkImportPapers` → `fetch-paper-metadata` → normalization → `safe_bulk_insert_papers`). No ESummary field is persisted, and a result carrying a DOI still imports by PMID.
- **Immediate display-only follow-up.** Acceptance surfaced one gap — discovery cards rendered raw PubMed publication types the user had already hidden in their Study Type Exclusion Pool. PR **#238** (merged **2026-08-23T11:48:50Z** as `0751607224074e63164ebe2aa93843d467136461`) filtered those badges at render time. It changed frontend files and tests only: **no** schema, RPC, RLS, Edge Function, classifier, normalization or importer change, and **no** Edge deployment — so it has no separate entry here. Its behaviour is current state, documented in [start-here.md](start-here.md).

---

## 2026-08-23 — AI-PROJECT-TAG-SUGGESTIONS-001A: `suggest-paper-organization` Edge rollout — backend live in Production

- **No database change.** This rollout applied **no migration** and made no schema, RPC, RLS, grant or generated-type change. It is recorded here because it added and deployed a **new Edge Function**, which is an operational change to the Production runtime. The Local = Remote migration ledger is unaffected — it stayed at 75 entries, latest `20260818120000_add_user_scoped_author_identity_resolution` — and **no new secret** was added: the function reuses the existing `GEMINI_API_KEY` and the optional `GEMINI_MODEL`.
- **Merge preceded deployment here, unlike `search-pubmed`.** The ordering rule is that the endpoint must not lag the UI that calls it; `001A` ships **no UI at all**, so merging first exposed nothing. PR **#240** merged **2026-08-23T19:04:42Z** as a regular two-parent merge commit **`57c17d7056edfcbf49551fe77061acb0404e8ab0`** — parent 1 the prior `main` `86b59b0c5f784d1465ff9505bd50d6512ef7893f`, parent 2 the approved head `3e783548bb8fa668804babee748d5a314bdc2bf9`. The merge commit's tree is identical to the reviewed head's tree, so no intervening commit contributed content.
- **Post-merge CI on the merge SHA.** Validate, DB Tests and E2E (local) all **completed / success** on `main`.
- **Edge deployment.** Under separate owner authorization, `suggest-paper-organization` was deployed from a worktree byte-identical to the merged commit `57c17d7056edfcbf49551fe77061acb0404e8ab0` with `supabase functions deploy suggest-paper-organization --project-ref lioxtgiputfniqbktcsz`. Production runtime **as observed at rollout — a dated snapshot, not a durable invariant** (query `supabase functions list --project-ref <project-ref>` for live state): slug `suggest-paper-organization`, id `7c17c669-e15a-44d2-aba0-6254b1da3cbf`, initial version **1**, `ACTIVE`, `verify_jwt = false`, bundle `d57d807cd4c233cf332e37969afbefeae1810b9545368791d1703bbde3899104`. Only that function was added — `fetch-paper-metadata`, `analyze-paper`, `get-gemini-provider-quota`, `delete-account` and `search-pubmed` were **not** redeployed and kept their versions and bundle hashes.
- **Non-destructive verification.** Against the Production endpoint: `OPTIONS` → **200** with the committed CORS headers; `GET` → **405 `method_not_allowed`**; unauthenticated `POST` → **401 `unauthenticated`**; authenticated `{}` → **400 `invalid_request` / `invalid_paper_id`**; authenticated title-only draft → **400 `invalid_request` / `insufficient_evidence`**; and an otherwise-valid request naming a paper owned by another user → **404 `paper_not_found`** with no disclosure that the row exists. **Those last two prove different things**: the title-only case stops at request validation, which runs *before* the ownership query, so it proves boot/auth/validation and **not** ownership; the foreign-paper case is the ownership proof, and only because its draft was otherwise valid and therefore reached the ownership check. None of the six consumed a quota unit — the counter's `used` and `updated_at` were both unchanged afterwards, so not even a consume/refund round trip occurred.
- **One authorized real generation.** A single live request returned **200** with exactly `existingProjects`, `existingTags`, `newProjects`, `newTags`, all arrays, no ephemeral `P#`/`T#` ref anywhere in the payload, and bounded reasons. It consumed **exactly one** `ai_analysis` unit and was not refunded. Deterministic before/after fingerprints over the account's Projects, Tags, `paper_projects`, `paper_tags`, every paper row and the specific paper sent were **byte-identical**, so the advisory/non-mutating contract held in Production, not only in tests.
- **The retry budget absorbed a real provider failure.** That one generation hit an upstream `503` on its first attempt, retried after 2 s and succeeded. The user-visible result was a normal 200 and no refund was issued, because a valid result was delivered — the first Production evidence of the bounded retry policy working on a genuine upstream fault rather than a simulated one.
- **Logging.** The rollout emitted only bounded operational lines — outcome labels, taxonomy counts, a provider status and class. An aggregate scan of the window found no bearer token, no API key, no JWT-like string, no UUID, no provider response body, and no line over 300 characters.
- **Vercel.** No Vercel action was taken. Production remained on the automatic deployment created by the #240 merge (`target: production`, `READY`, `meta.githubCommitSha = 57c17d7056edfcbf49551fe77061acb0404e8ab0`, `app.paperlume.app` bound). An Edge deployment does not touch Vercel, and a Vercel Preview can never be evidence about this endpoint.
- **Architecture boundary (C32).** The endpoint is **advisory**: it creates, assigns and persists nothing, and its only writes are the pre-existing `consume_ai_quota` / `refund_ai_quota` RPCs against the existing `ai_analysis` counter. No database identifier reaches Gemini — Projects/Tags cross as request-local `P#`/`T#` refs, and existing-entity suggestions resolve through that ref map alone.
- **Backend live is not feature shipped — at this date.** `AI-PROJECT-TAG-SUGGESTIONS-001B` — the Edit Paper suggestion UX — had **not** started. No frontend called this endpoint, so users had no suggestion feature; what the rollout bought is that the endpoint-before-UI gate is satisfied and `001B` could be built against a live, verified contract. It has since shipped and been accepted in Production — see the 2026-08-24 entry below.

---

## 2026-08-24 — AI-PROJECT-TAG-SUGGESTIONS-001B: Edit Paper AI organization surface — PR #242 merged; Production-accepted

- **No database change and no Edge change.** This rollout applied **no migration** and made no schema, RPC, RLS, grant or generated-type change, added **no new secret**, and changed **no file under `supabase/`** — so no Edge Function was deployed or redeployed. It is recorded here because it is the material rollout that made the AI organization feature reachable by users: `001A` shipped the endpoint with no caller, and `001B` shipped the caller. The Local = Remote migration ledger is unaffected — it stayed at 75 entries, latest `20260818120000_add_user_scoped_author_identity_resolution`.
- **Merge.** PR **#242** ("feat: add AI Project and Tag suggestions to Edit Paper") merged **2026-08-24T15:23:06Z** as a regular two-parent merge commit **`8159d353f3cdb76b332d6a0266f00c4d4772c566`** — parent 1 the prior `main` `558fc902b79cde847b0393889b720e94c06a333a`, parent 2 the approved head `aa8c3ce999bf09b0eab68a0ac8e4adefc45688d2`. The merge commit's tree is identical to the reviewed head's tree, so no intervening commit contributed content. The diff touches `README.md`, `docs/`, `e2e/`, `scripts/` and `src/` only — **zero** `supabase/` paths, which is the frontend-only claim as a checkable fact rather than an assertion.
- **What shipped.** A compact **AI organization** section in Edit Paper, above the Projects selector, with an explicit **Suggest Projects & Tags** action. Generation happens **only** on that click — never on dialog open, on abstract load, on keystroke, on Save or on opening a selector — and the request is built from the **unsaved draft** (title, abstract, parsed keywords, study type) plus the current local selection, because the user is asking about what they are editing rather than about the stored row. Suggested **existing** Projects/Tags become local dialog selections only; a proposed **new** Project/Tag requires its own explicit **Create & select** click, which creates the entity immediately through the existing `createProject` / `createTag` mutations. In both cases the **paper assignment persists only through the ordinary Save Changes path**, so cancelling assigns nothing. Because suggestions and paper analysis spend the one `ai_analysis` counter, the visible allowance is named **"AI requests"** — display copy only; no RPC, column or hook was renamed.
- **Deployment shape: frontend-only relative to `001A`.** No migration, no schema change, no new secret, no Edge source change, no Edge redeploy. The already-deployed `suggest-paper-organization` remained the backend contract throughout — the artifact the `001A` rollout deployed and verified on 2026-08-23, observed as version **1** at acceptance time (a dated observation, not a durable invariant; query `supabase functions list --project-ref <project-ref>` for live state). The frontend rolled out through the normal Vercel Git integration on the merge to `main`; no manual deployment or promotion was performed. The endpoint-before-UI rule was therefore satisfied *before* `001B` was built, which is the ordering the rule exists to produce — see [deployment.md](deployment.md) §7c.
- **Production acceptance — complete.** The owner completed full manual acceptance in the real Production application on **2026-08-24**. A real generation against the unchanged deployed function returned **HTTP 200** (`suggest-paper-organization` v1, POST event `1b0bc17d-2c51-4040-b716-612c834bcd50`, `2026-08-24T17:28:28.968Z`, 7,984 ms) and the UI rendered real Project and Tag suggestions with their reasons. Both persistence paths passed end to end:
  - **existing taxonomy** — Suggest → select a suggested existing Project/Tag → the selection enters ordinary Edit Paper state → **Save Changes** → reopen the paper → the assignment is still there;
  - **new taxonomy** — Suggest → **Create & select** on a genuinely new proposal → the taxonomy entity is created and the returned entity enters the Edit Paper selection → **Save Changes** → reopen the paper → the new entity and the intended assignment are both present.
- **Successful generation itself mutated nothing.** Acceptance confirmed the advisory contract in Production: nothing was automatically selected, nothing was automatically created, and a successful generation on its own persisted no paper or taxonomy assignment. Creation still requires the second explicit **Create & select** action, and assignment still requires Save. The successful generation consumed **one** unit of the shared `ai_analysis` allowance and was **not** refunded, which is the correct behaviour for a delivered result.
- **A temporary upstream provider degradation preceded acceptance.** The first manual acceptance attempts were blocked by degradation of the upstream Gemini Developer API (high latency and repeated HTTP 503s), affecting both `suggest-paper-organization` and the pre-existing `analyze-paper` in the same window. Read-only diagnosis — no code, deployment, secret, model or Google Cloud change — found **no** PaperLume code, key, model or quota defect, and **zero** Google 429 / quota-exceeded events; no timeout change was justified and none was made. The unchanged deployed functions later recovered and returned 200, and acceptance then completed successfully.

---

## 2026-09-02 — AI-MODEL-SELECTION-001A: entitlement, model catalog and per-user AI preference (`20260902120000`) — **implemented; Production migration NOT authorized**

- **One additive migration:** `20260902120000_add_ai_model_selection_foundation.sql`. It adds `public.user_entitlements.ai_model_selection_enabled` (BOOLEAN NOT NULL DEFAULT false, backfilled `true` for existing `pro` / `labs_team` rows in `active` or `trialing` status), the server-controlled `public.ai_model_catalog` seeded with exactly two rows (`google/gemini-3.5-flash`, `google/gemini-3.6-flash`), `public.user_ai_preferences` (one row per user, `ON DELETE CASCADE` from `auth.users`, FK to the catalog), the additive fail-closed `can_select_ai_model` column on `get_current_user_access()`, and two new SECURITY DEFINER RPCs — `set_current_user_ai_model(text)` and `clear_current_user_ai_model()`. It changes no plan name, quota, paper limit, storage limit, premium-taxonomy flag, Labs/Teams flag, billing identifier or usage counter. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C33**.
- **Security posture.** Both new tables carry `ENABLE` + `FORCE ROW LEVEL SECURITY`. Grants are stated REVOKE-first (the Supabase template grants ALL on a new table to `anon` / `authenticated` / `service_role` by default, so additive GRANTs alone would have described a surface the database did not have): `authenticated` holds **SELECT only** on both, and `anon` / `service_role` hold **nothing**. `ai_model_catalog` has a read-all SELECT policy for `authenticated`; `user_ai_preferences` has a SELECT-**own** policy and no write policy at all. Both new RPCs derive the caller from `auth.uid()`, take **no user-id parameter**, pin `search_path = public`, and hold EXECUTE for `authenticated` alone (revoked from PUBLIC, `anon` and `service_role`). The migration ends with a fail-closed `DO $verify$` block asserting the column shape, the exact backfill population, RLS enable+force, the grant matrix, the exact seed, the empty preference table, and the RPC posture — so a replay that produced a weaker schema fails at replay rather than shipping.
- **Runtime routing is deliberately unchanged.** `analyze-paper` and `suggest-paper-organization` still resolve the model solely through the global `GEMINI_MODEL` configuration and `supabase/functions/_shared/geminiModel.ts`, which is **byte-identical** to its prior blob (`9fde45a29c964aa1644b4bd455a7688ca03cc272`). Neither function reads `user_ai_preferences`, `ai_model_catalog` or `can_select_ai_model`, and **no file under `supabase/functions/` changed**. The current 90-second / zero-retry Gemini transport (PR #265) and the Production `gemini-3.6-flash` selection are untouched, as are prompts, request bodies, parsing and quota behaviour. There is **no per-model API key**: both seeded models are served by the same existing server-side `GEMINI_API_KEY`, and no secret was added, rotated or referenced.
- **No preference row was created for anyone.** Absence of a row means "use the system default", so backfilling would have converted "no opinion" into a choice the user never made. `handle_new_user()` was **not** extended.
- **Generated types:** `src/integrations/supabase/types.ts` regenerated from the **local** database (`supabase gen types typescript --local --schema public`); the diff is purely additive (+87 lines, 0 deletions) — the two new tables, `ai_model_selection_enabled` on `user_entitlements`, `can_select_ai_model` on `get_current_user_access`, and the two new RPC signatures. Not hand-edited (only the generator's trailing blank line normalized for `git diff --check`); the linked/Production schema was not used for generation.
- **Tests.** New pgTAP suite `011_ai_model_selection_foundation.test.sql` (**96 assertions**) covers the catalog allowlist and its constraints, the read/write posture, the fail-closed `can_select_ai_model` matrix (including the case that matters most — plan text `'pro'` with the flag `false` is **denied**), every setter rejection direction, upsert-stays-one-row, cross-user read and write isolation between two *entitled* users, the reset RPC, and the dormant-preference downgrade rules. `003_rpc_caller_scope_and_grants` moved from 29 to **31** SECURITY DEFINER functions and 23 to 25 directly-callable RPCs (plan 203 → 213); `008_account_deletion_cascade` proves `user_ai_preferences` cascades with the account while the shared catalog row survives (plan 79 → 84). `useCurrentUserAccess` gained `canSelectAiModel` with strict `=== true` comparison rather than truthiness coercion — `!!"false"` is `true`, which would hand a paid capability to a malformed row.
- **Not deployed.** No Production migration, no Production SQL, no Edge Function deploy, no secret or auth mutation, no Vercel action, and no provider request of any kind. The Local = Remote migration ledger is **unchanged** in Production; locally the chain is now 76 files, latest `20260902120000_add_ai_model_selection_foundation`.
- **The saved preference is in the full account export from day one** (`AI-MODEL-SELECTION-001A-CORRECTION-01`). `user_ai_preferences` ships as the singleton category `data/user_ai_preferences.json` carrying `user_id`, `preferred_model_id`, `created_at`, `updated_at`, with JSON `null` when the user has no explicit choice. The first draft of this work excluded it as "deferred to 001C, unreachable until a UI exists"; that reasoning was wrong, because `set_current_user_ai_model` is granted to `authenticated` and a real row can exist the moment the migration is applied. The read is S2-scoped (`.eq("user_id", …)`), singleton (`maybeSingle()`), ownership-verified, and never touches `ai_model_catalog` — the catalog stays out of the archive as global product metadata. A new narrow classifier (`isAiModelPreferenceSchemaMissing`) tolerates exactly one condition, a missing-object error naming `user_ai_preferences` during the schema-before-code rollout window; permission, RLS, auth, network, timeout, malformed and generic failures all still abort the whole export. `ACCOUNT_EXPORT_VERSION` stays **2** under the documented additive-category rule. The shared code/text matching behind both this and the 001C classifier moved to `src/lib/missingDatabaseObject.ts`; the 001C detector's own 19 tests pass unchanged, which is the evidence that extraction changed no behaviour.
- **Foundation is not feature.** There is **no Settings control** and **no AI operation consults the preference**. Runtime routing is `AI-MODEL-SELECTION-001B`; the visible Settings control is `AI-MODEL-SELECTION-001C`, which owns **no** export work.

---

## 2026-09-02 — AI-MODEL-SELECTION-001B: server-enforced per-user Gemini runtime routing — **implemented; NO migration; Production migration and Edge deploy NOT authorized**

- **No database change, and none was needed.** This work adds **no migration**, no schema, RPC, RLS, grant, seed or generated-type change; `src/integrations/supabase/types.ts` is untouched. It is recorded here because it changes the Production *runtime contract* of two Edge Functions, which is the same reason the `search-pubmed` and `suggest-paper-organization` rollouts appear above. The Local = Remote ledger is unaffected in both directions: Production still stands at 75 applied migrations (`20260902120000` is **not** applied there), and the local chain is unchanged at 76 files. The 001A schema, RPCs and posture are byte-identical — the whole task deliberately consumes the existing `get_current_user_access()`, SELECT-own preference and authenticated catalog SELECT surfaces rather than adding a resolver RPC for convenience. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C33**.
- **One shared implementation.** `supabase/functions/_shared/aiModelSelection.ts` (new) is the single runtime model-selection decision, used by **both** `analyze-paper` and `suggest-paper-organization`, so their authorization and fallback behaviour cannot drift. It is a pure module — no Deno APIs, no remote imports — so Vitest exercises the exact shipped code with a fake caller-scoped client.
- **Entitlement is re-checked on every AI operation.** Each function calls `get_current_user_access()` through the caller-authenticated (anon key + caller bearer token) client and honours a saved preference only on `can_select_ai_model === true`. Neither Edge Function compares `plan === 'pro'`, checks an email, or reads an internal role — the database access projection is the authority, and a dormant preference held by a downgraded user is **not even read**.
- **The catalog is the allowlist.** The preference is read from `user_ai_preferences` with an explicit `.eq("user_id", <authoritative auth.getUser() id>)` as defence in depth on top of the SELECT-own policy, then resolved against `ai_model_catalog` on an exact id filter. The returned row's `id` must match the requested id, `enabled` must be `true`, `provider` must be `google`, and `provider_model` must be non-empty and already trimmed. **No TypeScript list of model strings was created** — a second allowlist could disagree with the first, so the two seeded ids appear only as test fixtures.
- **`enabled` vs `selectable`, deliberately different at runtime.** A saved preference requires `enabled` only. `selectable = false` closes a model to *new* choices while the users who already chose it keep working; `enabled = false` retires it and falls back. Requiring **both** stays the setter's job — `set_current_user_ai_model` is unchanged.
- **Fail closed on the capability, open on the feature.** An access RPC error, a malformed access row, a preference read error, a malformed preference, a catalog read error, a missing/disabled/malformed catalog row and an unsupported provider **all** resolve to `resolveGeminiModel(GEMINI_MODEL)`. None becomes a PaperLume 402, none refunds a unit, and none fails the request: an unproven entitlement never routes, but nobody loses Analyze because a metadata read hiccuped. Selection happens after request validation and **before** the quota unit is spent, so it can cost nothing.
- **Only the model component of the provider URL changes.** Prompts, request bodies, `responseMimeType`, response parsing, extraction schemas, the Projects/Tags suggestion contract, quota consumption, refund semantics and provider-error classification are all untouched, and `_shared/geminiTransport.ts` is **byte-identical** — the 90-second / zero-retry policy (PR #265), a bounded Production diagnostic at this date and permanent Gemini policy since 2026-09-19 (C46), is unchanged in both direction and value. Both models are served by the **same existing `GEMINI_API_KEY`**, sent the same way (`x-goog-api-key`); no key was added, rotated, split per model or per user, and `GEMINI_MODEL` was not changed. The Gemini URL is now assembled in exactly one place, from a resolver-produced object rather than a bare string, so "send this user's request to an arbitrary model" has no expressible form.
- **`get-gemini-provider-quota` behaviour is unchanged.** It keeps resolving and reporting the configured **system default** and was edited only to say so accurately. It is manager-only observational monitoring of the shared Google project, not a per-user routing report, so it and the two generation functions may now legitimately name different models for the same request. No multi-model Monitoring call was added; **C29 remains deferred**.
- **Bounded diagnostics only.** One routing line per provider-bound request (`model_routing source=… provider=google model=…`) and one bounded reason on an unexpected fallback (`access_lookup_failed`, `invalid_access_row`, `preference_lookup_failed`, `invalid_preference`, `catalog_lookup_failed`, `model_missing`, `model_disabled`, `invalid_catalog_row`, `unsupported_provider`). No user id, email, bearer token, API key, raw database error body or request content. The two ordinary states — not entitled, no preference — emit **no** warning, so the common path stays quiet.
- **Tests.** New `_shared/__tests__/aiModelSelection.test.ts` (**75 tests**) covers every fallback direction, both honoured models, the `enabled` / `selectable` asymmetry, unsupported providers, blank/untrimmed values, user-id scoping, per-request re-checking, the bounded-log shape and the absence of a hard-coded allowlist. New `analyze-paper/__tests__/modelRouting.test.ts` (**27 tests**) executes the shipped decision for each required routing case and reads the committed source for the wiring the Deno shell cannot execute. `suggest-paper-organization/__tests__/handler.test.ts` grew 90 → **113 tests**, exercising the real handler end to end with a fake `fetch` and asserting the exact URL POSTed for each case plus unchanged ownership, taxonomy, payload, quota and refund behaviour. Repository totals: 139 → **141** test files and 4,333 → **4,458** tests, all passing, with `lint`, `typecheck`, `build` and `build:extension` green and no new warnings. Because `npm run typecheck` does not cover `supabase/functions/**`, the three Edge entrypoints were additionally checked with real Deno (`deno check`): `suggest-paper-organization` and `get-gemini-provider-quota` clean, and `analyze-paper` carrying **only** the two pre-existing `RpcClient` structural diagnostics that are identical on `main`.
- **No caller-supplied model and no caller-supplied identity.** Verified mechanically over the changed code: the only model sources are the trusted `GEMINI_MODEL` environment default and the catalog after entitlement verification. Neither request contract has a model field, neither function reads a query parameter or any header but `Authorization`, and the shared resolver reads no request input at all.
- **Not deployed, and no UI.** No Production migration, no Production SQL of any kind, no Edge Function deploy, no secret or auth mutation, no `GEMINI_MODEL` change, no Vercel or Chrome Web Store action, and **no provider request**. No Settings control was added — that is `AI-MODEL-SELECTION-001C` — so with no UI an ordinary user does not newly encounter this capability; only an already-existing server-side preference would affect AI execution. When these two functions are eventually deployed, note that `_shared/aiModelSelection.ts` joins the bundle closure of **both** of them.

---

## 2026-09-02 — AI-MODEL-SELECTION-001C: the Settings model-selection control — **implemented; NO migration; no Edge change; not deployed**

- **No database change, and none was needed.** This work adds **no migration** and no schema, RPC, RLS, policy, grant, seed or generated-type change; `src/integrations/supabase/types.ts` is untouched and `supabase/migrations/` is unchanged. It is recorded here for the same reason the 001B entry above is: it completes a feature whose contract this ledger tracks. The local chain stays at 76 files, latest `20260902120000_add_ai_model_selection_foundation`, and nothing was applied anywhere. Durable decisions: [decisions-and-triggers.md](decisions-and-triggers.md) **C33** and **C34**.
- **No Edge Function change either.** Nothing under `supabase/functions/` was modified. `_shared/aiModelSelection.ts`, `_shared/geminiModel.ts`, `_shared/geminiTransport.ts`, both generation entrypoints and `get-gemini-provider-quota` are byte-identical to their 001B state, so the runtime contract this ledger records is unchanged in both direction and value. `supabase/config.toml` is unchanged.
- **It consumes the 001A surfaces rather than adding new ones.** The Settings section reads `ai_model_catalog` (`id, provider, display_name, enabled, selectable, sort_order`, ordered `sort_order` then `id`) and the caller's own `user_ai_preferences` row (singleton, with an explicit `.eq("user_id", …)` over the SELECT-own policy), and writes **only** through `set_current_user_ai_model(p_model_id)` and `clear_current_user_ai_model()`. There is no direct `INSERT` / `UPDATE` / `UPSERT` / `DELETE` against either table anywhere in the frontend — which the grant matrix recorded in the 001A entry would deny regardless, since `authenticated` holds SELECT only. Opening Settings creates no preference row.
- **The gate is the server capability alone.** The control renders enabled only on `useCurrentUserAccess().access.canSelectAiModel === true`. No plan-name comparison, email, user-id allowlist, internal-role check or browser-storage flag exists in the surface, and the setter re-checks the same entitlement server-side regardless. Access loading renders no enabled control at all; an access-lookup error, a catalog read failure and a preference read failure each remove the control and offer a bounded retry, so a failed read is never displayed as "no preference".
- **The default stays server-side.** "Paperlume default" is a UI-only sentinel (`__paperlume_default__`) meaning *no saved row*; it calls the clear RPC, is never passed to the setter, and deliberately embeds no provider model — so a future `GEMINI_MODEL` change remains an environment change rather than a frontend-deploy dependency (**C34**). An explicit Gemini 3.5 pin and "no preference" are distinct states even though they currently route identically.
- **Lifecycle states are reported, not corrected.** `enabled = true, selectable = false` shows as the current, still-honoured choice but disabled for new selection — matching the runtime asymmetry the 001B entry records. A disabled or catalog-missing saved model reports that Paperlume is using the default, exactly as the runtime behaves, and is never silently rewritten. A downgraded user holding a dormant preference can clear it but not change it.
- **Tests.** New `src/hooks/__tests__/useAiModelSettings.test.ts` (**31 tests**) covers the approved catalog projection, deterministic ordering, the explicit `user_id` predicate, no-row-versus-failure, both RPC call shapes (including that the setter receives **only** `p_model_id` and the clear RPC receives **no argument at all**), every structured rejection class and its targeted refresh, and — read from the committed source — that no write verb and no third RPC appear in the module. New `src/components/settings/__tests__/AiModelSettingsSection.test.tsx` (**27 tests**) covers every rendered state and asserts statically that the component contains no plan/email/role gate, no hard-coded model id and no `GEMINI_MODEL` authority. New `src/components/settings/__tests__/SettingsDialog.aiModel.test.tsx` (**10 tests**) drives the composed dialog against a mocked Supabase client and proves opening Settings performs no preference write, saving does not close the dialog, and no direct table write is ever issued. Repository totals: 141 → **144** test files and 4,458 → **4,526** tests (+68: 31 + 27 + 10), all passing, with `lint` (18 pre-existing warnings, unchanged), `typecheck`, `build` and `build:extension` green.
- **E2E.** New `e2e/ai-model-settings.spec.ts` proves the non-entitled seeded user gets no selector and that Settings stays reachable at 390×640 (through the navigation drawer, with real geometry and `elementFromPoint` checks rather than `toBeVisible` alone), and — as a **disposable per-run entitled account** — that a saved model and a reset both survive Settings close/reopen. Entitlement for that account is granted the way Production would grant it: one server-side write to `user_entitlements.ai_model_selection_enabled`, with the plan left `free`, so the spec demonstrates that the **flag** and not the plan name is the gate. The deterministic seed users are unchanged and never acquire a preference row. The lifecycle then proves the privileged half the browser cannot see — that the final reset really removed the row — by reading it through the account's **own** authenticated SELECT-own path, because `service_role` is deliberately revoked on `user_ai_preferences` (001A) and must stay that way. No Edge Function is served and **no Gemini request is made**.
- **Not deployed.** No Production migration, no Production SQL, no Production database write, no Edge Function deploy, no secret or auth mutation, no `GEMINI_MODEL` change, no Vercel or Chrome Web Store action, and **no provider request of any kind**.

---

## 2026-09-03 — AI-MODEL-SELECTION-001D: Gemini 3.7 and 3.8 join the selectable catalog (`20260903120000`) — **implemented; Production migration NOT authorized**

- **One additive migration:** `20260903120000_add_gemini_37_38_catalog_models.sql`. It inserts exactly two rows into `public.ai_model_catalog` — `google/gemini-3.7-flash` (provider `google`, provider model `gemini-3.7-flash`, display name **Gemini 3.7 Flash**, `enabled`, `selectable`, `sort_order` 30) and `google/gemini-3.8-flash` (provider model `gemini-3.8-flash`, **Gemini 3.8 Flash**, `enabled`, `selectable`, `sort_order` 40) — bringing the catalog to four rows in order 3.5 / 3.6 / 3.7 / 3.8. It creates no table, column, index, constraint, policy, grant, function or trigger, and changes no plan, quota, paper limit, storage limit, entitlement flag, billing identifier or usage counter. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C35**.
- **The 001A migration is untouched and its history stands.** `20260902120000_add_ai_model_selection_foundation.sql` is byte-identical, and its own entry above continues to say — accurately — that it initially seeded exactly Gemini 3.5 Flash and Gemini 3.6 Flash. That was true when it ran; this later migration expands the catalog to four. Historical entries are not rewritten to match a later state.
- **A plain INSERT with no `ON CONFLICT`, deliberately.** The primary key and the `(provider, provider_model)` UNIQUE constraint are the drift detector: if either id or provider model already existed in a state nobody reviewed, replay must fail visibly rather than reconcile a row into the shape this file expects. `ON CONFLICT DO UPDATE` would overwrite exactly the evidence worth seeing, and `DO NOTHING` would accept conflicting metadata in silence. `sort_order` continues the sparse 10/20 spacing, so nothing was renumbered.
- **A fail-closed self-check proves what it did *not* touch.** The closing `DO $verify$` block asserts the catalog holds exactly the four approved rows as whole-row tuples in exactly that order, and then — by timestamp rather than by trust, since every row this transaction wrote carries `updated_at = now()` and every row it did not predates that — that **no pre-existing catalog row was modified**, **no `user_ai_preferences` row was written**, and **no `user_entitlements` row was written**. It separately asserts that no preference points at a model this migration created, that the catalog gained no credential-bearing column, and that the 001A grant posture (`anon` nothing, `service_role` nothing, `authenticated` SELECT-only) is intact. A replay that produced a weaker or wider result fails at replay rather than shipping.
- **The saved-preference invariant is the one that mattered most.** Production holds one preference row, `google/gemini-3.6-flash` — the owner's explicit selection from the C34 canary. Adding catalog rows is additive product metadata: it is not rewritten, not cleared, not moved to 3.7 or 3.8, and not converted to "Paperlume default". The verify block refuses to commit if it were.
- **No runtime and no frontend production code changed, and none needed to.** Nothing under `supabase/functions/` was modified — `_shared/aiModelSelection.ts`, `_shared/geminiModel.ts`, `_shared/geminiTransport.ts`, both generation entrypoints and `get-gemini-provider-quota` are byte-identical to their 001B/001C state — and nothing under `src/components/settings/` or `src/hooks/` was modified either. That is the architectural claim 001B and 001C were built for: **the database catalog is the allowlist**, so the resolver reads the row's trimmed `provider_model` into the Gemini URL and the Settings control renders whatever `enabled AND selectable` rows a supported provider offers. Concrete 3.7/3.8 ids appear only in tests, fixtures and documentation. `src/integrations/supabase/types.ts` is unchanged: this migration adds data, not schema.
- **The request contract and the system default are unchanged.** `GEMINI_MODEL` is not touched and the system default stays **`gemini-3.5-flash`** (**C34**) — a catalog row makes a model selectable, never default. Prompts, request bodies, the JSON `responseMimeType` structured-output contract, the absence of sampling overrides, parsing, quota consumption, refunds, the 90-second timeout and the zero-retry policy are all unchanged. All four models are served by the **same existing server-side `GEMINI_API_KEY`**; no key was added, split per model or rotated.
- **Provider acceptance (Google first-party documentation, re-read 2026-09-03).** `gemini-3.7-flash` — **stable**, released **2026-08-13**, structured outputs supported, 1,048,576 in / 65,536 out, **no shutdown date announced**. `gemini-3.8-flash` — **stable** and Google's newest Flash model, released **2026-09-02**, structured outputs supported, same token limits, thinking levels low/medium/high, stable endpoint `gemini-3.8-flash`, **no shutdown date announced**. Neither is preview-only and neither documents a contract change Paperlume's existing `generateContent` call would fail.
- **Tests.** New database suite `supabase/tests/database/012_ai_model_catalog_expansion.test.sql` (**50 assertions**) owns the catalog as a list: exactly four rows, the exact ordered ids / provider models / display names / providers / sort orders, all four enabled and selectable, whole-row metadata identity, the unchanged column set and credential-free posture, RLS still enabled *and* forced with exactly one SELECT-only policy, `anon` still refused, `authenticated` still unable to INSERT/UPDATE/DELETE, `service_role` still holding nothing on either table, the setter accepting 3.7 and 3.8 for an entitled caller while still refusing unknown ids, a bare provider model string, an unapproved sibling and the floating `-latest` alias, a Free user still refused, and a saved 3.6 preference surviving the expansion. Suite `011` keeps every 001A authorization claim — the exhaustive catalog-contents assertions moved to 012 rather than being softened there, since all database suites run against the final migration state. Runtime and UI suites grew by fixtures only: `_shared/__tests__/aiModelSelection.test.ts` 75 → **83**, `analyze-paper/__tests__/modelRouting.test.ts` 27 → **29**, `suggest-paper-organization/__tests__/handler.test.ts` 113 → **115**, `useAiModelSettings.test.ts` 31 → **36**, `AiModelSettingsSection.test.tsx` 27 → **29**, `SettingsDialog.aiModel.test.tsx` 10 → **12**.
- **E2E.** `e2e/ai-model-settings.spec.ts` now asserts the entitled disposable account's dropdown contains exactly **Paperlume default, Gemini 3.5 Flash, Gemini 3.6 Flash, Gemini 3.7 Flash, Gemini 3.8 Flash**, in that order, read out of the live local database after a full migration replay — and persists one 001D model (3.8) through the real setter RPC and the migrated catalog FK before continuing to the pre-existing 3.6 → 3.5 → default path. No Edge Function is served and **no Gemini request is made**.
- **Not deployed.** No Production migration, no Production SQL, no Production database write, no Edge Function deploy, no secret or auth mutation, no `GEMINI_MODEL` change, no quota/pricing/billing change, no Vercel or Chrome Web Store action, and **no provider request of any kind**. The Local = Remote ledger is unchanged in Production; locally the chain is now 77 files, latest `20260903120000_add_gemini_37_38_catalog_models`. **Neither Gemini 3.7 nor Gemini 3.8 has been provider-verified anywhere** — that canary is a separately authorized post-rollout step.

---

## 2026-09-03 (later the same day) — AI-MODEL-SELECTION-001D rollout closure: merged, migrated to Production, canaried — **catalog live; 3.7/3.8 provider generation still unverified**

*This entry follows the 001D implementation entry above and records what happened after it. That entry stays as written: at the time it describes, the migration genuinely was repository-only and Production migration was not authorized. Nothing below rewrites it.*

- **Merged.** PR #269 merged into `main` as the regular two-parent merge commit `7f981f445773e70cb61bbdc4e052037c2de40d9e` (parents `0ad72f6f4bb2a8cbb40ad1f7290f77b8c004de39` and `2c7c71ab5968bab0dfa14beb7448308bca28c202`), 18 files, +1033 / −53. Merged-main `Validate`, `DB Tests` and `Extension (package + real browser)` all passed on the merge SHA, and Vercel deployed Production automatically from it. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C35**.
- **Migrated to Production — database only.** `20260903120000_add_gemini_37_38_catalog_models.sql` was applied through the normal `supabase db push --linked` path against project `lioxtgiputfniqbktcsz`, after a dry run confirmed it was the **only** pending migration (no seeds, no roles). The Production ledger now records `20260903120000 add_gemini_37_38_catalog_models` and Local = Remote parity is restored at **77 / 77**, zero pending, zero orphans.
- **The live catalog is now exactly four rows:** `google/gemini-3.5-flash` (10), `google/gemini-3.6-flash` (20), `google/gemini-3.7-flash` (30), `google/gemini-3.8-flash` (40) — all `enabled`, `selectable`, `provider = google`. No fifth row, no floating `-latest` alias, no preview model, no non-Google provider.
- **The additive claim was verified, not assumed.** The 3.5 and 3.6 rows had their persisted field values and timestamps unchanged before and after (`created_at` = `updated_at` = 2026-09-02 13:13:54.380516+00 on both), confirmed by identical pre/post row fingerprints. Preference state, entitlement state (6 rows, 1 model-selection-enabled), RLS enable+force, the single SELECT-only policy, the grant matrix (`authenticated` SELECT only; `anon` and `service_role` nothing) and the nine-column credential-free schema were all unchanged. The migration's own fail-closed `DO $verify$` block passed inside the same transaction.
- **No deployment accompanied the rollout, and none was needed.** No Edge Function was deployed, no secret changed, `GEMINI_MODEL` and `GEMINI_API_KEY` were untouched, and no frontend deploy was required for the two new model names — the live `AI-MODEL-SELECTION-001C` Settings control reads `ai_model_catalog` dynamically. Database migrations are **not** shipped by a Vercel deploy; this was a separate, separately authorized step.
- **Live UI acceptance.** After the migration the owner refreshed the app and Settings → AI Model listed exactly **Paperlume default, Gemini 3.5 Flash, Gemini 3.6 Flash, Gemini 3.7 Flash, Gemini 3.8 Flash**, in that order, with the previously saved Gemini 3.6 preference still selected — so the catalog expansion neither required a frontend allowlist nor rewrote an existing choice. This is manual live acceptance of model-list discovery and preference preservation, not a browser regression suite.
- **Production canaries — routing verified, generation not.** The owner selected 3.7, then 3.8, through the live UI; both selections persisted. Three Suggest Projects & Tags runs followed, all on `suggest-paper-organization` **v10**, each logging `source=user_preference` with the exact provider model and then failing with Google **HTTP 503**:

  | Model | Attempt | Execution id | Request id | Outcome |
  |---|---|---|---|---|
  | `gemini-3.7-flash` | 1 of 1 | `1c9b20eb-51de-4773-a272-477cfdb3adae` | `01a0668b-759d-72f1-936c-d4b8552ae4a8` | `provider_failure class=provider_unavailable detail=http_503 provider_attempts=1 refund=attempted` |
  | `gemini-3.8-flash` | 1 of 2 | `28f26561-1c4b-4476-9074-5bfec19fca06` | `01a06692-85af-7c55-83d9-40187fe37ec3` | same |
  | `gemini-3.8-flash` | 2 of 2 | `0e0994bc-c97c-4eac-86ba-4e56c0e4e0ea` | `01a06692-ae29-750d-bd0a-5a61fcacc72b` | same |

  Provider counters moved 3.7 `0 → 1` and 3.8 `0 → 2` — one provider request per user action, consistent with the unchanged zero-retry policy. **No Analyze Paper canary was run on either model**; the only Analyze routing line in the window belongs to the earlier 3.6 canary.
- **Interpretation, stated precisely.** The rollout and the routing integration are confirmed; **successful generation on 3.7 and 3.8 is not**. The routing logs argue **against** a Paperlume model-selection/routing defect as the cause — each request resolved the saved preference and carried the exact intended provider model before the provider returned 503 — so nothing here indicates a repair to `_shared/aiModelSelection.ts`, the catalog, the setter or the Settings control. That is a statement about these requests, not a proof that the resolver is defect-free in every respect. Equally, nothing here establishes an officially confirmed Google outage or a global failure of either model; it is provider-availability evidence for these requests. `refund=attempted` records that the refund path ran and is **not** proof that a refund succeeded. The system default remains `gemini-3.5-flash` (**C34**), no automatic failover exists, and no model was disabled or removed.

---

## 2026-09-03 — CHROME-EXTENSION-IMPORT-001D: deterministic import duplicate resolution and additive assignment (`20260903180000`) — **implemented; Production migration NOT authorized**

- **One migration:** `20260903180000_add_deterministic_import_duplicate_resolution.sql`. It replaces the body of `public.safe_bulk_insert_papers(uuid, jsonb)` and adds two new RPCs, `public.bulk_add_paper_projects(uuid[], uuid[])` and `public.bulk_add_paper_tags(uuid[], uuid[])`. It creates no table, column, index, constraint, policy or trigger; it writes, backfills and repairs no row; it changes no RLS policy and loosens no existing grant. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C36**.
- **The gap it closes.** `safe_bulk_insert_papers` answered a `unique_violation` with `{ status: "duplicate" }` and no paper id, so the canonical importer recorded the identifier as skipped and its assignment phase ran only over newly inserted ids. `/extension-import` therefore told the user, truthfully, that their selected Projects and Tags were **not** applied to the paper already in their library. That was fail-closed by design until deterministic duplicate identity existed. This migration supplies it.
- **Resolution mirrors the unique indexes, and nothing else.** On `unique_violation` the function reads only the attempted row's own `pmid` and `doi` and queries only `public.papers` scoped to `user_id = p_user_id`, comparing `pmid` as stored and `doi` through `lower()` — the exact semantics `idx_papers_user_pmid_unique` and `idx_papers_user_doi_unique` enforce, including their "only when non-null" restriction. **No title, fuzzy, trigram, author, year, journal, abstract, URL or metadata comparison participates, and no provider is called.** Candidates are collected as `array_agg(DISTINCT id)`, so one row matched by both of its identifiers is one candidate rather than an ambiguity.
- **Exactly one distinct candidate returns an id; anything else returns none.** Zero candidates (a `unique_violation` from some other constraint) and two-or-more candidates (a PMID naming paper A and a DOI naming paper B) both keep `status = "duplicate"` with **no `id`**. The two-row case is the one that mattered: both indexes are violated by different rows, and every available tie-break — constraint evaluation order, id order, creation order, closest title — is an accident rather than an answer, so the function refuses rather than filing someone's taxonomy against a paper they did not pick.
- **Everything else about the function is byte-for-byte the prior behaviour.** Same signature, same `SECURITY DEFINER`, same `search_path=public`, same `p_user_id = auth.uid()` guard with explicit NULL-auth rejection, same statistical-method canonicalization, same structured publication-type and author-provenance handling, same column list, same per-row exception isolation, same `inserted` and `error` results, same result ordering and `index` alignment. Cross-user leakage is structurally impossible: the candidate query is scoped to the already-validated caller, and uniqueness is per-user, so another account holding the same PMID is not a collision and not a candidate.
- **The additive RPCs contain no `DELETE`, and that is checked rather than asserted.** `bulk_set_paper_projects` / `bulk_set_paper_tags` are replace-all setters — correct for a row created moments ago that owns nothing, and exactly wrong for a paper that has existed for months, where they would delete every Project and Tag it was already filed under. The new pair validates that **every** referenced paper and **every** referenced Project/Tag belongs to `auth.uid()` *before* any write (so one foreign or unknown id rejects the whole call rather than producing a partial mutation), rejects a NULL array element as malformed input, treats an empty or absent selection as a no-op, and inserts with `SELECT DISTINCT … ON CONFLICT DO NOTHING` so repeated ids and already-present memberships are idempotent rather than errors. Both are `SECURITY DEFINER` with `search_path=public`, and follow the repository's REVOKE-first posture from `20260802025704`: `PUBLIC`, `anon` and `service_role` hold nothing; `authenticated` holds EXECUTE. The migration's closing `DO $verify$` block fails the migration if either new function contains a `delete`/`truncate`, if either existing setter has *stopped* containing one, if any of the three functions loses `SECURITY DEFINER` or its pinned `search_path`, if any grant is wrong, or if either per-user unique index is missing.
- **Application: opt-in, and only one caller opts in.** `bulkImportPapers` gains `applyAssignmentsToResolvedDuplicates` (default `false`). Without it the duplicate id is not even read, so Add Papers, PubMed Search and `bulkImportFromParsedData` behave exactly as before — parsed-file import does not accept the option at all, so passing it there is a type error rather than a silent no-op. The importer also now returns a **terminal result** (`BulkImportOutcome`) reported *after* the assignment phase: per-identifier `inserted` / `duplicate-resolved` / `duplicate-unresolved` / `failed`, plus per-category `not-requested` / `applied` / `failed` evidence for the inserted group and the resolved-duplicate group separately. `/extension-import` opts in and speaks only from that result; it re-resolves nothing itself and issues no lookup of its own. Newly inserted rows keep the existing `bulk_set_*` path and its existing warning semantics unchanged. Counters keep their meaning: a resolved duplicate is still **skipped**, never **added**. Cache invalidation follows the *attempt*, so a partial success still becomes visible.
- **Deployment ordering — web first is safe, by construction and by test.** Before the migration is applied, the deployed `safe_bulk_insert_papers` returns duplicates with no id; the client treats that as unresolved, calls neither new RPC (so it cannot call a function that does not exist), and reports the selection as not applied. After the migration, the same already-deployed client activates the additive path with **no second frontend deploy**. See [deployment.md](deployment.md) §6.3.
- **Tests.** New database suite `supabase/tests/database/013_import_duplicate_resolution.test.sql` (**77 assertions**): the full resolution matrix (new insert, PMID duplicate, case-folded DOI duplicate, both-identifiers-one-row, the split two-row ambiguity asserted in both directions, three-user isolation including that a third account importing both shared identifiers simply inserts), title equality proving nothing (twice — a duplicate title is not a duplicate, and a PMID collision follows the identifier rather than a matching title), the unchanged null-auth / mismatched-caller / `anon` guards, preserved provenance and canonicalization, per-row isolation in a mixed batch, and the additive RPCs' additivity, idempotency, multi-paper behaviour, all six fail-closed reference cases with a state assertion proving no partial mutation, authorization, and no-op-on-empty. Suite `003` extends its **exhaustive** inventory from 31 to 33 SECURITY DEFINER functions and its EXECUTE matrix from 25 to 27 directly-callable RPCs (213 → **223** assertions). `src/hooks/papers/__tests__/useBulkMutations-assignment.test.ts` grows 10 → **23** tests covering the setter-versus-additive split, the opt-in gate in both directions, both single-category paths, the both-selected path, the no-selection path, per-category failure, partial truthfulness, cache invalidation, unchanged progress-callback semantics, and that parsed-file import ignores a resolved duplicate id. Repository totals: 144 test files, 4,526 → **4,561** tests.
- **E2E.** `e2e/extension-import.spec.ts` replaces the now-obsolete "does not assign the selection to the existing paper" case with five: the PMID duplicate adding the selection while the two memberships it already had survive (asserted at the wire — `bulk_add_*` called, `bulk_set_*` not — and against the persisted row); a duplicate with no selection issuing no assignment RPC at all; a partial failure where `bulk_add_paper_tags` is failed at the HTTP boundary and the page reports the Project applied and the Tag not, with the pre-existing Tag intact and still exactly one row; a DOI duplicate resolved twice, the second time through a differently-cased DOI, which lands on the existing paper and creates no second row (stated precisely: canonical normalization lowercases DOI before the insert, so that case is an **end-to-end** result about the whole pipeline and is deliberately *not* cited as isolated proof of the SQL resolver's case branch — that proof is suite `013`, which persists mixed-case DOI data and calls the RPC directly); and the pre-migration compatibility case, which forwards the real RPC call and deletes `id` from the real duplicate response (asserting the strip actually fired, so the test cannot pass vacuously) and then requires that **no** additive RPC was called and the page says the selection was not applied. 19 tests, all green against a full local migration replay. No Edge Function is served and no PubMed/Crossref egress occurs.
- **Not deployed.** No Production migration, no Production SQL, no Production database write, no Edge Function source change or deploy, no secret or auth mutation, no provider request of any kind, no Vercel action, and **no Chrome Web Store action** — `extension/**` is byte-identical and the packaged artefact, permissions, listing, privacy answers and submission state are all untouched. Locally the chain is 78 files, latest `20260903180000_add_deterministic_import_duplicate_resolution`; Production remains at 77 with this migration local-only.
- **Forward pointer.** The status above is the state **at the time this entry was authored** and is kept as written. `20260903180000` was subsequently applied to Production — when, the ledger cannot say — and on **2026-09-11** it was verified live and its database/RPC semantics passed a bounded Production acceptance; see the 2026-09-11 closure entry at the end of this file.

---

## 2026-09-04 — ATTACHMENT-ORPHAN-CLEANUP-HARDENING-001: durable, recoverable attachment cleanup (`20260904120000`) — **implemented; Production migration NOT authorized**

- **Two migrations, applied with an operator checkpoint between them.** `20260904110000_prepare_merge_lock_order.sql` (phase 1) replaces `merge_exact_duplicates` with a body that takes `LOCK TABLE public.papers IN ROW EXCLUSIVE MODE` before it re-parents `paper_attachments`, and does nothing else — no table, policy, grant, trigger or index, and no reference to the cleanup queue, tombstone, RPCs, drain or Storage fence, none of which exist at that point in the ledger. It exists because the merge as deployed in Production is the one writer that goes child-before-parent, which is the single order phase 2's barrier cannot tolerate; the two deadlock, with the migration as the victim. No barrier reordering fixes it, because a stale bundle's direct paper deletion goes parent-before-child, so one of the two always cycles. And it cannot be fixed inside phase 2, because `CREATE OR REPLACE FUNCTION` does not drain in-flight executions (measured on PostgreSQL 17.6: the replacement returned in 32 ms and the running call still completed with the old body). So phase 1 must commit, every transaction predating it must end — verified, not waited out, with the query in [deployment.md](deployment.md) §6.4 — and only then may phase 2 run. Phase 2 asserts both conditions itself, fail-closed, before taking a lock. Phase 1 is semantically inert and safe to leave in place indefinitely if phase 2 is delayed.
- **Phase 2:** `20260904120000_add_recoverable_attachment_cleanup_queue.sql`. It creates two tables (`public.attachment_cleanup_queue`, `public.attachment_cleanup_tombstone`), two indexes, two RLS policies, four `SECURITY DEFINER` functions executable by `authenticated` (`delete_attachment_with_cleanup(uuid)`, `delete_papers_with_attachment_cleanup(uuid[])`, `finalize_attachment_upload(uuid, text, text, text, integer)` and the Storage-policy predicate `attachment_object_has_live_metadata(text)`), one `SECURITY DEFINER` trigger function plus its BEFORE INSERT trigger on `paper_attachments` (`reject_attachment_over_cleanup_intent()` / `trg_paper_attachments_block_cleanup_intent`) and one `SECURITY INVOKER` internal helper (`attachment_cleanup_path_is_safe(uuid, text, uuid)`). It replaces exactly one existing object — the `attachments_owner_delete` Storage policy, which keeps its name and its owner-prefix condition and gains one more (see below) — and narrows exactly two existing grants: `INSERT`, `UPDATE`, `DELETE` and `TRUNCATE` on `public.paper_attachments`, and `DELETE` and `TRUNCATE` on `public.papers`, are revoked from `authenticated`, `anon` and `PUBLIC` — leaving `SELECT` on the former and `SELECT`/`INSERT`/`UPDATE` on the latter (see below). Otherwise it alters no existing table, column, trigger or policy, backfills and repairs no row, and touches no Storage *object*. The whole file runs inside an explicit `BEGIN … COMMIT`. It opens with the phase gate (which takes no lock, so a refusal costs nothing), then `LOCK TABLE auth.users IN SHARE ROW EXCLUSIVE MODE`, then `LOCK TABLE public.papers IN SHARE MODE`, then `LOCK TABLE public.paper_attachments IN ACCESS EXCLUSIVE MODE`. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C37**.
- **The gap it closes, stated as it actually was.** The 2026-era entries in this ledger describing attachment cleanup as best-effort were **accurate for their time and are not rewritten**. Until this migration, `deletePaper` / `bulkDeletePapers` read the attachment paths, deleted the papers (metadata cascading, quota refunding), and then made one `storage.remove()` whose failure was caught and logged as non-critical. After the delete, those paths existed nowhere but a local variable in one browser tab: a refresh, a crash or a closed laptop destroyed the only remaining knowledge of an object that still existed, and the orphan survived until account deletion swept the namespace. `deleteAttachment` had the mirror problem — Storage first, metadata second — where a failed metadata delete left a row and its quota charge pointing at a file that was already gone. Upload compensation had the same weakness in miniature: one un-inspected `remove()` with no durable record if it failed.
- **What replaced it: enqueue in the same transaction as the logical delete.** Both deletion RPCs write the Storage key into `attachment_cleanup_queue` and remove the metadata that named it in **one** Postgres transaction, so the key cannot stop being reachable without the database still holding it. `delete_papers_with_attachment_cleanup` additionally validates **every** paper id against `auth.uid()` before mutating anything (one foreign or unknown id rejects the whole call), normalises duplicate ids, rejects a NULL element, and refuses the entire call if any collected path fails namespace validation. The single-paper client calls the same function with a one-element array — one implementation, so the ordering cannot drift between the two paths.
- **The queue is closed to clients by both grant and policy.** RLS is enabled *and* forced; `authenticated` holds `SELECT` and `DELETE` on its own rows and **no INSERT and no UPDATE**, with the corresponding policies simply absent. A queue row is an instruction to delete a Storage object, so a client able to write one could schedule the destruction of its own valid attachments. `anon`, `PUBLIC` and `service_role` hold nothing. `(user_id, file_path)` is UNIQUE, so repeated intent is one job; `reason` is confined by CHECK to `attachment_delete` / `paper_delete` / `upload_compensation`; `user_id` cascades from `auth.users`; and there is deliberately **no FK to `papers` or `paper_attachments`**, because an intent that cascaded away with its paper would be destroyed by the statement it exists to outlive.
- **Upload is the subtle half, and a sequential existence check is not enough for it.** The failure to avoid is the mirror of the orphan: a metadata write whose HTTP response is lost *may still have committed*, so a browser that treats its own observed error as proof and deletes the object destroys a valid, quota-charged attachment. Asking the server "does a metadata row exist for this path?" in a second transaction does not fix that — it cannot see a transaction still in flight, and it answers "no" for exactly the write that is about to commit. So the metadata INSERT is not a client operation at all on this schema. `finalize_attachment_upload` takes a transaction-scoped advisory lock on `(auth.uid(), file_path)` **before it reads anything**, which makes two attempts at one object strictly ordered instead of concurrent; each then observes whatever the earlier one committed and returns `metadata_committed`, `metadata_present` (queueing nothing) or `cleanup_queued` (writing no metadata). Because the fresh-snapshot half of that argument is an isolation property rather than a lock property, the function asserts `READ COMMITTED` and refuses to run otherwise rather than silently reopening the window.
- **The metadata rejection path keeps the intent and drops the quota.** The INSERT runs inside a PL/pgSQL block with an `EXCEPTION` handler, so a rejection — a quota refusal, a constraint, anything — rolls back both the row and the storage quota the BEFORE INSERT trigger consumed for it, while the enclosing transaction survives to commit the `upload_compensation` queue row. Quota is charged exactly once on success and not at all on rejection.
- **A tombstone makes the exclusion a property of the table.** `trg_paper_attachments_block_cleanup_intent` refuses any `paper_attachments` INSERT whose `(user_id, file_path)` has a pending cleanup intent or a permanent cleanup tombstone. The trigger is where the exclusion stops depending on who is calling: it applies to the `SECURITY DEFINER` lifecycle writer as much as to anything else, so the rule belongs to the table rather than to one function's callers. (When this trigger was written, `authenticated` still held INSERT here and the trigger was the only thing standing between a stale bundle and a resurrected path; that grant is now revoked as well — see the lifecycle boundary below — so the two guards are independent rather than redundant.) It is named to sort before `trg_paper_attachments_check_storage_quota`, so a refusal costs no quota accounting.
- **The decision outlives the work, which is what makes idempotency durable.** A queue row is deleted the moment the drain confirms the object is gone. If that were the only record, a duplicated or delayed finalization arriving afterwards would find no metadata and no intent and would create metadata for a binary that no longer exists — and the ambiguous-response contract is precisely what asks clients to retry. `public.attachment_cleanup_tombstone` (`user_id`, `file_path`, `created_at`; PK on the pair) keeps that decision permanently. It has **no policy of any kind** and no privilege for any client role: it is read and written only by `SECURITY DEFINER` code, because a row a user could delete would be a row a user could delete in order to resurrect a removed object. Deletion-sourced cleanup is deliberately not tombstoned — only finalization creates metadata, so tombstoning those paths would grow a permanent record of every attachment ever deleted for no invariant.
- **Paper deletion and finalization are serialized on the paper.** `delete_papers_with_attachment_cleanup` snapshots the attachment paths it will queue and then deletes the papers; between those two steps an upload could commit an attachment the snapshot never saw, and the cascade would then destroy the only record of its Storage object. The FK does not help — it makes the cascade happen, it does not make the snapshot current. Both writers now take the same per-paper advisory lock (class `20260905`): deletion before its snapshot, in sorted id order; finalization before its first read, after the per-path lock. That ordering is what makes the protocol deadlock-free — finalization takes exactly one paper lock and deletion takes no path lock, so no cycle can form. When deletion wins, finalization finds no paper and declares the object garbage rather than raising, because raising would abort without recording anything and leave the binary as an orphan. A paper that exists but belongs to someone else is still refused outright: papers never change owner, so that is a bad request rather than a race.
- **The Storage delete policy gained a condition, and that is the stale-client answer.** After the migration a browser tab still running the pre-migration bundle can still attempt both historical destructive orderings, and no new client code reaches it. So `attachments_owner_delete` now also requires `NOT public.attachment_object_has_live_metadata(name)` — the owner may not delete an object a live `paper_attachments` row still names. Every legitimate path already removes or never had the metadata first, so all of them still work; both stale orderings fail safely instead. The fence exists **only after this migration commits** — it, the queue, the tombstone, the lifecycle RPCs and the drain are all created by it, together — so there is no pre-migration state in which the fence is active and none should be described. Against the **pre-migration** database there is no fence at all, and a stale tab's direct `DELETE FROM papers` behaves as it always has. Against the **post-migration** database that path is gone, because `DELETE` on `papers` is revoked: the stale tab is refused with `42501` at the `DELETE` itself and never reaches its Storage call. It fails early and strands nothing, which is the intent — but it is a refusal, not a path that keeps working. The predicate is `SECURITY DEFINER` so the fence sees all metadata rather than what the caller's RLS exposes, takes no user id and derives the caller from `auth.uid()`. `idx_paper_attachments_user_file_path` was added to serve it. Account deletion is unaffected: it sweeps with the elevated role, which bypasses RLS, and must — it exists to find objects no metadata row describes.
- **The lifecycle boundary: browsers stopped writing this table.** Every argument above is about which of two systems to trust when a browser and the database disagree, and that question only arises because the browser may write attachment metadata directly. It may not, after this migration: `REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON public.paper_attachments FROM PUBLIC, anon, authenticated`, with `GRANT SELECT ... TO authenticated` restated so the read path cannot be taken along by accident. Metadata is thereafter created and destroyed only by the three `SECURITY DEFINER` RPCs, the cascade they initiate, and account deletion — which makes *"Postgres never removes ordinary user attachment metadata without recording the Storage cleanup intent in the same transaction"* a database-enforced Product invariant rather than a convention of the current bundle. `TRUNCATE` is included because it removes every row without firing a row trigger or consulting RLS: it is the one statement that could refund no quota and record no intent. `UPDATE` was already unreachable (this table has no UPDATE policy and never has), but Production carries the old platform-default ACL that granted the API roles everything, so the privilege is revoked rather than left standing on a technicality. This is a deliberate, feature-specific narrowing of `20260731162729_reconcile_data_api_grants` — every other grant in that migration stands. `service_role` is untouched: nothing writes this table with it today, but the account-deletion cascade and any future privileged repair are exactly what a server role is for. The now-dead `owner insert` / `owner delete` RLS policies are kept as defense in depth. `REFERENCES`, `TRIGGER` and `MAINTAIN` remain, as they do on every table by platform default; none can create or remove a row, and `TRIGGER` is inert besides — neither client role holds `CREATE` on schema `public`, so neither can define the function a trigger would need.
- **The cutover barrier, because a revoke waits for nobody.** `REVOKE` locks catalog rows, not the table. Without a barrier a direct metadata `INSERT` permission-checked *before* the cutover commits after it, and the Storage fence — asked whether live metadata names the object — gets "no" from a row that has not committed yet, permits the delete, and the row then commits: metadata present, quota charged, binary gone, with every guard in place and none of them able to see it. The migration therefore opens with a three-table barrier — `SHARE ROW EXCLUSIVE` on `auth.users`, then `SHARE` on `public.papers`, then `ACCESS EXCLUSIVE` on `public.paper_attachments` — and holds all three to commit. `ACCESS EXCLUSIVE` conflicts with `ROW EXCLUSIVE`, so acquiring it waits out every open writer; it conflicts with everything, so nothing new begins between the barrier and the revoke; and a table lock is released exactly at commit, when the new privileges become visible — a statement queued behind it processes the relation's invalidation as it takes its own lock and is planned against them, so it gets `42501` rather than a successful write. The two parents come first, in that order, because the migration's own foreign keys need `SHARE ROW EXCLUSIVE` on `auth.users` and acquiring that *after* the downstream barriers is a lock-order inversion that deadlocks against an ordinary account deletion; the modes and the ordering are derived in full in [decisions-and-triggers.md](decisions-and-triggers.md) **C37**. The explicit `BEGIN … COMMIT` is required for two independent reasons: `supabase db reset` runs a migration file statement-at-a-time in autocommit, where `LOCK TABLE` raises `25P01`; and the Storage fence and the revoke must become visible in the same instant, since a live fence with writable metadata can be walked past by an uncommitted INSERT, while a revoke landing first leaves a window in which a stale tab can still delete the binary of a row that committed moments earlier. It also makes the closing `DO $verify$` block's own promise true — nothing lands unless everything verifies. There is deliberately **no `lock_timeout`**: a timeout turns a correctness barrier into a race the migration sometimes loses. Operationally the migration blocks all writes to `auth.users` (signup, account deletion and Auth user mutation — reads and foreign-key reference checks continue), all access to `paper_attachments` (reads included), and all writes to `papers`, for its catalog-only, millisecond duration; and it waits beforehand for the longest open Auth, paper or attachment transaction. Apply it as any DDL, not during a bulk operation, accepting that a signup or sign-in arriving in the window waits rather than fails.
- **The parent boundary: the cascade is the other door.** `paper_attachments.paper_id` is `ON DELETE CASCADE` from `papers`, so deleting the parent removes attachment metadata without any statement naming the child — and the pre-migration bundle deletes papers with a raw `DELETE FROM papers`, reads the attachment paths beforehand, and only afterwards asks Storage to remove the binaries. Revoking direct DML on `paper_attachments` alone would therefore have left the invariant true only for callers who happened to name that table. `REVOKE DELETE, TRUNCATE ON public.papers FROM PUBLIC, anon, authenticated`, with `GRANT SELECT, INSERT, UPDATE` restated so ordinary paper authoring cannot be taken along by accident. `TRUNCATE` is included because it empties the table without firing a row trigger or consulting RLS, and the local platform default grants it to `anon` here (`anon=Dxtm` on `papers`). Exactly two functions still delete papers, and suite `003` pins that count and their names: the lifecycle RPC, and `merge_exact_duplicates`, which re-parents attachment rows onto the kept paper before removing the discards so nothing cascades and no object is stranded. Account deletion is the third path and is unaffected — it deletes the auth user and sweeps Storage itself.
- **The two-table cutover order is derived, and one existing function was re-declared to make it provable.** The lock on `papers` has to conflict with `ROW EXCLUSIVE` (or it drains no paper writers) and must not conflict with `ROW SHARE`/`ACCESS SHARE` (or it blocks the foreign-key check of an in-flight `paper_attachments` INSERT while that transaction holds the child the migration is waiting for) — which leaves `SHARE`. The order is forced too: child-first would put an in-flight `DELETE FROM papers`, holding the parent and needing the child for its cascade, on the other side of a cycle that cannot be repaired because that transaction is a browser's raw statement. Parent-first inverts the cycle onto this repository's own code, so `delete_papers_with_attachment_cleanup` (section 4) and `merge_exact_duplicates` (section 4b) each now take `LOCK TABLE public.papers IN ROW EXCLUSIVE MODE` before touching `paper_attachments`. That is the ONLY line that differs in `merge_exact_duplicates`; it is otherwise byte-identical to `20260817120000`, `ROW EXCLUSIVE` is self-compatible and is the lock its own DELETE takes moments later, so no ordinary concurrency changes. The resulting proof is one sentence: while the migration holds `SHARE` on `papers` and waits for `paper_attachments`, every session that can hold a `paper_attachments` lock needs at most `ACCESS SHARE` or `ROW SHARE` on `papers`, and `SHARE` grants both.
- **What a stale bundle can and cannot do after the cutover.** It can no longer create a valid metadata row, so the half-state that made both historical orderings dangerous is unreachable: its upload fails visibly at `42501` instead of quietly producing a damaged attachment, its Storage-first deletion is refused by the fence, and its lost-response compensation is refused too. Its **paper deletion** now fails at `42501` as well — and because that path reads the paths, deletes the papers and only then calls Storage, a refusal at step two means step three never runs, so it cannot strip binaries off papers it did not delete. What it does **not** gain is durable cleanup — it cannot call functionality it does not know exists, so it writes no queue row and no tombstone. If its refused upload's own immediate Storage cleanup also fails, that binary remains as an untracked orphan in the owner's private namespace until the account-deletion Storage sweep finds it. That residual is bounded by how long stale tabs live after the deploy and is documented rather than described as retry.
- **The ordering is proved with real concurrent connections, because a sequential suite cannot see it.** `runAttachmentFinalizationProbe` in `scripts/e2e-local.mjs` joins the two existing bounded, fail-closed probes: it refuses a non-`READ COMMITTED` transaction; it releases two barrier-synchronised workers onto the same path and requires exactly one `metadata_committed` + one `metadata_present` with one row, no cleanup row and the bytes charged once; it forces a deterministic quota rejection and requires one cleanup row, no metadata and no quota charge, then requires the retry to refuse to create metadata even with quota restored; and — the case the design exists for — it holds one finalization open, shows that the superseded existence check reads the stale absence that would have destroyed the file, and requires the corrected RPC to block on the serialization lock and return `metadata_present` instead. The mirror ordering (cleanup committed first) is required to refuse metadata too. `runMigrationCutoverProbe` proves the deployment-time half on the same terms: it restores the pre-cutover grant, opens a real direct `INSERT` and holds it, requires the `ACCESS EXCLUSIVE` barrier to queue behind it (read from `pg_locks`, not inferred), starts a second writer that must not overtake the pending barrier, then releases the first and requires the barrier to acquire, revoke and commit — after which the queued writer must be refused with `permission denied` rather than committed. It then checks the resulting posture end to end (direct INSERT/UPDATE/DELETE/TRUNCATE all refused, `SELECT` preserved, all three lifecycle RPCs still working with no client table grant) and repeats the whole race on the PARENT table — P-CUT-1, an in-flight `DELETE FROM papers` the barrier must queue behind on `papers` itself; P-CUT-2, a second paper deletion that must not overtake the pending barrier and must be refused with `permission denied` once it runs; P-CUT-3/4, direct `DELETE` and `TRUNCATE` refused afterwards while `SELECT`/`INSERT`/`UPDATE` still work; and P-CUT-5, the lifecycle RPC still deleting. It restores the hardened ACL for both tables in a `finally`, which the lane's catalog fingerprint independently confirms.
- **Path validation is one function, and the second segment is deliberately unconstrained on the delete paths.** `attachment_cleanup_path_is_safe` requires exactly `<user>/<paper>/<name>`, no empty / `.` / `..` segment, no backslash, no control character, bounded length, and a first segment exactly equal to the caller — the same boundary `storage.foldername(name)[1]` enforces in Storage RLS. It compares the second segment to a given paper id only where the caller's contract names one (upload finalization). It must not do so on the deletion paths: `merge_exact_duplicates` re-parents attachment rows onto the kept paper and leaves `file_path` untouched by design, so a valid merged attachment legitimately carries a different paper id in its path than in its row, and requiring them to match would make merged attachments undeletable.
- **Storage is never touched from SQL, and that is checked rather than reviewed.** The migration's closing `DO $verify$` block fails the migration if any of the three RPCs loses `SECURITY DEFINER` or its pinned `search_path=public`, if any grant is wrong, if `PUBLIC`/`anon`/`service_role` can execute one, if the queue's RLS/force/policy-set/grant posture drifts, if the auth cascade or the (user_id, file_path) uniqueness is missing, if a FK to `papers`/`paper_attachments` appears, if the helper stops being internal or invoker, if the helper accepts a foreign or traversal path or rejects the canonical one, if either storage-quota trigger on `paper_attachments` has gone, if the four `attachments` Storage owner policies are not all present, if the bucket has become public — or if any cleanup function's body references the `storage` schema at all. It additionally fails if the tombstone trigger or its function is missing or mis-postured, if that trigger's name stops sorting before the quota trigger's, if `finalize_attachment_upload` takes no transaction advisory lock, if it reads `paper_attachments` **before** it takes that lock (an ordering a single-connection test cannot see), or if a `queue_untracked_attachment_cleanup` function exists at all — the superseded compensation RPC whose sequential existence check made the race reachable. It finally asserts the post-cutover authority model from inside the barrier: that BOTH locks are still held at the moment the check runs — `ACCESS EXCLUSIVE` on `paper_attachments` and `SHARE` (specifically, not something stronger) on `papers` — so the posture is verified on tables nobody else can touch; that `authenticated` keeps `SELECT` on `paper_attachments` and holds none of `INSERT`/`UPDATE`/`DELETE`/`TRUNCATE`; that it keeps `SELECT`/`INSERT`/`UPDATE` on `papers` and holds neither `DELETE` nor `TRUNCATE`; that `anon` holds nothing on either and `PUBLIC` nothing at all; that the table owner can still write both tables (or the RPCs would be broken by the revokes); that all three lifecycle RPCs are still `SECURITY DEFINER` and `authenticated`-executable; that both cross-table writers take their `papers` lock before they reach `paper_attachments`; and that an owner `SELECT` policy survives so the attachment list still reads.
- **Quota semantics are unchanged.** The BEFORE INSERT consume and AFTER DELETE refund triggers from `20260521030000` are untouched and still fire from the metadata DELETE inside the RPC's transaction. A queued-but-not-yet-removed object is therefore already refunded while physically present; making quota include pending bytes is a separate Product/accounting decision and was not taken.
- **Application: durable path plus a narrow pre-migration fallback.** New `src/lib/attachmentCleanup.ts` drains the caller's own queue — paginated, oldest-first with an `id` tie-break, bounded to 20 pages, `remove()` batched at 100, every path re-validated in the browser before the destructive call, both thrown and returned Storage failures treated as failures, acknowledgement only after Storage confirms, and no path/file name/paper id/user id ever logged. The bound is also honest about itself: the walk reports `completed` only when it actually observed the END of the queue (a short or empty window). If the last permitted window comes back FULL, more rows may exist that this pass will never see, so the result is `pending` with `pending: null` — an unknown remainder rather than a fabricated zero — even when every job it fetched was removed and acknowledged. The bound is not raised to avoid that state; the next authenticated session takes the following windows. And it holds no offset, because two tabs really do drain the same queue: offset paging is stable only if nothing shifts underneath it — read page 0, let another tab acknowledge twenty rows out of the front, read page 1, and twenty rows have slid into page 0's range where this pass will never look again, after which a short page 1 reads as "the queue is empty". The walk therefore repeatedly reads the HEAD and advances by DELETING what it finishes, which is the only thing that moves it forward.
- **Upload in the browser is now three lines of policy.** `useAttachments` uploads the binary, calls `finalize_attachment_upload`, and branches only on what the server said. It never INSERTs metadata on this schema and never decides that an object is garbage. On an ambiguous transport failure — thrown or returned — it repeats the same idempotent call once (`FINALIZE_MAX_ATTEMPTS = 2`, no timer, no backoff, no scheduled retry); if the database still does not answer, the object is left in place, no cleanup is claimed, and the user is told the upload could not be confirmed. Deleting on a guess is not reachable from any branch. `src/hooks/useAttachmentCleanupRecovery.ts` runs exactly one bounded pass per signed-in user, mounted once in the Dashboard shell, with no timer, no polling and no toast. `src/lib/attachmentCleanupAvailability.ts` recognises only the four cleanup object names under a missing-object code, and refuses even that once any cleanup object has answered in the session — so a partially installed schema fails visibly rather than silently downgrading every user to the old path. `src/hooks/papers/deletePapersCompat.ts` holds the pre-migration deletion sequence, with the one genuine fix that a returned `{ error }` from `remove()` now counts as a cleanup failure instead of being invisible.
- **Deployment ordering — web first is required, by construction and by test.** Before the migration is applied the deployed client sees `PGRST202`/`42883` naming a cleanup object, falls back, and behaves exactly as it did before this change (with the Storage-error observation fixed). After the migration the same already-deployed client activates the durable path with **no second frontend deploy**. Database-first is non-destructive but **not feature-functional**: until the new bundle reaches a tab, that tab cannot upload (its direct INSERT is refused) or delete (its Storage call is refused) an attachment. See [deployment.md](deployment.md) §6.4 and the four-combination table in §6.4a.
- **Generated types:** `src/integrations/supabase/types.ts` regenerated from the **local** database (`supabase gen types typescript --local --schema public`); the diff against `main` is purely additive (+81 lines, 0 deletions) — the two tables and the four function signatures. Not hand-edited (only the generator's trailing blank line normalized for `git diff --check`); the linked/Production schema was not used for generation.
- **Tests.** New database suite `supabase/tests/database/014_attachment_cleanup_recovery.test.sql` (**167 assertions**): the queue's shape, forced RLS, exactly-SELECT+DELETE policy set, grant matrix, uniqueness, reason domain, auth cascade and absent paper FK; cross-account read and acknowledgement isolation; the full matrix for all three RPCs including anonymous/foreign/unknown/NULL rejection with state assertions proving no partial mutation, the quota refund still firing, idempotent repeated intent and fail-closed unsafe paths; the whole upload state machine — metadata committed with the submitted values and the quota charged once, the repeated call reporting the committed row (and returning its id) while creating no second row, charging no second quota and scheduling nothing, the quota-rejected insert committing exactly one `upload_compensation` row with its quota rolled back, the retry refusing to create metadata even with quota to spare, the direct client INSERT over a queued path refused by the tombstone, and metadata and cleanup intent never coexisting for one path; the helper's posture and a 16-case accept/reject matrix; and that no cleanup function references the `storage` schema. The suite states in its own header what it CANNOT prove — the ordering, which every statement in a pgTAP transaction already serializes away — and names the concurrency probe that does. Suite `003` extends its exhaustive inventory from 33 to **38** SECURITY DEFINER functions (31 directly callable, 5 trigger-only, 2 internal) and gains a table-privilege inventory for the attachment lifecycle boundary — per privilege and per role on `paper_attachments`, plus the queue's read-and-acknowledge-only grant the tombstone's total absence of one, the parent table's own privilege matrix, and an exact inventory of the two functions that delete papers with the lock-order conformance both must satisfy — 223 → **281** assertions. Suite `007` gains the internal path helper as a fifth pinned non-RPC function (55 → **59**). Suite `008` proves both cleanup tables cascade with the account and have no paper FK, and that a neighbour's rows survive (84 → **94**). Suites `000`, `002` and `004` keep every ownership-trigger and quota-trigger assertion they had, but issue them as the table OWNER rather than as `authenticated`: after the revoke a direct client write is refused at the ACL before any trigger runs, so leaving them as they were would have made them pass while testing nothing. The writer that remains is the role the lifecycle RPCs execute as, which is exactly who those triggers now guard. `002` additionally re-proves cross-user deletion isolation through `delete_attachment_with_cleanup`, since the grant boundary now refuses everyone equally and that is a weaker statement than the one the suite exists to make (26 → **29**), and `004` pins the ACL boundary beside its existing `user_storage_usage` equivalents (25 → **28**). Suite `001` does the same for the parent: its cross-user paper-delete isolation moves to `delete_papers_with_attachment_cleanup`, with the raw `DELETE` and `TRUNCATE` refusals pinned alongside it (40 → **43**); and suite `010` routes its author-identity cascade case through the lifecycle RPC, which is the call the product now makes. The db-tests residue check now counts `attachment_cleanup_queue` **and** `attachment_cleanup_tombstone`. New application suites: `attachmentCleanup.test.ts` (41, including the five that pin the bounded drain's honesty — completion earned by a short final page, refused by a full one, the unknown remainder, no 21st page, and a truthful count when the walk did finish), `attachmentCleanupAvailability.test.ts` (17), `useAttachments.test.ts` (16), `useAttachmentCleanupRecovery.test.ts` (6), `usePaperMutations-attachmentCleanup.test.ts` (8), `useBulkMutations-attachmentCleanup.test.ts` (7), `deletePapersCompat.test.ts` (7). Repository totals: 144 → **151** test files, 4,561 → **4,669** tests; local pgTAP 1,326 → **1,455** assertions across 15 suites.
- **E2E.** `e2e/attachments.spec.ts` keeps all five existing cases and adds four, deterministic only at the HTTP boundary and never faking an outcome the server should decide: a forced Storage-delete failure on a direct attachment delete, proving the metadata row is gone, the queue row exists with `reason=attachment_delete`, the binary is still present and the UI says cleanup is pending — then, with the interception removed, that a reload's session-recovery pass removes the binary and clears the queue; the same for a **server-side** metadata rejection, produced by rewriting the finalization request's `p_file_name` to `null` so the database's own INSERT refuses it and commits the `upload_compensation` intent itself; the same for a paper deletion with `reason=paper_delete`, additionally asserting the paper row is **not** restored; and a genuine lost-response case — the first finalization request is DELIVERED with `route.fetch()` (so the transaction commits) and then its response destroyed with `route.abort()`, after which the browser's idempotent retry must produce exactly one metadata row, an empty queue and a binary that is still present. That last case is the correction's headline claim reproduced at the real HTTP boundary with no timing in it. Bulk deletion is pinned by unit and database tests rather than a browser case — it calls the identical RPC with a longer array — which is stated here rather than implied. Two further cases pin the lifecycle boundary at the browser: the pre-migration upload sequence replayed byte for byte against the post-migration schema — PUT the object, then a direct `.from("paper_attachments").insert(...)` — which must fail with `42501`, create no row, leave the binary in place, and then be cleanable by the old bundle's own `remove()` because no metadata names it (with the queue asserted **empty**, since a stale bundle gains no durable intent); and a live attachment whose metadata must survive a direct `.update()` and a direct `.delete()`, both `42501`, before the authoritative RPC removes both halves. A third pins the parent boundary at the browser: the old bundle's single and bulk `from("papers").delete()` calls must both fail with `42501`, and the paper, its attachment metadata, its binary and an empty cleanup queue must all survive. `e2e/attachments.spec.ts` therefore holds **15** cases.
- **Not deployed.** No Production migration, no Production SQL, no Production database write, no Production Storage read/write/delete, no Edge Function source change or deploy, no secret or auth mutation, no provider request of any kind, no Vercel action, and no Chrome Web Store action — `extension/**` and `supabase/functions/**` are byte-identical. Locally the chain is 79 files, latest `20260904120000_add_recoverable_attachment_cleanup_queue`; Production remains at 78 with this migration local-only.

## 2026-09-10 — ATTACHMENT-ORPHAN-CLEANUP-HARDENING-001 rollout closure: merged, migrated to Production, wet-accepted — **live and technically closed**

*This entry follows the 2026-09-04 implementation entry above and records what happened after it. That entry stays as written: at the time it describes, the work genuinely was repository-only and the Production migration was not authorized.*

- **Merged in two PRs.** `#272` (`feat/attachment-orphan-cleanup-hardening-001`) merged as `a3d9a2e85919fedbc8fb56150d09ef9cb1af7207`, and the follow-up `#273` (`fix/attachment-phase2-production-acl-parity-001`) merged as `daefd093b70b6c9780cab98f8dff924ec694b1c7` — the ACL-parity correction that makes both revokes name the **role** rather than the privileges, so the cutover converges hosted Production's legacy ACL as well as a clean replay. At the time this closure record was authored, `main` was `daefd093` with tree `a51d6880`.
- **Migrated to Production in the two authorized phases.** `20260904110000_prepare_merge_lock_order.sql`, then a verified drain of the legacy `merge_exact_duplicates` body, then `20260904120000_add_recoverable_attachment_cleanup_queue.sql` behind its three-table barrier. The corrected frontend was already deployed, which is the ordering [deployment.md](deployment.md) §6.4 requires. **The exact application date is not recoverable** — `supabase_migrations.schema_migrations` carries no applied-at column — but it falls between the `#273` merge (2026-09-05) and the acceptance below.
- **Measured Production outcome.** Ledger **79 → 80**, latest `20260904120000`, each phase present exactly once. Ordinary `public` tables **26 → 28** (the queue and the tombstone). Tables carrying a direct `anon` ACL **19 → 17** — the two reconciled tables dropped out, and *only* those two. Phase 1's `merge_exact_duplicates` definition MD5 stayed `0eadb7e71b8515fee7a5f364fa32a2a3` across phase 2, proving the cutover did not disturb it. The lifecycle RPCs and helpers, both indexes, `trg_paper_attachments_block_cleanup_intent` and the `attachment_object_has_live_metadata` fence on `attachments_owner_delete` are all live, and the target ACLs hold: `authenticated` = `SELECT` only on `paper_attachments`, `SELECT`/`INSERT`/`UPDATE` on `papers`, `anon` nothing on either.
- **Bounded wet acceptance passed (2026-09-10).** Run against the real Production application and Supabase project on a **dedicated, empty Production account**, under an explicit budget of two disposable papers and six uploads (five used), every artifact carrying a unique run marker. Cases A–E all passed: normal upload → persistence across a reload → normal deletion; a forced Storage `503` leaving a durable `attachment_delete` queue row with the binary intact and the metadata gone; a **real** first `finalize_attachment_upload` whose response was destroyed at the browser boundary, reconciled by one idempotent retry into exactly one metadata row with no queue row and **no tombstone**; the stale destructive doors all refused (a `storage.remove()` of a live attachment deleting **zero** objects, and `42501` for direct `paper_attachments` UPDATE/DELETE and for a raw `DELETE FROM papers`); and a corrected paper deletion under a forced `503` producing a durable `paper_delete` row without resurrecting the paper. In both failure cases the next authenticated session drained the queue by itself — queue read, Storage `DELETE`, then acknowledgement — with no manual queue write and no admin Storage deletion.
- **Zero residue.** The acceptance account returned to zero papers, zero attachments, zero queue rows, zero tombstones and zero Storage objects; no marker artifact remains anywhere in Production; Production-wide attachment metadata and attachment Storage objects both returned to **10**, and the queue and tombstone tables are both empty globally. One zero-valued `user_storage_usage` row persists for that account — ordinary quota accounting created by the first upload, not an orphan. The acceptance account itself remains present by design.
- **Deliberately not done.** No `upload_compensation` tombstone was manufactured in Production; that permanent-rejection branch stays proven by the migration's own `DO $verify$`, pgTAP `014_attachment_cleanup_recovery.test.sql` and deterministic local E2E. No historical orphan hunt was performed, and none is claimed. No service-role or admin mutation, no SQL DML, no Auth or Storage admin action, no Edge deploy, no Vercel action, no secret or provider change, and no Chrome Web Store action.
- **Still open, and separate.** The repository-wide legacy Data API ACL item is untouched: **17 of Production's 28** ordinary `public` tables still carry a direct `anon` grant inherited from Supabase's old platform default. This initiative reconciled exactly two tables as a consequence of its own invariant; it is not a global ACL remediation and must not be read as one.

## 2026-09-10 — DATA-API-ACL-RECONCILIATION-001: the complete Data API client-role relation/sequence ACL matrix (`20260910212202`) — **implemented; Production migration NOT authorized**

- **What it is.** `20260910212202_reconcile_data_api_acls.sql` converges `PUBLIC`, `anon` and `authenticated` on all 28 ordinary `public` tables and the one sequence, and hardens the default privileges `postgres` hands to future tables and sequences in `public`. It is the completion of the work `20260731162729_reconcile_data_api_grants.sql` started; that migration was deliberately additive (GRANT only, no REVOKE) and **is not rewritten** — it remains an accurate record of what it did and why.
- **What it converges.** Three groups. **G1** — the 17 tables still carrying the platform's original blanket ACL (`anon` holding all eight privileges): revoked by role, then `authenticated` re-granted its exact surface, with `usage_counters` / `subscriptions` / `subscription_events` getting no re-grant at all because they have no client policy of any kind. **G2** — `papers` and `paper_attachments`, whose DML `20260904120000` already converged but whose `REFERENCES`/`TRIGGER`/`MAINTAIN` for `authenticated` it did not name; the DML sets are restated exactly as that migration left them. **G3** — the nine already-narrow tables, swept for `PUBLIC`/`anon` only: `authenticated` is deliberately not re-issued there, because `REVOKE ALL … FROM authenticated` would also drop the column-level `author_identities.preferred_name` grant, which a table-level re-GRANT cannot restore.
- **The sequence.** `papers_insert_order_seq` keeps `USAGE` for `authenticated` — the minimum a direct `papers` INSERT needs to evaluate its `nextval()` default — and loses `SELECT` and `UPDATE`, the latter being what permits `setval()`. `service_role` is absent from the sequence's `REVOKE`/`GRANT`, so its posture (`rwU` on Production, `wU` on a clean replay) is preserved as-is.
- **Future objects (D1a).** `ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE ALL ON TABLES/SEQUENCES FROM PUBLIC, anon, authenticated`. A new table whose author forgets its ACLs is now unreachable rather than exposed. This does **not** replace the per-object REVOKE-first rule; see [decisions-and-triggers.md](decisions-and-triggers.md) C38.
- **Explicitly not touched:** `service_role` (relation privileges, sequence privileges and default privileges), function EXECUTE and function default privileges, `supabase_admin` defaults, RLS/policies/FORCE-RLS flags, and the `paper_tags`/`paper_projects` SELECT/INSERT/DELETE contract. The migration references `service_role` in its preconditions and verification but names it in no privilege-mutating `GRANT`, `REVOKE` or `ALTER DEFAULT PRIVILEGES` statement; its exact observed posture is preserved, and the migration refuses to commit if that posture moved.
- **Two supported starting histories.** Hosted Production (`anon` holds everything on the 17; broad `postgres` defaults) and a clean replay under the current CLI (non-DML residue only). The preconditions accept exactly those two and refuse anything else — including a table or sequence added since the audit, whose intended surface nobody has reviewed. `postgres`'s `public` table and sequence default entry is judged **whole**: `anon` and `authenticated` must each match the same history, no direct `PUBLIC` default and no grantee outside `postgres`/`anon`/`authenticated`/`service_role` may exist, and `service_role` must be a recognised platform shape. An unexplained state stops the migration before it changes anything, rather than being normalized or silently preserved. Supabase's own 2026-10-30 move of existing projects to opt-in defaults produces the second shape, so it does not break the migration in either order.
- **Transactional and fail-closed.** The file is wrapped in explicit `BEGIN … COMMIT` — the second migration to do so, for the reason `20260904120000` documents: `supabase db reset` runs statements in autocommit, where a closing verification block would abort with earlier statements already committed (here: `authenticated` revoked and not yet re-granted). Its `DO $verify$` asserts direct **and** effective privileges for `PUBLIC`, `anon` and `authenticated`; an allowlist over every other grantee (on relations and the sequence only the owner, `authenticated` and `service_role`; in `postgres`'s `public` table/sequence defaults only the owner and `service_role`); the single column grant; the sequence; and `service_role` equality against a snapshot taken before the mutation — and it creates a throwaway table, view and sequence to prove what a *new* object actually inherits, dropping them again before commit.
- **Tests.** New suite `supabase/tests/database/015_data_api_acl_matrix.test.sql` (**88 assertions**): catalog-driven, exact-match, failing on over-grant as well as under-grant, and checking allowlists as well as named roles so a grantee nobody named fails it. It enumerates every Data-API-reachable relation kind (ordinary, partitioned, view, materialized view, foreign) so an unclassified future relation fails CI; asserts direct and effective privileges separately, using `grantee = 0` for PUBLIC rather than the `pg_get_userbyid(0)` idiom that silently matches nothing; pins the one column grant and proves no other `author_identities` column is updatable; pins `service_role` to the posture this initiative preserved rather than to a narrower target; proves the future-object defaults on real probe objects; runs Supabase's own documented revoke and proves it restores nothing; and adds a fail-closed SECURITY INVOKER routine inventory so a new invoker function cannot inherit PUBLIC EXECUTE unnoticed.
- **Hosted-parity replay lane.** `scripts/e2e-local.mjs` gains `runHostedAclParityLane`, run inside `npm run test:db:local` only. It resets to baseline `20260904120000`, proves suite 015 **fails** there, proves the migration **refuses** a default-privilege entry outside the audited histories before any change (an unexplained `authenticated` shape, NC6a; an unreviewed `authenticator` grantee, NC6b — each absent from the ledger afterwards, every ACL untouched, the lane restored byte-for-byte), seeds hosted Production's ACL state from `scripts/acl-parity/hosted-baseline-20260904120000.sql`, proves the seed reproduced Production **exactly** against the committed read-only reference (relations, routines and default privileges), proves suite 015 still fails, applies the pending migration through `supabase migration up --local` — the real file, never a transcription — then proves suite 015 passes, that `service_role` is byte-identical across the migration, that a new table with no ACL statements is both unreachable by every client role and rejected by CI, and that a third-party grantee added after the migration fails CI through the allowlist assertions (NC6c).
- **Review fix before merge (2026-09-11).** Independent review found the first version's default-privilege precondition checked `anon` alone. Because D1a revokes by name, that let an unexplained `authenticated` default be silently normalized and let a grantee nobody named (for example `authenticator`) survive the migration — both reproduced locally against that version, which committed in each case. The precondition now validates the whole entry and the final verification is an allowlist; the corrected file refuses both states before any change. The `authenticated` matrix, the sequence target, D1a, D2 and the `service_role` boundary are unchanged.
- **Repository state.** Local chain is 81 files, latest `20260910212202_reconcile_data_api_acls`. **Production remains at 80 with this migration local-only.** No Production migration, SQL, DML, DDL, GRANT/REVOKE or default-privilege change was performed; no Storage, Auth, Edge Function, Vercel, secret or Chrome Web Store action occurred. `src/**`, `extension/**` and `supabase/functions/**` are byte-identical, and `src/integrations/supabase/types.ts` is untouched — object privileges are not part of the generated types.
- **Forward pointer.** The status above is the state **at the time this entry was authored** and is kept as written. The migration was subsequently applied to Production on **2026-09-11**; see the closure entry below.

## 2026-09-11 — DATA-API-ACL-RECONCILIATION-001: Production closure (`20260910212202` applied) — **live in Production**

- **Merged.** PR #275 (`security/data-api-acl-reconciliation-001`) merged into `main` as the regular two-parent merge commit `9ca298ba14c32459200ea84db4fa16bd75e20057` — parents `8661c476f7121beffa5c3f2097e8c9a368b5ee25` (base) and `296eb43aa974c08e21c6db80f1318838b5a36908` (approved head) — with merge tree `b5632accc62708d832a07f898141429d38a8c51d`, identical to the approved head tree. `Validate`, `DB Tests` and `Extension` were green on the merge commit, and the automatic Vercel Production deployment reached `READY`.
- **Applied to Production exactly once.** `20260910212202_reconcile_data_api_acls.sql` was applied to `lioxtgiputfniqbktcsz` during a bounded operator rollout on **2026-09-11**, in **one** successful `supabase db push` that reached `Applying migration …` and exited `0`. No retry was needed, no `--include-all`, no migration repair, and no hand-copied SQL — the CLI applied the tracked file. **The exact application date is operator-log evidence, not ledger metadata**: `supabase_migrations.schema_migrations` carries no applied-at column, so the ledger can only prove the row exists, never when it landed.
- **Ledger.** **80 → 81**, latest migration `20260904120000` → **`20260910212202`**, present exactly once. `supabase migration list --linked` then showed full alignment at **81/81** — no local-only, no remote-only, no mismatch.
- **Measured outcome — the client-role surface.** Ordinary `public` tables carrying a direct `anon` grant: **17 → 0**. `PUBLIC` (grantee `0`) and `anon` now hold **no** direct entry and **no** effective privilege on any of the 28 ordinary tables or the one sequence. `authenticated` matches the reviewed matrix on **28 of 28** tables, direct and effective, with zero mismatches and no `TRUNCATE`, `REFERENCES`, `TRIGGER` or `MAINTAIN` surviving anywhere. No grantee outside `postgres`, `authenticated` and `service_role` holds anything on a `public` relation or sequence. Relation inventory is unchanged: 28 ordinary tables and one sequence, all owned by `postgres`, and no other Data API relation kind.
- **The sequence and the column grant.** `papers_insert_order_seq` is now `{postgres=rwU, authenticated=U, service_role=rwU}` — `authenticated` holds `USAGE` only (effective `USAGE`, no `SELECT`, no `UPDATE`), `anon` and `PUBLIC` nothing. `author_identities.preferred_name → authenticated UPDATE` remains the **sole** explicit column-level ACL in `public`, unchanged.
- **Future objects (D1a) hardened in Production.** `postgres`'s default privileges in `public` now name only the owner and `service_role`: TABLES `{postgres=arwdDxtm, service_role=arwdDxtm}`, SEQUENCES `{postgres=rwU, service_role=rwU}`. `PUBLIC`, `anon` and `authenticated` are absent from both, and there is still no global (schema-less) TABLE or SEQUENCE default entry. D1a does **not** replace the per-object rule: D2 still requires every migration creating an API-reachable object to `REVOKE` by role and `GRANT` its exact surface.
- **`service_role` preserved, not narrowed.** Its relation-privilege digest (`e8010e80…`, 163 entries across 20 tables plus the sequence), its sequence privileges (`SELECT, UPDATE, USAGE`) and all three of its default-privilege entries (tables `ALL`, sequences `SELECT, UPDATE, USAGE`, functions `EXECUTE`) are **byte-identical** to the pre-migration snapshot. It was compared only against its own preflight state, never against a narrower target.
- **Out of scope and verified unchanged.** Function privileges (44 routines in `public`) and function default privileges — **not hardened by this initiative**; `supabase_admin`'s default privileges; every default-privilege entry outside the migration's scope; RLS and FORCE-RLS flags (28 enabled, 26 forced, of 28 tables) and all 59 `public` policies. The migration's in-transaction probe objects left no residue.
- **Severity, stated correctly.** No anonymous Data API row exposure had been proven, and none is claimed: every policy requires `auth.uid() = user_id`, and PostgREST exposes neither a TRUNCATE verb nor DDL. This closed a **least-privilege / defense-in-depth** defect with a latent destructive capability behind it (RLS never governed `TRUNCATE`, `TRIGGER` or `MAINTAIN`) plus a **replay/regression** problem. It must **not** be described as a Production data-exposure incident.
- **Nothing else was mutated.** No application-table DML, no ad-hoc DDL, no manual `GRANT`/`REVOKE`, no manual `ALTER DEFAULT PRIVILEGES`, no migration repair, no Auth or Storage mutation, no Edge Function deployment, no secret or provider change, no manual Vercel deployment, and no Chrome Web Store action. Runtime observation was limited to non-mutating signed-out `GET`s of `/` and `/auth` (both `200`); no Playwright ran against Production.
- **Still open, and separate.** `service_role` least-privilege narrowing; function `EXECUTE` and function default-`EXECUTE` hardening; `supabase_admin` default hardening; FORCE-RLS harmonization; and any junction-table narrowing beyond today's contract. None of those is a regression introduced here — each is separate future security work.

## 2026-09-11 — CHROME-EXTENSION-IMPORT-001D Production closure: `20260903180000` verified live, database/RPC acceptance passed — **live in Production**

*This entry follows the 2026-09-03 implementation entry above and records what is true now. That entry stays as written: when it was authored, the migration genuinely was local-only and its Production application was not authorized.*

- **Historical implementation.** PR #271 (`feat/chrome-extension-import-001d-duplicate-resolution`) merged into `main` as `ef199d0bc1f2994db4736ae0c1184c499e78b9fc` on 2026-09-03 (21:25 UTC). It was built web-first on purpose: a database that predates `20260903180000` answers every duplicate with no `id`, and the client treats that as unresolved and calls neither additive RPC, so the web half was correct before, during and after the migration window ([deployment.md](deployment.md) §6.3).
- **Application date — not recoverable, and not asserted.** `supabase_migrations.schema_migrations` carries no applied-at column, so Production cannot say when the row landed. The earliest repository record that describes `20260903180000` as applied is the 2026-09-04 attachment-cleanup addendum in [privacy-data-flow-audit.md](privacy-data-flow-audit.md) §27 (commit `8e91ce9`), which names it as Production's latest applied migration at that time. This entry asserts only what was verified: **live in Production as of 2026-09-11**.
- **Live Production verification (2026-09-11, read-only).** Queried as `postgres` inside transactions that reported `transaction_read_only = on`, before and after the acceptance. Ledger **81** rows, latest `20260910212202`, with `20260903180000` (`add_deterministic_import_duplicate_resolution`) present **exactly once**. Both identity indexes are exactly `(user_id, pmid) WHERE (pmid IS NOT NULL)` and `(user_id, lower(doi)) WHERE (doi IS NOT NULL)`. The live `prosrc` of `safe_bulk_insert_papers`, `bulk_add_paper_projects` and `bulk_add_paper_tags` is **byte-identical** (MD5) to this migration's definitions — and so are the replace-all `bulk_set_paper_projects` / `bulk_set_paper_tags`, which still delete and then insert, unchanged since `20260802025704`, and the `delete_papers_with_attachment_cleanup` used for cleanup. Each of the six is `SECURITY DEFINER` with `search_path=public`, has exactly one overload, and carries ACL `{postgres=X/postgres,authenticated=X/postgres}`: `authenticated` executes it; `anon`, `PUBLIC` and `service_role` do not.
- **Bounded wet acceptance (2026-09-11) — the database/RPC layer.** Every mutation went through an ordinary authenticated `supabase-js` session signed in as the **dedicated Production acceptance account** (`f896de5d-ffd5-49ff-a6a7-f06b44f4dc41`, created 2026-09-10 for acceptance and holding no user content) and called the same RPCs the importer calls — no service role, no SQL write, no impersonation. The data was synthetic: every title and name carried the run marker `_prod_extdup_accept_20260911T143748Z_881674`, PMIDs were nine digits and DOIs used the reserved `10.5555` prefix, and no PubMed or Crossref request was made.
  - **A — PMID.** The first insert returned `inserted`; the same PMID again returned `duplicate` **with the first row's id**, and exactly one row carries that PMID.
  - **B — DOI case.** Stored `10.5555/PaperLume-ExtDup-881674-CaseB`, then sent `10.5555/PAPERLUME-EXTDUP-881674-CASEB` straight to the RPC: `duplicate` with the existing id, still one row, stored DOI unchanged. This is the live proof of the resolver's `lower(doi)` rule, not of application-side normalization.
  - **C — ambiguity.** One row owning a PMID and another owning a DOI; an attempt carrying both returned `duplicate` with **no `id` key**, both rows were byte-identical afterwards, and no third row existed.
  - **D — Projects.** Project A was set through `bulk_set_paper_projects`, then `bulk_add_paper_projects` with Project B left exactly {A, B}; repeating the add changed nothing.
  - **E — Tags.** The same through `bulk_set_paper_tags` / `bulk_add_paper_tags`: exactly {A, B}, idempotent on repeat.
- **Zero residue.** Cleanup used the ordinary authenticated paths: `delete_papers_with_attachment_cleanup` for the four papers (`deleted_count 4`, `queued_count 0`), then owner deletes of the two Projects and two Tags. A read-only audit afterwards found zero marker papers, Projects or Tags anywhere in Production, zero junction rows for any run id, and — for the account — zero attachment metadata, cleanup-queue rows, tombstones and Storage objects; its pre-existing zero-byte `user_storage_usage` row is unchanged. The postflight matched the preflight exactly in ledger, function bodies and ACLs, indexes, and the aggregate papers/Projects/Tags/junction fingerprint of every other account.
- **The application boundary — current source and local regression, not Production.** `/extension-import` is the only shipping caller passing `applyAssignmentsToResolvedDuplicates: true`; `bulkImportPapers` acts on a duplicate id only when that option is `=== true`, and `bulkImportFromParsedData` does not accept it. Against this tree: `useBulkMutations-assignment.test.ts` 24/24 (23 when 001D merged; the 24th came with the attachment-cleanup work), `npm run test:db:local` PASS — 16 pgTAP suites, 1,543 assertions, including suite `013` — and `e2e/extension-import.spec.ts` 18/18 plus its auth setup.
- **Not wet-tested in Production.** The `/extension-import` browser flow itself rests on current source plus the deterministic local E2E. Add Papers, PubMed Search and file import were not exercised in Production; their default-off behaviour is proven from source and unit tests.
- **Nothing else was changed beyond ordinary sign-in bookkeeping.** No schema, migration, DDL, `GRANT`/`REVOKE`, default-privilege or policy change. No Auth account was created or deleted, and no administrative or product Auth change was performed: the acceptance used one ordinary password sign-in, which caused normal GoTrue authentication/session bookkeeping on the acceptance account (including the usual sign-in metadata), followed by a local-scope sign-out that revoked that acceptance session. No attachment, Storage, Edge Function, secret, provider, manual Vercel or Chrome Web Store action. Chrome Web Store publication and review are separate from this initiative.

## 2026-09-12 — AI-MULTI-PROVIDER-001A: provider-neutral AI runtime foundation — **implemented; NO migration; no second provider; NOT deployed**

- **Repository-only architectural refactor.** No migration, no regenerated types, no catalog row, no seed change, no secret, no Edge deploy, no Production SQL, no provider request of any kind. Nothing under `supabase/migrations/`, `supabase/tests/database/`, `src/` or `extension/` changed. Decision: **C39**.
- **What the seam is.** Model selection decides *which* model, a **provider adapter** decides *how* to speak to that provider, and a **runtime registry** decides which provider protocols PaperLume can speak at all. New: `_shared/aiProvider.ts` (contract), `_shared/aiProviderRegistry.ts` (registry + `resolveSystemDefaultAiModel`), `_shared/googleAiProvider.ts` (the Google adapter), `analyze-paper/prompt.ts` (the analysis prompt, moved verbatim so the request is executable in a test).
- **Registered providers: `google` only.** No Anthropic entry, no OpenAI entry, no placeholder that throws "not implemented", no endpoint, no credential accessor. `ai_model_catalog` remains the **model** allowlist; the registry is the **provider-protocol** allowlist and names no model string, so the two cannot become duplicates of each other. A hypothetical enabled `anthropic` / `openai` row still resolves to the system default with `unsupported_provider`, proven through the real handler on the wire.
- **`_shared/aiModelSelection.ts` is now provider-neutral.** It takes the system default as `{ provider, providerModel }`, returns the provider it was given or one a catalog row named and the registry accepted, builds no URL and contains no provider literal (asserted against its own source). `buildGeminiGenerateContentUrl` and `SUPPORTED_AI_PROVIDER` moved out; the resolver's provider type is narrowed to a *registered* provider, so the adapter lookup has no failure branch to mishandle after a quota unit is spent.
- **Both operations refactored; their provider requests and user-visible behaviour are unchanged** (the one log-only difference is below). `analyze-paper/index.ts` and `suggest-paper-organization/handler.ts` build a provider-neutral request (system instruction, one user-content string, `responseFormat: "json"`), call the resolved adapter, receive normalized text and run their **existing** parsers. `suggest-paper-organization/prompt.ts`'s `buildGeminiRequestBody` became `buildSuggestGenerationRequest`; `parse.ts`'s `extractProviderText` moved into the adapter. Prompts, suggestion caps (3/5/2/3, 400-char reason), the 60,000-character input bound, quota consume/refund, provider-error classification, status codes and user-facing wording are untouched.
- **Transport untouched.** `_shared/geminiTransport.ts` is byte-identical: the `AI-PROVIDER-90S-PROD-DIAGNOSTIC-001A` policy (90 s per attempt, zero retries) — at this date still a bounded Production diagnostic — is neither resolved nor generalised, and 001A explicitly does **not** assert it is right for any future provider. *(Those same two constants were later adopted as PaperLume's permanent Gemini policy on 2026-09-19; see C46 in [decisions-and-triggers.md](decisions-and-triggers.md).)*
- **Golden equivalence, mechanically.** The pre- and post-refactor implementations of **both** operations were executed side by side across 24 scenarios each, comparing status, response body, response headers, the exact provider URL / method / headers / request-body SHA-256, the RPC sequence, the table reads and every emitted log line. 47 of 48 comparisons are identical including logs; the differences that appear and disappear between runs are the transport's own `elapsed_ms` wall-clock jitter. Byte-for-byte golden request pins were also added for both operations (analyze body SHA-256 `3285186f…`, 1,973 bytes; suggest body SHA-256 `667b4e29…`, 3,530 bytes; the analysis system instruction SHA-256 `636b4ff6…` is unchanged from `main`).
- **The one deliberate difference, stated rather than hidden.** In `analyze-paper` only, a 2xx whose body is not JSON is now logged as the bounded `gemini_unreadable_response` instead of the JSON parse error's own message, which quotes a fragment of the provider's body. Status, response body, `provider_unavailable` classification and the refund are unchanged; this is required by the adapter contract (no provider error body may cross the boundary) and narrows what can be logged. Two pre-existing facts are recorded and preserved rather than fixed: the two functions have always classified that case differently (`provider_unavailable` vs `malformed_response`), and consolidating the two envelope readers onto the stricter one changes `analyze-paper` only for envelopes Google's proto3 JSON cannot emit (a non-string `text`, or `candidates`/`parts` as objects instead of arrays).
- **Tests.** New `_shared/__tests__/aiProviderRegistry.test.ts` (39), `_shared/__tests__/googleAiProvider.test.ts` (46) and `_shared/__tests__/aiProviderTypeBoundary.test.ts` (5), new `analyze-paper/__tests__/geminiRequestGolden.test.ts` (10), plus new cases in the suggest handler and prompt suites and updated wiring assertions in the two `analyze-paper` suites. The type-boundary suite pins the adapter/provider binding tightened in review — an adapter's `generate` accepts only its own provider's models and the registry lookup preserves the provider, so pairing the Google adapter with another provider's model is a compile error; its `@ts-expect-error` assertions are enforced by a direct strict typecheck of the Edge modules, and each was shown to fire when the binding it guards is reverted. That review fix changed types, comments and docs only: the emitted JavaScript of every changed runtime module is byte-identical. Full local suite **4,776 tests / 155 files green**; lint clean (18 pre-existing warnings, 0 errors); typecheck and build green; the Edge modules and the `analyze-paper` entrypoint were additionally type-checked directly, which CI does not do.
- **Deployment state.** **Not deployed.** Production runs the pre-001A artifacts. The Edge bundle closure of **both** generation functions now includes the three new `_shared` modules (and `analyze-paper/prompt.ts` for Analyze), so a future authorized deploy must cover both functions together — see [deployment.md](deployment.md) §7c. `GEMINI_API_KEY` remains the only AI provider credential.

## 2026-09-12 — AI-MULTI-PROVIDER-001B: Anthropic + OpenAI provider adapters — **implemented; deliberately UNREGISTERED; NO migration; NOT deployed**

- **Repository-only.** No migration, no regenerated types, no catalog row, no seed change, no secret, no Edge deploy, no Production SQL, and no provider request of any kind: no Gemini, Anthropic or OpenAI call was made, and every test uses an injected fake `fetch`. No package, lockfile, workflow, extension or Chrome Web Store change. Decision: **C40**.
- **Adapters implemented: `google`, `anthropic`, `openai`. Registered providers: `google` only.** New `_shared/anthropicAiProvider.ts` implements the Claude Messages API: `POST https://api.anthropic.com/v1/messages`, `x-api-key`, `anthropic-version: 2023-06-01`, top-level `model`/`system`/`messages`/`max_tokens`, and structured output via `output_config.format` (`json_schema`) with no beta header. New `_shared/openAiProvider.ts` implements the OpenAI Responses API: `POST https://api.openai.com/v1/responses`, `Authorization: Bearer`, `model`/`instructions`/`input`/`max_output_tokens`, `text.format` (`json_schema`, `strict: true`), and **`store: false` on every request**. Both use native `fetch`, with no SDK. `aiProviderRegistry.ts`, `aiModelSelection.ts` and `src/hooks/useAiModelSettings.ts` (`SUPPORTED_PROVIDERS = ["google"]`) are byte-identical to `main`, so a hypothetical `anthropic`/`openai` catalog row still falls back with `unsupported_provider`. No shipping Edge Function imports either new module, so neither is in any deployment closure.
- **Why unregistered.** Per the official provider documentation reviewed 2026-09-12, Claude Sonnet 5 runs adaptive thinking by default at effort `high`, and GPT-5.6 Terra reasons at effort `medium` by default. Neither adapter sends any reasoning or thinking configuration. The per-operation policy, and a real output budget replacing each adapter's provisional 4,096-token ceiling, belong to `AI-MULTI-PROVIDER-001C`.
- **Provider-neutral contract.** `AiGenerationRequest` gained a required `jsonSchema: { name, schema }`, owned by each operation. `ANALYZE_JSON_SCHEMA` in `analyze-paper/prompt.ts` is exactly `tldr`/`studyType`/`statisticalMethods`, all strings. `SUGGEST_JSON_SCHEMA` in `suggest-paper-organization/prompt.ts` is the four suggestion arrays, with the current 3/5/2/3 caps and the 400-character reason bound unchanged. Neither provider dialect supports `maxItems`/`maxLength`, so the caps, the length bounds and the request-local ref rule stay enforced by the existing parsers. `AiProviderFailureKind` gained `incomplete_response` for a provider-reported unfinished generation (Anthropic `stop_reason` other than `end_turn`; OpenAI `status` other than `completed`), because `empty` would misdescribe a truncated answer. Both operations classify it explicitly as `malformed_response`, with a refund. Google cannot produce it.
- **Response extraction.** Anthropic: every `content` block is walked, and only `type: "text"` blocks are concatenated, in order; `thinking`, `redacted_thinking`, `tool_use` and unknown blocks are ignored. OpenAI: every `output` item is walked, and only `output_text` blocks inside `message` items are concatenated; `reasoning` items, `refusal` blocks and unknown items are ignored, and the SDK-only top-level `output_text` field is never read. The terminal state is checked before any text is read, so a truncated answer is never returned as a success.
- **Transport.** Each adapter makes one attempt under its own 60-second ceiling, with no retry and no backoff. Neither calls or generalises `geminiTransport.ts`, whose 90 s / zero-retry policy — a bounded Production diagnostic at this date, permanent Gemini policy since 2026-09-19 (C46) — is untouched. Failures cross as `http` (status only; the body is never read), `network`, `timeout`, `unreadable_response`, `empty` or `incomplete_response`. No body, header, request id, URL or key crosses.
- **Google non-regression.** `googleAiProvider.ts`, `geminiTransport.ts` and `analyze-paper/__tests__/geminiRequestGolden.test.ts` are byte-identical to `main`, and the golden pins still pass: URL, method, both headers, body SHA-256 `3285186f…` (1,973 bytes), system-instruction SHA-256 `636b4ff6…`, and `responseMimeType: "application/json"`. The Google adapter ignores `jsonSchema`: a schema sentinel never reaches the wire, and the bytes are identical with or without a schema.
- **Tests.** New `_shared/__tests__/anthropicAiProvider.test.ts` (78), `openAiProvider.test.ts` (86) and `aiOutputSchemas.test.ts` (74), plus new registration-gate, type-boundary, Google-ignores-schema, Settings provider-filter and operation-classification cases. Full local suite **5,034 tests / 158 files green**; lint clean (18 pre-existing warnings, 0 errors); root typecheck and build green. A direct strict typecheck of the Edge provider modules, both operations (including both `index.ts` entrypoints) and their suites reports only diagnostics identical to `origin/main`'s. Five type-level negative controls were each caught by the checker: `jsonSchema` made optional, each new adapter widened to any provider, `generate` turned back into a bivariant method, and the new failure kind removed.
- **Deployment state.** **Not deployed.** Production runs the pre-001A Google-only artifacts. The closure of **both** generation functions changes again (see [deployment.md](deployment.md) §7c), and the two new adapter modules are in neither closure. No secret was added: `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` are documented as intended future names only.

## 2026-09-12 — AI-MULTI-PROVIDER-001C: model-aware reasoning policy and provider activation foundation (`20260912120000`) — **COMPLETE (repository/application); schema + frontend LIVE; generation Edge NOT deployed; reasoning activation and paid providers deferred**

- **Additive migration, applied to Production ahead of the merge.** One new additive migration, `20260912120000_add_model_aware_ai_reasoning_policy.sql`, replayed locally from zero before any Production step. Developing and validating the PR made no Production SQL, secret, Edge deploy or live Gemini/Anthropic/OpenAI request, and every test uses an injected fake `fetch`. No package, SDK, lockfile, workflow, extension or Chrome Web Store change. Decision: **C41**. The 001C PR had to stay unmerged until this migration was separately authorized, applied to Production and verified while the old application was still live ([deployment.md](deployment.md) §6.6). That gate was satisfied on 2026-09-12, and the PR merged on 2026-09-13 — see *Production schema phase*, *Application merge* and *Closure* below.
- **Provider contracts re-verified against first-party documentation (2026-09-12).** Every one matches the owner-approved matrix, so there was no BLOCKED condition.
  - **Gemini:** `generationConfig.thinkingConfig.thinkingLevel` takes lowercase `minimal|low|medium|high`. 3.5/3.6 accept all four; 3.7/3.8 reject `minimal`. The default is `medium`. `max_output_tokens` includes thought tokens. `thinkingBudget` is legacy, and there is no documented incompatibility with JSON response mode.
  - **Claude Sonnet 5:** thinking is "adaptive only" and on by default. `"enabled"` returns 400 and `"disabled"` is accepted. `output_config.effort` takes `low|medium|high|xhigh|max`, defaults to `high`, applies with or without thinking, and sits as a sibling of `output_config.format`. `max_tokens` bounds thinking plus text.
  - **GPT-5.6 Terra:** `reasoning.effort` takes `none|low|medium|high|xhigh|max`, defaults to `medium`, and has no `minimal`. Reasoning tokens count against `max_output_tokens`.
- **Schema.**
  - `ai_model_catalog` gains `reasoning_levels TEXT[] NOT NULL DEFAULT '{}'`, `auto_analyze_reasoning_level`, `auto_suggest_reasoning_level` and `reasoning_selectable BOOLEAN NOT NULL DEFAULT false`, with five CHECK constraints: canonical vocabulary with no NULL element, no duplicates, canonical Automatic levels, Automatic levels supported by their own model, and a selectable control that is usable.
  - The four Gemini rows carry the C41 matrix: 3.5/3.6 `{minimal,low,medium,high}` with analyze `minimal`, 3.7/3.8 `{low,medium,high}` with analyze `low`, suggest `medium` on all four, and **`reasoning_selectable = false` on all four**.
  - `user_ai_preferences.preferred_reasoning_level` is added as `TEXT NULL`, where NULL means Automatic and `'automatic'` is refused.
  - `set_current_user_ai_model` is re-created with an additive `reasoning_reset` result. It locks the caller's preference row (`FOR UPDATE`), then re-validates the saved level against the new model and writes the pair under that lock, preserving a supported level and nulling an unsupported one. `clear_current_user_ai_model` keeps its body; only its comment changed.
  - New `set_current_user_ai_reasoning(text)` is **revoked from PUBLIC, anon, authenticated and service_role and granted to no one (staged)**. New `clear_current_user_ai_reasoning()` is granted to authenticated only.
  - **Both setters serialize on the caller's preference row** (review fix 01, made before any Production application). `set_current_user_ai_reasoning` locks the row before reading the model it names. The model setter's no-row path is a bounded lock-or-insert loop (three passes, then `40001`). A final write that lands on no row raises `XX000` rather than reporting `saved = true`. The `db-tests` preference-lock probe proves both orders of each race with real concurrent sessions.
  - The migration's self-check asserts every one of these facts, including the ungranted setter.
  - No Sonnet/Terra row, credential column, grant change on either table, entitlement, quota or pricing change. Generated types were regenerated locally (`supabase gen types typescript --local --schema public`), purely additive (+33 lines), and not hand-edited apart from the trailing newline.
- **Runtime.**
  - New `_shared/aiReasoningPolicy.ts` is the single shared policy for both operations. It reads the effective model's catalog row by `(provider, provider_model)` and resolves `manual`, `automatic` or `provider_default_fallback`, each with bounded reasons.
  - New `_shared/aiProviderCredentials.ts` maps `google→GEMINI_API_KEY`, `anthropic→ANTHROPIC_API_KEY` and `openai→OPENAI_API_KEY`.
  - `AiProviderAdapter<Provider, Level>` now takes a typed `AiCallPolicy<Level>` (a reasoning directive plus `maxOutputTokens`), and each adapter declares its own level union and `supportsReasoningLevel` guard.
  - The registry registers **google, anthropic and openai** and adds `generateWithRegisteredAiProvider`, one exhaustive dispatch that narrows each request's policy.
  - `aiModelSelection.ts` reads `preferred_reasoning_level` and carries it **only** on an honoured preference. Every fallback nulls it.
  - Both operations resolve reasoning before the quota unit and read only the selected provider's credential. `analyze-paper` still refunds a missing key after the unit. `suggest-paper-organization` still fails it before the unit, and its credential check moved from before model selection to after it.
- **Wire changes.**
  - **Google** gains exactly `generationConfig.thinkingConfig.thinkingLevel`. The new golden body is SHA-256 `e26b9bca…` (2,018 bytes) for analyze `minimal`, and deleting that one field yields the pre-001C golden `3285186f…` (1,973 bytes). The `provider_default` path emits `3285186f…` exactly, and no Gemini output ceiling is sent.
  - **Anthropic:** `off` → `thinking.type: "disabled"` + `effort: "low"`; `low…max` → `adaptive` + that effort. `format` is preserved beside `effort`, and `max_tokens` is 4096 for Analyze or 8192 for Suggest.
  - **OpenAI:** `reasoning.effort` verbatim, `store: false`, and a `max_output_tokens` of 4096 or 8192.
  - The provisional 001B `*_PROVISIONAL_*` ceilings are removed, and transports are unchanged.
- **Frontend.** `SUPPORTED_PROVIDERS` is `["google","anthropic","openai"]`, which adds no option today because no such catalog row exists. The catalog projection adds the four reasoning columns, and the preference read adds `preferred_reasoning_level`. New `saveReasoning`/`clearReasoning` guard on an active named model and `reasoningSelectable`. The model and reasoning rejection classifiers are separate, because `model_disabled` means different things to the two setters. A `reasoning_reset` toast is added. The Settings **Reasoning level** control shows Automatic (Recommended) plus exactly the model's own levels; it is disabled on PaperLume default and while staged, and it explains the exact Automatic policy from catalog metadata in visible text wired by `aria-labelledby`/`aria-describedby`. New presentation-only `src/lib/aiReasoning.ts` holds the labels and descriptions and fails closed on unknown values.
- **Account export v3.** `USER_AI_PREFERENCE_EXPORT_COLUMNS` gains `preferred_reasoning_level`, and `ACCOUNT_EXPORT_VERSION` goes 2 → 3 because an existing file changed shape.
- **Tests.**
  - **Vitest:** 5,206 tests in 160 files, all green. New suites are `_shared/__tests__/aiReasoningPolicy.test.ts` (37) and `aiProviderCredentials.test.ts`. The adapter, registry, type-boundary, model-selection, golden, routing, handler, hook, component and export suites were extended.
  - **Database:** new pgTAP suite `016_ai_reasoning_policy.test.sql` (104 assertions, including grantee ALLOWLISTS that judge every role that can execute the staged setter, not just the named ones). Suites 003, 011 and 012 were updated deliberately (classification lists and pinned column/result sets), not loosened. A clean local replay from zero ran 17 suites and 1,655 assertions, **all green**, plus the framework-free 18/18 and the concurrency probes.
  - **Lint:** 0 errors, the 18 pre-existing warnings. Root typecheck and build are green.
  - **E2E (local):** the full lane ran **254 passed**. `ai-model-settings.spec.ts` now proves the staged Reasoning level end to end against the migrated local catalog: Automatic and disabled on PaperLume default, the exact "Analyze: Low · Organization suggestions: Medium" (3.8) and "Analyze: Minimal · …" (3.5) policy text read from catalog rows, no manual choice while staged, and an explanation wired by `aria-describedby`. `account-export.spec.ts` pins version 3. Manual save/reset is deliberately not driven from the browser, because that would require granting the staged setter locally. It is covered by suite 016 (owner context) and the unit tests. The run needed the documented temporary dev-server port move off 8080, which another project's `limactl` holds. It was reverted before commit and is not part of this change.
  - **Direct Edge typecheck:** zero new diagnostics. Everything reported is a pre-existing class shared with `origin/main`: Deno globals, remote imports, `index.ts` handler params, the three golden-test tuple lines and the identifierDetection suite.
- **Production schema phase — applied 2026-09-12.** A separately authorized rollout applied `20260912120000` exactly as reviewed at PR head `8001fce8182859a7cfd1d597573112df502ef3fd` (migration blob `9ea674c6…`), with `supabase db push --linked` after a dry run proposed only that file. There was no migration repair, no `--include-all` and no hand-run SQL.
  - **Ledger 81 → 82**, Local = Remote, latest `20260912120000`, present exactly once.
  - **Existing data preserved.** Preference rows (model assignments and timestamps) and entitlement rows are fingerprint-identical before and after. Every `preferred_reasoning_level` is NULL. The catalog's base columns are unchanged; its `updated_at` moved only because the migration's own metadata `UPDATE`s fire the table's timestamp trigger.
  - **Still staged off.** `reasoning_selectable = false` on all four rows, `set_current_user_ai_reasoning` is executable by its owner only, and no Sonnet/Terra or other non-Google row exists. No provider secret was installed.
  - **The old application stayed compatible**, verified read-only: the old frontend bundle kept serving, and its exact catalog and saved-preference reads succeeded as `authenticated`. Saving a model, Analyze, Suggest and export were not exercised, because the rollout authorized no preference write and no AI quota use.
  - The Supabase security advisor gained only the expected `authenticated`-executable SECURITY DEFINER notice for the granted `clear_current_user_ai_reasoning`.
- **Application merge — 2026-09-13.** PR #280 merged at its independently approved head `d62994ef67ff8f27763a442bd9f63f4d7f7b54f5` as the regular two-parent merge `1c4c9b5882628cbe6ab7bead60e7f4eac0bed0b4` (parents `d5c0201…` and `d62994e…`; tree identical to the approved head). Merged-main Validate, DB Tests and Extension passed, and the automatic Vercel Production deployment of the merge commit reached READY, so the 001C frontend (Reasoning level control, account export v3) is live. The merge changed no Production data: the migration is still present once, `reasoning_selectable` is false on all four rows, no manual level exists and the setter is still ungranted.
...
- **Runtime.**
  - New `_shared/aiUsage.ts`: the provider-neutral usage vocabulary (reported / unreported / not applicable per dimension; `not_returned` / `invalid` per report) and the shared nesting finalizer.
  - `AiProviderResult` gains a required `usage`. The Google, Anthropic and OpenAI adapters read their own usage objects; usage survives `empty` and `incomplete_response`, and every no-body outcome carries none. Request bytes are unchanged (Google golden `e26b9bca…`).
  - New `_shared/aiPriceBook.ts` (append-only, effective-dated list prices for the four Gemini models, verified 2026-09-13) and `_shared/aiCostEstimate.ts` (pure, exact integer arithmetic, five cost statuses).
  - New `_shared/aiUsageTelemetry.ts`: row builder, never-throwing recorder with bounded logs, and the lazily built insert-only client from the platform secret key. `selectEdgeSecretKey` moved to `_shared/edgeSecretKey.ts` and is re-exported by `accountDeletion.ts`, so `delete-account` is unchanged.
  - `analyze-paper` and `suggest-paper-organization` record exactly one event per provider call after the outcome is decided; quota, refunds, prompts, parsers, response bodies and the Gemini 90 s / zero-retry transport policy are unchanged.
- **Scripts.** `scripts/e2e-local.mjs`: the residue check counts telemetry rows, and the hosted-parity lane's NC4 `service_role` snapshot is scoped to relations present at its baseline, because `migration up` also applies later migrations that create their own relations.
...
- **Deployment (operator record; outcome verified).** `supabase functions deploy analyze-paper --project-ref lioxtgiputfniqbktcsz --no-verify-jwt`, then the same for `suggest-paper-organization` (CLI 2.111.0), one attempt each, 07:59:03–07:59:18Z:
  ...
  Both kept `verify_jwt = false`. Downloaded again after the deploy, both live artifacts were byte-identical to the `f962b44d` closures. The Gemini transport remains 90 s / zero retries — still a bounded Production diagnostic at this date, and adopted as the permanent Gemini policy on 2026-09-19 (C46) without either constant moving.
...
## 2026-09-17 — AI-MULTI-PROVIDER-001E: paid-provider catalog staging (`20260917201856`) — **implemented; migration NOT applied to Production; no credential; no paid provider reachable**

- **Migration `20260917201856_add_paid_provider_catalog_models.sql`.** Inserts exactly two rows into `public.ai_model_catalog`:
  - `anthropic/claude-sonnet-5` — provider `anthropic`, provider model `claude-sonnet-5`, "Claude Sonnet 5", `enabled = true`, **`selectable = false`**, `sort_order = 50`, reasoning levels `{off,low,medium,high,xhigh,max}`, Analyze `off`, Suggest `medium`, `reasoning_selectable = false`.
  - `openai/gpt-5.6-terra` — provider `openai`, provider model `gpt-5.6-terra`, "GPT-5.6 Terra", `enabled = true`, **`selectable = false`**, `sort_order = 60`, reasoning levels `{none,low,medium,high,xhigh,max}`, Analyze `none`, Suggest `medium`, `reasoning_selectable = false`.
- **Additive and fail-closed.** A plain `INSERT` with **no** `ON CONFLICT`, so a pre-existing row in an unreviewed state fails the migration visibly instead of being silently reconciled. The in-transaction verify block asserts: exactly six catalog rows; both new rows as whole rows; neither paid row `selectable`; **no** row `reasoning_selectable`; all four Google rows unchanged as whole rows; the six-model order; no catalog row modified except the two inserted; no `user_ai_preferences` row written, none pointing at a paid model, and none carrying a manual reasoning level; no entitlement written; `set_current_user_ai_reasoning` still ungranted to `authenticated`; no credential-shaped column; and the 001A client-role grant posture intact.
- **What it does NOT do.** Does not set `selectable = true`; does not set `reasoning_selectable = true`; does not grant the reasoning setter; does not touch a Google row, a preference, an entitlement, a quota or the system default; adds no column, credential, grant, policy, function, trigger or index.
- **Database tests.** New suite `018_paid_provider_catalog_staging.test.sql` (45 assertions). Suites `012` and `016` were updated — not softened — for the six-row catalog: `012` still asserts the whole list and now scopes its selectability claim to Google while asserting the paid rows are **not** selectable; `016` keeps its manual-reasoning lock and now asserts no non-Google row beyond the two approved ones.
- **Price book.** `_shared/aiPriceBook.ts` gains `anthropic/claude-sonnet-5@2026-09-17` and `openai/gpt-5.6-terra@2026-09-17`, verified from first-party pricing pages that day (C44).
- **Deployment state.** **Migration NOT applied to Production.** Production's catalog still holds the four Google rows. No `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` is installed. The deployed generation runtime (`analyze-paper` v27, `suggest-paper-organization` v11, Phase 6) already **contains** both adapters, so no redeploy is required to add a provider protocol — but a redeploy from the accepted merge **is** required for the new price records to reach the estimator. **No request has been sent to Anthropic or OpenAI.**
- **Privacy.** Amended `src/pages/Privacy.tsx` §6 is a **draft pending owner approval**; the amendment must be published before the first paid request. Audit: [privacy-data-flow-audit.md](privacy-data-flow-audit.md) §30. Decisions: **C43**, **C44**.

---

## 2026-09-18 — AI-MULTI-PROVIDER-001E Phase 7 executed, Edge-log hardening deployed, and Phase 8 prepared (`20260918210017`) — **staging migration APPLIED; paid canaries PASSED; activation migration NOT applied**

This entry brings the log's current state up to date. The 2026-09-17 entry above is the record of that day and is deliberately not rewritten; several of its "not yet" statements were superseded the following day.

- **Staging migration applied.** `20260917201856` was applied to Production on 2026-09-18, taking the ledger 83 → **84**. The catalog holds six rows; both paid rows landed `enabled = true`, **`selectable = false`**, `reasoning_selectable = false`.
- **Privacy Policy published**, effective September 18, 2026 (PR #287, merge `ef8ad768`), before any paid request was sent.
- **Both credentials installed.** `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` were set in one operation (12 → 14 secrets), each digest verified equal to `sha256` of the intended value, with no other secret changed.
- **Both generation functions redeployed** from `ef8ad768`: `analyze-paper` v29, `suggest-paper-organization` v13, so the bundles carry the reviewed paid-provider price records.
- **Phase 7 canaries PASSED** — four provider calls, one attempt each, on the dedicated acceptance account's synthetic paper: Claude Analyze (`automatic` → `off`, 1,105/102 tokens, $0.003230), Claude Suggest (`medium`, 1,863/200, $0.005726), Terra Analyze (`none`, 555/67, $0.001914), Terra Suggest (`medium`, 887/125, $0.003274). Total $0.014144. Telemetry 3 → 7 content-free events, all `estimated` against the reviewed price records; quota 2 → 6; no library mutation; both bounded log windows clean. Detail in [deployment.md](deployment.md) §14.1a.
- **A canary prerequisite was discovered and corrected.** `resolveEffectiveAiModel` checks entitlement *before* it reads a saved preference, so a preference-only canary on the free acceptance account would have fallen back to Gemini silently (`not_entitled` is a quiet reason). The bounded fix — a temporary, immediately-restored `ai_model_selection_enabled` on that one account, per provider block — is documented in [deployment.md](deployment.md) §14.2 and as a dated correction inside C43.
- **Edge-log privacy hardening merged and deployed** (EDGE-LOG-PRIVACY-HARDENING-001 + 001A, PR #288, merge `a3c7d910`): `analyze-paper` v29 → **v30**, `fetch-paper-metadata` v21 → **v22**, both read back byte-identical to the merge. No arbitrary throwable message can reach a log from either function. **No migration** was involved. Audit: [privacy-data-flow-audit.md](privacy-data-flow-audit.md) §10.1.
- **Phase 8 prepared, NOT applied.** `20260918210017_activate_paid_provider_model_selection.sql` sets `selectable = true` on exactly `anthropic/claude-sonnet-5` and `openai/gpt-5.6-terra` and nothing else. It is fail-closed at both ends: a preflight block asserts the six-row catalog, the two target rows in their exact staged state field by field, the four Google rows unchanged and manual reasoning off everywhere, then updates and requires exactly two affected rows; a verify block re-asserts the whole post-state, that no other catalog row was written, that no preference or entitlement row was written, and that `set_current_user_ai_reasoning` is still ungranted.
- **Deployment state.** **The activation migration is NOT applied.** Production's two paid rows remain `selectable = false`, so the setter still refuses them and Settings still does not list them; manual reasoning remains disabled catalog-wide and the reasoning setter remains granted to nobody. Applying it is a separate, separately authorized step.

---

## 2026-09-19 — AI-MULTI-PROVIDER-001E Phase 8: paid-provider user activation (`20260918210017`) — **APPLIED; Claude Sonnet 5 and GPT-5.6 Terra are user-selectable; 001E CLOSED**

The closure of the rollout recorded in the two entries above. Those entries are dated history and are deliberately not rewritten: the 2026-09-18 entry's "the activation migration is NOT applied" was true when written, and this entry is what supersedes it.

- **PR #289 merged** as the two-parent commit `38b22c209591a5f5ac2d80498bb55d082dde6d22` (parents `a3c7d910` + `459ac2f`, tree `ad3e4220` = the approved head's tree; branch kept). Required merged-main CI passed (Validate, DB Tests; Extension also ran and passed).
- **Migration `20260918210017_activate_paid_provider_model_selection.sql` applied 2026-09-19**, 05:40:28–05:40:38Z, by one `supabase db push --linked --yes` — one attempt, exit 0, no seeds, no roles. `migration list` and a dry run had each shown **exactly one** pending migration first, and a final read-only gate re-confirmed both rows were still `selectable = false`. Ledger **84 → 85**, latest `20260918210017`, present once.
- **Exactly two rows changed, one flag each**: `anthropic/claude-sonnet-5` and `openai/gpt-5.6-terra`, `selectable` **false → true**. Every other column on both rows — provider, provider model, display name, `enabled`, `sort_order`, `reasoning_levels`, both Automatic levels, `reasoning_selectable` — is unchanged. Catalog md5 `49e844e5…` → `7b5530da…`.
- **Google rows unchanged**, proven positively: their whole-row md5 is still `09cb913f…`, and the migration's verify block asserted them field by field.
- **Manual reasoning untouched**: `reasoning_selectable` is false on all six rows and `set_current_user_ai_reasoning` is still granted to nobody (`{postgres=X/postgres}`).
- **System default unchanged** — still the Google model resolved from `GEMINI_MODEL` (C34), which is environment configuration, not a catalog row.
- **No entitlement, preference or backfill mutation.** The migration's verify block asserted that no preference and no entitlement row was written, and the entitlements md5 was unchanged across the push. Nobody was moved onto a paid model; entitlement is still the authority for *who* may select one.
- **No Edge deployment and no secret change.** All six functions are byte-identical across the activation (`analyze-paper` v30, `suggest-paper-organization` v13, `fetch-paper-metadata` v22, `delete-account` v6, `get-gemini-provider-quota` v9, `search-pubmed` v5+). No secret digest moved.
- **No provider call or canary during activation.** Phase 7 (2026-09-18) had already exercised Analyze and Suggest against both providers live; telemetry was unmoved by this task.
- **Discovery verified read-only.** The exact query the Settings hook runs, executed as an authenticated caller, returns all six models in render order — four Gemini, Claude Sonnet 5, GPT-5.6 Terra — and an authenticated but **non-entitled** caller still reports `can_select_ai_model = false`.

Cross-references rather than repetition: the Phase-7 canary evidence is in the 2026-09-18 entry and [deployment.md](deployment.md) §14.1a; the Edge-log hardening is in the same entry and [privacy-data-flow-audit.md](privacy-data-flow-audit.md) §10.1; the activation runbook is now an executed operator record in [deployment.md](deployment.md) §14.4.

---

## 2026-09-19 — AI-MANUAL-REASONING-001: manual reasoning user activation (`20260919075655`) — **implemented; migration NOT applied to Production; Production remains Automatic-only**

The release of the two staging locks `AI-MULTI-PROVIDER-001C` put on manual reasoning on 2026-09-12 (C41), prepared for all six currently selectable models at once (C45). **Draft only: nothing here is applied, deployed or canaried.**

- **Migration `20260919075655_activate_manual_ai_reasoning_selection.sql` — prepared, NOT applied.** It makes two changes and no others: `reasoning_selectable` **false → true** on exactly the six approved ids (`google/gemini-3.5-flash`, `google/gemini-3.6-flash`, `google/gemini-3.7-flash`, `google/gemini-3.8-flash`, `anthropic/claude-sonnet-5`, `openai/gpt-5.6-terra`), and `GRANT EXECUTE ON FUNCTION public.set_current_user_ai_reasoning(text) TO authenticated`. It also rewrites two catalog COMMENTs that would otherwise still describe the staged state.
- **Fail-closed before it touches anything.** One atomic `DO` block holds the preconditions, the `UPDATE` and the `GRANT`, so `supabase db reset` — which runs a file statement-at-a-time — cannot leave the flag set without the grant. It requires exactly six catalog rows, exactly the six approved ids, each matching its reviewed tuple field for field (flags, `sort_order`, `reasoning_levels` in order, both Automatic columns), `reasoning_selectable` false everywhere, the setter still executable by its owner **only** (an allowlist over every grantee, so PUBLIC at grantee 0 fails it too), the setter still SECURITY DEFINER with a pinned `search_path` and its reviewed argument list, its body still byte-identical to 001C's by `md5(prosrc)` = `2f3db664…`, and the other three preference RPCs still owner + `authenticated` only. The `UPDATE` requires exactly 6 affected rows.
- **Self-check after.** Six rows still, each matching its full post-activation tuple; exactly six `reasoning_selectable`; Gemini 3.7/3.8 still without `minimal`; the column DEFAULT still `false`; `authenticated` can execute the setter and **no other role can**, with no `WITH GRANT OPTION`; the body digest unchanged; no preference row written (so no backfill), no entitlement row written; clients still hold no direct write on either table.
- **Sixteen negative controls, run locally.** Each precondition and each verify assertion was re-run against a deliberately perturbed state inside a rolled-back transaction — already activated, drifted vocabulary, drifted Automatic level, a seventh row, one row pre-opened, the setter pre-granted to `anon`/PUBLIC/`authenticated`, an altered `search_path`, the clear RPC revoked, the model setter widened, and (post-mutation) an extra grant, a grant option, a written preference, a flipped DEFAULT and a replaced body. All sixteen aborted with the intended message; the positive control passed; the database fingerprint was unchanged afterwards.
- **No application code changed.** The Settings control renders whatever `reasoning_levels` a row lists, the deployed Edge runtime already reads `preferred_reasoning_level` and applies it to **both** operations, and all three adapters already express every level. The repository change is the migration, tests, comments and docs. **No Edge Function was deployed and no provider was called.**
- **Tests.** New pgTAP suite `019_manual_reasoning_activation` (79 assertions) owns the activated state and drives the setter as `authenticated` — 48 setter calls covering every canonical level against every model, accepting each model's own list and refusing everything else (Gemini 3.7/3.8 refuse `minimal`; Claude refuses `none`; Terra refuses `off`), plus the entitlement, `model_required`, disabled-model and closed-model refusals, the cross-provider model-switch resets, and a future row proving activation opened six rows rather than the column. Suites 003, 016 and 018 were moved to the final state rather than softened: the setter moved out of 003's "staged" class into the client-RPC matrix (291 assertions), and 016's owner-context calls became `authenticated` calls. The lock probe in `scripts/e2e-local.mjs` now calls the reasoning setter as `authenticated` too. A new Edge chain test composes selection → policy → adapter → telemetry for all six rows.
- **Production state at preparation time, verified read-only (2026-09-19):** ledger **85**, latest `20260918210017`; six catalog rows, all `enabled` and `selectable`, all `reasoning_selectable = false`; `set_current_user_ai_reasoning` ACL `{postgres=X/postgres}`, not executable by `authenticated`, `anon` or `service_role`; `clear_current_user_ai_reasoning` granted to `authenticated`; **zero** preference rows carry a manual reasoning level. The three reasoning RPC bodies are byte-identical to the repository's. No Production mutation was made.
- **Deployment state.** **The activation migration is NOT applied.** Production keeps `reasoning_selectable = false` on all six rows and the setter granted to nobody, so the Settings reasoning control stays disabled for every account and every user stays on Automatic. Applying it is a separately authorized rollout step ([deployment.md](deployment.md) §15).

---

## 2026-09-19 — AI-MANUAL-REASONING-001 closure: manual reasoning APPLIED and Production-accepted (`20260919075655`) — **APPLIED; manual reasoning is live for entitled users; C45 CLOSED**

The closure of the rollout prepared in the entry above. That entry is dated history and is deliberately not rewritten: its "the activation migration is NOT applied" was true when written, earlier the same day, and this entry is what supersedes it.

- **PR #291 merged** as the two-parent commit `96cca6fbe46651790aeffd6a278c65b653cc510b`. Required merged-main CI passed.
- **Migration `20260919075655_activate_manual_ai_reasoning_selection.sql` applied 2026-09-19**, by one `supabase db push --linked` — one attempt, no repair, no `--include-all`, no manual SQL. A read-only preflight and a dry run had each shown **exactly one** pending migration first. Ledger **85 → 86**, latest `20260919075655`, present once.
- **Exactly the two staged locks released, together.** `reasoning_selectable` **false → true** on exactly the six approved ids (`google/gemini-3.5-flash`, `google/gemini-3.6-flash`, `google/gemini-3.7-flash`, `google/gemini-3.8-flash`, `anthropic/claude-sonnet-5`, `openai/gpt-5.6-terra`), and `EXECUTE ON FUNCTION public.set_current_user_ai_reasoning(text)` granted to **`authenticated` only**, without `WITH GRANT OPTION`. Verified read-only afterwards: the setter's ACL is `{postgres=X/postgres,authenticated=X/postgres}`; `anon`, `service_role` and PUBLIC cannot execute it; the body is still `md5(prosrc) = 2f3db664…`, byte-identical to 001C's.
- **Every other catalog field unchanged**, proven field by field by the migration's own verify block and re-verified read-only: provider, provider model, display name, `enabled`, `selectable`, `sort_order`, `reasoning_levels` in order, and both Automatic columns. The Automatic matrix did not move — Analyze `minimal` (3.5/3.6), `low` (3.7/3.8), `off` (Claude Sonnet 5), `none` (GPT-5.6 Terra); organization suggestions `medium` everywhere.
- **No backfill and no entitlement change.** No preference row was written and no manual level was backfilled, so every user stayed on Automatic until they chose otherwise; no entitlement row was written, and the capability is still the same `can_select_ai_model`. The column DEFAULT stays `false`, so a future catalog row still starts closed.
- **No Edge deployment and no secret change.** All six functions were byte-identical across the activation (`analyze-paper` v30, `suggest-paper-organization` v13, `fetch-paper-metadata` v22, `delete-account` v6, `get-gemini-provider-quota` v9, `search-pubmed` v6), and no secret's value or timestamp moved. The system default is still the Google model resolved from `GEMINI_MODEL` (C34).
- **Bounded Production acceptance — PASSED 2026-09-19.** Six intentional provider operations on the dedicated acceptance account's retained synthetic canary paper, one attempt each, no retry: Gemini 3.5 Flash at `minimal` (Analyze), Gemini 3.8 Flash at `high` (Analyze), Claude Sonnet 5 at `low` (Analyze + Suggest), GPT-5.6 Terra at `low` (Analyze + Suggest). **All six resolved `model_selection_source = user_preference`, `reasoning_source = manual` and the exact saved level**, each recording one content-free telemetry event. Claude and Terra are the load-bearing cases: one saved level overrode **both** halves of each Automatic split (`off`/`medium` and `none`/`medium`).
- **Gemini 3.8 Flash returned HTTP 503 — a provider-availability exception, not a manual-reasoning routing failure.** Its telemetry event records `google/gemini-3.8-flash` at `manual`/`high` with `provider_attempts = 1`, so the intended model and manual level reached the provider boundary; Gemini 3.8 did **not** generate a response. The quota unit consumed for it was refunded, per the existing refund semantics.
- **Quota.** Five successes consumed one PaperLume quota unit each and the 503 was consumed then refunded, so the acceptance's net delta was **+5**. Reasoning effort introduced no weighted quota accounting: one successful AI operation is one unit at every level.
- **Acceptance account restored.** The temporary `ai_model_selection_enabled = true` was returned to its prior `false`; the original state was the **absence** of a preference row and that absence was restored (not merely reset to Automatic); the substantive entitlement fields matched the captured baseline afterwards, with `updated_at` moved by its ordinary trigger; the durable synthetic canary paper remained; and no non-acceptance account was intentionally changed.

Cross-references rather than repetition: the executed runbook and the full acceptance record are in [deployment.md](deployment.md) §15, §15.1 and §15.2; the decision and its rollout posture are C45 in [decisions-and-triggers.md](decisions-and-triggers.md); the privacy position — no new data category, no Privacy Policy amendment — is the 2026-09-19 addendum in [privacy-data-flow-audit.md](privacy-data-flow-audit.md).

---

## 2026-09-24 — SEC-AI-QUOTA-REFUND-AUTHORITY-001: server-only AI-quota refund (`20260924193915`) — **APPLIED to Production and both generation functions DEPLOYED, 2026-09-25**

Closes an authorization defect in the AI quota (C47). **Live in Production since the 2026-09-25 migration-first rollout** (last bullet). No canary was run.

- **The defect.** `refund_ai_quota(uuid)` was executable by `authenticated` and checked only `p_user_id = auth.uid()` before decrementing the counter. Nothing tied a refund to a consumption, so a signed-in browser could consume, refund itself and consume again without limit. Re-proven on a disposable local PG 17.6 replay whose function body is byte-identical to Production (prosrc md5 `36d1bdb04fc5d163a04cc32afce0ee66`): a Free user consumed 15, refunded 15 directly and consumed 15 more. Never reproduced against Production.
- **Migration `20260924193915_make_ai_quota_refund_server_only.sql` — applied to Production 2026-09-25.** Explicitly transactional. It refuses to run unless it sees exactly the reviewed pre-state: run as `postgres`; exactly one `refund_ai_quota` overload; owner `postgres`, SECURITY DEFINER, plpgsql, `search_path=public`, the `TABLE(refunded, period_type, used)` signature; the reviewed body by digest and by content (the exempt bucket and the caller guard); EXECUTE held by owner + `authenticated` and nobody else; `service_role` present and executing **no** SECURITY DEFINER function in `public`; `consume_ai_quota` and `get_ai_quota_status` at their reviewed digests and grants. Then it revokes every non-owner grantee, replaces the body (the `auth.uid()` comparison removed, every bucket rule preserved, NULL refused), rewrites the COMMENT, and grants EXECUTE to `service_role` — in that order, so the server-style body is never executable by `authenticated` even inside the transaction. It changes authority only (ACL and function body) and is not meant to write any quota or entitlement row. A closing self-check asserts:
  - the exact final matrix: owner + `service_role`; no `authenticated`, `anon` or PUBLIC; no grant option;
  - the new body digest `4224750ddbff3651e7e0aaa2576f4de4`;
  - that `service_role` now executes exactly this one SECURITY DEFINER function;
  - that consume and status are untouched;
  - that **this transaction** inserted, updated or deleted no `usage_counters` or `user_entitlements` row.

  The last check reads the backend's own per-transaction table statistics (`pg_stat_get_xact_tuples_*`) against a baseline taken at the start of the transaction. Other sessions' writes never appear there, so legitimate AI traffic during the migration cannot fail it, and no traffic pause is needed. It requires `track_counts` (on in Production, read-only verified 2026-09-24) and refuses to run without it. An earlier draft of this file, never applied anywhere, compared rows' `updated_at` with `now()`, and a concurrent `consume_ai_quota` could fail that check. It was replaced before review completed (correction `4e18b35`), so the applied file is the concurrency-safe one.
- **Nine negative controls, run locally.** Each on a disposable clone of a hosted-parity lane at the pre-migration state: `service_role`, `anon` or PUBLIC pre-granted; `authenticated` missing; an extra overload; a drifted refund body; a drifted `consume_ai_quota` body; `service_role` granted on another SECURITY DEFINER function; and the wrong executing role. Every one was refused with its named precondition, and every public function's ACL and body fingerprint was byte-identical afterwards.
- **No-write self-check, proven locally on PG 17.6.**
  - **Concurrent traffic.** With the file paused just before the check, a second session ran a real `consume_ai_quota` as `authenticated` plus an entitlement update, and committed them. The earlier `updated_at` draft aborted on that traffic; the current file committed, and the other session was not blocked.
  - **Same-connection writes.** Counter and entitlement writes committed on the same connection moments earlier do not fail the check. A zero-baseline variant of the file does fail on them, which is why the check compares against a baseline.
  - **Writes the check must catch.** It catches an UPDATE, a DELETE, a backdated INSERT, an entitlement UPDATE that leaves `updated_at` alone, and a call to `refund_ai_quota` from inside the migration; each was rolled back. The old timestamp check missed the DELETE and the backdated INSERT.
  - **Fail-closed guards.** Stripping `BEGIN`/`COMMIT` trips the missing-baseline guard, and `track_counts = off` is refused before anything changes.
- **Edge (both generation functions).** New `_shared/aiQuotaRefund.ts`: a server-only client from the platform-injected key (same `selectEdgeSecretKey` rule), no caller Authorization header, no session, a 5 s bound, typed to exactly `rpc("refund_ai_quota", { p_user_id })`, plus a never-throwing best-effort `refundAiQuotaUnit`. `analyze-paper` (both refund sites) and `suggest-paper-organization` (its one refund site) now refund through it with the `auth.getUser()` id; consumption stays on the caller's client. `suggest-paper-organization`'s caller-client type now names only `consume_ai_quota` and `get_current_user_access`. The telemetry writer is unchanged and stays insert-only.
- **Tests.** pgTAP `003` gains a server-only RPC classification (298 assertions), `004` gains the regression proof and the preserved accounting rules (41), `000` pins the new posture (139, unchanged count); the framework-free verification's cases 13/16 now refund as `service_role` and case 13 also proves the owner's own JWT is refused (18/18). All 20 suites pass on a fresh replay (**1,862** assertions) and the key suites on a simulated hosted-parity lane. New executed Edge suite `analyze-paper/__tests__/quotaRefundAuthority.test.ts` runs the real `Deno.serve` shell under Vitest with the remote import mocked; new `_shared/__tests__/aiQuotaRefund.test.ts`; the suggest handler suite gains a refund-authority block. Six source mutations reintroducing the defect were each caught.
- **Generated types unchanged.** Types generated with postgres-meta 0.99.0 before and after the migration on a local replay are byte-identical, and the `refund_ai_quota` entry matches the committed `src/integrations/supabase/types.ts`.
- **Production state before the rollout (historical), verified read-only (2026-09-24):** ledger **86**, latest `20260919075655`; `refund_ai_quota` ACL `{postgres=X/postgres,authenticated=X/postgres}`, body digest as above; `analyze-paper` v32 and `suggest-paper-organization` v15, both byte-identical to `main` `f06107b6` and refunding through the caller-scoped client.
- **Production rollout (2026-09-25) — migration first, then both functions from the same merge commit** ([deployment.md](deployment.md) §6.8):
  - **Merge.** PR #299 merged as the two-parent commit `dca8a0be3daa61f45d1f18e2aa526e8a444a320e` (parents `f06107b6` and the approved head `4e18b352`); merged-`main` Validate, DB Tests and Extension passed on it.
  - **Migration.** `20260924193915` applied under its repository version (statements in the Production Postgres log at 04:37:16–04:37:17Z). Ledger **86 → 87**, latest `20260924193915`.
  - **Edge.** `analyze-paper` **v32 → v33** (04:38:04Z, `ezbr_sha256` `7562f5a8bb954368e2a0cddb28a446faed879f72e7e461193048bdbfed107271`) and `suggest-paper-organization` **v15 → v16** (04:38:11Z, `ezbr_sha256` `fa7f5ebcdaa71946d6eed733d769d8bbb9913b3c51f3f53af52350e521997127`). `fetch-paper-metadata` v22, `get-gemini-provider-quota` v9, `delete-account` v6 and `search-pubmed` v6 were not redeployed.
  - **Final Production verification (read-only):** `refund_ai_quota(uuid)` has ACL `{postgres=X/postgres,service_role=X/postgres}` and body digest `4224750ddbff3651e7e0aaa2576f4de4`. EXECUTE is **false** for `authenticated`, `anon` and PUBLIC and **true** for `service_role`. The set of `public` SECURITY DEFINER functions `service_role` can execute is exactly `{refund_ai_quota(uuid)}`. `consume_ai_quota` (`8b3f8c3b…`) and `get_ai_quota_status` (`212b9a3e…`) are unchanged and still `authenticated`-only. The independent Production verification read back the deployed v33 and v16 bundles: both contain the reviewed server-only refund path.
  - **Not done, by design.** No AI-provider request, no quota consume or refund canary and no application-data write. The closure rests on the live ACL and body plus the deployed bundles. The refund remains best-effort.
  - **Rollback** is a security rollback that re-opens the defect; see [deployment.md](deployment.md) §6.8. None has been performed.

## 2026-09-25 — DB-JUNCTION-DML-GRANT-HARDENING-001: assignment junctions become SELECT-only for the browser (`20260925134526`) — **APPLIED to Production, 2026-09-25**

Follow-up to C38; decision **C48**. **Live in Production since the 2026-09-25 migration-only rollout** ([deployment.md](deployment.md) §6.9; last bullet). No canary was run.

| `authenticated` | `paper_projects` / `paper_tags` | `projects` / `tags` |
|---|---|---|
| **Before** (read-only, 2026-09-25; ledger **87**, latest `20260924193915`) | `SELECT, INSERT, DELETE` (`authenticated=ard/postgres`) | `SELECT, INSERT, UPDATE, DELETE` (`authenticated=arwd/postgres`) |
| **After** (ledger **88**, latest `20260925134526`) | `SELECT` only (`authenticated=r/postgres`) | `SELECT, INSERT, UPDATE, DELETE` — **unchanged** |

- **What changes.** One statement: `REVOKE INSERT, DELETE ON TABLE public.paper_projects, public.paper_tags FROM authenticated;`. Not `REVOKE ALL` — `SELECT` is still used by the dashboard list, Project/Tag filtering, CSV/BibTeX export and the account export.
- **Projects/Tags entity permissions are unchanged, and AI-assisted creation remains supported.** AI "Create & select" inserts the new Project/Tag into `projects` / `tags` (the same `createProject` / `createTag` mutation the management UI uses) and only stages its id; the paper assignment is written by `set_paper_projects` / `set_paper_tags` on Save. No junction write is involved at any step.
- **Why the assignment paths keep working.** Every junction write goes through `set_paper_*`, `bulk_set_paper_*`, `bulk_add_paper_*` or `merge_exact_duplicates`. All seven are SECURITY DEFINER, owned by `postgres` (which owns both junctions and holds `arwdDxtm` on them), `search_path=public`, and executable by owner + `authenticated` only — verified read-only in Production on 2026-09-25, with body digests `8104be4a…` / `8b0537b3…` / `a348cebf…` / `e3b6bcfe…` / `1d1c9125…` / `01da7404…` / `b43400b3…`. Their junction writes are checked against the owner, not the caller. FK cascades from deleting a Project, Tag or paper also run as the junction's owner.
- **Audit result.** A repository-wide call-graph audit found no direct junction INSERT / UPSERT / UPDATE / DELETE in the web app, the Chrome extension or any Edge Function; every direct junction access is a read (`usePapers`, `useFilterState`, `useExportPapers`, `fetchAccountExportData`).
- **Fail-closed preconditions.** Runs as `postgres` with `track_counts` on; all four tables exist once as ordinary `postgres`-owned tables with RLS enabled and forced; the junctions' `authenticated` grant is exactly `SELECT, INSERT, DELETE` direct **and** effective with no grant option; `projects` / `tags` exactly `SELECT, INSERT, UPDATE, DELETE`; no `anon`, PUBLIC or unknown grantee; no browser-role column grant (a column-level INSERT would survive a table REVOKE); exactly six junction policies — SELECT/INSERT/DELETE per table, permissive, both-owner — pinned by digest `04b973b09f1aadcd8a5389b8899818b7` (identical in Production and on a clean PG 17.6 replay); and all seven RPCs pinned by overload count, owner, SECURITY DEFINER, language, `search_path`, result type, argument names, body digest and EXECUTE ACL. It then snapshots the junctions' non-`authenticated` ACL entries, `service_role`'s direct and effective junction privileges, the entity tables' whole ACLs, RLS flags, every policy on all four tables and the seven RPCs.
- **Fail-closed verification before COMMIT.** Junctions `SELECT`-only for `authenticated` (each of the eight privileges checked by name, direct and effective, no grant option); the owner still holds `SELECT`, `INSERT` and `DELETE` on both; every snapshot equal (so `service_role`, `projects` / `tags`, RLS, policies and RPCs are proven unchanged); no browser-role column grant; and — from this transaction's own `pg_stat_get_xact_tuples_*` counters against a baseline taken at its start — no row inserted, updated or deleted in `papers`, `projects`, `tags` or either junction. No timestamp heuristic is used.
- **Controls run locally (PG 17.6).** On the reviewed pre-state the file applied cleanly (positive control). 10 precondition controls (column INSERT grant, dropped junction policy, weakened junction policy, drifted RPC body, RPC EXECUTE widened to `anon`, `merge_exact_duplicates` EXECUTE widened to `service_role`, extra junction UPDATE, `projects` INSERT already missing, `anon` grant, FORCE RLS off) were each refused with their named message; 11 verification controls (`REVOKE ALL`, INSERT-only revoke, `projects` or `tags` also narrowed, `service_role` changed, a row written, policy dropped, RPC EXECUTE changed, column grant added, FORCE RLS dropped, grant option added) were each caught before COMMIT.
- **Tests.** pgTAP `015` now expects `paper_projects` / `paper_tags` = `SELECT` and adds section I (entity tables stay `DELETE,INSERT,SELECT,UPDATE` as literals; junctions `SELECT / SELECT`; no junction column grant; the RPC write authority intact). `002` proves the junctions in two layers — the real browser boundary (reads work, every direct write is an ACL refusal, Projects/Tags are still created directly and assigned by RPC, cascades still clean up), and the dormant both-owner policies under a test-only, rolled-back re-grant, with every refusal required to be an RLS refusal. `000` replaces its positive direct-INSERT cases with ACL denials and adds a section proving all four setters' INSERT and DELETE halves still write with the direct grant gone. `003`, `005` and `013` are unchanged and pass. New Vitest suites: `src/test/junctionWriteBoundary.test.ts` (TypeScript-AST guard over the web app, extension and Edge Functions: direct `.from()` junction mutations on the chain or through a same-file builder alias, failing closed on unresolved mutated tables and on bare junction builders that leave local view; not cross-file data flow or hand-built REST requests) and `EditPaperDialog.aiCreateAssignBoundary.test.tsx` (the real AI Create & select → Save path against a client that refuses junction writes).
- **Generated types unchanged** — a grant is not part of the schema shape; `src/integrations/supabase/types.ts` is untouched.
- **History — not the cross-owner fix.** Before `20260802025704` the junction policies checked paper ownership only, and cross-owner junction insertion was a confirmed defect, reproduced on a replay of that schema (PFA-C03B1); whether a real account ever exercised it is not established. `20260802025704` remediated it with both-owner RLS and setter ownership validation. C48 is the later least-privilege cleanup that removed the redundant direct browser `INSERT` / `DELETE` authority (decision C48).
- **Production rollout (2026-09-25) — migration only** ([deployment.md](deployment.md) §6.9):
  - **Merge.** PR #301 merged as the two-parent commit `043efee0b9477537cf125fffc32434a89d0c5bb5` (parents `69f51a66` and the approved head `285ea61b`). Merged-`main` Validate, DB Tests and Extension passed on it; `E2E (local)` does not run on a push to `main`, and its evidence is the pull-request run `36193186293` on the exact approved head `285ea61b`, which passed.
  - **Migration.** `20260925134526` applied under its repository version, present exactly once. Ledger **87 → 88**, latest `20260925134526`. No Edge Function, secret, Auth or Storage change accompanied it. The merge had already triggered the normal automatic Vercel Production deployment for `043efee` (`dpl_FYqqhWU5TBJqbZHGypApzhcrMPrk`, READY); it required no manual Vercel action and carried no C48 application-behavior change.
  - **Final Production verification (read-only):** `paper_projects` / `paper_tags` grant `authenticated` `SELECT` only (every other privilege false; normalized direct ACL `postgres=arwdDxtm`, `service_role=arwdDxtm`, `authenticated=r`). Verified unchanged: RLS and FORCE RLS on all four tables; the six junction policies (digest `04b973b0…`), still present as dormant defense-in-depth; the seven assignment/merge RPCs (owner `postgres`, SECURITY DEFINER, `search_path=public`, the body digests above, EXECUTE owner + `authenticated` only); `service_role`'s junction privileges; the `projects` / `tags` ACL (`authenticated=arwd/postgres`); and the deployed Edge Function versions.
  - **Not done, by design.** No AI suggestion, no Project/Tag creation and no paper-assignment canary, and no application-data write. The rollout is established by the live ACL/catalog state and the tracked migration; the product paths under the hardened schema are proven by the CI suites above.
  - **Rollback** would be a least-privilege regression; see [deployment.md](deployment.md) §6.9. None has been performed.

## 2026-09-26 — DB-INVOKER-EXECUTE-HARDENING-001A: the five caller-scoped read RPCs become SECURITY INVOKER (`20260926152414`) — **APPLIED to Production, 2026-09-26**

First bounded result of the `DB-INVOKER-EXECUTE-HARDENING-001` design audit; decision **C49**. **Live in Production since the 2026-09-26 migration-only rollout** ([deployment.md](deployment.md) §6.10; last bullet). No canary was run.

| | Five read RPCs | `public` SECURITY DEFINER | … executable by `authenticated` (advisor lint 0029) |
|---|---|---|---|
| **Before** (read-only, 2026-09-26; ledger **88**, latest `20260925134526`) | SECURITY DEFINER | 40 | **32** |
| **After** (Production, read-only after the 2026-09-26 apply; ledger **89**, latest `20260926152414`) | SECURITY INVOKER | 35 | **27** |

- **What changes.** Five `ALTER FUNCTION … SECURITY INVOKER` statements — `search_papers(uuid,text,integer,integer)`, `search_papers_short(uuid,text)`, `filter_papers_by_keywords(uuid,text[])`, `get_keyword_options(uuid,uuid[],integer,integer,text[])`, `get_duplicate_papers()` — and nothing else. Only `prosecdef` changes: the bodies (md5 `d4a5f3af…` / `ce353564…` / `b2f5a8e5…` / `531010c1…` / `3c914811…`), signatures, return types, argument defaults, volatility, parallel mode, owner, `search_path=public` and EXECUTE ACL `{postgres=X/postgres,authenticated=X/postgres}` are all unchanged. No `CREATE OR REPLACE`.
- **Why it is safe.** Each function reads only the caller's rows of `papers` (and `synonym_pool` for `filter_papers_by_keywords`). `authenticated` already holds table-level `SELECT` on both, the caller-owned PERMISSIVE SELECT policies (`USING (auth.uid() = user_id)`) admit exactly those rows, and `authenticated` is neither SUPERUSER nor BYPASSRLS. Before this change the owner `postgres` (BYPASSRLS) ran them, so each body's `auth.uid()` predicate was the only boundary. After it, RLS is the primary boundary and the unchanged predicates are defense-in-depth.
- **`search_papers`' historical inline comment** ("SECURITY DEFINER bypasses table-level RLS …", from `20260518010000`) is left in the stored body on purpose, because recreating the body only to edit a comment would widen the change. The migration states above its `ALTER` that it supersedes that comment. C49 and [architecture-read-path.md](architecture-read-path.md) describe the current mode. The comment may be dropped the next time the body is legitimately recreated.
- **Fail-closed preconditions.** Runs as `postgres` with `track_counts` on. `authenticated` is not SUPERUSER/BYPASSRLS and holds USAGE on `public` / `auth` and EXECUTE on `auth.uid()`. Each of the five is pinned on: exactly one overload, owner, SECURITY DEFINER, `plpgsql`, volatility (`v` / `s` / `s` / `s` / `v`), parallel `u`, strictness, leakproofness, set-returning, exact result type, exact argument list with defaults, `search_path=public`, body md5, the literal ACL, and effective EXECUTE (`postgres` / `authenticated` true; `anon` / `service_role` / PUBLIC false). `papers` / `synonym_pool` are pinned on: owner `postgres`, RLS **and** FORCE RLS, `authenticated` exactly `INSERT,SELECT,UPDATE` / `DELETE,INSERT,SELECT,UPDATE` direct and effective, no `anon` / PUBLIC / unknown grantee, and no column-level grant. All eight policies are pinned by name, command, permissive, roles, USING and WITH CHECK, and by digest `07603cbe4e78a4d6097e7ec33bd1e6c8` (identical in Production and on a clean PG 17.6 replay). The inventory is pinned at 40 definer functions, 32 of them `authenticated`-executable. Before the change it snapshots: each of the five's whole `pg_proc` row minus `prosecdef` (oid included), every other `public` function's whole row, the 27 retained authenticated definer functions, the existing authenticated INVOKER set, and both relations' owner / RLS / ACL.
- **Fail-closed verification before COMMIT.** All five are `prosecdef = false`, with the ACL and effective posture unchanged. Their rows minus `prosecdef` are identical, and the body digests are restated literally. No other `public` function changed. The definer count is 35 and the authenticated definer set is exactly the 27 retained. The authenticated INVOKER set grew by exactly the five. Relations, grants, policies (digest) and `authenticated`'s role attributes are unchanged. The transaction wrote no `papers` / `synonym_pool` row (its own `pg_stat_get_xact_tuples_*` counters).
- **Migration controls run locally (PG 17.6).** Each was run inside a rolled-back transaction on the reviewed pre-state (the five reverted to DEFINER):
  - *Positive control:* the clean pre-state converted all five.
  - *Refused, each with its named message:* SELECT policy broadened; extra RESTRICTIVE policy; `synonym_pool` SELECT policy dropped; `papers` SELECT revoked; `synonym_pool` SELECT revoked; `search_papers` body drift; `anon` EXECUTE granted; an extra authenticated definer function (count 41); running as `service_role`; and re-running on the already-converted state.
  - *Not constructible:* the BYPASSRLS drift. Supabase reserves `authenticated` ("only superusers can modify it"), so the platform itself blocks it; the precondition stays as a guard.
- **Tests.**
  - `003` (309 assertions): 35 definer functions (27 directly callable + 1 server-only + 5 trigger-only + 2 internal). The five are removed from the definer classification, so a revert is an unclassified definer and fails. A new `invoker_rpcs()` classification pins NOT `prosecdef`, the byte-identical ACL, and anon / PUBLIC / service_role refused, authenticated and owner allowed. The authenticated definer set is asserted to equal the 27. The null / mismatch / valid caller tests are unchanged and pass.
  - `015` (103): ACL-H1 inventory 6 → 11 (ordered under `COLLATE "C"`). New section J pins each of the five (owner, INVOKER, `search_path`, body digest, ACL, "exec authenticated") plus the authenticated-only INVOKER set. Relation expectations are unchanged.
  - New `020_read_rpc_security_invoker` (67): posture; boundary (both tables' grants, RLS/FORCE, all eight policies by value and digest, a non-BYPASSRLS `authenticated`); anon refused; own-user results (FTS, attribution, short search, keyword filter incl. own-synonym expansion, keyword options incl. id narrowing, duplicate groups); the unchanged cross-user / null-auth / NULL-id contract; foreign ids, keywords, text and synonyms yielding nothing; the identity logic still present; **RLS is live** (a rolled-back RESTRICTIVE deny policy empties all five, and on `synonym_pool` removes only the synonym expansion); and **the grant is live** (a rolled-back SELECT revoke makes exactly the dependent functions fail with "permission denied for table").
  - `000`: comment only.
  - Full local `npm run test:db:local`: **21 suites, 1,998 assertions, PASS**, plus the harness's own negative control, concurrency probes and the hosted-Production ACL parity lane, which applied this migration from Production's legacy ACL shape and kept `015` green.
- **Negative controls (tests must fail).** Each was injected into a rolled-back copy of the suite. `020` caught every one:
  - each of the five left SECURITY DEFINER, one at a time (posture + RLS-live + grant-live);
  - authenticated EXECUTE revoked;
  - anon EXECUTE granted;
  - service_role EXECUTE granted;
  - a body change;
  - a `search_path` change;
  - `papers` SELECT policy broadened, and dropped;
  - `synonym_pool` SELECT policy broadened, and dropped;
  - `papers` SELECT revoked;
  - `synonym_pool` SELECT revoked;
  - a cross-user leak: `search_papers_short` with its guard removed and owner authority restored, which really returned the victim's row to another user.

  `003` and `015` catch the posture subset. The same guard removal while staying INVOKER returned **0** rows, because RLS alone stopped it — the point of C49 — and is still caught by the guard and posture assertions.
- **Product regression.** `npm test` 173 files / 5,658 tests; lint, typecheck, `build` and `build:extension` pass. Local E2E through the repository harness passed **69/69** across `search-attribution`, `notes`, `filters`, `eager-load`, `pools` and `scrollarea-reachability`. A PostgREST-level probe (owner / other user / anon over `/rest/v1/rpc`) passed 15/15. No application or Edge Function source changed.
- **Generated types unchanged.** `supabase gen types typescript --local --schema public` produces byte-identical output with the five as DEFINER or INVOKER, and `src/integrations/supabase/types.ts` is untouched.
- **Advisor.** Before the rollout Production reported **32** `authenticated_security_definer_function_executable` warnings; after it, **27** — 24 intentional functions plus the three later INVOKER candidates (`bulk_update_keywords`, `bulk_update_study_types`, `safe_bulk_insert_papers`). The CLI's bundled local advisor does not implement lint 0029, so the pre-rollout local figure was that lint's catalog predicate (definer ∧ `authenticated` EXECUTE in `public`) = **27**, which suite `003` pins as a set, and the Production advisor now agrees with it.
- **Deliberately separate follow-ups (not in this change):**
  - `pg_temp` placement in the retained definer functions' `search_path`;
  - an explicit posture/behaviour test for `attachment_object_has_live_metadata` (which must stay SECURITY DEFINER);
  - the `bulk_update_*` INVOKER group;
  - the `safe_bulk_insert_papers` INVOKER group;
  - `DB-SEARCH-VECTOR-EXPRESSION-PARITY-001`;
  - C30 (leaked-password protection) — unchanged.
- **Production rollout (2026-09-26) — migration only** ([deployment.md](deployment.md) §6.10):
  - **Merge.** PR #303 merged as the two-parent commit `77b7a4ba0470eb3645d53217e6155cd17c65c15b` (parents `a4ca8238` and the approved head `a68fdaad`). Merged-`main` Validate, DB Tests and Extension passed on it; `E2E (local)` does not run on a push to `main`, and its evidence is the pull-request run `36254210375` on the exact approved head `a68fdaad`, which passed.
  - **Migration.** The linked migration comparison showed exactly one pending migration, and `supabase db push --dry-run` listed exactly `20260926152414_harden_read_rpcs_security_invoker.sql`. The normal linked `supabase db push` applied exactly that file under its repository version (the five `ALTER FUNCTION … SECURITY INVOKER` statements are in the Production Postgres log at 17:21:31–17:21:32Z). Ledger **88 → 89**, latest `20260925134526` → `20260926152414`, present exactly once. No Edge Function deployment, secret, Auth, Storage, AI/provider or quota change accompanied it, and no manual Vercel action was part of it.
