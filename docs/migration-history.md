...
## 2026-09-12 — AI-MULTI-PROVIDER-001C: model-aware reasoning policy and provider activation foundation (`20260912120000`) — **implemented; open PR; Production migration NOT authorized; NOT deployed; MERGE-GATED on the migration**

- **Repository-only, and merge-gated.** One new additive migration, `20260912120000_add_model_aware_ai_reasoning_policy.sql`. It was replayed locally from zero and **not applied to Production**. No Production SQL, secret, Edge deploy or live Gemini/Anthropic/OpenAI request was made, and every test uses an injected fake `fetch`. No package, SDK, lockfile, workflow, extension or Chrome Web Store change. Decision: **C41**. **The 001C PR must not be merged until this migration has been separately authorized, applied to Production and verified while the old application is still live** ([deployment.md](deployment.md) §6.6).
...
- **Deployment state.** **Not deployed.** Production still runs the pre-001A `analyze-paper` v26 and `suggest-paper-organization` v10. Once authorized, the deploy must cover both generation functions together and must follow the migration (§6.6a). No Anthropic/OpenAI secret is installed, and no catalog row names either provider.
