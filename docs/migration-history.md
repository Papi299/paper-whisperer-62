...
- **Explicitly not touched:** `service_role` (relation privileges, sequence privileges and default privileges), function EXECUTE and function default privileges, `supabase_admin` defaults, RLS/policies/FORCE-RLS flags, and the `paper_tags`/`paper_projects` SELECT/INSERT/DELETE contract. The migration names `service_role` in no statement and refuses to commit if its privileges moved.
