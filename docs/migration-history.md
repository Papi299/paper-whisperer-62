...
- **Migration `20260818120000_add_user_scoped_author_identity_resolution.sql`.** Strictly additive: four tables (`author_identities`, `author_identity_aliases`, `author_identity_links`, `author_identity_merges`), nine functions (six client RPCs, two internal helpers, one trigger function), one trigger on `papers`, and no change to any existing table, column, constraint, policy, grant, function or row. **Not applied to Production** — it requires explicit owner authorization.
...
- **User-scoped, enforced structurally.** Every alias, link and merge foreign key is **composite on `(user_id, …)`** against `author_identities (user_id, id)`, so a cross-user relationship cannot be *stored*, not merely cannot be reached. No global uniqueness on a preferred name or an ORCID — two users may reach different conclusions about the same person, and two people may share a name.
...
- **Analytics.** Counts one entity per resolved identity root and one per unresolved 001A mention key; a paper is credited once per entity. Selections are held as stable entity keys so a rename or a change of representative spelling cannot drop them.
