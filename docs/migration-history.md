...
- **001B is complete. 001C is not.** `AUTHOR-IDENTITY-RESOLUTION-001C` remains **unimplemented**: there is no person, author, alias or identity-link table, no global ORCID uniqueness, no auto-linking, no fuzzy/affiliation/coauthor/AI resolution and no author-merge UI. A matching ORCID across two rows is **provenance a source stated, not a claim that the mentions are the same person**. Resolving mentions to people remains a separate, later product decision.
