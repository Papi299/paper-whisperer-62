...
- **Nothing else was mutated.** No schema, migration, DDL, `GRANT`/`REVOKE`, default-privilege or policy change. No Auth user was created, deleted or modified — the only Auth activity was one ordinary password sign-in (with GoTrue's own sign-in bookkeeping) and a local-scope sign-out that revoked that session. No attachment, Storage, Edge Function, secret, provider, manual Vercel or Chrome Web Store action. Chrome Web Store publication and review are separate from this initiative.
