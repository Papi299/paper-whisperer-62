...
## 2026-09-24 — SEC-AI-QUOTA-REFUND-AUTHORITY-001: server-only AI-quota refund (`20260924193915`) — **implemented; migration NOT applied to Production; Edge NOT deployed**

Closes an authorization defect in the AI quota (C47). **Draft only: nothing here is applied, deployed or canaried.**
...
- **Migration `20260924193915_make_ai_quota_refund_server_only.sql` — prepared, NOT applied.** Explicitly transactional. It refuses to run unless it sees exactly the reviewed pre-state: run as `postgres`; exactly one `refund_ai_quota` overload; owner `postgres`, SECURITY DEFINER, plpgsql, `search_path=public`, the `TABLE(refunded, period_type, used)` signature; the reviewed body by digest and by content (the exempt bucket and the caller guard); EXECUTE held by owner + `authenticated` and nobody else; `service_role` present and executing **no** SECURITY DEFINER function in `public`; `consume_ai_quota` and `get_ai_quota_status` at their reviewed digests and grants. Then it revokes every non-owner grantee, replaces the body (the `auth.uid()` comparison removed, every bucket rule preserved, NULL refused), rewrites the COMMENT, and grants EXECUTE to `service_role` — in that order, so the server-style body is never executable by `authenticated` even inside the transaction. It changes authority only (ACL and function body) and is not meant to write any quota or entitlement row. A closing self-check asserts:
  ...
  The last check reads the backend's own per-transaction table statistics (`pg_stat_get_xact_tuples_*`) against a baseline taken at the start of the transaction. Other sessions' writes never appear there, so legitimate AI traffic during the migration cannot fail it, and no traffic pause is needed. It requires `track_counts` (on in Production, read-only verified 2026-09-24) and refuses to run without it. An earlier draft of this same unapplied file compared rows' `updated_at` with `now()`, and a concurrent `consume_ai_quota` could fail that check. It was replaced before review completed.
...
- **Production state at preparation time, verified read-only (2026-09-24):** ledger **86**, latest `20260919075655`; `refund_ai_quota` ACL `{postgres=X/postgres,authenticated=X/postgres}`, body digest as above; `analyze-paper` v32 and `suggest-paper-organization` v15, both byte-identical to `main` `f06107b6` and refunding through the caller-scoped client.
- **Deployment state.** **The migration is NOT applied and neither function is deployed.** Rollout is migration first, then both functions from the same merge commit ([deployment.md](deployment.md) §6.8).
