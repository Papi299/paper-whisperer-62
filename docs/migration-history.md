...
- **Choices between people are told apart by evidence, never by a UUID.** Preferred names are deliberately not unique, so the chooser and the undo controls annotate each option with the user's own data — ORCIDs, aliases, linked spellings, linked-mention counts, a recognisable paper title — in the accessible name as well as on screen. `A → B → C` therefore renders "Undo merge of A into B" and "Undo merge of B into C" rather than two identical controls.
- **Stale links are reported, not obeyed.** A link is used only when its paper is present, its index still exists, and the author text is byte-for-byte its `author_name_snapshot` — the read-side twin of the invalidation trigger. A row failing that check contributes no grouping and no ORCID/name evidence, is counted in an explicit warning, and its mention is re-offered as unresolved (replacing the surviving row rather than colliding with the unique `(paper_id, author_index)`).
...
- **Local verification.** Full migration replay from empty; 933 pgTAP assertions across 11 suites (108 new in suite 010) plus the framework-free verification, both concurrency probes and the catalog-fingerprint residue checks; 2552 unit tests across 99 files; 155 local E2E tests including five identity acceptance flows and a seed-time cross-user isolation probe over the real Data API; lint at its 0-error baseline; typecheck and build clean.
