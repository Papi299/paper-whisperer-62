...
- **Tests.** New `_shared/__tests__/aiProviderRegistry.test.ts` (39) and `_shared/__tests__/googleAiProvider.test.ts` (46), new `analyze-paper/__tests__/geminiRequestGolden.test.ts` (10), plus new cases in the suggest handler and prompt suites and updated wiring assertions in the two `analyze-paper` suites. Full local suite **4,771 tests / 154 files green**; lint clean (18 pre-existing warnings, 0 errors); typecheck and build green; the Edge modules and the `analyze-paper` entrypoint were additionally type-checked directly, which CI does not do.
