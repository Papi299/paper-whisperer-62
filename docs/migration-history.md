...
- **One migration:** `20260904120000_add_recoverable_attachment_cleanup_queue.sql`. It creates one table (`public.attachment_cleanup_queue`), one index, two RLS policies, three `SECURITY DEFINER` client RPCs (`delete_attachment_with_cleanup(uuid)`, `delete_papers_with_attachment_cleanup(uuid[])`, `finalize_attachment_upload(uuid, text, text, text, integer)`), one `SECURITY DEFINER` trigger function plus its BEFORE INSERT trigger on `paper_attachments` (`reject_attachment_over_cleanup_intent()` / `trg_paper_attachments_block_cleanup_intent`) and one `SECURITY INVOKER` internal helper (`attachment_cleanup_path_is_safe(uuid, text, uuid)`). Apart from adding that one trigger it alters no existing table, column, trigger, policy or grant, backfills and repairs no row, and touches no Storage object or `storage.*` object. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C37**.
...
- **A tombstone makes the exclusion a property of the table.** `trg_paper_attachments_block_cleanup_intent` refuses any `paper_attachments` INSERT whose `(user_id, file_path)` already has a queued cleanup intent. `authenticated` still holds INSERT on `paper_attachments` — the pre-migration client path needs it, and a browser tab loaded before the frontend deploy still uses it — so without this the exclusion would hold only for writes that came through finalization. It is named to sort before `trg_paper_attachments_check_storage_quota`, so a refusal costs no quota accounting.
