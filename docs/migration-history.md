...
- **Backend live is not feature shipped.** `AI-PROJECT-TAG-SUGGESTIONS-001B` — the Edit Paper suggestion UX — has **not** started. No frontend calls this endpoint, so users have no suggestion feature; what the rollout bought is that the endpoint-before-UI gate is satisfied and `001B` may now be built against a live, verified contract.
