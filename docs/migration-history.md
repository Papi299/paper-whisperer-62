...
- **One migration:** `20260904120000_add_recoverable_attachment_cleanup_queue.sql`. It creates one table (`public.attachment_cleanup_queue`), one index, two RLS policies, three `SECURITY DEFINER` client RPCs (`delete_attachment_with_cleanup(uuid)`, `delete_papers_with_attachment_cleanup(uuid[])`, `queue_untracked_attachment_cleanup(uuid, text)`) and one `SECURITY INVOKER` internal helper (`attachment_cleanup_path_is_safe(uuid, text, uuid)`). It alters no existing table, column, trigger, policy or grant, backfills and repairs no row, and touches no Storage object or `storage.*` object. Durable decision: [decisions-and-triggers.md](decisions-and-triggers.md) **C37**.
...
- **Generated types:** `src/integrations/supabase/types.ts` regenerated from the **local** database (`supabase gen types typescript --local --schema public`); the diff is purely additive (+43 lines, 0 deletions) — the queue table and the four function signatures. Not hand-edited (only the generator's trailing blank line normalized for `git diff --check`); the linked/Production schema was not used for generation.
- **Tests.** New database suite `supabase/tests/database/014_attachment_cleanup_recovery.test.sql` (**95 assertions**): the queue's shape, forced RLS, exactly-SELECT+DELETE policy set, grant matrix, uniqueness, reason domain, auth cascade and absent paper FK; cross-account read and acknowledgement isolation; the full matrix for all three RPCs including anonymous/foreign/unknown/NULL rejection with state assertions proving no partial mutation, the quota refund still firing, idempotent repeated intent, fail-closed unsafe paths, and the metadata-exists guard; the helper's posture and a 16-case accept/reject matrix; and that no cleanup function references the `storage` schema. Suite `003` extends its exhaustive inventory from 33 to **36** SECURITY DEFINER functions and its EXECUTE matrix from 27 to **30** directly-callable RPCs (223 → **238** assertions). Suite `007` gains the internal path helper as a fifth pinned non-RPC function (55 → **59**). Suite `008` proves the queue cascades with the account and has no paper FK, and that a neighbour's pending cleanup survives (84 → **89**). The db-tests residue check now counts `attachment_cleanup_queue`. New application suites: `attachmentCleanup.test.ts` (36), `attachmentCleanupAvailability.test.ts` (17), `useAttachments.test.ts` (13), `useAttachmentCleanupRecovery.test.ts` (6), `usePaperMutations-attachmentCleanup.test.ts` (8), `useBulkMutations-attachmentCleanup.test.ts` (7), `deletePapersCompat.test.ts` (7). Repository totals: 144 → **151** test files, 4,561 → **4,656** tests.
- **E2E.** `e2e/attachments.spec.ts` keeps all five existing cases and adds three, deterministic only at the HTTP boundary (`page.route` fails exactly the Storage object `DELETE`, and in one case the `paper_attachments` `POST`): a forced Storage-delete failure on a direct attachment delete, proving the metadata row is gone, the queue row exists with `reason=attachment_delete`, the binary is still present and the UI says cleanup is pending — then, with the interception removed, that a reload's session-recovery pass removes the binary and clears the queue; the same for a forced metadata-insert failure with `reason=upload_compensation`; and the same for a paper deletion with `reason=paper_delete`, additionally asserting the paper row is **not** restored. Bulk deletion is pinned by unit and database tests rather than a fourth browser case — it calls the identical RPC with a longer array — which is stated here rather than implied.
