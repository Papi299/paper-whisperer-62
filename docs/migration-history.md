...
- **Tests.**
  ...
  - **Database:** new pgTAP suite `016_ai_reasoning_policy.test.sql` (104 assertions, including grantee ALLOWLISTS that judge every role that can execute the staged setter, not just the named ones). Suites 003, 011 and 012 were updated deliberately (classification lists and pinned column/result sets), not loosened. A clean local replay from zero ran 17 suites and 1,646 assertions, **all green**, plus the framework-free 18/18 and the concurrency probes.
...
- **Deployment state.** **Edge not deployed.** Production still runs the pre-001A `analyze-paper` v26 and `suggest-paper-organization` v10, unchanged in version and bundle hash across the migration. Once authorized, the deploy must cover both generation functions together (§6.6a). No Anthropic/OpenAI secret is installed, and no catalog row names either provider. **PR #280 was still open and unmerged when this was recorded (2026-09-13).**
