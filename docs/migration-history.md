...
## 2026-09-25 — DB-JUNCTION-DML-GRANT-HARDENING-001: assignment junctions become SELECT-only for the browser (`20260925134526`) — **prepared; NOT applied to Production**

Follow-up to C38; decision **C48**. Repository change only — nothing here has been applied, and the rollout is migration-only and separately authorized ([deployment.md](deployment.md) §6.9).

| | `paper_projects` / `paper_tags` (`authenticated`) | `projects` / `tags` (`authenticated`) |
|---|---|---|
| **Repository target** (this migration) | `SELECT` | `SELECT, INSERT, UPDATE, DELETE` — **unchanged** |
| **Production today** (read-only, 2026-09-25; ledger **87**, latest `20260924193915`) | `SELECT, INSERT, DELETE` (`authenticated=ard/postgres`) | `SELECT, INSERT, UPDATE, DELETE` (`authenticated=arwd/postgres`) |
...
- **Nothing applied.** No Production DDL/DML, no Edge or frontend deploy. Production continues to grant `SELECT, INSERT, DELETE` on the junctions until the §6.9 rollout.
