...
- **Only the model component of the provider URL changes.** Prompts, request bodies, `responseMimeType`, response parsing, extraction schemas, the Projects/Tags suggestion contract, quota consumption, refund semantics and provider-error classification are all untouched, and `_shared/geminiTransport.ts` is **byte-identical** — the temporary 90-second / zero-retry diagnostic policy (PR #265) is unchanged in both direction and value. Both models are served by the **same existing `GEMINI_API_KEY`**, sent the same way (`x-goog-api-key`); no key was added, rotated, split per model or per user, and `GEMINI_MODEL` was not changed. The Gemini URL is now assembled in exactly one place, from a resolver-produced object rather than a bare string, so "send this user's request to an arbitrary model" has no expressible form.
...
- **Transport untouched.** `_shared/geminiTransport.ts` is byte-identical: the temporary `AI-PROVIDER-90S-PROD-DIAGNOSTIC-001A` policy (90 s per attempt, zero retries) is neither resolved nor generalised, and 001A explicitly does **not** assert it is right for any future provider.
...
- **Transport.** Each adapter makes one attempt under its own 60-second ceiling, with no retry and no backoff. Neither calls or generalises `geminiTransport.ts`, whose temporary 90 s / zero-retry diagnostic policy is untouched. Failures cross as `http` (status only; the body is never read), `network`, `timeout`, `unreadable_response`, `empty` or `incomplete_response`. No body, header, request id, URL or key crosses.
...
- **Runtime.**
  ...
  - `analyze-paper` and `suggest-paper-organization` record exactly one event per provider call after the outcome is decided; quota, refunds, prompts, parsers, response bodies and the Gemini 90 s / zero-retry diagnostic are unchanged.
