...
## 2026-09-26 — DB-INVOKER-EXECUTE-HARDENING-001A: the five caller-scoped read RPCs become SECURITY INVOKER (`20260926152414`) — **PREPARED, not yet Production**

First bounded result of the `DB-INVOKER-EXECUTE-HARDENING-001` design audit; decision **C49**. **Not applied to Production.** The rollout is the pending migration-only step in [deployment.md](deployment.md) §6.10. Until it runs, Production keeps all five functions SECURITY DEFINER.

| | Five read RPCs | `public` SECURITY DEFINER | … executable by `authenticated` (advisor lint 0029) |
|---|---|---|---|
| **Production now** (read-only, 2026-09-26; ledger **88**, latest `20260925134526`) | SECURITY DEFINER | 40 | **32** |
| **After this migration** (local replay; expected in Production after §6.10) | SECURITY INVOKER | 35 | **27** |
...
- **Advisor.** Production today: **32** `authenticated_security_definer_function_executable` warnings. Expected after the rollout: **27** — 24 intentional functions plus the three later INVOKER candidates (`bulk_update_keywords`, `bulk_update_study_types`, `safe_bulk_insert_papers`). The CLI's bundled local advisor does not implement lint 0029, so the local figure is that lint's catalog predicate (definer ∧ `authenticated` EXECUTE in `public`) = **27**, which suite `003` pins as a set.
