...
- **Schema.**
  ...
  - `set_current_user_ai_model` is re-created with an additive `reasoning_reset` result. It re-validates the saved level against the new model in the same transaction, preserving a supported level and nulling an unsupported one. `clear_current_user_ai_model` keeps its body; only its comment changed.
...
- **Tests.**
  ...
  - **Database:** new pgTAP suite `016_ai_reasoning_policy.test.sql` (95 assertions, including grantee ALLOWLISTS that judge every role that can execute the staged setter, not just the named ones). Suites 003, 011 and 012 were updated deliberately (classification lists and pinned column/result sets), not loosened. A clean local replay from zero ran 17 suites and 1,646 assertions, **all green**, plus the framework-free 18/18 and the concurrency probes.
