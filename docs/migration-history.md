...
Adds **one** additive migration, `20260731162729_reconcile_data_api_grants.sql`, that makes the Data API's table/sequence GRANTs **reproducible on a fresh replay**. Opened as PR against `main` for independent review; **not merged, not applied to Production.**
...
- **Production not modified.** Read-only ledger comparison (remote `supabase_migrations.schema_migrations` = 68 rows, latest `20260726120000`) vs. local shows exactly **one** pending migration (`20260731162729`) and no remote-only migration; the CLI `--linked` dry-run itself was environment-blocked at the link step, so equivalence was established from the read-only ledger. No Production DDL/DML was executed. Grants are a strict **subset** of Production's existing ACLs, so applying this migration to Production later (separately authorized) does not broaden client access.
...
- **Deployment remains separately gated** (C24: apply to the linked project via the runbook, under its own authorization). This PR **unblocks but does not complete** `PFA-C03A1-L`; the local-stack foundation work stays uncommitted and preserved in its own workspace. **C03A2-L not begun; PFA-C03 not complete; C27–C29 unchanged.**
