...
- **A tombstone makes the exclusion a property of the table.** `trg_paper_attachments_block_cleanup_intent` refuses any `paper_attachments` INSERT whose `(user_id, file_path)` has a pending cleanup intent or a permanent cleanup tombstone. `authenticated` still holds INSERT on `paper_attachments` — the pre-migration client path needs it, and a browser tab loaded before the frontend deploy still uses it — so without this the exclusion would hold only for writes that came through finalization. It is named to sort before `trg_paper_attachments_check_storage_quota`, so a refusal costs no quota accounting.
