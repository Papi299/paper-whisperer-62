...
**Filtering semantics.** State gained `projectMatchMode`/`tagMatchMode` (`"any" | "all"`, default `"any"`), with guarded setters, in `useFilterState`. `"any"` is the OR-union shipped previously; `"all"` requires membership in **every** selected member. Each category still issues exactly **one** bounded junction query — now selecting the entity ID column too (`paper_id, project_id` / `paper_id, tag_id`) — and a new pure helper `resolveJunctionPaperIds(rows, selectedIds, mode)` in `src/lib/filterSets.ts` resolves the paper-ID set: Any unions matching papers (deduped); All groups rows by paper and includes a paper only when its count of **unique** matched selected entities equals the number of unique selected IDs, so duplicate junction rows cannot inflate the count and unselected rows are ignored defensively. React Query keys include the mode (`junctionQueryKey`), so Any and All never share a cache entry while order-insensitive selections still reuse one; the four-state (`null`/`undefined`/`[]`/resolved) contract is preserved. Emptying a category (clear or deselecting the last member, or a global Clear) resets that category's mode to `"any"`, upholding the "no selection ⇒ Any" invariant.
