...
| 4 | **Attachments privacy + storage-quota enforcement** — tighten bucket SELECT policy; `BEFORE INSERT` quota trigger; `AFTER INSERT/DELETE` usage triggers. | #2 (`user_entitlements` must exist for the trigger to read the quota). | **Unblocked.** Recommended next implementation PR (C14 launch blocker). |
