...
| **Cloudflare domain hygiene** for `paperlume.app` (C19). Confirm auto-renew is enabled. Confirm transfer-lock / registry-lock is enabled. Save the domain receipt + RDAP info in a private password manager (not in the repo). | Auto-renew prevents accidental domain loss at expiry. Transfer-lock prevents social-engineering domain hijacks. The receipt + RDAP info are needed for future Paddle KYB and any domain-recovery scenario. | **Pending owner verification in the Cloudflare dashboard.** Recommended within the first few days of purchase. |
| **DNS / hosting / email setup** on `paperlume.app` (C19). Connect Vercel custom domain `app.paperlume.app` (DNS-only Cloudflare records during initial connection); Google Workspace on `paperlume.app`; Resend on `auth.paperlume.app` with SPF / DKIM / DMARC; Supabase Auth Custom SMTP pointed at Resend; Supabase Auth `Site URL` + `Redirect URLs` updated to `https://app.paperlume.app`. | Required before paid beta — see [`deployment.md §8a`](deployment.md) for the full checklist. None of these are configured yet. | Pending owner setup. Sequence: domain hygiene first; marketing site + Vercel custom domain next; Google Workspace + Resend + Supabase Auth SMTP last. |
