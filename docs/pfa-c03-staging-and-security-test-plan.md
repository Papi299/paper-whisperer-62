...
>
> **Owner selection.** The owner selected **PFA-C03** on 2026-07-31. Its first bounded subtask, **PFA-C03A0** (contract + readiness audit), produced this contract; the next subtask, **PFA-C03A1-L** (local-stack seed/reset + env/guard foundation), has since been **implemented and is open for independent review** (not yet merged or accepted — see §7.3, §10, §14). This document still ships no project/workflow/framework of its own. **PFA-C03 is not complete.**
>
...
**Grant-parity prerequisite uncovered by C03A1-L — resolved (merged + applied 2026-07-31).** Implementing C03A1-L surfaced a tracked defect that had to land **before** the local six-spec run could pass: a fresh local `supabase db reset` does not reproduce the Data API table GRANTs the app needs (the tracked chain relied on Supabase's old auto-grant default, now moving to **opt-in Data API exposure**; Production still carries the broad legacy grants), so `authenticated` received `42501` and the app could not run on a clean replay. This was fixed by the **separate bounded migration PR `PFA-C03A1-L-GRANT-PARITY-001`** (one least-privilege `20260731162729_reconcile_data_api_grants.sql`; local-replay-proven), now **merged** to `main` (merge commit `1f00800e5c2f1e1c2d405aa0061ecc38c8738004`) and **applied to Production** via the tracked linked CLI — the ledger is aligned at **69 (Local = Remote), latest `20260731162729`**, and the Production application was an **effective no-op** (existing ACLs already broader; no privilege revoked, no `anon`/`PUBLIC` grant added). **The grant-parity prerequisite remains complete, and C03A1-L has now been implemented and locally validated** (`PFA-C03A1-L-LOCAL-STACK-FOUNDATION-RESUME-001`, resumed on finalized `main` `bf78406bb…`). The local lifecycle (`npm run test:e2e:local`) starts an **ephemeral local Supabase stack**, **replays all 69 tracked migrations**, seeds two local-only confirmed Auth users plus a deterministic fixture (120 primary + 5 secondary papers, ≥ 2 note-less newest rows, a disposable highest-`insert_order` paper), and proves RLS cross-user isolation with authenticated (non-service-role) clients. The **two-layer fail-closed guard is implemented** (Layer 1 pre-server in `playwright.config.ts` via Vite `loadEnv`; Layer 2 browser-observed in `e2e/global-setup.ts` before any credential), its pure logic is unit-tested (22 cases), and the **guard negative controls pass** (`npm run test:e2e:local:verify-guards`: the Layer 1 Production-target control fails closed before Vite starts with no secret leaked; the Layer 2 ordering check confirms the guard precedes credential entry). The **six selected read-only specs pass locally** (`auth`, `bulk-actions`, `eager-load`, `filters`, `paper-import`, `pools` — 32 tests green) and a **second reset/reseed cycle is deterministic** (fresh ephemeral stack, all 69 migrations replayed again, identical fixture counts and ownership-relative assertions, bounded `auth.spec` green, clean teardown). **No Production or cloud Supabase access occurred** (only local `supabase start` / `db reset --local` / `status` / `stop`; no `--linked` command, no remote query, no Edge Function deploy/invoke) and **no GitHub workflow was created**. **C03A1-L is implemented and open for independent review (not yet merged or accepted); C03A2-L begins only after C03A1-L review/merge/finalization; C03A2-L has not begun; and PFA-C03 remains incomplete.** D3–D5 remain unresolved.
...
| **C03A1-L** *(local-first / hybrid)* — **implemented; in review** | Local-stack seed/reset + env + guard contract (repo) | `playwright.config.ts`/`e2e/*` env wiring + guards + seed/reset scripts | none (local only) | none (local path needs no cloud action) | none for the local path (`TEST_USER_*` seeded locally) | **met:** local stack replays all 69 + seeds; both guard layers reject Production/remote; six read-only specs green; deterministic second reset/reseed | Prohibited | No |
...
- **D2 — Test-backend architecture (local-first / cloud-first / local-first + optional cloud). `[RESOLVED 2026-07-31 → local-first]`** *Why:* the substantive architecture choice; it selects the required Phase A branch (§10) and drives determinism, cost, secret surface, and which specs run first. *Recommended default:* **local-first**, adding cloud only when a wave demonstrably needs it. *Alternatives:* cloud-first; local-first + optional cloud. *Trade-off:* realism vs cost/complexity/secret surface. *Selects the Phase A branch:* local-first → `C03A1-L → C03A2-L`; cloud-first → `C03A1-C → C03A2-C`; hybrid → `C03A1-L → C03A2-L` first, optional `C03A1-C → C03A2-C` later. *Blocks:* the Phase A branch shape (local-first unblocks `C03A1-L` immediately without any cloud decision; cloud-first additionally needs D1). Phase B (`C03B1 → C03B2`) is shared and unaffected. **Owner outcome:** local-first selected; path `C03A1-L → C03A2-L`; cloud phases not selected. The Data API grant-parity prerequisite (`PFA-C03A1-L-GRANT-PARITY-001`, §7.3) is **complete — merged and applied to Production 2026-07-31**, and **C03A1-L has now been implemented and locally validated** (`…-RESUME-001`: local stack replays all 69 migrations, deterministic seed, two-layer fail-closed guards + negative controls, six read-only specs green, deterministic second reset/reseed). **C03A1-L is open for independent review — not yet merged or accepted; C03A2-L has not begun; PFA-C03 remains incomplete.**
...
**PFA-C03 is NOT complete.** The C03A0 contract shipped, and **C03A1-L is now implemented and open for independent review** (local-first foundation; not yet merged or accepted). The next gate is independent review and merge of C03A1-L, after which the separately authorized **C03A2-L** (non-required `E2E (local)` CI) may begin; **C03A2-L has not begun** and **no `E2E (local)`/`db-tests` workflow exists**. Phase B (`C03B1 → C03B2`) remains unstarted. No further implementation phase is authorized by this task.
