...
- **Status:** C03A2-L is **merged and accepted**; **Phase A is complete for the selected local-first path**; **Phase B has started — C03B1 is blocked by pre-existing DB-security defects, with a bounded remediation under review (§9.6); C03B2 is unstarted**; **D3 is resolved (2026-08-02, pgTAP); D4–D5 remain unresolved**; **PFA-C03 is incomplete.**
...
### 9.6 C03B1 status — blocked by pre-existing security defects; bounded remediation under review `[FACT]`
...
- **Independent-review expansion incorporated:**
  ...
  8. **anon EXECUTE on `consume_ai_quota`/`refund_ai_quota`:** flagged by review; on the **current** base these RPCs are already `{owner, authenticated}` (anon cannot execute) — `20260731162729_reconcile_data_api_grants` closed this earlier. The migration **re-asserts** their least-privilege ACL defensively (idempotent no-op).
  9. **Broader privileged-function ACL inventory:** a complete `public` `SECURITY DEFINER` inventory (20 functions: 17 directly-callable + 3 trigger-only) was taken and an explicit intended ACL matrix encoded.
- **Bounded remediation — implemented and under independent review; NOT merged, NOT applied to Production.** Task `PFA-C03B1-PREEXISTING-SECURITY-DEFECT-REMEDIATION-001` on branch `fix/pfa-c03b1-security-remediation`: one CLI-generated migration `20260802025704_harden_rpc_and_relational_ownership.sql` + one focused pgTAP file `supabase/tests/database/000_preexisting_security_regressions.test.sql` (plus these docs). It adds explicit NULL-auth guards to all affected user-ID RPCs, a bounded `search_path` on `search_papers`, least-privilege EXECUTE across the whole SECURITY DEFINER surface (directly-callable → `{owner, authenticated}`; trigger-only → owner-only), both-owner RLS for `paper_projects`/`paper_tags`, and attachment↔paper ownership RLS plus a defense-in-depth ownership check in `check_and_consume_storage_quota`. Verified across **two fresh 70-migration replays**: focused pgTAP **93/93**; the preserved 18-case legacy file passes (hash unchanged); application regression probes pass; standard `Validate` passes with `package-lock.json` byte-identical.
- **Documented residual (bounded follow-up, not fixed here):** the `set_paper_projects` / `set_paper_tags` / `bulk_set_paper_projects` / `bulk_set_paper_tags` setter RPCs validate **paper** ownership but not referenced **project/tag** ownership. This task's scope (Section J/K) is the **RLS policies**, tested via the direct-insert path; the new RLS `SELECT` policy already makes any setter-created cross-owner row **cross-tenant-invisible** (no disclosure). A separately authorized follow-up should add referenced-object ownership validation to those four RPC bodies.
- **State:** the remediation **PR is open for review and is not merged; Production is unchanged** (69 migrations, latest `20260731162729`; Edge Functions v10/v15/v3) and **remains vulnerable until a separately authorized merge + deploy task**. **C03B1 remains unimplemented; C03B2 is unstarted; Phase B is incomplete; D3 resolved, D4–D5 unresolved; PFA-C03 incomplete; C27–C29 unchanged.**
