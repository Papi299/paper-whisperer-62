...
> **Status: active contract — C03A1-L merged and accepted; C03A2-L implemented and in review.** This document is the durable implementation and decision contract for **PFA-C03 — Safe staging environment and automated security tests**. **C03A0** (this contract) is complete; **C03A1-L** (local-stack seed/reset + env/guard foundation) is **merged to `main` and independently accepted** (PR #173, merge `299b19a…` — see §7.3, §10, §14); **C03A2-L** (non-required local Playwright CI workflow, `E2E (local)` / job `e2e-local`) is now **implemented and open for independent review** in an unmerged PR (see §8.1, §10) — its workflow file `.github/workflows/e2e-local.yml` exists **only on the C03A2-L feature branch until merge**. **Phase A is not yet finalized; Phase B is unstarted; PFA-C03 is incomplete.** The required **`Validate / validate`** gate is **unchanged and remains the sole required merge gate**; `E2E (local)` is **non-required**. No **cloud** Supabase project, GitHub secret/variable/Environment, DB-test workflow, database-test framework, or cloud fixture exists — every such artifact named below remains a **recommendation or future artifact**; the merged deliverable is the repo-local C03A1-L lifecycle, and C03A2-L adds only the non-required local E2E workflow that reuses it.
>
...
**C03A2-L is implemented and open for independent review (unmerged).** It realizes the §8 local-first (Model A) contract as a single new workflow file `.github/workflows/e2e-local.yml`, present **only on the C03A2-L feature branch until merge**. It records:
...
- **CI acceptance:** three consecutive exact-head `E2E (local)` executions on fresh GitHub-hosted runners pass (each provisioning a fresh ephemeral stack and running guard verification + six-spec lifecycle + cleanup assertions); all three job-log scans contain **zero credential-value matches**; no green-run artifact is produced; and exact-head `Validate` remains green. Exact run IDs and attempt numbers are recorded in the PR description and the execution report (kept out of durable docs before merge finalization).
- **Status:** C03A2-L remains **unmerged and unaccepted**; **Phase A is not yet finalized**; **Phase B is unstarted**; **D3–D5 remain unresolved**; **PFA-C03 is incomplete.**
...
| **C03A2-L** *(local-first / hybrid)* — **implemented; in review** (PR open, unmerged) | Non-required **local** Playwright CI (`E2E (local)` / `e2e-local`) | new `.github/workflows/e2e-local.yml` (feature branch only) | CI only (ephemeral local stack) | approve running the workflow | none cloud (local anon key + seeded `TEST_USER_*`); no secret/variable/Environment | **met in review:** guard verification + exact six read-only specs run; three consecutive exact-head green attempts on fresh runners; fork PRs skip; `contents: read`; all actions SHA-pinned; cleanup + clean-state assertions enforced; zero credential-marker log matches; no credential artifact; `Validate` unchanged & required | Prohibited | No |
...
**PFA-C03 is NOT complete.** The C03A0 contract shipped, **C03A1-L is merged and independently accepted** (local-first foundation; PR #173, merge `299b19a…`), and **C03A2-L is now implemented and open for independent review** (non-required `E2E (local)` / `e2e-local` CI reusing the merged C03A1-L lifecycle — see §8.1). The `E2E (local)` workflow exists **only on the C03A2-L feature branch until merge**; it is **unmerged and unaccepted**, so **Phase A is not yet finalized**. The required `Validate / validate` gate is unchanged and remains the only required merge gate; the **`db-tests` workflow still does not exist**. Phase B (`C03B1 → C03B2`) remains unstarted and **D3–D5 remain unresolved**. The next gate is independent review, merge, and documentation finalization of C03A2-L; no further implementation phase is authorized by this task.
