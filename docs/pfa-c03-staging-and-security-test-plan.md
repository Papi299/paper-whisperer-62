...
Resuming from `main` at the PR #178 merge commit `4bfe689e587c7a562cb43720678f33fc3c7536b0` (which finalized the §9.6 security-remediation documentation), the C03B1 local database-security architecture is now **implemented on the feature branch `test/pfa-c03b1-db-security-resumed`** and is **review-ready but not merged**. Scope: **eight files, three commits** (`test: add C03B1 database security suites` → `docs: record C03B1 implementation` → `test: harden C03B1 detector guarantees`); **no migration, no workflow, no dependency, no lockfile change; no Production mutation**. D3 remains resolved to **pgTAP**.
...
**Catalog-residue proof** — before the negative control and again after all tests + concurrency cleanup, the runner captures a deterministic fingerprint (per-category md5 over stably-ordered rows) of: public function signatures (identity args, owner, `SECURITY DEFINER`, `proconfig`, ACL); public policies (schema, table, name, command, roles, USING, WITH CHECK); public-table RLS flags (`relrowsecurity`, `relforcerowsecurity`); non-internal triggers on public tables; and persistent relations in the tested schemas. Final equality on a fresh connection proves **no persistent function, policy, trigger, relation, RLS-flag, extension, helper, fixture, or advisory lock changed**, alongside the retained zero-row and zero-advisory-lock checks.

**True-concurrency AI-quota probe (bounded + fail-closed)** — a committed fixture user sits one unit below a cap of 2; a coordinator session holds an EXCLUSIVE advisory lock while two worker sessions block requesting the SHARED lock; once both are confirmed waiting on `pg_locks`, the coordinator releases so both call `consume_ai_quota` concurrently. Every coordinator/worker child process is now tracked, bounded by an explicit timeout (coordinator acquire 15 s / barrier 30 s / worker exit 30 s / coordinator exit 15 s), and **must exit 0 without signal termination** — output from a nonzero or killed process is never accepted; the coordinator must additionally report a successful acquire and unlock. Required result: **exactly one `ok` + one `quota_exceeded`; final usage = cap; exactly one unit consumed; one counter row (no duplicate)**. After all three sessions terminate, a fresh connection proves **zero barrier locks and zero ungranted waiters** before the fixture is removed; fixture absence (user, entitlement, counter, storage, access, advisory lock) is then re-proven on another fresh connection. On any timeout/error the remaining processes are killed within a bounded window and the error is re-thrown so the mandatory teardown still runs.

**Evidence (local):** two complete `npm run test:db:local` runs both passed — negative control detected the injected papers-RLS regression (nonzero + marker) and its rollback + baseline restoration were verified; pgTAP **394/394** each (139 + 40 + 26 + 164 + 25), legacy **18/18**; coordinator and both workers exited 0 within their timeouts with one allowed + one quota_exceeded, usage = cap, one counter row, no barrier locks; catalog fingerprint identical before and after; row/advisory residue zero; mandatory current-project teardown. The `E2E_KEEP_LOCAL_STACK=1` escape hatch was verified unable to bypass db-tests cleanup. The existing E2E lifecycle still passes (`verify-guards`: **32** guard tests, Layer 1 fail-closed, Layer 2 ordering; `test:e2e:local`: 70 migrations, 120/5 seed + isolation, six specs / **32** Playwright tests, clean teardown; `stop`). Credential scans of every retained log are **zero**. Standard validation: `npm ci` lockfile byte-identical, lint 0 errors, typecheck 0, Vitest all pass, build succeeds.
