...
**Grant-parity prerequisite uncovered by C03A1-L.** Implementing C03A1-L surfaced a tracked defect that must land **before** the local six-spec run can pass: a fresh local `supabase db reset` does not reproduce the Data API table GRANTs the app needs (the tracked chain relied on Supabase's old auto-grant default, now moving to **opt-in Data API exposure**; Production still carries the broad legacy grants), so `authenticated` receives `42501` and the app cannot run on a clean replay. This is addressed by a **separate bounded migration PR, `PFA-C03A1-L-GRANT-PARITY-001`** (one least-privilege `20260731162729_reconcile_data_api_grants.sql`; local-replay-proven; Production not modified). **C03A1-L itself remains uncommitted, preserved, and incomplete** until that grant PR merges and is finalized, after which C03A1-L resumes and must pass its six-spec and reset/reseed idempotence proofs. **No GitHub E2E workflow exists yet; C03A2-L has not begun; PFA-C03 is not complete.**
