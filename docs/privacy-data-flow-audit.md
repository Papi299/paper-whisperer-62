...
> - **Manual reasoning remains disabled in Production**: `reasoning_selectable` is false on all six catalog rows and `set_current_user_ai_reasoning` is granted to nobody. `AI-MANUAL-REASONING-001` (C45) prepares the migration that opens both; it is **not applied**, and when it is, it introduces **no new public data category** — see the addendum below.
>
