...
**Addendum (2026-09-10, DATA-API-ACL-RECONCILIATION-001 / C38).** The rows above describe Production's live state and are unchanged. Repository migration `20260910212202_reconcile_data_api_acls.sql` — **implemented and not yet applied to Production** — extends the `internal_user_access` treatment named in the "Server-only tables" row to every client role on every table: `PUBLIC` and `anon` are reduced to no privilege on all 28 ordinary `public` tables and the one sequence, `authenticated` to an exact per-table matrix, and the other three server-only tables (`subscriptions`, `subscription_events`, `usage_counters`) lose the client-role privileges they still carry from the platform default. This is a least-privilege and defense-in-depth change, **not** a response to demonstrated exposure: every policy on those tables requires `auth.uid() = user_id`, no anonymous row access was shown, and the Data API exposes no path to the non-DML privileges (`TRUNCATE`, `TRIGGER`, `MAINTAIN`) that RLS does not cover. Nothing about data collection, retention, processors or user-facing flows changes. When the rollout is authorized, the "Server-only tables" row becomes true of all four tables rather than one.
