...
| Any signed-in client could INSERT, UPDATE or DELETE its own `paper_attachments` rows directly | Browser roles hold `SELECT` only. `INSERT`, `UPDATE`, `DELETE` and `TRUNCATE` are revoked from `authenticated` and `anon`, and `PUBLIC` holds nothing, so attachment metadata is created and destroyed only by the three lifecycle RPCs, the cascade they initiate, and account deletion. The migration takes an `ACCESS EXCLUSIVE` lock on the table before making that change, so no write authorized under the old privileges can commit under the new posture |
