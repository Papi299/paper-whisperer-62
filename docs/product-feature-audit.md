...
- **Recommended top three (owner-gated):** **PFA-C01** AI-quota UX completion (recommended), **PFA-C02** account-level data export (alternative), **PFA-C03** isolated staging + CI E2E/DB tests (strategic alternative).
...
- **Status taxonomy:** `Shipped — complete`, `Shipped — partial/fragile`, `Infrastructure only`, `Planning only`, `Dormant/orphaned`, `Unknown — manual verification required`. **Confidence:** `High` / `Medium` / `Low`.
...
|---|---|---|---|---|---|
| `/` (`Index`) | Public | Redirect to `/dashboard` or `/auth` | `src/pages/Index.tsx` uses `useAuth` | Shipped — complete | High |
| `/auth` (`Auth`) | Public | Sign in / sign up / forgot-password | `src/pages/Auth.tsx`, `supabase.auth.*`, zod validation | Shipped — complete | High |
| `/reset-password` | Public (recovery link) | Set new password | `src/pages/ResetPassword.tsx`, `updateUser` | Shipped — complete | Medium |
| `/dashboard` (`Dashboard`) | Guarded redirect | Full app: list, filter, import, edit, bulk, dedup, AI | `src/pages/Dashboard.tsx` + ~15 hooks | Shipped — complete | High |
| `*` (`NotFound`) | Public | 404 fallback (logs to `console.error`) | `src/pages/NotFound.tsx` | Shipped — complete | High |
| Add Papers dialog | Guarded | Identifier / file / manual ingestion + assignment | `AddPaperDialog.tsx` → `useBulkMutations` | Shipped — complete | High |
| Edit Paper dialog | Guarded | Edit metadata, notes, attachments, abstract | `EditPaperDialog.tsx` → `usePaperMutations`, `useAttachments` | Shipped — complete | High |
| Find Duplicates dialog | Guarded | Scan + merge PMID/DOI duplicates | `DeduplicationDialog.tsx` → `useDeduplication` | Shipped — complete | High |
| Sidebar manage modals (Projects, Tags, Keywords, Study Types, Synonyms, Exclusions) | Guarded | Taxonomy + pool CRUD | `Sidebar.tsx` + `PoolsContext` + per-pool hooks | Shipped — complete | High |
| Settings dialog | Guarded | Set/clear PubMed API key | `SettingsDialog.tsx` → `useSettings` | Shipped — complete | High |
| Analytics panel | Guarded | Aggregate stats over filtered set | `AnalyticsPanel.tsx` → `useAnalyticsData` | Shipped — complete | Medium |
| Sign-out | Guarded | End session | Sidebar dropdown → `useAuth().signOut` | Shipped — complete | High |

## 4. Capability matrix

| ID | Domain | Capability | Status | Conf. | User-facing evidence | Data/backend evidence | Test evidence | Material gap |
|---|---|---|---|---|---|---|---|---|
| CAP-AUTH-01 | Auth | Email/password sign-in/up, session restore, sign-out | Shipped — complete | High | `Auth.tsx`, `useAuth.ts`, `Sidebar.tsx` | Supabase Auth, `handle_new_user` trigger | `e2e/auth.spec.ts` | Leaked-password protection disabled (advisor) |
| CAP-AUTH-02 | Auth | Password reset (email → set new) | Shipped — complete | Medium | `Auth.tsx` forgot flow, `ResetPassword.tsx` | `resetPasswordForEmail`, `updateUser` | none direct | Email delivery is runtime-only |
| CAP-ONBOARD-01 | Onboarding | Empty-library first action | Shipped — partial | Medium | `PaperList.tsx:361` "No papers yet…" | n/a | none | Doesn't mention file import / taxonomy (PFA-F005) |
| CAP-IMPORT-01 | Ingestion | PMID/DOI identifier batch import | Shipped — complete | High | `AddPaperDialog.tsx` | `fetch-paper-metadata`, `bulk_insert` RPC | `e2e/paper-import.spec.ts` | — |
| CAP-IMPORT-02 | Ingestion | Title-based import (auto first match) | Shipped — complete | High | `AddPaperDialog.tsx:568` static warning | `fetch-paper-metadata` | `importParsers.test.ts` | No per-paper confirm — accepted by decision |
| CAP-IMPORT-03 | Ingestion | File import BibTeX/RIS/NBIB/ENW/CSV/TSV | Shipped — complete | High | `AddPaperDialog.tsx` dropzone | `importParsers.ts`, `bulk_insert` | `importParsers.test.ts`, `e2e/file-import-order.spec.ts` | — |
| CAP-IMPORT-04 | Ingestion | Manual paper creation | Shipped — complete | High | `AddPaperDialog.tsx` manual tab | `usePaperMutations.addPaperManually` | `AddPaperDialog.test.tsx` | — |
| CAP-IMPORT-05 | Ingestion | Import continuation + assignment loop | Shipped — complete | High | `AddPaperDialog.tsx` (PR #164) | junction RPCs | `useBulkMutations-assignment.test.ts` | — |
| CAP-LIB-01 | Library | Server-side list, pagination, lazy load, sort | Shipped — complete | High | `PaperList.tsx`, `Dashboard.tsx` | `buildPapersQuery.ts`, pagination indexes | `e2e/eager-load.spec.ts`, `filters.spec.ts` | Large-library thresholds unverified at scale |
| CAP-LIB-02 | Library | Filters (year, study type, notes, keyword, project, tag) | Shipped — complete | High | `SearchFilters.tsx`, `useFilterState.ts` | filter RPCs, junction pagination | `e2e/filters.spec.ts` | — |
| CAP-LIB-03 | Library | Project/Tag Any/All match modes | Shipped — complete | High | `useFilterState.ts` | `resolveJunctionPaperIds` | `useFilterState.test.tsx` | — |
| CAP-SEARCH-01 | Search | Phrase / FTS / short search + "Matched in:" attribution | Shipped — complete | High | `useFilterState.ts`, `PaperList.tsx` | `search_papers`, `search_papers_short` RPCs | `e2e/search-attribution.spec.ts` | Function `search_path` mutable (advisor) |
| CAP-PROJ-01 | Projects | Create/edit/delete + assignment + filtering | Shipped — complete | High | `ManageProjectsModal.tsx`, `EditProjectDialog.tsx` | `useProjectMutations`, FK cascade | `projectTagMutations-cache.test.ts` | — |
| CAP-TAG-01 | Tags | Full tag lifecycle (mirror of projects) | Shipped — complete | High | `ManageTagsModal.tsx`, `EditTagDialog.tsx` | `useTagMutations`, FK cascade | `projectTagMutations-cache.test.ts` | — |
| CAP-PRESET-01 | Presets | Saved searches (create/load/update/rename/delete, dirty state) | Shipped — complete | High | `FilterPresetsMenu.tsx`, `Dashboard.tsx` | `filter_presets` table, payload v3 + v1/v2 reads | `useFilterPresets.test.ts`, `e2e/filter-presets.spec.ts` | — |
| CAP-DETAIL-01 | Detail | View/edit metadata, on-demand abstract, keywords, study type | Shipped — complete | High | `EditPaperDialog.tsx`, `useAbstract.ts` | `papers` table, RLS | `e2e/mutations.spec.ts` | — |
| CAP-POOLS-01 | Curation | Keyword / synonym / study-type / exclusion pools | Shipped — complete | High | Sidebar modals, `PoolsContext.tsx` | pool tables, FK cascade, re-eval hooks | `e2e/pools.spec.ts` | Gating flag `premium_taxonomy_enabled` unenforced (infra-only) |
| CAP-NOTES-01 | Notes | Per-paper notes, indexed into search | Shipped — complete | High | `EditPaperDialog.tsx` | `notes` column + search RPC | `e2e/notes.spec.ts` | — |
| CAP-ATTACH-01 | Attachments | Upload/list/signed-URL view/delete, size+MIME limits | Shipped — complete | High | `EditPaperDialog.tsx`, `useAttachments.ts` | private bucket, owner policies, storage-quota triggers | `e2e/attachments.spec.ts` | Over-quota surfaces raw DB error; no usage gauge (PFA-F004) |
| CAP-AI-01 | AI | Single + bulk AI analysis (TL;DR, study type, stats) | Shipped — partial | High | `usePaperAnalysisActions.ts`, `PaperList.tsx` | `analyze-paper`, `consume/refund_ai_quota` | `usePaperAnalysisActions.test.ts` | 402 not surfaced; no usage/plan display (PFA-F001) |
| CAP-DEDUP-01 | Dedup | PMID/DOI detection, transitive merge, keep-suggestion | Shipped — complete | High | `DeduplicationDialog.tsx`, `useDeduplication.ts` | `get_duplicate_papers`, `merge_exact_duplicates` | `mergeOverlappingGroups.test.ts`, `parseDuplicateGroups.test.ts` | PMID/DOI-only by decision |
| CAP-BULK-01 | Bulk | Select-all across filtered set, bulk project/tag/delete/analyze | Shipped — complete | High | `BulkActionsToolbar.tsx`, `useBulkSelection.ts` | `allFilteredIds`, atomic junction RPCs | `e2e/bulk-actions.spec.ts` | — |
| CAP-EXPORT-01 | Export | CSV / RIS / BibTeX, chunked large-library pipeline | Shipped — complete | High | `SearchFilters.tsx`, `useExportPapers.ts` | `exportUtils.ts`, `fetchAllPages` | `largeExportPipeline.test.ts` | — |
| CAP-ANALYTICS-01 | Analytics | Aggregate stats over filtered set | Shipped — complete | Medium | `AnalyticsPanel.tsx` | `useAnalyticsData.ts` | none direct | Weak automated coverage |
| CAP-SETTINGS-01 | Settings | Per-user PubMed API key | Shipped — complete | High | `SettingsDialog.tsx`, `useSettings.ts` | `profiles.pubmed_api_key`, used by metadata fn | `useSettings.test.ts` | — |
...
| CAP-COMM-02 | Commercial | AI quota enforcement (server) | Shipped — complete (backend) | High | Edge fn only | `consume/refund_ai_quota` live | `usePaperAnalysisActions.test.ts` (client path) | No client UX (see CAP-AI-01) |
| CAP-COMM-03 | Commercial | Storage quota enforcement (server) | Shipped — complete (backend) | High | Edge/DB only | `check_and_consume_storage_quota` trigger | none (no DB tests) | No usage UX (PFA-F004) |
...
| CAP-A11Y-01 | A11y/Responsive | Keyboard/focus/labels; narrow-screen layout | Unknown — manual verification | Low | Radix primitives, form labels present | n/a | none | Desktop-first shell; runtime-only (PFA-F008, §15) |
...
|---|---|---|
| 7.1 Authentication & account entry | Shipped — complete | sign-in/up/reset/sign-out all wired; no profile page beyond API key |
| 7.2 Initial library onboarding | Shipped — partial | valid first action; thin zero-state (PFA-F005) |
| 7.3 Paper ingestion | Shipped — complete | identifier + title + BibTeX/RIS/NBIB/CSV + manual + continuation |
| 7.4 Library & read path | Shipped — complete | server filter/sort/paginate/lazy-load; scale unproven |
| 7.5 Search | Shipped — complete | phrase/FTS/short + "Matched in:" attribution |
| 7.6 Projects | Shipped — complete | full CRUD + assignment + Any/All |
| 7.7 Tags | Shipped — complete | mirror of projects |
| 7.8 Saved filters / presets | Shipped — complete | v3 payload, v1/v2 back-compat, dirty state, stale-ref guard |
| 7.9 Paper detail & editing | Shipped — complete | metadata/notes/abstract/attachments |
| 7.10 Curation pools | Shipped — complete | keywords/synonyms/study-types/exclusions; gating flag unenforced (PFA-F007) |
| 7.11 Notes | Shipped — complete | CRUD + search-indexed |
| 7.12 Attachments | Shipped — partial | upload/view/delete; no usage gauge (PFA-F004) |
| 7.13 AI analysis & classification | Shipped — partial | live path; 402 UX gap (PFA-F001) |
| 7.14 Duplicate handling | Shipped — complete | PMID/DOI-only by decision; transitive merge |
| 7.15 Bulk operations | Shipped — complete | select-all across filtered set |
| 7.16 Export | Shipped — complete | CSV/RIS/BibTeX, chunked |
| 7.17 Responsive & accessibility | Unknown — manual | desktop-first; runtime-only (PFA-F008, §15) |
| 7.18 Error/loading/empty states | Shipped — complete | consistent toasts; no isolated-env test (PFA-F006) |
| 7.19 Account lifecycle & portability | Planning only | no deletion/export (PFA-F003) |
| 7.20 Commercial foundations | Infrastructure only / backend-live | enforcement live, no UX; C27 paused (§14) |
...
- **Severity:** Medium · **Classification:** Shipped — partial/fragile · **Confidence:** High · **Affected user:** every user, especially Free (15-call lifetime cap).
...
- **Severity:** Low · **Classification:** Dormant/orphaned · **Confidence:** High.
...
- **Expected complete behavior:** a Settings surface to export the full library (papers, notes, projects, tags, presets) and to delete the account with its data.
...
- **Severity:** Low · **Classification:** Shipped — partial/fragile · **Confidence:** High.
...
- **Severity:** Low · **Classification:** Shipped — partial · **Confidence:** Medium.
...
- **Severity:** Medium · **Classification:** (test-risk) · **Confidence:** High.
...
- **Severity:** Low · **Classification:** (security-hardening, already recorded) · **Confidence:** Low (advisors recorded 2026-07-17; not re-verified here — no Production access).
...
| PFA-C02 | Product feature | No way to export/back up the full account | PFA-F003; `SettingsDialog.tsx` | Settings action that fetches the full library (papers, notes, projects, tags, presets) and downloads a single JSON/ZIP | reuses existing read hooks | Medium | Low–Medium |
| PFA-C03 | Test-environment enablement | RLS/quota/E2E unverifiable in CI; Playwright hits Production | PFA-F006; `validate.yml` | Provision an isolated staging Supabase project; wire Playwright (+ optional pgTAP) against it in a **non-required** CI job | owner decision on staging project | Epic — must be phased | Medium |
...
Commercial/launch work (C27) is excluded. Ties are broken toward higher evidence confidence and lower dependency risk.
...
| 2 | PFA-C02 | Product feature | 4 | 1 | 3 | 2 | 3 | 3 | 1 | **23** | Real data-ownership value; composable from existing reads; low dependency |
| 3 | PFA-C03 | Test-env enablement | 3 | 2 | 3 | 5 | 3 | 5 | 2 | **23** | Highest strategic leverage (unlocks safe RLS/quota/E2E testing) but owner-gated + large |
| 4 | PFA-C05 | Product enablement | 2 | 2 | 2 | 2 | 3 | 1 | 0 | **21** | Cheap, self-contained; complements PFA-C01 |
...
| 7 | PFA-C08 | Remediation | 2 | 0 | 2 | 2 | 2 | 2 | 1 | **13** | Defense-in-depth; not currently exploitable |
| 8 | PFA-C09 | UX/accessibility | 2 | 1 | 2 | 1 | 2 | 3 | 1 | **11** | Overlaps deferred mobile (C7); desktop-first tool |
| 9 | PFA-C07 | Remediation | 1 | 0 | 1 | 1 | 3 | 1 | 0 | **9** | Pure hygiene; trivial |

## 13. Recommended owner shortlist

Exactly three options. Option 1 is the audit recommendation; **the final choice is owner-gated** and no option was selected or implemented.
...
- **Dependencies:** none.
- **Why top three:** highest score; fixes a live, user-facing, high-confidence gap at minimal cost and risk.

### Option 2 — Alternative

- **Candidate:** PFA-C02 — Account-level "export all my data."
- **Target user:** any user wanting a backup/portability of their whole library.
- **Exact problem:** the only export is per-selection citation formats; there is no full-account export.
- **Current repository evidence:** `SettingsDialog.tsx` exposes only the PubMed key; README lists account-level export as not implemented; full-library reads already exist (`useExportPapers`, `usePapers`, presets/pools hooks).
- **Bounded MVP scope:** a Settings action that assembles papers + notes + projects + tags + presets into one downloadable JSON (or ZIP) client-side.
- **Non-goals:** account deletion (separate, PFA-C04); scheduled/automated backups; re-import.
- **Acceptance criteria:** the file contains every owned paper and its taxonomy/notes/presets; empty account yields a valid empty export; large libraries stream via the existing chunked/paged pattern.
- **Likely files/subsystems:** `SettingsDialog.tsx`, a new `useAccountExport` hook reusing existing read paths, `exportUtils.ts`.
- **Schema/RPC/Edge expectation:** none (client composition of existing reads).
- **Testing strategy:** Vitest for the assembler/serializer over fixtures.
- **Key risks:** low–medium — completeness across tables; memory on very large libraries (mitigated by paging).
- **Dependencies:** none. **Why top three:** clear data-ownership value with low dependency and strong evidence.

### Option 3 — Strategic alternative

- **Candidate:** PFA-C03 — Isolated staging Supabase project + CI E2E/DB tests.
- **Target user:** indirectly all users (quality/safety); directly the maintainer.
- **Exact problem:** Playwright runs against **production** and is excluded from required CI; RLS/S1/quota atomicity have no automated verification.
- **Current repository evidence:** `start-here.md` testing baseline; `validate.yml` (no Playwright/DB); no `supabase/tests`.
- **Bounded MVP scope (phased):** provision a staging Supabase project mirroring migrations; point Playwright at it via `.env`; add a **non-required** CI job; optionally seed pgTAP for RLS/quota. Keep required CI unchanged (no mutating Production tests).
- **Non-goals:** making E2E a required merge gate on day one; touching production data.
- **Acceptance criteria:** Playwright runs green against staging in a non-required job; documented setup; required `validate` unchanged.
- **Likely files/subsystems:** `playwright.config.ts`, `e2e/global-setup.ts`, `.github/workflows/*`, `supabase/` seed/tests, `deployment.md`.
- **Schema/RPC/Edge expectation:** no product schema change; staging provisioning + optional pgTAP.
- **Testing strategy:** the deliverable **is** test enablement.
- **Key risks:** medium — owner must approve/provision staging; cost/secrets management.
- **Dependencies:** owner decision on a staging project. **Why top three:** highest strategic leverage — de-risks every subsequent feature.
