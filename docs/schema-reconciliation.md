...
- **RLS remains unchanged and authoritative** for row-level isolation; **no schema shape, policy, function, ownership, generated type, or user data changed**; Production was not modified (the migration is one pending migration, applied separately under its own authorization). Verified on a clean local replay (twice): authenticated own-user reads/writes succeed, cross-user access stays RLS-blocked, and server-only tables stay `42501` for client roles.
