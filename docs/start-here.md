...
- **Schema drift (reconciliation nearly complete):** production predates the first tracked migration, so a clean replay of the tracked migrations produces a schema that materially differs from production even though the migration ledger fully matches. **RECON-JUNCTIONS-001, RECON-STATISTICAL-METHODS-001, RECON-INTEGRITY-001, and RECON-LEGACY-COLUMNS-001 are complete** — merged, applied remotely, and verified; the aligned ledger holds **64** migrations (last `20260719060025`). The **final** reconciliation task, **`RECON-METADATA-PARITY-001`** — drop `projects.updated_at` (+ its trigger), remove the duplicate `papers` updated-at trigger, converge the eight drifted `created_at` defaults to `now()`, enforce `study_type_pool.created_at` NOT NULL, converge `tags.color` default to `'#e2e8f0'`, and drop seven redundant single-column indexes superseded by production's covering composite/unique indexes — is prepared as a **local-only, not-yet-deployed** migration open for review; it is applied remotely in a later C24 task. `papers.search_vector` (proven semantically equivalent) and the SEC-4 default-grant diff are approved benign/artifact exclusions. Generated Supabase types stay deferred (C25) until this migration is deployed and exact parity is verified; reconciliation precedes the TypeScript baseline and CI. Full inventory, owner decisions (C20–C25), and roadmap: [schema-reconciliation.md](schema-reconciliation.md). The audit confirmed RLS policies, security RPCs, and all commercial tables are **in parity** — enforcement is not broken.
...
The four applied reconciliation migrations (**RECON-JUNCTIONS-001, RECON-STATISTICAL-METHODS-001, RECON-INTEGRITY-001, RECON-LEGACY-COLUMNS-001**) are done (merged, applied remotely, verified; **64** aligned ledger rows). **RECON-METADATA-PARITY-001** — the final metadata/index parity migration — is prepared and open for review as a **local-only** change (unapplied remotely). After it is merged and applied remotely (C24) and exact schema/type parity is verified, resume the TypeScript baseline and then establish CI and branch protection. The full ordered sequence and per-migration rules are in [schema-reconciliation.md](schema-reconciliation.md) (decisions C20–C25).
...
1. RECON-METADATA-PARITY-001 (2026-07-19, PR #156): the final metadata/index parity migration (`20260719162013_reconcile_metadata_parity.sql`) prepared and opened for review — **local-only, unapplied remotely** (deployed in a later C24 task). Converges `projects.updated_at` removal, the duplicate `papers` updated-at trigger, `created_at` → `now()`, `study_type_pool.created_at` NOT NULL, `tags.color` default, and seven redundant indexes; `search_vector`/SEC-4 grants classified benign/artifact. Hardened per review (RECON-METADATA-PARITY-001A) with complete state-specific metadata manifests, a proactive `projects.updated_at` dependency inventory, an expanded preservation inventory, a full routine-inventory gate (owner/ACL/volatility/secdef/config), and read-only S2 validation against live production.
2. RECON-LEGACY-COLUMNS-001 completed end-to-end (2026-07-19): the three empty production-only legacy columns (`papers.urls`, `synonym_pool.primary_term`, `synonym_pool.variants`) dropped per C21 (PR #155), applied remotely and verified; aligned ledger now **64** migrations.
3. RECON-INTEGRITY-001 completed end-to-end (2026-07-19): twelve NOT NULL constraints enforced and the `synonym_pool.synonyms` empty-array default restored per amended C23 (PR #154), applied remotely and verified.
4. RECON-STATISTICAL-METHODS-001 completed end-to-end (2026-07-18): `papers.statistical_methods` converged to canonical JSON-string storage (PR #153), applied remotely and verified.
5. RECON-JUNCTIONS-001 completed end-to-end (2026-07-18): junction tables converged to composite PKs, migration applied remotely, exact junction parity verified (PR #152).
