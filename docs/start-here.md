...
- **PROJECT-TAG-SELECTOR-UX-001 — completed baseline (PR #162, merge `dedde4d0`).** The dashboard Project and Tag filters are searchable multi-select with long-name overflow fixed; saved presets are payload **version 2** with backward-compatible version-1 reads. This is the current baseline the match-mode task below builds on.

1. **PROJECT-TAG-MATCH-MODE-001** — add an independent **Any/All** match mode to each of the Project and Tag filters (Any = OR-union within the category, All = AND-by-membership within the category; Projects and Tags stay ANDed with each other and with every other filter — there is **no** global cross-category OR). Default is **Any**, which preserves PROJECT-TAG-SELECTOR-UX-001 behavior and the meaning of every preset saved before modes existed; saved presets upgrade to payload **version 3** with backward-compatible version-1/version-2 reads (both normalized to Any/Any), **no automatic DB rewrite, no schema migration**. This work is implemented in a pull request on branch `feat/project-tag-match-modes`. **If you are reading this from the `feat/project-tag-match-modes` branch, that PR is still under independent review — the immediate gate is its approval and merge; do not treat the task as done and do not start the next task yet. Once this document is present on `main` (i.e. the PR has merged), treat PROJECT-TAG-MATCH-MODE-001 as completed, and the immediate next engineering task is `IMPORT-CONTINUATION-WORKFLOW-001`.**
2. **IMPORT-CONTINUATION-WORKFLOW-001** — the next active task once PROJECT-TAG-MATCH-MODE-001 is on `main`: fix the Add Papers flow where the Import Results Summary replaces/hides the Project/Tag assignment controls (separate task; **not** part of #1).
3. **PRODUCT-FEATURE-AUDIT-001** — the focused product-feature and incomplete-workflow audit and prioritization exercise. **Deferred, not cancelled**, until the remediation tasks above are complete; do not select or implement a feature before that audit.
4. Owner selection of the first new feature from that audit.
