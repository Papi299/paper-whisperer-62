...
- **Post-deletion focus continuity in the paper table.** Dismissing a confirmation now restores focus correctly everywhere (§3), but *confirming* a paper deletion unmounts the row — and its Delete button — before close-focus runs, so focus still falls to `<body>`. Restoration cannot answer this: the correct target is a post-deletion design decision (next row's action, or the table with a status announcement), not an opener to return to. Deliberately not invented under the restoration work.
