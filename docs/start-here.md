...
1. **PRODUCT-FEATURE-AUDIT-001** — a repository-wide, evidence-based product-feature and incomplete-workflow audit that produces a prioritized backlog and a bounded owner decision packet (three shortlist options). It is a documentation-and-prioritization task; it does **not** select or implement a feature. Deliverable: [product-feature-audit.md](product-feature-audit.md). **If you are reading this from the `docs/product-feature-audit` branch, that audit PR is still under independent review — the audit is complete but awaiting approval and merge. Owner feature selection is not yet unlocked; no feature has been selected or started. Once this document and the audit are present on `main` (i.e. the PR has merged), treat PRODUCT-FEATURE-AUDIT-001 as completed.**
2. **Owner selection** of one candidate from the audit shortlist (or authorization of a blocking remediation) — the immediate next gate once the audit is merged. Implementation remains **unstarted** until the owner chooses; no candidate may begin without a new explicit task prompt.

The audit's blocking-remediation gate is **Outcome A**: no Critical or High remediation blocks feature selection.
