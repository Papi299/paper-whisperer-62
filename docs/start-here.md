...
- **Backend:** Supabase — Postgres 17, Auth (JWT, localStorage-persisted sessions), Storage, PostgREST, and Edge Functions (see §5 — four deployed, one implemented and awaiting its rollout).
...
- **In-app PubMed discovery (PUBMED-IN-APP-SEARCH-001) — implemented; NOT yet rolled out.** Add Papers gained a fourth mode, **PubMed Search**: a query goes to the new read-only `search-pubmed` Edge Function (ESearch → ESummary), results are paged 20 at a time, and the user multi-selects records and imports them. The load-bearing boundary is that **discovery finds PMIDs and the existing importer imports them**: the selected PMID strings go into the exact `onBulkImport` callback the Import IDs tab uses, so there is no second normalization, no second insert payload and no second duplicate check, and no ESummary field is ever persisted. Search is explicit (never on keystroke), the query reaches PubMed unrewritten, a stale response cannot repopulate a newer or reset session, the four modes share **one** `selectedProjectIds`/`selectedTagIds` intent, and the whole discovery session is ephemeral — nothing is persisted to storage, URL or profile. Requires the `search-pubmed` function to be deployed **before** the frontend merges; see [deployment.md](deployment.md) §7b.
...
- **Edge Functions — four of the five in `supabase/functions/` are deployed and ACTIVE:** `fetch-paper-metadata`, `analyze-paper`, `get-gemini-provider-quota`, and `delete-account`. Deployed versions are volatile; read them back (`supabase functions list --project-ref <project-ref>`) instead of quoting a snapshot.
  - **`search-pubmed` is implemented but NOT deployed.** It backs the PubMed Search tab and must be deployed before that frontend merges, or the tab would be reachable with no endpoint behind it — [deployment.md](deployment.md) §7b holds the artifact list, the ordering and the non-destructive verification. It is read-only, uses no elevated key, needs no new secret and no migration, and its deploy artifact does **not** include any file the other four functions bundle, so none of them needs redeploying.
...
- All five authenticate the caller with the caller's JWT via in-function `auth.getUser()`; `verify_jwt = false` at the gateway is intentional, and CORS `*` is an accepted decision under bearer-token auth. Only `delete-account` uses an elevated key, and only after the caller is authenticated.
...
- **What does not exist:** a required E2E gate, and Edge Function tests executed by a **Deno** runtime. `delete-account` and `search-pubmed` are the partial exceptions — each keeps its whole request path in a runtime-agnostic `handler.ts` plus pure `_shared` modules, so the real handler is covered by Vitest (CORS-before-auth, method gating, `getUser()`, request bounds, the server-side API-key read, retry/backoff, and the rule that no identity is read from the body). `delete-account` is additionally served for real under the local lifecycle; `search-pubmed` is not, because its E2E stubs at the browser boundary to keep CI off the live NCBI network. Other Edge Function validation remains manual post-deploy smoke (established metadata case: PMID `41912805`).
...
1. **In-app PubMed discovery (PUBMED-IN-APP-SEARCH-001) — implemented, NOT rolled out.** Add Papers gained a fourth mode. A new read-only `search-pubmed` Edge Function authenticates the caller in-function, reads that user's optional `profiles.pubmed_api_key` server-side, and answers ESearch + ESummary as an application-owned page — never raw NCBI JSON, never the key, and never the query text in a log line (only its length, because a research query can disclose a patient or a diagnosis). The frontend discovers; the **existing** importer imports: selected PMID strings go into the same `onBulkImport` the Import IDs tab calls, so duplicate classification, normalization, authorship provenance and assignment stay exactly where they were, and a result carrying a DOI still imports by PMID. Four modes did not fit one 390px row, so the tab list becomes a 2×2 grid below `sm` with 40px targets — measured, not assumed, along with the result row's shrinkability (a negative control restoring the nowrap/min-content shape puts a 1,539px hidden overflow into the dialog and pushes the PubMed link's centre off a 390px screen). **Reachable is not the same as tappable:** the row deliberately is not a click target (it contains an external link), which leaves the shared 16×16 `Checkbox` as the only selection affordance, so it carries a transparent `::before` that delivers a measured 41×43 hit region without changing anything visible or coupling the link. Touch targets are measured with `elementFromPoint`, not `getBoundingClientRect()`, and only after the dialog's `zoom-in-95` open animation settles — mid-animation every length inside a dialog reads 5% small, which is how a 40px target measures 38. **No migration, no new secret, no change to any existing Edge Function.** Rollout is blocked on deploying `search-pubmed` *before* the frontend merges — [deployment.md](deployment.md) §7b.
