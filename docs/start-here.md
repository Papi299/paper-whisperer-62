...
1. TYPESCRIPT-BASELINE-001 (2026-07-20, branch `baseline/typescript`, PR open — **not merged**): regenerated the authoritative `src/integrations/supabase/types.ts` from the reconciled linked schema (local and linked `public` types semantically identical), then eliminated the former ~48-diagnostic application TypeScript baseline. `npm run typecheck` now passes with **0 diagnostics** (application + Node); added the truthful `typecheck` / `typecheck:app` / `typecheck:node` npm scripts. No migration, remote mutation, CI workflow, or branch-protection change.
