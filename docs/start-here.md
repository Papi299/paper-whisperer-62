...
- Reported tablet issues are addressed in the accepted implementation: Analytics is reachable at coarse-pointer tablet sizes (the controls region is an explicit bounded scroll owner), touch hit targets meet size minimums, and surfaces that open on a coarse pointer do not autofocus a text field.
...
- **`E2E (local)` and `DB Tests` exist on `main` and are NOT required checks.** Both run on same-repository pull requests and `main` pushes, skip fork-origin PRs before any execution, use `contents: read` only, and need **no** GitHub secret, variable, or Environment. Promoting either to a required gate is decision **D5**, deliberately unresolved.
...
| [pfa-c03-staging-and-security-test-plan.md](pfa-c03-staging-and-security-test-plan.md) | PFA-C03 contract and evidence record (complete); D4–D5 rationale |
