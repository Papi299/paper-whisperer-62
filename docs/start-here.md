...
**Remaining grounded next-candidates** (none urgent — only pursue if the user explicitly asks): broader `Dashboard.tsx` responsibility-split planning (it remains the orchestration hub for many flows); other small prop-grouping refactors only if a similar pass-through pattern is found in another component; **plan-only AI-analysis test coverage before any AI-analysis refactor** (the bulk + single-paper Gemini flows have no E2E or unit coverage today, so a safe extraction must start with test scaffolding); analytics-section extraction planning (only if the analytics surface grows). These are candidates, not commitments — do not invent a roadmap beyond this list.
