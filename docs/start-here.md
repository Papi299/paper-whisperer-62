...
1. RECON-METADATA-PARITY-001 (2026-07-19): the final metadata/index parity migration (`20260719162013_reconcile_metadata_parity.sql`) prepared and opened for review — **local-only, unapplied remotely** (deployed in a later C24 task). Converges `projects.updated_at` removal, the duplicate `papers` updated-at trigger, `created_at` → `now()`, `study_type_pool.created_at` NOT NULL, `tags.color` default, and seven redundant indexes; `search_vector`/SEC-4 grants classified benign/artifact.
