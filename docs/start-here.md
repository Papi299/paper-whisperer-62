...
- **Author identity resolution is user-scoped and explicit** (AUTHOR-IDENTITY-RESOLUTION-001C — implemented, **not yet in Production**; see §5). Three deliberately separate layers: 001A compares author *strings* (`authorMentionKey`), 001B records what a *source* stated (`papers.author_provenance`), and 001C records what *this user decided*. Only the third creates a person, and only from an explicit action — there is no backfill, no import-time resolution and no background linker, so two historical papers sharing a valid ORCID stay two authors until someone links them. Identities never cross users: every foreign key is composite on `(user_id, …)`, so a cross-user alias, link or merge edge cannot be stored. A merge is a reversible edge (`source → target`), never a reassignment, so undo is one row deleted rather than a state reconstruction. Links are bound to `paper_id + author_index + author text` and a trigger clears a paper's links in the same transaction as any real change to its `authors` array. Suggestions come only from an exact valid ORCID or an exact 001A key, and a contradicting ORCID suppresses the weaker name match; there is no fuzzy, affiliation, coauthor or AI matching anywhere. Analytics counts one entity per resolved identity and one per unresolved 001A mention.
...
- **Database:** every tracked migration in `supabase/migrations/` is applied to the linked project **except `20260818120000_add_user_scoped_author_identity_resolution.sql`** (AUTHOR-IDENTITY-RESOLUTION-001C), which is awaiting explicit owner authorization and has deliberately **not** been applied. It is additive-only, so the current Production frontend is unaffected by applying it, and the 001C frontend degrades to 001A author grouping while it is absent — the two orderings are both safe, which is what makes the migration → verify → merge rollout possible. **The live ledger is authoritative**; read it back with `supabase migration list --linked` rather than trusting a number written here.
