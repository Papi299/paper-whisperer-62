...
1. PROJECT-TAG-MATCH-MODE-001 (2026-07-24, branch `feat/project-tag-match-modes`): each of the dashboard Project and Tag filters gains an independent **Any/All** match mode — Any is OR-union within the category, All is AND-by-unique-membership within the category (one bounded junction query per category; the returned entity ID is used to count matched members). Projects and Tags stay ANDed with each other and every other filter; there is **no** global cross-category OR. Default **Any** preserves the shipped behavior; saved presets upgrade to payload **version 3** (`projectMatchMode`/`tagMatchMode`) with backward-compatible version-1 and version-2 reads normalized to Any/Any — **no automatic DB rewrite, no schema migration, no remote Supabase mutation, no generated-type change**. Second of the high-priority UX/workflow-remediation sequence. While read from the feature branch this entry is pending its merge; once present on `main` it records the completed remediation.
