...
The active priority is **product feature and workflow development**. A short, **high-priority UX/workflow-remediation sequence runs first**, before broader feature expansion:

- **PROJECT-TAG-SELECTOR-UX-001 — completed baseline (PR #162, merge `dedde4d0`).** The dashboard Project and Tag filters are searchable multi-select with long-name overflow fixed; saved presets are payload **version 2** with backward-compatible version-1 reads.
- **PROJECT-TAG-MATCH-MODE-001 — completed (PR #163, merge `b406e2f2`).** Each of the Project and Tag filters has an independent **Any/All** match mode (Any = OR-union within the category, All = AND-by-membership within the category; Projects and Tags stay ANDed with each other and with every other filter — **no** global cross-category OR). Default **Any** preserves prior behavior; saved presets are payload **version 3** with backward-compatible version-1/version-2 reads (both normalized to Any/Any), no automatic DB rewrite, no schema migration.

1. **IMPORT-CONTINUATION-WORKFLOW-001** — the active remediation: fix the Add Papers flow where the Import Results Summary replaces/hides the Project/Tag assignment controls and dead-ends the user, so both identifier and file imports support a continuous **Import More / Import Another File** loop that preserves the Project/Tag selections for the next batch (a completed run is never retroactively re-assigned). No callback-contract change, no RPC change, no schema migration. This work is implemented in a pull request on branch `fix/import-continuation-workflow`. **If you are reading this from the `fix/import-continuation-workflow` branch, that PR is still under independent review — the immediate gate is its approval and merge; do not treat the task as done and do not start the next task yet. Once this document is present on `main` (i.e. the PR has merged), treat IMPORT-CONTINUATION-WORKFLOW-001 as completed, and the immediate next task is `PRODUCT-FEATURE-AUDIT-001`.**
2. **PRODUCT-FEATURE-AUDIT-001** — the focused product-feature and incomplete-workflow audit and prioritization exercise. **Deferred, not cancelled**, until the remediation above is complete; do not select or implement a feature before that audit.
3. Owner selection of the first new feature from that audit.

The infrastructure baseline is ready for this work: schema reconciliation is complete (all five RECON tasks merged and verified; **65** aligned ledger rows, last `20260719162013`), `npm run typecheck` is at **0 diagnostics** (TYPESCRIPT-BASELINE-001), and the required GitHub Actions `Validate` workflow with `main` branch protection is live (CI-BASELINE-001) — **this CI and branch-protection baseline remains mandatory for every remediation and feature PR**. The paused commercial direction remains valid future-facing work: C17/C18 (Paddle as the future MoR provider) and the built entitlement/quota/subscription/storage infrastructure are preserved, not cancelled — see C27 in [decisions-and-triggers.md](decisions-and-triggers.md) and [owner-decisions.md](owner-decisions.md).
...
1. IMPORT-CONTINUATION-WORKFLOW-001 (2026-07-24, branch `fix/import-continuation-workflow`): the Add Papers dialog no longer dead-ends after an import completes. Both identifier and file imports now show the result summary **plus** the Project/Tag assignment controls under an explicit **"Assignments for next import"** heading, with a primary **Import More** / **Import Another File** action that resets only that run's state and returns to the ready/dropzone view without closing the dialog. Selected Projects/Tags persist across the continuation and across tab changes; a completed run is never retroactively re-assigned (completed-run vs next-run copy is explicit, and changing selections while a summary is shown invokes no import/DB callback). The already-processed identifier batch cannot be accidentally re-imported (the source input and its Import action are replaced by Import More in completed state, with a defensive guard). Full dialog close still resets everything. **No callback-contract change, no `useBulkMutations`/RPC semantic change, no schema migration, no remote Supabase mutation, no generated-type change, no CI/branch-protection change.** Third of the high-priority UX/workflow-remediation sequence. While read from the feature branch this entry is pending its merge; once present on `main` it records the completed remediation.
2. PROJECT-TAG-MATCH-MODE-001 (2026-07-24, PR #163, merge `b406e2f2`): each of the dashboard Project and Tag filters gained an independent **Any/All** match mode — Any is OR-union within the category, All is AND-by-unique-membership within the category. Each active category runs **one deterministic paginated junction-query pipeline, never one query per selected Project or Tag**: it pages through PostgREST via `fetchAllPages` under a stable composite ordering (`paper_id`, then the entity ID, ascending) and concatenates all raw rows before a single Any/All resolution. Projects and Tags stay ANDed with each other and every other filter; there is **no** global cross-category OR. Default **Any** preserves the shipped behavior; saved presets upgraded to payload **version 3** (`projectMatchMode`/`tagMatchMode`) with backward-compatible version-1 and version-2 reads normalized to Any/Any — no RPC, no automatic DB rewrite, no schema migration, no remote Supabase mutation, no generated-type change.
3. PROJECT-TAG-SELECTOR-UX-001 (2026-07-24, PR #162, merge `dedde4d0`): the dashboard Project and Tag filters became **searchable multi-select** (zero/one/many, OR within each category, AND across categories and other filters), and long Project/Tag names no longer overflow the fixed-width trigger (truncation + full-name title/aria). Saved filter-presets upgraded to payload **version 2** (`selectedProjectIds`/`selectedTagIds` arrays, normalized to duplicate-free sets) with backward-compatible version-1 reads — no automatic DB rewrite, no schema migration, no remote Supabase mutation, no generated-type change.
4. PRODUCT-DIRECTION-RESET-001 (2026-07-24, docs only, PR #161 merged): by owner decision (C27), public-launch and commercial-launch **implementation** are **paused** and off the active critical path — Paddle integration, checkout, webhooks, paywalls, billing and public-launch work must not start without a new explicit owner decision. The active priority returns to **feature and workflow development**. Commercialization is paused, **not cancelled**: C17/C18 and the built entitlement/quota/subscription/storage infrastructure are preserved as future-facing. The required CI + branch-protection baseline stays active for all feature PRs.
5. CI-BASELINE-001 established the required GitHub Actions CI baseline: the **`Validate`** workflow (`.github/workflows/validate.yml`) runs `npm ci`, lint, `npm run typecheck`, Vitest and the production build on Node 22 for pull requests to `main`, pushes to `main`, and manual dispatch — official actions pinned to full commit SHAs, read-only token, inert placeholder build env (no secrets). `main` protection now requires the `validate` check (strict/up-to-date, administrators included, zero human approvals, force-push and deletion disabled, regular merge commits allowed, Vercel not required). Playwright is intentionally excluded from required CI (production-backed E2E, no isolated staging). No migration or database mutation occurred.
