...
**Test counts after this wave:** Vitest at 228/228, Playwright at 65/65 (both confirmed locally on PR #106). Update test counts in this section if they drift.
...
**Remaining grounded next-candidates** (none urgent — only pursue if the user explicitly asks): focused E2E for the `Matched in:` server-driven attribution sub-line; small extraction of a `PresetNameForm` shared between the Save and Rename dialogs; broader Dashboard / SearchFilters refactor planning. These are candidates, not commitments — do not invent a roadmap beyond this list.
