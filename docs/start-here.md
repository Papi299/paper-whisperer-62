...
- Billing / paywall / upgrade UI. **PFA-C01-AI-QUOTA-UX-001** adds AI-quota **transparency** — a header usage indicator plus a clear, actionable message when the AI `402` is hit — but **no** upgrade/checkout/paywall path (under review on `feat/pfa-c01-ai-quota-ux`; live only once merged **and** its additive migration is applied). Storage-quota errors are still surfaced only as a raw message; no upgrade path exists for either.
...
1. **PFA-C01-AI-QUOTA-UX-001** — complete the AI-analysis quota experience: a read-only `get_ai_quota_status` RPC (server-only `usage_counters` stays server-only — no client SELECT), a header usage indicator (used/remaining, lifetime vs monthly, reset date), a specific actionable message when the server returns the AI `402`, and a bulk run that stops after the first authoritative quota response. It adds **quota transparency only — no checkout, paywall, or upgrade path** (C27 unchanged). **If you are reading this from `feat/pfa-c01-ai-quota-ux`, the implementation is under independent review and not yet shipped; its additive migration (`20260724120000`) is committed but not applied remotely. Once merged and the migration is applied, PFA-C01 becomes complete.**
2. **Next**: after PFA-C01 ships, the owner selects the next candidate from the audit shortlist/backlog; no candidate begins without a new explicit task prompt.
