...
**Remaining grounded next-candidates** (none urgent — only pursue if the user explicitly asks): small extraction of a `PresetNameForm` shared between the Save and Rename dialogs; broader Dashboard / SearchFilters refactor planning. These are candidates, not commitments — do not invent a roadmap beyond this list.
