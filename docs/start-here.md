...
1. **PROJECT-TAG-SELECTOR-UX-001** — convert the dashboard Project and Tag filters to searchable multi-select and fix long-name overflow (**in progress** on `feat/project-tag-multifilters`; PR open and unmerged — do not treat as done until it is independently reviewed and merged).
2. **IMPORT-CONTINUATION-WORKFLOW-001** — fix the Add Papers flow where the Import Results Summary replaces/hides the Project/Tag assignment controls (separate task; **not** part of #1).
...
1. PROJECT-TAG-SELECTOR-UX-001 (2026-07-24, in progress — PR open, **not merged**): the dashboard Project and Tag filters become **searchable multi-select** (zero/one/many, OR within each category, AND across categories and other filters), and long Project/Tag names no longer overflow the fixed-width trigger (truncation + full-name title/aria). Saved filter-presets upgrade to payload **version 2** (`selectedProjectIds`/`selectedTagIds` arrays) with backward-compatible version-1 reads — **no automatic DB rewrite, no schema migration, no remote Supabase mutation, no generated-type change**. First of the high-priority UX/workflow-remediation sequence; the Import continuation bug (IMPORT-CONTINUATION-WORKFLOW-001) is intentionally a separate later task.
