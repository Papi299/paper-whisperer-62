...
- **Edge Functions — all five in `supabase/functions/` are deployed and ACTIVE:** `fetch-paper-metadata`, `analyze-paper`, `get-gemini-provider-quota`, `delete-account`, and `search-pubmed`. Deployed versions are volatile; read them back (`supabase functions list --project-ref <project-ref>`) instead of quoting a snapshot.
