...
- **Backend:** Supabase — Postgres 17, Auth (JWT, localStorage-persisted sessions), Storage, PostgREST, and two **deployed** Edge Functions: `fetch-paper-metadata` (PubMed/Crossref metadata) and `analyze-paper` (Google Gemini analysis). (A third, `get-gemini-provider-quota`, is added by the active-under-review OWNER-MANAGER-ACCESS task but is **not deployed** yet.) No service-role key is used anywhere; Edge Functions authenticate with the caller's JWT via in-body `auth.getUser()` (`verify_jwt = false` at the gateway is intentional; CORS `*` is an accepted decision under bearer-token auth).
...
**PRODUCT-FEATURE-AUDIT-001 is complete** — merged via PR #165 (merge `214bc329`); its deliverable [product-feature-audit.md](product-feature-audit.md) is the point-in-time capability audit and owner decision packet (blocking-remediation gate **Outcome A**: no Critical/High blocker). **Owner selection is complete:** on 2026-07-24 the owner selected **PFA-C01** (complete the AI-analysis quota experience) as the first implementation candidate. Candidates **PFA-C02–PFA-C09 remain unselected** and must not be started without a new explicit owner decision.

**One active implementation task is under independent review — not yet deployed:**

- **OWNER-MANAGER-ACCESS-AND-GEMINI-QUOTA-001 (2026-07-25, decision C28).** A new **owner-selected objective, separate from the PFA candidate list**: internal `owner`/`manager` roles decoupled from the commercial plan (`internal_user_access` + read-only `get_current_user_access` RPC), an owner Paperlume **AI-quota exemption** (owner is never blocked, usage still recorded/refundable; AI indicator shows "Unlimited"), and a **manager-only view of the shared Google Gemini provider quota** (a `get-gemini-provider-quota` Edge Function reading Google Cloud Monitoring, kept distinct from each user's per-user allowance). Also centralizes the Gemini model string and adds structured provider-error handling in `analyze-paper` (no prompt/schema/retry/timeout change; a provider limit is never turned into a Paperlume 402). Ships **two additive migrations** — `20260725090000` (**applied in Production 2026-07-26**) and the local-only grant-hardening `20260726120000` (**awaiting approval**). **Staged Production deployment is partially done under individual authorizations:** the Google Monitoring API + narrowly-privileged service account + `roles/monitoring.viewer` + key, the three Google Edge secrets, migration `20260725090000`, and the **owner bootstrap** are complete and verified; **still pending** — `get-gemini-provider-quota` and the PR `analyze-paper` are **undeployed**, `20260726120000` is **unapplied**, and PR #168 is **unmerged** (each a separately-authorized step; see [deployment.md §13](deployment.md)). Runtime authorization is **UUID/role-based, never email-based**; the server (`get-gemini-provider-quota` re-checking `get_current_user_access`) stays authoritative. **C27 unchanged** — no billing/paywall/upgrade; `usage_counters` privacy and Free/Pro quota values are unchanged. **PFA-C02–PFA-C09 remain unselected.**
...
2. **Next PFA gate: explicit owner selection.** Within the PFA track no further candidate is active — **PFA-C02–PFA-C09 remain unselected** and no PFA candidate begins without a new explicit task prompt. The currently active engineering work is the owner-selected **OWNER-MANAGER-ACCESS-AND-GEMINI-QUOTA-001** (above, under review), a separate objective and not a PFA candidate.

The infrastructure baseline supports this work: schema reconciliation is complete, **66** aligned ledger rows (last `20260724120000`, Local = Remote), `npm run typecheck` is at **0 diagnostics** (TYPESCRIPT-BASELINE-001), and the required GitHub Actions `Validate` workflow with `main` branch protection is live (CI-BASELINE-001) — **this CI and branch-protection baseline remains mandatory for every remediation and feature PR**. The paused commercial direction remains valid future-facing work: C17/C18 (Paddle as the future MoR provider) and the built entitlement/quota/subscription/storage infrastructure are preserved, not cancelled — see C27 in [decisions-and-triggers.md](decisions-and-triggers.md) and [owner-decisions.md](owner-decisions.md).
...
1. OWNER-MANAGER-ACCESS-AND-GEMINI-QUOTA-001 — **active; staged Production deployment in progress — migration `20260725090000` applied and owner bootstrap complete/verified, but functions undeployed and PR unmerged** (2026-07-25→26, decision C28). A new owner-selected objective (separate from the PFA list): internal `owner`/`manager` roles decoupled from the commercial plan (`internal_user_access` + read-only `get_current_user_access` RPC; server-only, FORCE RLS, no client policy), an owner **AI-quota exemption** (owner never blocked, usage still recorded/refundable; additive `is_exempt` on `get_ai_quota_status`; AI indicator shows "Unlimited"), and a **manager-only shared Google Gemini provider-quota** view (`get-gemini-provider-quota` Edge Function over Google Cloud Monitoring, distinct from the per-user allowance). Also centralizes the Gemini model string and adds structured provider-error handling in `analyze-paper` (no prompt/schema/retry/timeout change; a provider limit is never a Paperlume 402). One additive **local-only** migration `20260725090000`; 17-case local DB verification; regenerated local types (`43eb1a93…` → `30f0137a…`). Runtime authorization is UUID/role-based, never email-based. **No remote migration, no owner grant, no Google secrets, no Edge deploy, no Google setup** — each is a later, separately-authorized step ([deployment.md §13](deployment.md)). **C27 unchanged**; `usage_counters` privacy and Free/Pro quotas unchanged; **PFA-C02–PFA-C09 remain unselected**. Correction pass **001A** (same PR #168, migration unchanged) fixed the reviewed Monitoring/UX edge cases: one metric type per Monitoring request, newest-complete-60s-bucket minute usage, DST-safe Pacific-day boundary, structured provider-500 client parsing (provider failures are not a Paperlume 402), authoritative "Unlimited" indicator condition, no fabricated blocked-attempt zeros, and identity-keyed credential/response caches. Correction pass **001B** (same PR #168, migration unchanged) then corrected the Monitoring math: minute DELTA usage/exceeded use **ALIGN_SUM** (never ALIGN_DELTA), minute totals combine only synchronized bucket timestamps (never cross-minute), the provider-response cache identity includes the full credential (a rotated key is exercised on the next call), and pagination overflow fails **unavailable** rather than presenting partial data. Staged Production deployment then ran (2026-07-26): Google Monitoring API/service-account/`roles/monitoring.viewer`/key, the three Google Edge secrets, migration `20260725090000`, and the **owner bootstrap** are all complete and verified (setting secrets re-provisioned `fetch-paper-metadata`→v10 and `analyze-paper`→v14 with unchanged pre-PR code). A **grant-hardening correction** (local-only migration `20260726120000`, PR correction pass 001C) then `REVOKE`s direct `internal_user_access` table privileges from PUBLIC/anon/authenticated as defense in depth atop FORCE RLS + no policy (18-case local SQL verification). Still pending: `get-gemini-provider-quota` and the PR `analyze-paper` undeployed, `20260726120000` unapplied, PR #168 unmerged.
2. PFA-C01-AI-QUOTA-UX-001 — **complete, deployed, and accepted** (2026-07-25, PR #166, merge `fb876c8b6020de83d23ce2877effa0881ed5c4e4`): the AI-analysis quota experience — a **read-only** `get_ai_quota_status` SECURITY DEFINER RPC (additive migration `20260724120000`; S1-guarded, `authenticated`-only, no write/consume/refund, no client SELECT on the server-only `usage_counters`), a header **AI-quota indicator** (used/remaining, lifetime vs monthly, UTC reset date; fails soft), a pure structured-`402` parser so quota exhaustion shows a specific actionable message (not the generic non-2xx string), and a **bulk run that stops after the first authoritative quota `402`** with complete `analyzed + failed + not attempted = total` accounting. **Quota transparency only — no checkout/paywall/upgrade (C27 unchanged).** The server remains the enforcement boundary. Migration `20260724120000` is **applied in Production** (project `lioxtgiputfniqbktcsz`); merged-main `Validate` succeeded; the automatic Vercel **Production** deployment is live on `app.paperlume.app`. The initial MCP-generated migration-history label `20260725054400` was repaired to the canonical `20260724120000` (history metadata only; no schema SQL re-executed). PFA-C02–PFA-C09 remain unselected; the next gate is an explicit owner selection.
3. PRODUCT-FEATURE-AUDIT-001 (2026-07-24, PR #165, merge `214bc329`): a repository-wide, evidence-based product-feature and incomplete-workflow audit at baseline `main` `5a92229f`, delivered as [product-feature-audit.md](product-feature-audit.md). It classifies 31 capabilities (22 shipped-complete / 3 partial / 4 infrastructure-only / 1 planning-only / 0 dormant / 1 unknown), records 9 findings and a 9-candidate ranked backlog, and prepares exactly three owner shortlist options. Blocking-remediation gate **Outcome A** (no Critical/High blocker). Documentation-only; the owner subsequently selected **PFA-C01**.
4. IMPORT-CONTINUATION-WORKFLOW-001 (2026-07-24, PR #164, merge `5a92229f`): the Add Papers dialog no longer dead-ends after an import completes. Both identifier and file imports show the result summary **plus** Project/Tag assignment controls with a primary **Import More** / **Import Another File** action that resets only that run's state and returns to the ready/dropzone view without closing the dialog. Selected Projects/Tags persist across the continuation and tab changes; a completed run is never retroactively re-assigned, and the already-processed identifier batch cannot be re-imported. **No callback-contract change, no `useBulkMutations`/RPC semantic change, no schema migration, no remote Supabase mutation, no generated-type change.** Final item of the high-priority UX/workflow-remediation sequence.
5. PROJECT-TAG-MATCH-MODE-001 (2026-07-24, PR #163, merge `b406e2f2`): each of the dashboard Project and Tag filters gained an independent **Any/All** match mode — Any is OR-union within the category, All is AND-by-unique-membership within the category. Each active category runs **one deterministic paginated junction-query pipeline, never one query per selected Project or Tag**: it pages through PostgREST via `fetchAllPages` under a stable composite ordering (`paper_id`, then the entity ID, ascending) and concatenates all raw rows before a single Any/All resolution. Projects and Tags stay ANDed with each other and every other filter; there is **no** global cross-category OR. Default **Any** preserves the shipped behavior; saved presets upgraded to payload **version 3** (`projectMatchMode`/`tagMatchMode`) with backward-compatible version-1 and version-2 reads normalized to Any/Any — no RPC, no automatic DB rewrite, no schema migration, no remote Supabase mutation, no generated-type change.