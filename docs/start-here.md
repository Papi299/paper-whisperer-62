...
1. **PROJECT-TAG-SELECTOR-UX-001** — convert the dashboard Project and Tag filters to searchable multi-select and fix long-name overflow. This work is implemented in **PR #162**. **If you are reading this from the `feat/project-tag-multifilters` branch, PR #162 is still under independent review — the immediate gate is its approval and merge; do not treat the task as done and do not start the next task yet. Once this document is present on `main` (i.e. PR #162 has merged), treat PROJECT-TAG-SELECTOR-UX-001 as completed** — it is then the current completed baseline for the dashboard Project/Tag filters — **and the immediate next engineering task is `IMPORT-CONTINUATION-WORKFLOW-001`.**
2. **IMPORT-CONTINUATION-WORKFLOW-001** — the next active task once PROJECT-TAG-SELECTOR-UX-001 is on `main`: fix the Add Papers flow where the Import Results Summary replaces/hides the Project/Tag assignment controls (separate task; **not** part of #1).
