...
>
> **Strategy pivot (2026-05-21).** The MVP is now **web-first** with **Merchant of Record (MoR)-first** billing per [C17](decisions-and-triggers.md) (which superseded the earlier C8 Stripe-first direction the same day). **Provider selected: Paddle** per [C18](decisions-and-triggers.md) (2026-05-21). Lemon Squeezy is retained as a fallback only. **Working commercial brand: Paperlume**; **primary working domain: `paperlume.app`** (secured via Cloudflare Registrar) per [C19](decisions-and-triggers.md) (2026-05-21). Paperlume is **not a registered trademark**; trademark registration was deferred.
>
> **Operational-setup progress (2026-05-22).** Branded app URL ✅ live on `https://app.paperlume.app` (Vercel custom domain connected); ✅ Production transactional Auth email via Resend on `auth.paperlume.app` with Supabase Auth Custom SMTP; ✅ Paperlume-branded Auth email templates; ✅ Multi-mailbox smoke-tested. **Still pending:** Google Workspace business email; marketing site at root `paperlume.app` with legal URLs; Paddle KYB / Product / Price / webhook / customer-portal setup per C18. Details in [`deployment.md §8a`](deployment.md).
>
> Mobile / App Store / Play Store launch is **deferred to a later roadmap phase**. The full content of this document remains relevant for that later phase but is not on the critical path for the web MVP. Items that are *also* required before the **web** paid beta (privacy policy, terms, support URL, account deletion, AI disclosure, production domain + email setup) are still required and are tracked in [commercial-architecture.md §6](commercial-architecture.md) and [`deployment.md §8a`](deployment.md). MoR / Paddle adoption does **not** remove the privacy / terms / support / account-deletion requirements. The web launch will land them first; the mobile checklist reuses them when the time comes.
...
**Billing-provider direction.** A Merchant of Record (MoR) provider is the web MVP path per C17 (2026-05-21 — supersedes C8 Stripe-first the same day); **provider selected: Paddle per C18 (2026-05-21)**. Lemon Squeezy retained as fallback only. Apple IAP and Google Play Billing remain the planned ingestion paths when mobile work begins; the [commercial-architecture.md §8](commercial-architecture.md) provider-neutral ingestion model is intact and supports adding them as purely additive Edge Functions.
...
- [x] **In-app account deletion path — implemented (PFA-C04).** Account → **Danger zone** → Delete account invokes the `delete-account` Edge Function, which authenticates the caller, re-checks the typed confirmation phrase server-side, removes every Storage object under the account's bucket prefix through the Storage API, and then hard-deletes the Auth user (`auth.admin.deleteUser(userId, false)`); the user's database rows go with it through the existing `ON DELETE CASCADE` foreign keys. Required by recent Apple and Google policy — re-verify their exact current requirements at submission time (this checklist item records that the capability exists, not that any specific store's wording has been re-checked).
- [x] **Data export — implemented (PFA-C02).** Account → Account data downloads one versioned ZIP holding the signed-in user's whole dataset: JSON for papers (notes included), projects, tags, both relationship tables, presets and all four pools, attachment metadata, and a non-secret profile, plus the attachment **binaries**. Credentials are excluded by construction. Citation-format export (CSV/RIS/BibTeX) remains available separately for the current selection. See [start-here.md](start-here.md) §Implemented capabilities.
