...
import { PapersPage, CacheSnapshot } from "./papers/types";
...
// PapersPage and CacheSnapshot are now imported from ./papers/types
...
export function usePapers(userId: string | undefined, normalizationConfig?: NormalizationConfig) {
  ...
  // ── Cache helpers (extracted to usePaperCacheHelpers) ──
  const { snapshotCache, rollbackCache, cancelQueries, updatePapersCache, updateMetaCache, adjustCount } = usePaperCacheHelpers(userId);

  // ── Taxonomy mutations (extracted to useProjectMutations / useTagMutations) ──
